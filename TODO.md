...
| item | owner | status |
|---|---|---|
| **Self-hosting C compiler** — chibicc compiled *to* SVM IR (via the LLVM on-ramp, artifact kept for speed) and run as a guest that compiles C → SVM IR in-sandbox; browser playground (W5) is one deployment | SELFHOST_C.md | **steps 1–2 done + step-3 planned 2026-07-24** — `chibicc.svmb` builds via the on-ramp (`demos/chibicc_selfhost/`: 258 funcs, verifies). Step-3 reshaped (App. B): the guest libc is *assembled from the proven Postgres shims* (26/41 symbols incl. the `%.17g` dtoa already byte-exact) + a small `chibicc_extra.c` (~15 trivial). **No substrate changes.** Blocked on one owner call: where the shared guest libc lives (factor vs in-place `#include` vs copy) |
