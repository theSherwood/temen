...
| item | owner | status |
|---|---|---|
| **Self-hosting C compiler** — chibicc compiled *to* SVM IR (via the LLVM on-ramp, artifact kept for speed) and run as a guest that compiles C → SVM IR in-sandbox; browser playground (W5) is one deployment | SELFHOST_C.md | **steps 1–2 + step-3 slice 1 done 2026-07-24** — `chibicc.svmb` builds via the on-ramp and, with the self-host libc linked (`demos/chibicc_selfhost/`: reused Postgres shims + `chibicc_extra.c`), **translates with NO `--stub-externs`** — 333 funcs, trap-free, verifies + bytecode-compiles. **No substrate changes.** Next: step-3 slice 2 — the run harness (instantiate on the `fs` cap + memfs, compile a `.c`, byte-match native `--emit-ir`). Deferred owner call: factor the shared guest libc out of `demos/postgres/` once both guests are green (in-place `#include` for now) |
