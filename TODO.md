...
| item | owner | status |
|---|---|---|
| **Self-hosting C compiler** — chibicc compiled *to* SVM IR (via the LLVM on-ramp, artifact kept for speed) and run as a guest that compiles C → SVM IR in-sandbox; browser playground (W5) is one deployment | SELFHOST_C.md | **steps 1–3 done 2026-07-24 (slices 1+2)** — `chibicc.svmb` builds trap-free (333 funcs, verifies) AND runs on the SVM: `examples/chibicc_run.rs` + `run_selfhost_diff.sh` show the guest's emitted SVM IR **byte-matches native** across int / float (`%.17g`→`__vm_fmt_gen`) / header-`#include` cases. **No substrate changes.** Next: widen the corpus toward chibicc's `test/*.c` + wire the differential into CI. Deferred owner call: factor the shared guest libc out of `demos/postgres/` (in-place `#include` for now) |
