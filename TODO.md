...
| item | owner | status |
|---|---|---|
| **Self-hosting C compiler** — chibicc compiled *to* SVM IR (via the LLVM on-ramp, artifact kept for speed) and run as a guest that compiles C → SVM IR in-sandbox; browser playground (W5) is one deployment | SELFHOST_C.md | **ALL 5 STEPS DONE 2026-07-24 — chibicc compiles C in the browser.** `chibicc.svmb` builds trap-free (333 funcs, verifies), emits SVM IR **byte-matching native** across int/float/`#include` on treewalk/bytecode/jit, and the playground has a **"C compiler (chibicc → SVM)" card** (`browser/web/play.js`): edit C → compile with chibicc.svmb in-browser (new cdylib `svm_run_onramp_fs`) → `svm_parse` → run → result, verified in Chromium by the `real-browser` Playwright gate. **No substrate changes.** Follow-ups: `#include` in the demo (header image — export already accepts one), `printf`→Stream output convention, factor the shared guest libc out of `demos/postgres/` (in-place `#include` for now) |
