# GC.md — temen ⇄ guest GC support contract (conservative, non-moving)

> Status: draft RFC, converged between the temen and JACL sides. Expect iteration.
> Scope: what temen provides so a **guest** can run its own garbage collector. temen
...
pointers, so by default temen reports candidate root words as raw data and lets the guest
...
temen contributes exactly one thing the guest cannot do itself: enumerate the roots
...
| **temen** | enumeration of roots on **control stacks + saved-register blocks** — out-of-band, unnameable by guest masking (§3d/§5), so the *only* place the guest cannot reach. |

## 2. Stop-the-world: cooperative, guest-coordinated (no async preemption)

temen provides **no** preemptive, any-PC stop with register capture, and will not —
...
**No new temen world-stop primitive is required.** At most an optional thin `quiesce`
...
`i32.atomic.wait` / `atomic.notify`), so temen ships it only as a **tested reference**, not an
API. temen can wrap only the collector↔vCPU barrier; the safepoint poll + `suspend` (step 2)
lives in the guest scheduler. temen exposes **no vCPU-count intrinsic**, so the guest passes `N`;
...
`crates/temen/tests/gc_quiesce.rs` (parametric in `N`, run for N=2 and N=4) — one STW cycle; a
...
## 3. The one new temen primitive: range-filtered root enumeration
...
`heap_lo`/`heap_hi` are **window offsets** bounding the guest heap; temen walks every
...
**masked** (`m = w & mask`), and the masked value `m` is what is range-tested and emitted; temen
...
- **It conveys ~no authority.** Every word it returns is an in-window value the guest's own
  ...
  pointers) are filtered *inside* temen and never returned (§3, property 1). It cannot read
  out-of-window memory and writes only to a guest-provided, masked buffer — **zero
  escape-TCB**, nothing to gate.
- **temen treats a domain as one trust principal.** Capabilities gate the inter-domain / host
...
1. **Range-filtered (and mask-constrained against host leaks).** Return only in-window
   *masked* candidate words. Out-of-window words — host JIT return addresses (code arena),
   saved frame pointers, host pointers — are filtered **inside temen** and never cross the
   boundary. No host ASLR/layout leaks. This is what keeps the control-stack *read* inside
   temen's TCB and makes option-B ("let the GC read control stacks") safe.
   ...
   *into* the window (e.g. keep only low bits, so a return address's low 24 bits land in
   `[heap_lo, heap_hi)`), leaking host-address bits past the filter. So temen **constrains the
...
4. **Mechanism, not policy.** temen does only the window range-check; the guest does
...
> temen scans every fiber **not actively executing guest mutator code** = all **parked**
...
(interp). temen walks the caller from the `gc.roots` site down, exactly like a parked fiber.
So **the collector never reads its own out-of-band control stack** (it cannot) — temen reads
...
fibers, suspended coroutines; `temen-interp/src/bytecode.rs`), and a module combining `gc.roots` with
...
- `gc.roots` is sound **only under STW** — temen does the range-check; the guest
  ...
  RUNNING fiber (`temen-jit/src/fiber_rt.rs`); the interp scans the shared registry
  without it.
...
4. **`gc.roots` on the JIT** (the intricate, unsafe piece). *(done — a conservative native-stack
   walk.)* New `temen-fiber` accessors expose a parked fiber's `[ctx, top)` saved extent and a
   ...
   *preserves* those registers across the call, so the thunk boundary did **not** force the caller
   ...
   (`fiber_rt::temen_gc_roots_flush`, one naked variant per target) that spills every callee-saved
   register onto the scanned stack before the walk runs (`crates/temen-jit/GC_ROOTS_AUDIT.md`; A/B
   test in `crates/temen/tests/gc_roots.rs`). Scanning a running fiber's / another vCPU's stack assumes
...
6. **Reference STW `quiesce` barrier** (§2.1) — pure-guest futex barrier, shipped only as a both-backends
   test (`crates/temen/tests/gc_quiesce.rs`), not an temen API. *(done.)*
...
- Any temen object model, allocator, write barrier, or moving/evacuating collector.
- Exposing raw control-stack views or word locations to the guest (§3 keeps the read
  inside temen and returns values only).
