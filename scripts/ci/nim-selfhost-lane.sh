...
#
#   nifler  every module parsed: `system`, the program and each import as a step of the plan, and
#           each file a module includes by nimsem itself — fork → execve("/bin/sh", ["-c",
...
echo "[1/4] the compiler phases, through the POSIX edge"
...
# compiler, whose hash tables iterate in another order (#1753) — so they are not the reference.
...
"$B/nim_selfhost_lane" --sh "$W/sh.ir" --check "$N/prog.nim" --hexer "$W/hexer.temen" \
  --expect "$N/nimcache" "$W/nimsem.temen" "$W/nifler2.temen" nimony/lib "$W/out.s.nif" \
  >"$W/temen.out"
