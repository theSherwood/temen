...
- **Durability / snapshot** — `durable.*`, checkpoint/restore.
...
The window is a flat buffer; guest pointers stay i64 offsets; an access is `win + off` with **no
`& (size-1)` mask and no bounds trap** (INVARIANT #2's masking lowering deliberately dropped). Emit
loads/stores via `memcpy` into a typed temp (not a cast-deref) so the output stays correct under
default flags regardless of target alignment / strict-aliasing — the `wasm2c` choice. **Fuel:** omit
the safepoint polls entirely. This is coherent *because the artifact has left the sandbox* — the
"undisableable fuel/epoch kill" rule (`DEBUGGING.md`) exists to kill a runaway **guest**; a trusted
native binary is the user's own process to bound and kill.
...
> Identical results and traps to the oracle **on in-window, terminating executions**; **UB (not a
> trap) on out-of-window access**; **no `OutOfFuel`.**
...
- **Slice 2 — powerbox ABI.** `_start` + extern-import header (§4.3); the chibicc/LLVM libc corpus
  (SHA-256, xxHash, jsmn, …) runs as emitted C against a reference powerbox impl. **Gate:** the §20a
  eight-library corpus is byte-identical to its native `clang` build *and* to the oracle.
...
| #2 confinement = masking lowering | **explicitly renegotiated for this out-of-sandbox artifact only** — masking is dropped in emitted C by design; the sandbox's masking pass is untouched because the sandbox never runs this output. *Owner must record the dated renegotiation in `INVARIANTS.md` when accepting.* |
