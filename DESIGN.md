...
is therefore small and bounded: our verifier (tiny, fuzzable) + our CLIF
confinement lowering + our memory/capability plumbing. Caveat: Wasmtime's security is
...
**isolate confinement into one tiny, separately-fuzzable lowering pass**
with a crisp invariant (every access proven bounded, or dominated by a bounds check of
the *final effective address* into `[0, reserved)`, trapping out-of-window),
differential-tested against the interpreter and native — see §4, §18.
...
guard-when-bounded / bounds-check-when-not, bounding the *final effective address*
and trapping out-of-window (trap-confinement); (3) confinement is one isolated,
fuzzed unit. All three are folded in below (§4, §18) and logged (D36–D38).
...
The dominant escape-TCB component is the **JIT** (Cranelift + our trap-confinement lowering +
...
things are out of reach: memory access is trap-confined (I1); return addresses live
...
- **Escape-TCB** (a bug breaks the sandbox for everyone): verifier; JIT incl.
  trap-confinement lowering; runtime memory-model plumbing (window setup, guard pages,
...
- **The adversary controls:** the IR (any verifier-passing module), all window
  memory (arbitrary reads/writes), the timing/sequencing of `call.cap`s, and
...
|---|---|---|
| **I1 Memory confinement** — every access ∈ `[base, base+size)`, out-of-window ⇒ `MemoryFault` | trap-confinement lowering (JIT) | the isolated confinement-fuzz unit (`fuzz/mask`) + differential vs interpreter (§18) |
...
| **I5 Stack integrity** — control stack unreachable by confinement; data overflow → guard fault | runtime (placement) + JIT (stack probes) | guard-page + stack-clash tests |
...
isolated confinement-fuzz unit, and verifier fuzzing — strong bug-finders, not proofs.
...
- **Plain data** — scalars (`i32`/`i64`/`f32`/`f64`/`v128`) and **pointers**. A
  forged pointer is harmless: every access is bounds-checked + MMU-confined to the window
  (§4) and traps if out-of-window, so the worst case is the guest corrupting its *own*
  memory (allowed). So
...
- **Pointers** are a CHERI-ready-but-erasable refinement of `i64` (§10): off-CHERI
  a no-op; the type exists for the JIT's confinement and a future CHERI *host* backend.

**Central safety theorem (what the verifier + runtime buy):** escape is impossible
because (a) the memory model confines all plain-data access (bounds check of the final
effective address + guard region, trapping out-of-window, JIT-enforced — §4), (b) **the authority tables
...
**JIT codegen (the trap-confinement lowering), not the verifier** — the verifier secures
typing, control flow, and table-index ranges; confinement of memory is the
confinement pass (§4, §18). "Verified ⇒ cannot escape" is shorthand for "verified
**and** the trap-confinement lowering is correct."
...
invocation: `call.cap (handle, op-index, args…)` → handle type + op-index resolve
...
- **Memory:** `{i32,i64,f32,f64}.load/store`; narrow `load8_s/u load16_s/u load32_s/u`
  ...
  *hint* (unaligned allowed). Trap-confinement is implicit (JIT-inserted).
...
```
**`call.cap <op_index>(h, args)`** — `op_index` is an immediate and the handle's
...
- **Dispatch is per-entry, not per-type** (D45). One interface type has many
  ...
  **devirtualizes** it to a direct, inline-able call (§9's "inline-able to ~free")
  ...
  - **Devirtualization is deferred, and the cost of doing it is the reason (not just
    ...
    scalar `call.cap` is ~1.24× a wasm import today; the defensible §1a interface win —
  ...
  - **Update — D45 now implemented (opt-in), and it cleared the TCB concern above.** An optional
    ...
    (`n_args`/`n_res`) so an odd-signature `call.cap` can't C-ABI-mismatch the fn. The production
...
Since `call.cap` results can already be handle-typed, attenuation and the initial
...
ptr is a guest window offset, so the trampoline bounds-checks `(ptr,len)`
...
**windowed (trap-confined) memory** (§4) — *force* most of the ABI's shape; the rest is
...
it is bounds-checked + confined, §4). The control stack is **out-of-band** (§5), *not* in
the window — so an address-taken local cannot live there, or its `&` would name a
location outside the window (and trap). Hence the SafeStack split:
...
### Shared conventions
- **Invocation:** `call.cap <handle> <op-index> args… → results` (§3c); each
...
- **Trap-confinement (guard-when-bounded, bounds-check-when-not)** (D36). An
  out-of-window access raises `Trap::MemoryFault` **at the offending access** — it is
  never masked/aliased back into the window:
  ...
  - *Branchless confinement (D63), matching Wasmtime.* A top-level access lowers to
    `cmp; select_spectre_guard(oob, reserved, addr+offset); load` — no per-access
    branch. An out-of-bounds address is redirected (via cmov) to `reserved`, the
    first byte of the trailing guard page, so the access **itself** faults there
    (`MemoryFault`), keeping D38's clear-fault-at-the-offending-access property. This
    is the *identical primitive* default-config Wasmtime uses for bounds-checked
    heaps (`select_spectre_guard` → `cmp→cmov`); the `reserved` bound is hoisted into
    a register, so per access it is one cmp + one cmov, no branch.
  - *Spectre-v1: the guard-select IS the hardening.* `select_spectre_guard` is a
    speculation barrier — a data-dependent cmov, not a predicted branch — so a
    misspeculated out-of-bounds access **also** receives `reserved` and lands on the
    guard; it can never speculatively read a neighbouring window or host memory, the
    same confinement the old AND mask gave. *Measurement corrected the earlier claim
    that our AND beat Wasmtime's cmov:* the 40-bit reservation mask is not an x86
    immediate, so the AND was a RIP-relative **load** per access, and the `trapnz`
    added a per-access **branch** — together ~1.06×/1.34× behind Wasmtime-w64 on
    `matmul`/`matmul_eb` (`confine` harness). Branchless cmov-to-guard reaches
    **0.94×/0.97×** (parity/faster). (A `trapnz`-keeping cmov variant — clamp only,
    branch kept — *regressed* to 1.23×/1.55×, confirming the branch removal is the
    win, not the cmov.) **Nested §14 sub-windows take the same branchless path** — an
    out-of-child access redirects to the *parent's* guard page (`round_up(parent
    reserved, page)`), the only guaranteed fault site since the child slice has
    committed parent memory on both sides, so it faults out-of-child instead of
    aliasing the parent — no nesting penalty. A per-domain unhardened mode (no spectre
...
itself is either proven in-window or bounds-checked. "Bounded" here means *proven by the
JIT's upper-bound analysis*, **not** free-by-type; a genuinely unknown 64-bit
address is always bounds-checked (and traps out-of-window). Two strengths of elision
follow, and they differ in what they trust:
- *Conservative (no guard reliance):* the whole access `[addr+offset, +width)` is
  proven `< size`, so no check is needed — the address is already in-window. Sound on
...
- **Interface signature** — an ordered list of op signatures (params/results in IR
  ...
  **guest module's own type section** (§3a), so the verifier statically type-checks
  every `call.cap` with zero host knowledge — self-contained and verifiable.
...
A punted host call **inside a fiber** parks the FIBER, not the vCPU: the `call.cap` unwinds
...
- **`notify`'s count is a promise, and it is differentially pinned.** `notify(addr, n)`
  wakes **at most `n`** waiters on that key — **oldest first**, in arrival order (owner ruling
  2026-09-22) — and returns exactly how many it woke; the rest stay parked. FIFO because LIFO
  starves: under steady notify traffic a waiter that parked early sits behind every later arrival
  forever, and a guest work queue whose oldest waiter never runs is a hard failure to attribute. A guest lock or semaphore handing out one unit of
  ...
  1:1 vCPU's OS thread and an event-parked fiber queue the same kind of cell, and the
  wake is latched in that cell rather than inferred from shared state, so a waiter reports
  woken iff a `notify` claimed it. Two representations behind one key is how the JIT once
  woke three OS waiters while reporting one (#1615, invariant #15).
...
> A `call.cap` through an offer **runs that op's handler over the provider's world** and
> returns its results to the calling fiber. If the handler blocks, the caller waits.

- **Every call is synchronous from the guest's perspective** — `call.cap` in, results out. No
...
- **Wider widths (256/512) are feature-detected, deferred — and the blocker is the
  ...
  *(a)* a **host-provided vectorized capability** (`dot`/`gemm`/`memcpy`/crypto/codec the
  host implements with its own tuned AVX-512, invoked via `call.cap` + a zero-copy
...
## 19. Debugging & observability  [PARTIALLY BUILT — interpreter stepping + DAP + memory-access hooks shipped; DWARF & call.cap record/replay staged]
...
with no DWARF and no JIT in the loop. **Still staged:** the `call.cap` record/replay log (pillar 1)
...
1. **Record/replay & time-travel — nearly free *in deterministic mode*, a genuine
   ...
   capabilities; logging `call.cap` inputs/outputs and seeding the deterministic mode (§12)
   ...
   **Caveat (inherited from §12):** "all nondeterminism via capabilities" holds *only in
   ...
   stronger substrate than plain seed-replay). The `call.cap` I/O log itself is not yet built.
...
(`temen-opt`) that inserts a `call.cap` before each memory op and re-verifies (fail-closed, §2a) —
...
- **One call shape, and it is synchronous.** `call.cap` produces a result (§3b); the MVP caps
...
- **Nesting (§14):** a child capability resolves at grant time to a **pass-through** (one hop
  ...
  fiber** — child `call.cap` → trampoline → parent handler → return — composing to any depth.

**The governing principle:**

> **Synchrony is interface-guaranteed; cost is host policy the guest cannot observe.**

`call.cap` is always synchronous in *shape*, and the child "cannot tell whether a capability
...
- **`temen-durable` — the IR→IR freeze/thaw transform.** It instruments a function so an in-flight
  may-suspend op (a `call.cap`, or a `Call` into a suspended chain) can be **unwound** into
  ...
  **shadow stack**, both in the window; unwind spills the live set + a resume id and returns out to
  ...
  CFGs (branches, loops, joins), leaf `call.cap`s, and propagated call chains (re-issue vs. continue).
...
**Shape (everything crosses `call.cap`, D55).** The guest builds a serialized IR blob (the binary
`temen-encode` format) in its own window, `call.cap`s a granted **`Jit`** capability with a `(ptr, len)`,
...
`call.cap 11 <op> …` on a granted `Jit` domain handle (negative-errno `i64` ABI, D42):
...
- **Model A (shipped, the default) — sidestep the mask.** A submitted unit is reached through the host
  `call.cap` thunk, never installed in the shared table, so the mask never moves; incremental
...
structural equality. The registry is consulted only inside a synchronous `call.cap` and is **never read at
...
that keeps "no escape-TCB change" true:** trap-confinement (I1) bounds accesses to `[0, reserved)` for the
...
`call.cap` it runs inside — while **threads** (`thread.spawn`/`join`) and the **futex** (`wait`/`notify`)
stay rejected (a spawned vCPU would outlive the `call.cap` and collide with the serialized `Mutex<Host>`
...
- **Native Cranelift.** `define_extra` admits a `thread.*`/futex unit once the domain hosts threads
  ...
  locked-`Host` path (a hosted unit's spawned vCPUs are concurrent `call.cap`ers). A unit's
...
(the guest is suspended *inside* the module during a `call.cap`, so it can never self-trigger compaction).
...
works via `temen_run::cap_thunk_locked`, which serializes `call.cap` through a per-domain `Mutex<Host>` so
...
guest `call.cap 11` iface is unchanged. `JitSession` owns the boxed `Mutex<Host>` and re-bakes the locked
...
| 4 | LOW | call.cap result buffer partially uninitialized on host arity mismatch → zero-filled |
...
- **Phase 3.5 — Cross-platform parity** — port the runtime to **Windows** and lock
  ...
  is already portable (confinement is pure arithmetic), so only the non-TCB
...
- **Fuzz the trap-confinement lowering as its own unit** (D38) — it is the part
  ...
  generated memory access is dominated by a bounds check of the final effective address
  into `[0, reserved)` (trapping out-of-window), or proven bounded behind a guard.*
  Differential-test confined addresses against the interpreter's checked addresses;
  add a self-test that asserts no access instruction reaches a raw, unbounded address.
...
| D38 | Confinement = **guard-when-bounded, check-and-clamp-when-not** (trap-confinement): bounds-check the **final effective address** against `[0, reserved)` with a cold `MemoryFault` trap, and clamp the address feeding the access with `& (reserved−1)` (architecturally a no-op past the check; on the speculative path it confines Spectre-v1 exactly as the old mask did). One isolated separately-fuzzable lowering pass; ONE configuration — no unhardened check-only tier | Settled; **the per-access shape is revised by D63** (branchless `select_spectre_guard` for top-level windows; this row's `trapnz` + AND is retained for nested §14 sub-windows) | Matches wasm32 hot path (zero instructions when elided); final-address checking closes the large-immediate escape; trap-at-the-access matches wasm UX and keeps the §18 escape oracle; isolation makes it fuzzable as the security hinge. (The original "1-op AND beats Wasmtime's cmp→cmov" claim was refuted by measurement — the 40-bit mask is a RIP-relative load, not an immediate — and is corrected in D63.) |
