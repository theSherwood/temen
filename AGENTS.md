...
**Fast local pre-push check (optional):** `git config core.hooksPath .githooks` (or run `scripts/ci/install-git-hooks.sh`) enables a pre-push hook that mirrors CI's `check` job (`build · test · fmt · clippy`, plus the excluded crates above), so the common failures surface before a CI round-trip. It's fast feedback, **not** the gate — the CI matrix (cross-OS, miri/asan/tsan/loom, fuzz, differential) still runs on the PR and remains authoritative. Bypass once with `TEMEN_HOOK_SKIP=1 git push`. See `.githooks/README.md`.
