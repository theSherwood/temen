...
**Status:** owner-accepted 2026-09-04 — Decision A in progress (#1253 tracks the slices), Decision B tracked in #1289. all three §4a rulings **accepted 2026-09-08 and recorded in INVARIANTS.md** — R1 (freeze authority as an explicit capability — supersedes "durable ⇒ nested"), R2 (window minter → `Budget.mem`, with a top-up cascade), R3 (placement is a parameter, not a form). Decision B's §4a gate is met (R1–R3 recorded + #1299 merged). **R2's `WindowMinter`→`Budget.mem` fold is now built** (op 15 charges `Budget.mem` via `budget_mem_take`; the `WindowMinter` cap + `cap_id` 15 retired; embedders grant `HostCap::detached_budget(mem)`), and `Budget.transfer` (the top-up primitive) landed — so the historical `WindowMinter` references below are the pre-fold record. The rest here is not yet built; the interpreter half it
