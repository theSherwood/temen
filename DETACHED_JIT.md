...
**Status:** owner-accepted 2026-09-04 — Decision A in progress (#1253 tracks the slices), Decision B tracked in #1289. all three §4a rulings **accepted 2026-09-08 and recorded in INVARIANTS.md** — R1 (freeze authority as an explicit capability — supersedes "durable ⇒ nested"), R2 (window minter → `Budget.mem`, with a top-up cascade), R3 (placement is a parameter, not a form). Decision B's §4a gate is met (R1–R3 recorded + #1299 merged). Nothing here is built; the interpreter half it
