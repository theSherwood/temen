...
// the *shipping wasm artifact* through the exact FFI the browser console uses (`temen_pg_open` /
// `temen_pg_query` / `temen_pg_snapshot` / `temen_pg_close`). The native `pg_snapshot_roundtrip` test proves
...
//
//   node pg_snapshot_test.mjs [temen_browser.wasm] [postgres_resolved.temen] [pgdata.img]
//
...
const modPath = process.argv[3] ?? 'web/assets/postgres_resolved.temen';
...
function open(imgBytes) {
  ...
  const rc = ex.temen_pg_open(modP, N(modBytes.length), imgP, N(imgBytes.length));
  ex.temen_dealloc(modP, N(modBytes.length));
  ex.temen_dealloc(imgP, N(imgBytes.length));
  return Number(rc);
}
function query(sql) {
  const b = Buffer.from(sql.endsWith('\n') ? sql : sql + '\n');
  const p = put(b);
  const rc = Number(ex.temen_pg_query(p, N(b.length)));
  ex.temen_dealloc(p, N(b.length));
  return { rc, out: readStdout() };
}

// 1) Boot from the pristine image.
if (open(readFileSync(imgPath)) !== 0) fail(`initial boot: status ${ex.temen_status()}`);
...
if (query('CREATE TABLE persist_probe (x int);').rc !== 0) fail(`CREATE: status ${ex.temen_status()}`);
if (query('INSERT INTO persist_probe VALUES (424242);').rc !== 0) fail(`INSERT: status ${ex.temen_status()}`);
console.error('created table + inserted row');

// 3) Snapshot the live data dir, copy the image out, tear the backend down.
if (Number(ex.temen_pg_snapshot()) !== 0) fail('snapshot');
const sp = Number(ex.temen_pg_snapshot_ptr());
const sl = Number(ex.temen_pg_snapshot_len());
...
if (open(snapshot) !== 0) fail(`reboot from snapshot: status ${ex.temen_status()}`);
console.error('rebooted from snapshot');

// 5) The row survives.
const { rc, out } = query('SELECT x FROM persist_probe;');
if (rc !== 0) fail(`SELECT after reboot: status ${ex.temen_status()}`);
