// Real-browser (Chromium) end-to-end check for the **cooperative-session bash card** (#1122 route
// (a)): the same live `bash -i` session as the interactive card, but on the SUSPEND/RESUME driver —
// one Worker owns a resumable `CoopRun` (`temen_bash_coop_open`), pumps it to the first prompt, and
// on every keystroke batch feeds the #797 line discipline and pumps again; the engine returns IDLE
// whenever bash waits at the prompt instead of blocking a thread on a doorbell. Nothing here relies
// on cross-Worker wakes. The session drives the identical script as the interactive E2E: prompt, a
// builtin, ^C → rc=130 (readline's handler re-raising into bash's), a fork+exec'd `seq`, ^Z of a
// foreground `cat` (SIGCHLD wakes bash's waitpid across an idle/pump cycle), `fg`, ^D exit.
//
...
// actually run. Run: node browser-bash-coop-session-test.mjs
...
if (ASSETS.some((a) => !existsSync(`${ROOT}/web/assets/${a}`))) {
  console.log('– coop-session bash test skipped (web/assets/bash.temen, bin_seq.temen or bin_cat.temen absent — run build-bash-assets.mjs)');
  process.exit(0);
}
...
catch { console.log('– coop-session bash test skipped (playwright not found)'); process.exit(0); }
...
const CARD = 'section[data-demo="bash -i (cooperative session)"]';
...
try {
  ...
  console.error(`coop-session bash failed: ${e.message}`);
...
if (ok) { console.log('✓ cooperative-session bash card (#1122 route (a)): idle/feed/pump session — prompt, builtin, ^C→rc=130, fork+exec\'d seq, ^Z suspends cat + usable prompt, fg resumes, ^D exit'); process.exit(0); }
process.exit(1);
