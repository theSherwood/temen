...
try {
  ...
  const WORK_IDS = ['powerbox', 'threads', 'jit', 'inst', 'capio', 'wasmjit', 'tierup', 'jitcodegen', 'instcodegen', 'instnested', 'jitruntime', 'jitb2'];
  ...
  let pageOk = false;
  let isolated, powerbox, threads, jit, inst, capio, wasmjit, tierup, jitcodegen, instcodegen, instnested, jitruntime, jitb2;
  for (let attempt = 1; attempt <= INDEX_ATTEMPTS; attempt++) {
    ...
    if (attempt < INDEX_ATTEMPTS) {
      const bad = WORK_IDS.filter((id, i) => [powerbox, threads, jit, inst, capio, wasmjit, tierup, jitcodegen, instcodegen, instnested, jitruntime, jitb2][i].status !== 'pass');
