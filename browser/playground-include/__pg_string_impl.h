#ifndef __PG_STRING_IMPL_H
#define __PG_STRING_IMPL_H

// The bodies of the seeded <string.h> (#1392) — see that header for why they live in their own
// file, and `__pg_linkage.h` for what `__PG_FN` means in each of the three compile modes.

#include <string.h>

__PG_FN size_t strlen(const char *s) {
  size_t n = 0;
  while (s[n]) n++;
  return n;
}
__PG_FN int strcmp(const char *a, const char *b) {
  while (*a && *a == *b) { a++; b++; }
  return (unsigned char)*a - (unsigned char)*b;
}
__PG_FN int strncmp(const char *a, const char *b, size_t n) {
...
__PG_FN char *strcpy(char *d, const char *s) {
  char *r = d;
  while ((*d++ = *s++)) {}
  return r;
}
__PG_FN char *strncpy(char *d, const char *s, size_t n) {
  size_t i = 0;
  for (; i < n && s[i]; i++) d[i] = s[i];
  for (; i < n; i++) d[i] = 0;
  return d;
}
__PG_FN char *strcat(char *d, const char *s) {
  char *r = d;
  while (*d) d++;
  while ((*d++ = *s++)) {}
  return r;
}
__PG_FN char *strchr(const char *s, int c) {
  for (; *s; s++) if (*s == (char)c) return (char *)s;
  return (char)c ? 0 : (char *)s;
}
__PG_FN char *strrchr(const char *s, int c) {
  const char *last = 0;
  for (; *s; s++) if (*s == (char)c) last = s;
  return (char *)last;
}
__PG_FN char *strstr(const char *hay, const char *needle) {
...
__PG_FN void *memcpy(void *d, const void *s, size_t n) {
  char *dd = d;
  const char *ss = s;
  for (size_t i = 0; i < n; i++) dd[i] = ss[i];
  return d;
}
__PG_FN void *memmove(void *d, const void *s, size_t n) {
  char *dd = d;
  const char *ss = s;
...
__PG_FN void *memset(void *d, int c, size_t n) {
  char *dd = d;
  for (size_t i = 0; i < n; i++) dd[i] = (char)c;
  return d;
}
__PG_FN int memcmp(const void *a, const void *b, size_t n) {
  const unsigned char *aa = a, *bb = b;
  for (size_t i = 0; i < n; i++) if (aa[i] != bb[i]) return aa[i] - bb[i];
  return 0;
}
__PG_FN void *memchr(const void *s, int c, size_t n) {
  const unsigned char *p = s;
  for (size_t i = 0; i < n; i++) if (p[i] == (unsigned char)c) return (void *)(p + i);
  return 0;
}

__PG_FN char *strncat(char *d, const char *s, size_t n) {
  char *r = d;
  while (*d) d++;
  size_t i = 0;
  for (; i < n && s[i]; i++) d[i] = s[i];
...
__PG_FN size_t strspn(const char *s, const char *set) {
  size_t i = 0;
  for (; s[i]; i++) {
    const char *p = set;
    while (*p && *p != s[i]) p++;
...
__PG_FN size_t strcspn(const char *s, const char *set) {
  size_t i = 0;
  for (; s[i]; i++) {
    const char *p = set;
    while (*p && *p != s[i]) p++;
...
__PG_FN char *strpbrk(const char *s, const char *set) {
  for (; *s; s++) {
    const char *p = set;
...
__PG_FN char *strtok(char *s, const char *delim) {
...
__PG_FN int __pg_lower(int c) { return (c >= 'A' && c <= 'Z') ? c + 32 : c; }
__PG_FN int strcasecmp(const char *a, const char *b) {
...
__PG_FN int strncasecmp(const char *a, const char *b, size_t n) {
  for (size_t i = 0; i < n; i++) {
...
__PG_FN char *strndup(const char *s, size_t n) {
  size_t l = 0;
  while (l < n && s[l]) l++;
