// THREADS.md step 4 — run the real Temen engine over a guest window in **shared** wasm linear memory.
...
// `Region::shared` view of the window via `temen_run_shared`. In the threads build the window lives in
...
//
// Usage:  node threads-engine.mjs <module.wasm> [guest.temenc] [expected]
...
const guestPath = process.argv[3] ?? 'corpus/threads.temenc';
...
const mp = ex.temen_alloc(guest.length);
new Uint8Array(buf()).set(guest, mp);
const winSize = 1 << 16;
const win = ex.temen_alloc(winSize);
new Uint8Array(buf(), win, winSize).fill(0);

const got = ex.temen_run_shared(mp, guest.length, win, winSize, 0n);
const pass = BigInt(got) === expect;

console.log(`module: ${wasmPath}`);
console.log(`  linear memory: ${buf().byteLength >> 20} MiB  shared=${shared}`);
console.log(`  temen_run_shared(guest over Region::shared window @0x${win.toString(16)}) = ${got}` +
  `  expect ${expect}  ${pass ? 'PASS' : 'FAIL'}`);
console.log(`\n${pass ? 'PASS' : 'FAIL'}: the Temen engine runs over a ${shared ? 'SHARED' : 'private'} ` +
