...
// Event counters, observed from outside: one TIERUP service reads svm_onramp_tierup_func once; a
// JIT_INVOKE reads svm_onramp_tierup_jit_wasm_ptr once; every cross-tier bounce goes through
// svm_onramp_tierup_call_interp.
let forcePump = false;
...
const exCounted = Object.fromEntries(Object.entries(Object.getOwnPropertyDescriptors(ex)).map(([k, d]) => {
  ...
  if (k === 'svm_onramp_tierup_func') return [k, (...a) => { counts.tierups++; return v(...a); }];
  if (k === 'svm_onramp_tierup_jit_wasm_ptr') return [k, (...a) => { counts.invokes++; return v(...a); }];
  if (k === 'svm_onramp_tierup_call_interp') return [k, (...a) => { counts.bounces++; return v(...a); }];
  if (k === 'svm_onramp_jit_run_call_interp') return [k, (...a) => { counts.bounces++; return v(...a); }];
  if (k === 'svm_onramp_jit_run_open')
    return [k, (...a) => {
      if (forcePump) return -2; // pretend "_start not emittable" so runJitModule falls to the pump
      const r = v(...a);
      if (r === 0) counts.path = 'whole-program';
      return r;
    }];
  if (k === 'svm_onramp_tierup_open')
    return [k, (...a) => { const r = v(...a); if (r === 0) counts.path = 'tierup-pump'; return r; }];
  if (k === 'svm_coop_open')
...
for (const [name, rel, workload] of cards) {
  ...
  // The #839 column proper: the tier-up pump forced (what a guest gets when the whole-program
  // emit refuses), same workload.
  forcePump = true;
  try {
    const pcold = await pumpRun(modBytes, stdin, `bench#${name}#pump`);
    let pBest = Infinity, pw;
    for (let i = 0; i < N; i++) {
      const r = await pumpRun(modBytes, stdin, `bench#${name}#pump`);
      if (r.ms < pBest) { pBest = r.ms; pw = r; }
    }
    const pparity = pw.out === b[0].out && pw.st === b[0].st ? 'parity=OK' : 'parity=MISMATCH';
    console.log(
      `${name}: pump-forced cold ${pcold.ms.toFixed(1)}ms, warm best ${pBest.toFixed(1)}ms ` +
      `(${(bBest / pBest).toFixed(2)}x) | path ${pw.path} | tierups ${pw.tierups} invokes ${pw.invokes} bounces ${pw.bounces} | ${pparity}`,
    );
  } catch (e) {
    console.log(`${name}: pump-forced REFUSED/failed (${e.message})`);
  }
  forcePump = false;
}
