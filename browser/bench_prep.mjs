...
// fast-loading demo pays per page load (translation is done at build time). It times the Temen's
// decode + verify + bytecode-compile of a *pre-translated, pre-resolved* `.temen`, running the Temen's
// own Rust compiled to wasm on V8 (the `temen-browser` cdylib's `temen_prep_bench`). Its ratio to the
// native `prep_temen` example is the sandbox tax on loading — the counterpart of `bench.mjs`'s
// interpreter-execution tax. See BOOTSPEED.md.
//
//   node bench_prep.mjs <temen_browser.wasm> <module.temen>
//
...
if (!modPath) {
  console.error('usage: node bench_prep.mjs <temen_browser.wasm> <module.temen>');
  process.exit(2);
}
...
const once = () => {
  ...
  const r = ex.temen_prep_bench(ptr, len);
  const e = performance.now();
  const st = ex.temen_status();
  if (st !== 0) {
    console.error(`temen_prep_bench status ${st} (1=decode 2=unsupported 5=verify)`);
    process.exit(3);
  }
