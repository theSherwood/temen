...
// `warm-snapshot-test.mjs`. Drives the shipping engine FFI (`svm_warm_open`/`svm_warm_eval`) plus the
// warm+JIT path (`runWarmJit`) over the committed `lua_snapshot.svmb` (built from
// `lua_snapshot_harness.c`), and asserts:
//   - warm `eval_run` over the restored snapshot matches the cold `_start` (`svm_run_onramp`) output
...
//
//   node lua-warm-snapshot-test.mjs [svm_browser.wasm] [lua_snapshot.svmb]
//
...
const modPath = process.argv[3] ?? 'web/assets/lua_snapshot.svmb';
...
function warmInterp(js) { const s = put(enc(js)); ex.svm_warm_eval(s.p, s.len); const out = readStdout(); const st = ex.svm_status(); s.free(); return { out, st }; }
