// Live host-import demo: run the host-backed powerbox (`temen_run_live`) and prove the guest's
...
// is `--features live` and instantiated WITH a host `temen_host` import object.
//
...
const guestPath = process.argv[3] ?? 'corpus/live.temenc';
...
const ptr = ex.temen_alloc(N(guest.length));
...
const ret = ex.temen_run_live(ptr, N(guest.length));
console.log('--------------------------------------------------');
ex.temen_dealloc(ptr, N(guest.length));
