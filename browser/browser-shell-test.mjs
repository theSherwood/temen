...
// `temen_browser.wasm` the page runs, fetch `web/assets/shell.temen` + its PATH registry
// (`stage_runner.temen`, `primes.temen`), and drive the `temen_run_shell` entry the card's Run calls —
...
const ASSETS = ['shell.temen', 'stage_runner.temen', 'primes.temen', 'upper.temen'];
...
const res = await page.evaluate(async (script) => {
  ...
  const bytes = await fetchB('./assets/shell.temen');
  // Build the PATH-registry blob temen_run_shell parses: u32 count, then per entry u32 name-len + UTF-8
  // name + u32 module-len + module bytes (little-endian) — the __stage runner and the primes command.
  const reg = [
    { name: '__stage', bytes: await fetchB('./assets/stage_runner.temen') },
    { name: 'primes', bytes: await fetchB('./assets/primes.temen') },
    { name: 'upper', bytes: await fetchB('./assets/upper.temen') },
  ...
  const sp = eng.ex.temen_alloc(stdinBytes.length); new Uint8Array(eng.memory.buffer).set(stdinBytes, sp);
  const gp = eng.ex.temen_alloc(blob.length); new Uint8Array(eng.memory.buffer).set(blob, gp);
  eng.ex.temen_run_shell(mp, bytes.length, sp, stdinBytes.length, gp, blob.length);
  const status = eng.ex.temen_status();
  const stdout = dec(Number(eng.ex.temen_stdout_ptr()), eng.ex.temen_stdout_len());
  eng.ex.temen_dealloc(mp, bytes.length); eng.ex.temen_dealloc(sp, stdinBytes.length);
  eng.ex.temen_dealloc(gp, blob.length);
...
if (ok) { console.log(`✓ shell card: temen_run_shell produced the expected output (${res.stdout.length}B)`); process.exit(0); }
process.exit(1);
