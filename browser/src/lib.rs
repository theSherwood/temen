...
/// **Link a program against a library and hand back the linked program's IR text** (#1392) — the
/// *debugger* path, the twin of [`temen_link_run`]'s run path. A host that wants to **step** a
/// separately-compiled program passes this text to its DAP session (`programText`): the linker now
/// merges both units' debug info, and the text waist carries it, so stepping and variable
/// inspection work across the link. Read the text back through [`temen_stdout_ptr`] /
...
pub extern "C" fn temen_link_text(
    ...
    let Some(lib) = link_load_unit(link_slice(lib_ptr, lib_len)) else {
        return fail(STATUS_DECODE_ERR);
    };
...
/// the powerbox `_start` and verify — everything [`temen_link_run`] does *except* running. Public so
/// a host can hand the linked program to a debugger instead (#1392): the module's IR text
/// ([`temen_text::print_module`]) carries the debug info the linker now merges across units.
///
/// The program's entry is resolved through the unit's **own** export table rather than assumed to be
/// func 0. A chibicc `--emit-object` TU exports `main` *and* its own whole-program `_start`
/// bootstrap, and that bootstrap sits at func 0 — so the func-0 assumption picked it, whose
/// paramless signature then failed `synth_manifest_start`'s "entry takes one i64" rule. That stale
/// `_start` is dropped, too: a *linked* program's entry is the caller's choice, so a unit's own
/// bootstrap export is meaningless after linking and would collide with the `_start` synthesized
/// here. A unit that exports nothing under `entry` keeps the original convention (func 0).
pub fn link_program(
...
/// [`link_program`] + run, over two already-decoded units — the native core the browser FFI and the
/// native tests share.
pub fn link_run_units(
    lib: &temen_ir::Module,
    program: &temen_ir::Module,
    entry: &str,
    stdin: &[u8],
...
/// The `i`-th param the emitted `f{entry}` takes after `(win, env)` (`0` if out of range / no run),
/// as an **i64** so the JS driver passes it as a `BigInt` — a §14 child-entry's data-stack pointer is
/// `i64` (`child_entry_ok`). A paramless `_start` has no slots; the legacy i32 cap-handle form fits i64.
#[no_mangle]
pub extern "C" fn temen_onramp_jit_run_slot(i: usize) -> i64 {
...
/// child module it spawns and the state the step/deliver protocol threads. The program is leaked so the
/// vCPU can borrow it `'static` (reclaimed on close), mirroring [`PgSession`].
struct Op13JitDriver {
    prog: *mut bytecode::VcpuProgram,
    root: bytecode::Vcpu<'static>,
...
/// Shared counter the mini-driver's granted `"fs"` ticks (observable proof the *marshaled* cap ran inside
/// the emitted child). Set at `_open`, read at [`temen_op13jit_counter`].
...
fn op13jit_open_driver(driver: temen_ir::Module, child: temen_ir::Module, minter: bool) -> i32 {
    ...
    // The granted "fs": a forkable HOST_PROC counter (the re-grantable shape a shared fs takes).
    let counter = std::sync::Arc::new(std::sync::Mutex::new(0i64));
    let c1 = std::sync::Arc::clone(&counter);
    let handler: temen_interp::HostProc = Box::new(move |_op, _a, _m, _| {
    ...
    // SAFETY: non-zero 8-aligned layout; owned until `temen_op13jit_close` deallocs it, after the root
    // vCPU + any child run views are dropped. Linear-memory backed, so the emitted child can access it.
    ...
    // SAFETY: `prog` outlives the vCPU (freed only in `close`, after the vCPU is dropped).
    ...
    unsafe {
        *core::ptr::addr_of_mut!(OP13_JIT_COUNTER) = Some(counter);
...
pub extern "C" fn temen_op13jit_step() -> i32 {
    ...
    loop {
        match d.root.run() {
            bytecode::VcpuEvent::Done(v) => {
                d.result = v.first().map_or(0, |x| match x {
                    Value::I64(n) => *n,
...
#[no_mangle]
pub extern "C" fn temen_op13jit_counter() -> i64 {
    unsafe {
        (*core::ptr::addr_of!(OP13_JIT_COUNTER))
            .as_ref()
            .map_or(0, |c| *c.lock().unwrap())
    }
}

/// Close the op-13 loop: drop the driver (its root vCPU, then its window + leaked program) and the run.
/// Idempotent.
#[no_mangle]
pub extern "C" fn temen_op13jit_close() {
    // SAFETY: single-threaded wasm; drop the run + driver in order (vCPU before its program/window).
    unsafe {
        *core::ptr::addr_of_mut!(JIT_RUN) = None;
        if let Some(d) = (*core::ptr::addr_of_mut!(OP13_JIT)).take() {
            drop(d.root);
            std::alloc::dealloc(d.mem_base, d.layout);
            drop(Box::from_raw(d.prog));
        }
        *core::ptr::addr_of_mut!(OP13_JIT_COUNTER) = None;
    }
}
...
// subset, and slower).
//
// An `InterpDriven` guest — it `vm_map`s, streams, hosts fibers, `thread.spawn`s — runs here: this
// driver wraps [`bytecode::CoopRun`], the cooperative multi-vCPU scheduler, so the interpreter
// owns `_start` and multiplexes every vCPU of the run on **one** wasm thread, no Workers. `CoopRun`
// owns its `Domain`/window/powerbox and services `Spawn`/`Join`/`Wait`/`Notify`/fibers/
// `Jit.install`/`Jit.invoke` **internally**, so the only events that reach the host are tier-up, a
// §22 `Jit.invoke` of an emitted unit, and the run's end. The cross-tier `call_interp` bounce is
...
fn coop_emit_for(m0: &temen_ir::Module, shared: bool, win_log2: u8) -> Result<CoopEmit, i32> {
    ...
    // whole function to the interpreter (the #888 widened fixpoint serves each wrapper as a
    // marshallable cross-tier callee over the live bounce). Mutates the module BOTH tiers use, so
    ...
    // #1009: a rodata guest (a `readonly` data segment — every real on-ramp card lays out its
...
// ---- #926 slice 2f: the B2 driver-table accessors — the set the JS host rebuilds its shared
...
"#;
    ...
    #[test]
    fn op13jit_loop_drives_a_nested_emitted_child_end_to_end() {
        // The JS-orchestrated op-13 loop's engine side, end-to-end: the driver marshals `fs` and stages the
        // child; here we substitute JS's `driveJitRun` with the interpreter oracle (`run_cross_tier(0)` over
        // the *staged* run — identical value to the emitted `f0` by tier parity, INVARIANTS §9), then
        // `_deliver` + resume. The driver returns the child's join result (41) and the marshaled fs ticked.
        assert_eq!(temen_op13jit_open(), STATUS_OK);
        let mut guard = 0;
        loop {
            guard += 1;
            ...
            match temen_op13jit_step() {
                ...
                OP13JIT_CHILD => {
                    ...
                    let v = unsafe { (*core::ptr::addr_of_mut!(JIT_RUN)).as_mut() }
                        .expect("child staged")
                        .run_cross_tier(0, &[Value::I64(0)])
                        .expect("child f0 runs over the marshaled host");
                    unsafe {
                        RUN_VALUE = match v.first() {
                            Some(Value::I64(n)) => *n,
                            _ => panic!("child value"),
                        }
                    };
                    assert_eq!(temen_op13jit_deliver(), STATUS_OK);
                }
                other => panic!("unexpected step code {other}"),
            }
        }
        assert_eq!(
            temen_op13jit_result(),
            41,
            "the driver returned the nested child's join result (40 + marshaled fs = 41)"
        );
        assert_eq!(
