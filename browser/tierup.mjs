...
// `Vcpu` FFI (`temen_par_run` → `PAR_TIERUP` → run the emitted `f{func}` → `temen_par_deliver_tierup`)
// exactly as `worker.js` does, but on one thread so it runs fast under Node. Asserts the tier-up run
// is byte-identical to the pure bytecode interpreter (`temen_run`) for the same guest — the same oracle
...
// func 0 sums `f1(i)` over 0..5 (bound baked in — `temen_par_root` passes no args); func 1 is the
...
async function main() {
  ...
  if (ex.__tls_size.value > 0) ex.__wasm_init_tls(Number(ex.temen_par_alloc(ex.__tls_size.value + ex.__tls_align.value)));
  ...
  const toGuest = (src) => {
    ...
    const g = u8().slice(Number(ex.temen_parse_ptr()), Number(ex.temen_parse_ptr()) + ex.temen_parse_len());
    const gptr = Number(ex.temen_par_alloc(g.length));
    u8().set(g, gptr);
  ...
  const buildDriver = async ({ gptr, glen }) => {
    if (ex.temen_par_enable_jit(gptr, glen) !== 1) throw new Error('temen_par_enable_jit rejected the guest');
  ...
  const runTierUp = async ({ gptr, glen }) => {
    ...
    const prog = ex.temen_par_compile(gptr, glen);
    if (prog === 0) throw new Error('temen_par_compile null');
    ex.temen_par_powerbox_none();
    const win = Number(ex.temen_par_alloc(winSize));
    const v = ex.temen_par_root(prog, win, winSize, 0);
    if (v === 0) throw new Error('temen_par_root null');
    let tierups = 0;
    for (;;) {
      const evc = ex.temen_par_run(v);
      if (evc === PAR_DONE) { const r = ex.temen_par_ev_a(v); ex.temen_par_free(v); return { value: r, tierups }; }
      if (evc === PAR_TRAP) { ex.temen_par_free(v); return { trap: true, tierups }; }
      if (evc === PAR_TIERUP) {
        tierups++;
        const func = Number(ex.temen_par_ev_a(v));
        const argvPtr = Number(ex.temen_par_tierup_argv_ptr(v)), an = Number(ex.temen_par_tierup_argv_len(v));
        const args = [];
  ...
  const g = toGuest(SRC);
  const oracle = ex.temen_run(g.gptr, g.glen, 0n);
