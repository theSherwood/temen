...
fn main() {
    for path in std::env::args().skip(1) {
        ...
        let m = svm_encode::decode_module(&bytes).expect("decode");
        let n = m.funcs.len();
        ...
        let tally = |m: &svm_ir::Module| -> (usize, usize) {
            let mut em = m.clone();
            ...
            let r = if all_shimmable {
                svm_wasm_jit::compile_module_tierup_b2(&em, false, 10)
            } else {
                svm_wasm_jit::compile_module_tierup(&em, false)
            };
            match r {
        ...
        let mut om = m.clone();
        svm_wasm_jit::outline_cap_calls(&mut om);
