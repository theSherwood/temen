...
//! export) — STAGE1.md "real posix shell in the playground", slice 3. The committed fixture
//! `fixtures/shell.svmb` is the *same* shell the differential `crates/svm/tests/c_shell.rs` runs
//! (`shim.c + ring.c + shell_main.c`, chibicc-compiled, imports resolved by name), regenerated with
...
//! in the personality's captured stdout. Run on the tree-walk interpreter (the shell carries
//! `Instantiator` cap.calls the bytecode reserved-window engine refuses); this is the exact module the
//! native differential exercises, so a match here is the shell itself running in the browser engine.
