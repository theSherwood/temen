...
//! (`temen_pg_open` / `temen_pg_query` / `temen_pg_snapshot` / `temen_pg_close`) against the real 20 MB Postgres
...
//!   3. `temen_pg_snapshot` the live data dir to an image, then `temen_pg_close`,
//!   4. `temen_pg_open` a **fresh** backend from that snapshot image,
...
/// Read back the stdout the last `temen_pg_*` call captured (the cdylib-managed `OUT` buffer).
fn engine_stdout() -> String {
    let ptr = temen_browser::temen_stdout_ptr();
    let len = temen_browser::temen_stdout_len();
    if ptr.is_null() || len == 0 {
        return String::new();
    }
    // SAFETY: `temen_stdout_ptr/_len` name a live cdylib allocation valid until the next capturing call.
    let bytes = unsafe { std::slice::from_raw_parts(ptr, len) };
    String::from_utf8_lossy(bytes).into_owned()
}

fn open(module: &[u8], image: &[u8]) -> i32 {
    temen_browser::temen_pg_open(module.as_ptr(), module.len(), image.as_ptr(), image.len())
}

fn query(sql: &str) -> (i32, String) {
    let rc = temen_browser::temen_pg_query(sql.as_ptr(), sql.len());
    (rc, engine_stdout())
}
...
fn session_survives_snapshot_and_reboot() {
    let module = asset("postgres_resolved.temen");
    ...
    assert_eq!(temen_browser::temen_pg_snapshot(), 0, "snapshot failed");
    let sptr = temen_browser::temen_pg_snapshot_ptr();
    let slen = temen_browser::temen_pg_snapshot_len();
