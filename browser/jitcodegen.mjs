...
async function main() {
  ...
  const prove = async (name, guestSrc, serviceKind) => {
    ...
    ex.temen_par_jit_codegen_service(serviceKind); // 0 = i32 service, 1 = f64 service
    if (ex.temen_par_powerbox_jit_codegen(gptr, glen) !== 1) throw new Error(`${name}: powerbox failed`);
    ...
    const run = (codegen) => {
      ...
      const prog = ex.temen_par_compile_jit(gptr, glen);
