...
async function timeCoop(stdin, label) {
  const t0 = performance.now();
  const status = await runJitModule(ex, memory, guest, stdin, label);
  return { dt: performance.now() - t0, status };
}

// Min of N samples: the floor is the signal, the tail is scheduler/GC noise.
async function best(stdin, label, reps) {
  let interp = Infinity, coop = Infinity, ok = true;
...
for (const [label, program, reps] of rows) {
  const { interp, coop, ok } = await best(enc.encode(program), label.replace(/\W+/g, '-'), reps);
  if (!ok) { console.log(`  ${label.padEnd(32)} FAILED (see above)`); continue; }
  const speedup = interp / coop;
  console.log(`  ${label.padEnd(32)} ${interp.toFixed(1).padStart(10)} ms  ${coop.toFixed(1).padStart(13)} ms  ${speedup.toFixed(2).padStart(7)}x`);
}
