// V8 (Node) driver: **boot PostgreSQL --single inside wasm** and measure it. Loads the Temen bytecode
// engine compiled to wasm (the `temen-browser` cdylib), the pre-translated+resolved Postgres module, and
...
//
//   node bench_pg.mjs <temen_browser.wasm> <postgres_resolved.temen> <pgdata.img> [sql-file]
...
if (!imgPath) {
  console.error('usage: node bench_pg.mjs <temen_browser.wasm> <module.temen> <pgdata.img> [sql-file]');
  process.exit(2);
}
const sql = sqlPath
...
const modPtr = ex.temen_alloc(N(modBytes.length));
const imgPtr = ex.temen_alloc(N(imgBytes.length));
const inPtr = ex.temen_alloc(N(sql.length));
...
const rv = ex.temen_run_pg(modPtr, N(modBytes.length), imgPtr, N(imgBytes.length), inPtr, N(sql.length));
const ms = performance.now() - t0;

const status = ex.temen_status();
const exit = ex.temen_exit_code();
const outLen = Number(ex.temen_stdout_len());
const outPtr = Number(ex.temen_stdout_ptr());
