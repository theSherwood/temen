// Browser wasm-JIT tier — the JS linker (BROWSER.md § "wasm-JIT tier", slice 2). Given the Temen
// browser cdylib instance and an encoded Temen module, this: (1) asks the cdylib to emit a
// WebAssembly module for it (`temen_wasmjit_compile`), (2) instantiates the emitted bytes against the
// **cdylib's own linear memory** (so an `temen_alloc`ed window + env cell are addressable in both),
...
// Compile `moduleBytes` (an encoded Temen module, already in a JS Uint8Array) to a callable JIT
...
export async function compileJit(ex, moduleBytes, { memory = null } = {}) {
  ...
  let lastTrap = 0;
  const envBytes = ex.temen_wasmjit_env_bytes(); // fuel counter + cross-tier scratch
  ...
  const instance = await WebAssembly.instantiate(module, {
    env: {
      ...
      // hit an interp leaf. Re-enter the engine in-Rust (`temen_wasmjit_call_interp` runs the leaf on
  ...
  return {
    // `args` must already be correctly typed for the Temen entry's params: a JS Number for an i32
    ...
    call(args = [], { fuel = DEFAULT_FUEL, winSize = 1 << 16 } = {}) {
      ...
      const win = Number(ex.temen_alloc(winSize));
      ex.temen_wasmjit_init_window(win, winSize); // lay the module's data segments into the window
