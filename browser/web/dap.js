...
// DAP-over-bytecode path. The cdylib hosts the real `temen-dap` server (selecting the bytecode backend);
...
//
// `createDapClient(ex, memory)` starts a fresh session (`temen_dap_reset`) and returns `{ send }`.
// `send(command, args)` returns `{ response, events, all }` for the one request.
export function createDapClient(ex, memory) {
  let seq = 0;
  const enc = new TextEncoder();
  const dec = new TextDecoder();
  ex.temen_dap_reset(); // begin a clean session
  ...
  const send = (command, args = {}) => {
    ...
    const rc = ex.temen_dap_request(p, bytes.length);
    ex.temen_dealloc(p, bytes.length);
    if (rc !== 0) throw new Error(`DAP request rejected (bad JSON): ${command}`);
    const rp = ex.temen_dap_response_ptr();
    const rl = ex.temen_dap_response_len();
