...
// reactor (`temen_onramp_*`) runs the guest's `tick` on the bytecode engine each frame; this runs the
// guest's **whole `tick` on emitted wasm**, near-natively. The cdylib opens the reactor
// (`temen_onramp_jit_open_fs`: decode, enlarge the window, run `_start` on the interpreter, emit the
...
// their `env.call_interp` back to `temen_onramp_jit_call_interp`, which runs them on the interpreter over
...
export async function openJitReactor(ex, memory, moduleBytes, wadName, wad) {
  ...
  if (wad) {
    ...
    const wadP = Number(ex.temen_alloc(wad.length));
    {
      const v = u8();
    ...
    opened = ex.temen_onramp_jit_open_fs(modP, moduleBytes.length, nameP, nameBytes.length, wadP, wad.length);
    ex.temen_dealloc(modP, moduleBytes.length);
    ex.temen_dealloc(nameP, nameBytes.length);
    ex.temen_dealloc(wadP, wad.length);
  } else {
    const modP = Number(ex.temen_alloc(moduleBytes.length));
    u8().set(moduleBytes, modP);
    opened = ex.temen_onramp_jit_open(modP, moduleBytes.length);
    ex.temen_dealloc(modP, moduleBytes.length);
  }
  if (opened !== 0) {
  ...
  const wptr = Number(ex.temen_onramp_jit_wasm_ptr());
  const wlen = ex.temen_onramp_jit_wasm_len();
  const emitted = u8().slice(wptr, wptr + wlen);
  const win = Number(ex.temen_onramp_jit_win_ptr());
  const entrySp = ex.temen_onramp_jit_entry_sp();
  const tick = ex.temen_onramp_jit_tick();
  const envBytes = ex.temen_onramp_jit_env_bytes();
  ...
  const instance = await WebAssembly.instantiate(module, {
    env: {
      ...
      call_interp: (func, argsPtr) => {
        const s = ex.temen_onramp_jit_call_interp(func, argsPtr);
  ...
  return {
    ...
    frame() {
      ...
      ex.temen_onramp_jit_present();
      return status;
    },
    close() {
      ex.temen_dealloc(env, envBytes);
      ex.temen_onramp_jit_close();
    },
  };
}
