...
// --target=wasm -msimd128), and Temen IR (clang -O2 -emit-llvm -> temen-llvm; vectorization ON, the
...
// survives the LLVM->Temen on-ramp (which rejects inline asm); i32 throughout so JS can match via
...
// linear-congruential recurrence (4 unrolled steps -> one multiply by M^4), which temen-jit/Cranelift
// doesn't, so it reads ~8x native. It is the ONLY kernel where temen-jit trails native; `xorshift`
// below is the representative scalar-throughput kernel (no recurrence to collapse -> temen-jit ~= native).
...
// on (for `vadd`), clang would otherwise turn these fills into vector stores the temen-llvm→temen-jit path
...
// AVX, wasm SIMD, and temen-jit via the on-ramp's v128) collapse it to vector adds; the interpreters stay
// scalar. (Replaces the old `vsum`, whose known-content array let Cranelift fold the loop to a bogus ~0
// on temen-jit and read out of bounds at large n.)
