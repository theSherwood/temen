...
### 3.2 Provider-side exports (`impl` exports) and wiring  [SUPERSEDED — folded]

*The v1 offer machinery this section designed (landed 2026-07-20) was rebuilt as the unified
...
### 3.6 The unified execution model — one world per domain  [SUPERSEDED — built via CALLS]

*This section's end-state design (one world per domain; handler fibers over `main`'s own
window; `svc.wait`/`svc.poll` service points; the serve queue) was built by the CALLS.md
increments and is settled as DESIGN.md §12a — provider taxonomy, admission, direct handoff,
promotion, caller-pays fuel. The design text lives in git history with the tracker. The one
deliberate divergence from the end state sketched here: library instances keep the animated
transport rather than dissolving into serve-loop-driven domains — `OFFER_TRANSPORT.md`
(owner, 2026-08-04) records why.*
