...
> Status: **`std` I/O + args + stderr + time + env + fs + net + process + threads + HashMap work on svm.**
> (S0 + S1a–e + S2 time/env + S3 fs + S4 HashMap; `std::{net, process::Command}` and the full
> `std::thread`/sync/TLS surface landed via #775 and the #779 threads epic. `env::var`/`vars`
> unblocked — #755 fixed. Deferred: unwinding/`panic=unwind` (needs the on-ramp EH substrate, #883).)
> _Historical S0–S2 detail below; the table in §8 is the live per-slice status._
>
> Status (original, 2026-08-11): **`std` I/O + args + stderr + time + env work on svm (S0 + S1a–e + S2 time/env).**
...
| **S4** | ✅ **DONE.** `HashMap` end-to-end. `RandomState` reaches the base `sys/random` leaf (the `unsupported` arm the overlay routes `os=svm` into), whose `hashmap_random_keys` derives its seed from allocation addresses — **deterministic per guest** on svm (the window/bump-allocator layout is fixed), which is exactly the §5 pin's intent (per-guest determinism the differential harness wants); no host RNG op, no `fill_bytes` call. Surfaced + fixed a general on-ramp gap on the way: `llvm.bitreverse.iN` for **non-power-of-2** `N` (core's `fmt` placeholder packing emits an `i3` bitreverse, so `format!`→`String` tripped it) — `emit_bitreverse` now reverses the full power-of-2 container and shifts the reversed low-`N` bits down (`intrinsic_coverage.rs::non_power_of_two_bitreverse_lowers`). Test: `std_hashmap_round_trips` (order-independent aggregates → byte-identical to native despite differing seeds). A real fixed-seed / non-panicking `fill_bytes` svm random leaf is a hardening follow-up only if a consumer needs `getrandom`. | base `sys/random` leaf; on-ramp bitreverse fix | Retires the `NIM.md` §3e `RandomState` blocker; a `HashMap`-heavy program byte-identical to native. |
