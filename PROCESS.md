...
1. **Canonical-key futex** (was S9): key `Backed` pages on `(backing, region_off)` on both
   ...
   a process-global `abs_page → (backing_fd, region_off)` registry (`svm-run` records a §13 `map`'s
