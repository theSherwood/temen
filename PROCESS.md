...
- **The capability seam is open by construction** (§7/D46): interfaces are data + host
  code, bound by name at instantiation; `self.*` reflection is an always-available,
...
1. **Children are born destitute** — powerbox = `Instantiator` + `AddressSpace` only;
   *"pass-through of the parent's other handles is a follow-up"* (`temen-interp`,
...
grant(domain, name_ptr, name_len, handle) -> 0 | -errno    (pre-start only; re-grants MY handle
                                                             ...
                                                             child finds it via self.resolve)
...
- **Revocation-unparks — the guest-side cancel, pinned (2026-07-22, jacl).** The
  ...
  **[BUILT 2026-07-22 — §3.6 slice 1, reference interpreter.]** `Stream.close` is real
  ...
  `temen-interp/tests/revocation_unparks.rs`. Slot-parked (`call.import`) calls, their
...
code and pinned by `crates/temen/tests/futex_cross_domain.rs`:
...
### S9 rescope after §3.6 (2026-07-23) — most of Endpoint v1 already exists

The §3.6 unified-execution build (IMPORTS.md §3.6 as-built, slices 1–5b +
separate-module children) constructed, under other names, most of what this
section proposed. Reconciliation, so S9 starts from what is rather than from
the 2026-era sketch:

| S9 proposal | what §3.6 built | status |
|---|---|---|
| `mint(sig) -> (serve_end, client_template)` | `Instantiator.child_offer` (op 14): a client cap over a **running child's** impl-export, shape from the callee's module, structurally interned (D59) | built for the child topology; **guest-side self-mint is the gap** (see below) |
| `serve(serve_end, buf)` — park until a call arrives | `svc.wait` (`call.cap CAP_SELF 10`): parks the domain on its bounded FIFO inbound queue; a caller's enqueue wakes it | built — serve is a *service point*, not a per-endpoint object: one queue per domain, offers multiplexed by `(export, op)` |
| `reply(serve_end, caller, result)` | the handler's **return value** (`cap_reply` wakes the ticket-parked caller; an unclaimed result rides the completion cell) | built — reply is implicit in handler completion; a parked handler is a completed-but-not-replied dispatch (5b), so slow replies don't block the domain |
| client `call.cap` parks its fiber, indistinguishable from a host cap | caller-side parking (slice 3): enqueue + `Blocked::CapReply`, fiber-level since 5a; the slot route (`import.attach` + `call.import`) rides it too | built |
| kill-safe cancel + revocation-unparks | slice 1 + rebind revocation (slice 4); death-is-revocation (D37) covers a dead servicer | built |
| FIFO rendezvous, fixed | the svc queue is FIFO by construction; woken parked handlers resume before new admissions (5b) | built |
| sibling data plane: `SharedRegion` + canonical-key futex | S1b (both backends; per-domain-handle caveat folded into the S1c residue) | built |
| budget: the servicer pays | handlers run on the serving domain's own fuel | built |

**What S9 v1 still is, then — two items, both small next to what's done:**
...
2. **The serve/reply *split* question**: the proposal's explicit
   ...
   — handler-per-dispatch, parked handlers don't block the loop — which is
   simpler and already covers slow replies. **Recommendation: drop explicit
   serve/reply from v1** (YAGNI until a personality demonstrably needs
   zero-fiber multiplexing); revisit only with benchmark evidence.

**Answered 2026-07-23 (owner):**

1. **No self-mint, no peer transfer channel.** A domain must not discover or
   coordinate with arbitrary other domains; every capability transfer stays
   **mediated by an authority in the grant graph** — which is exactly the
   built machinery (spawn grants, `child_offer` + regrant). Self-mint item 1
   above is *dropped*, not deferred. The one residue that survives is
   mediation-consistent: a domain minting an offer over its **own** export to
   grant *down its own grant graph* (a parent serving its children — what
   guest-served exec needs); the granter has authority over the grantee, so
   no new arrow appears.
2. **"Endpoint" is retired as a concept name.** The consumer wants low-level
   substrate primitives, and the substrate's real nouns are what stays: live
   offers, service points (`svc.wait`/`svc.poll`), handler returns. The
   recommendation to drop the explicit `serve`/`reply` split is **confirmed**.

**Serving-substrate review (same day):** a critical walkthrough recorded seven
issues — verdict: the design holds (it is the actor model: domain = actor, svc
queue = mailbox, one world = actor state), but ISSUES.md **I36** (the
`module_serves` fold drops a serving module's whole program to the tree-walk
oracle) is promoted as not-acceptable, and **I37–I40** record the blast-radius,
fairness, serialization-ceiling, and reply-orphan items (two more were already
tracked: fiber-level parks, durability × serving).
...
|---|---|---|
| host `call.cap` | 1 trampoline, **0 fiber switches** | the §1a fast path; a host handler runs on the caller's stack |
...
a host `call.cap` (top row) — you only pay the endpoint RTT for capabilities a parent
*chooses* to interpose. Real measured figures (endpoint RTT vs `call.cap` vs syscall, all
...
across repeated same-slot spawns. Built and pinned by `crates/temen/tests/jit_instantiate_cache.rs`:
the same `(module, entry, size)` spawned at two different offsets JIT-compiles **once**
(`temen_jit::child_compiles()` advances by 1), and each spawn still runs correctly confined
...
1. **Canonical-key futex** (was S9): key `Backed` pages on `(backing, region_off)` on both
   backends — **done** (S1b). Interp keys via `PageProt::Backed`; the JIT canonicalizes through
   a process-global `abs_page → (backing_fd, region_off)` registry (`temen-run` records a §13 `map`'s
   pages, the JIT futex thunks read it) — `jit_futex_region_canonical.rs` pins two aliases of one
   region rendezvousing on the JIT. Unblocks the comm plane for siblings *and* JIT concurrency.
...
- **Detached**: a window minted by a **platform window-minter capability** — an ordinary
  ...
  *read*, not lifecycle: the child's guarantee is "my memory is private and `attest`
  ...
  detached 1), quota exhaustion, forged-budget refusal. **Folded into `Budget.mem` (R2, done):**
  the standalone `WindowMinter` capability and its side table are **retired** — op 15's arg 0 is a
...
## 6. `self.attest` — the trust anchor  [PROPOSED — amends one §14 sentence]
...
- **Read-only report, no negotiation.** A domain that dislikes the answer exits before
  touching secrets. Fits D46's `self.*` contract exactly: reflection confers nothing,
...
- **The friction, named**: §14 currently says *"There is no 'am I nested?' query by
  ...
  (`self.provenance(handle)` — platform- vs ancestor-terminated + depth, IMPORTS.md
...
(`call.cap` is precisely the `temen-durable` transform's suspension unit). So:
...
- **Cost/coverage, honestly**: v1 clone is a full window copy (CoW rides `DURABILITY.md`
  ...
  today treats `call.dyn` to may-suspend targets as out of scope (R8) — Bash
...
- **§7/D46**: every substrate object is an ordinary open-set capability interface —
  signature in the type section, verifier-checked `call.cap`, named binding, fail-closed.
...
- **pipes**: a channel capability *of the personality* — either a host-served
  stream-pair cap (temen-run library, park-on-empty/full via endpoint semantics) or a
...
- **L1 — interruptible parks (small runtime slice). BUILT (#799) for blocking pipe parks,
  disposition-gated.** A parked call wakes with an interrupted status instead of its result; the
  libc runs pending handlers and re-issues, or returns `EINTR`. **Built the faithful way:** the
  interrupt is driven from the **L2 safepoint** — when the personality hands the interp a
  *delivery* (`SignalSource::take_deliverable` returns `Some`, i.e. a **caught + unmasked** signal),
  the core also `interrupt_interruptible_parks()`, re-admitting every parked interruptible syscall
  (`Blocked::PipeRead`/`PipeWrite`) with a `-EINTR` (a value, invariant 5). The **disposition gate
  is free and lives entirely in the personality**: an ignored (`SIG_IGN`) or masked signal yields
  `None` and never interrupts. Nothing signal-specific lives in temen — the guest raises via the
...
- **L2 — safepoint-injected handlers (rides existing polls). BUILT (#796).** At the per-op
  ...
  handler `void handler(int)` — pushed like a `call.dyn` on a guest-registered signal
  ...
  The seam is a `SignalSource` trait the interp calls (mechanism) and temen-posix implements
...
blocked capability pipe read/write, an ignored/masked one does not, and temen holds no signal policy;
...
| S1b | Canonical-key futex — key `Backed` pages on `(backing, region_off)` | — | **done, both backends.** Interp: `FutexKey::{Anon,Region}` keyed via `PageProt::Backed` (`futex_region_canonical.rs`, negative-checked). **JIT half now landed** (`jit_futex_region_canonical.rs`): the JIT futex thunks (`thread_wait`/`thread_notify`) canonicalize an address through a process-global `abs_page → (backing_fd, region_off)` registry — recorded by the `Host` region hook `temen-run` installs at a §13 `map` (it owns the backing's `os_fd`; the trampoline knows `mem_base`), forgotten on `unmap` + purged at run teardown — so two aliases of one region at different window offsets produce the same `FutexKey` and rendezvous. Not a confinement-lowering change (a futex key is a rendezvous coordinate; a wrong key misses a wakeup, it can't escape). **Caveat for true sibling domains:** the interp's `FutexKey::Region` first field is still the *per-domain handle id* (canonical within a domain / for the JIT's one-window aliasing); cross-**domain** siblings need the backing-interned identity (the JIT already uses `os_fd`) — folded into S1c when concurrent separate-window children exist |
| S1c | OS-thread children on the JIT: `instantiate` spawns the cached child in its **own guarded window** (keeps the trailing guard — in-place would break carve isolation), `join` parks on the completion cell; live channel is a granted `SharedRegion`, not the copied carve | S1b | **done — async OS-thread executor landed.** A non-durable op-0/5 child now runs on its **own OS thread** in its own guarded window: `instantiate` compiles once (cached `Arc<ChildCode>`, `Send+Sync`), spawns `run_child_code` on a `std::thread` that arms `mem::install_guard` and publishes `(result, trap)` into a `ChildDone` cell, then returns the join slot immediately. `join` parks on the cell (bounded re-check so the §5 kill-path still unwinds a waiter); `poll` reports the live cell state (0 running / 1 returned / 2 trapped); every child thread is joined at teardown (`Nursery::join_children`, before the window frees, mirroring the vCPU `Domain::join_all`). Blast radius stays narrow: durable + op-8/11/13 children remain synchronous. Confinement lowering (D38) untouched — a wrong spawn/wakeup misses a rendezvous, never escapes. **Deterministic concurrency proof without a live channel:** `jit_poll_observes_a_concurrently_running_child` — the child runs a long bounded loop, the parent's first `poll` right after `instantiate` sees it running (0), spins to done (1), returns `0*10+1=1`; a synchronous impl would return `11`. This distinguishes async from sync using only op-0/5, so the (a) `SharedRegion` live-channel rendezvous is **no longer gating S1c** — deferred to the two-children pipeline slice along with (b) per-child targeted kill (the confinement-codegen kill point). Stress-verified 20/20. **Prior scoping finding** (kept for the deferred pieces): a `SharedRegion` live channel needs deep PAL aliasing (Linux `memfd` / Windows `MapViewOfFile3`) into a separate child window; per-child kill collides with the S1a compile cache unless the §5 kill-path codegen reads the interrupt pointer at runtime (a TCB-sensitive confinement-lowering change). **2026-07-23 update — the live-channel piece landed** (STAGE1.md item 6 JIT-pipeline BUILT block): the aliasing PAL already existed (`MprotectWindow::map_region` maps a region fd into *any* `GuestWindow`, incl. a child's), so the real work was async granted children (ops 8/11/13 on the S1c OS-thread path, powerbox released via the `run_child_code_then` teardown before the window VA frees) + child `atomic.wait`/`notify` compiled against the parent domain's shared futex (a nesting run always stands up the `Domain` now, futex-only if the module has no thread ops). Only per-child targeted kill remains deferred |
| S2 | Grant capabilities into a child's powerbox ("children born destitute" fix). **Done (interp): (a)** `instantiate_granted` (op 8) — single coordinate-free cap (`Stream`/`Exit`/`Clock`) as the child's 3rd entry arg (`instantiate_granted.rs`); **(b)** `instantiate_named` (op 11) — a **multi-cap grant list** (`grants_n` × 16-byte `{name_off,name_len,handle,flags}` records) re-granted **by name**, child discovers via `self.resolve` (`instantiate_named.rs`). stdout/stderr grants share the parent's sink (stdio inheritance); non-copyable caps refused (`CapFault`). **JIT parity (ops 8 & 11):** `instantiate_granted` (op 8, single positional cap) and `instantiate_named` (op 11, multi-cap by name) on the JIT — the child powerbox `Host` is built host-side by `temen_run::grant_child_build` / `grant_named_child_build` (freed by `grant_child_release`; the JIT keeps the `Host` opaque, the same seam as `ModuleResolver`) via the shared `Host::spawn_granted_child` / `spawn_named_child`, so both backends hand the child identical handles + shared stdout/stderr sinks; op 11's grant records are read from the parent window host-side (out-of-window ⇒ `MemoryFault`, non-UTF-8/non-copyable ⇒ `CapFault`), and the named child finds its caps by `self.resolve` (lowered to the run's `call.cap` thunk with the child host as ctx). Granted children run synchronously, uncached (per-spawn child ctx), non-nesting (`InstEnv::null`). Differential-tested: `jit_instantiate_granted.rs` (`granted_child_writes_stdout_matches_interp`, `non_copyable_grant_capfaults_on_both`), `jit_instantiate_named.rs` (`named_child_resolves_two_grants_matches_interp`). Remaining: create-suspended/start split + teardown/refcount + recursive nesting of a granted child (rides JIT async children, S1c) | — | **grant list + names done (interp); ops 8 & 11 JIT parity done** |
| S3 | Lifecycle: `poll`/`kill`/`detach` (+ per-child kill cell on JIT) | S1 | **poll + detach + kill done, both backends** (`Instantiator` ops 9/10/12). Interp: `poll` non-destructive 0/1/2 probe, `detach` drops the join claim, `kill` sets a per-§14-child `Arc<AtomicBool>` polled at the per-op `step` — the child + inherited-flag `thread.spawn` descendants trap (`ThreadFault` → `poll` 2), `lifecycle_kill.rs` (negative-checked). **JIT parity** (`jit_lifecycle.rs`): thunks reading the `children` table. With S1c async children an op-0/5 child now reports the live state — `poll` **0 (running)** while its thread runs, then 1/2; `kill`/`detach` are harmless 0 (a running async child is reached only by the run-wide §5 kill-path today). `poll`'s *running* (0) parity is proven by `jit_poll_observes_a_concurrently_running_child`; `kill_detach` matches interp differentially. Follow-ups: prompt-wake of a **parked** interp child, deep-§14 subtree kill, and **per-child targeted kill** of a running async JIT child (the confinement-codegen kill point, deferred with the S1c live channel) |
| S4 | fs dir ops (**done** — landed with the Postgres `initdb` work: `FS_STAT`/`MKDIR`/`RMDIR`/`OPENDIR`/`READDIR`/`CLOSEDIR`); POSIX personality lib: fd table, **host-served** pipe, proc ABI | S2 | fs ops done. **Host-served pipe done** (`Host::grant_pipe() -> (write, read)`): one FIFO (`Host::pipes`), both ends granted as `Stream`-typed handles (a pipe end *is* a stream — read/write/close), so it dispatches through the generic `call.cap` → `Host` path and **JIT parity is free** — differential `pipe.rs` (`write_then_read` byte flow; `empty_read` non-blocking `0`; wrong-direction `-EINVAL`). Index-carrying ⇒ non-copyable; durable when the frozen tree holds every end (#1680), otherwise `NonDurableKind::Pipe`. **Cross-domain grant done:** the FIFO is an `Arc<Mutex<VecDeque>>` (`PipeBacking`), so a pipe end **re-grants into a §14 child** — the child's `Host` aliases the same queue. Both the interp op-8/11 path and the JIT's `temen_run::grant_child_build` route through one shared `Host::regrant_into_child` (pipe end → alias backing; coordinate-free cap → copy/sink-share), so a parent handing a child its write end and reading the child's bytes back is a cross-backend differential (`pipe_cross_domain.rs`). Remaining: fd table + proc ABI (personality lib, guest-side) |
| S5 | `Budget` split/read; detached window minter behind a granted authority | S2 | **`Budget` split/read done (both backends)** — iface 14, a passable/splittable `(fuel, mem, spawn)` quota vector (`Binding::Budget` → `Host::budgets`). op 0 `split(fuel, mem, spawn) -> sub_handle` attenuates a sub-budget out of the holder's remaining (a field of `-1` = "all remaining"; an unbounded `-1` parent field stays unbounded; over-asking a bounded field ⇒ `-EINVAL`, whole split fails closed, nothing deducted — D19 attenuation via `try_grant`, like `AddressSpace.sub`); op 1 `read(field) -> remaining` reports fuel/mem/spawn; **op 2 `transfer(dst, fuel, mem, spawn) -> 0 | -errno`** (#1289 R2) moves quota from the holder DOWN into an existing budget it holds — the top-up primitive, `split`'s lazy inverse (conservation; `-1` = all remaining; over-asking a bounded field ⇒ `-EINVAL`, nothing moves). An **ordinary cap** (generic `call.cap` → `Host::cap_dispatch_slots`), so **JIT parity is free** — differential `budget.rs` (`split_and_read`, `over_split`, `split_all`, `transfer_moves_quota_down_conserving`, `over_transfer_…moves_nothing`, `transfer_from_an_unbounded_holder`). Index-carrying ⇒ non-copyable across tables (a child gets a *sub*-budget by `split`/`transfer`, never the handle); **durable** since #1502 — the artifact carries the remaining quotas and the thaw may only attenuate them (`Host::set_budget_thaw_hook`). **The detached-window minter folded into `Budget.mem` (R2, done):** op 15 charges `Budget.mem` (`budget_mem_take`) and the standalone `WindowMinter` cap + `cap_id` 15 retired (reserved-not-reused); embedders grant via `HostCap::detached_budget(mem)`. Remaining: **charging** live *fuel/spawn* consumption against a budget (the `create(module, window, budget)` accounting) |
| S6 | `self.attest` incl. freeze authority (+ the §14 amendment PR into DESIGN.md) | S5 | **done (interp + JIT root parity)** — `self.attest` (a new `self.*` intrinsic, self-dispatch op 4) reports a domain's platform-vouched provenance packed into an `i32`: `tier (§2 isolation, 0/1/3) | (window_exposed << 8) | (freeze_exposed << 9)`. `Host::attestation` (`Attestation { tier, window_exposed, freeze_exposed }`), embedder-set via `set_attestation`; the §14 spawn path stamps a nested child's report (**window_exposed = true** — the parent reads its carve; **freeze_exposed = durable**; tier inherited). Non-interposable by construction (a `self.*` intrinsic is never a handle-table entry — D46). Wired across all engines (tree-walk / bytecode `Op::CapSelfAttest` / JIT `call.cap` thunk → shared `self_dispatch`) + text/verify/encode/peval/spec. **O5 resolved:** window provenance is a single exposed bit, not an authority set (smaller non-interposable surface). Differential `attest.rs`: root report (tier/exposure packing) matches both backends; a nested §14 child sees `window_exposed` (interp). DESIGN.md §14 "no am-I-nested query" sentence **amended** (the narrow opt-in carve-out). Follow-ups: **O14** — durable freeze must capture/restore a child's attestation (thaw re-attach currently defaults it); JIT plain-child `self.*` (empty powerbox) |
| S7 | BusyBox port; stage-1/2 demo gates (**endpoint-free**) | S3,S4 | **Stage-0 mini-shell done** — a real read-eval loop (chibicc → `temen-posix`, libc by name) runs a script from preloaded stdin with builtins (`echo` incl. `$VAR`, `pwd`, `cd`, `exit`, unknown→`not found`), differential interp/JIT (`c_shell.rs`). Proves the command-interpreter pipeline end to end on the personality. **Grown since:** (1) **argv delivery** — the personality carries a host-side arg vector (`argc`/`argv` ops, the symmetric analogue of `getenv`), so `sh -c "echo $HOME"` works (`c_shell.rs`); a guest crt bridges these to a standard `main(int, char**)` for unmodified programs later (the chibicc `_start` args-buffer parse is the alternative, deferred — it needs a globals-layout shift past `POWERBOX_ARGS_END`); (2) **`stat` + `opendir`/`readdir`/`closedir`** over the memfs (S-`temen-posix` ops 13-16), driven by an `ls` builtin; (3) **I/O redirection (`<`/`>`/`>>`) + `cat`/`wc`** — the full redirection triad rebinds globals `in_fd`/`out_fd` through `open`/`close` (S-`temen-posix` ops 5/6, `O_CREAT`/`O_TRUNC`/`O_APPEND`); `cat`/`wc` read a path arg or the redirected `in_fd` (`wc` reports `lines words bytes`), and multiple redirects on one line (`wc < in > out`) are honored — exercising the full `open`/`read`/`write`/`close` round-trip differentially (`c_shell.rs`); (4) **argv tokenizer + text filters** — the line is split into a real `argv[]` (unlocking multi-arg builtins), on which ride `grep PATTERN [file]`, `head`/`tail -n N`, and `rm` (`unlink`, op 8), all differentially tested; (5) **exit status** — `exec_line` returns a status threaded into `last_status`/`$?` (grep-miss → 1, unknown → 127), with `true`/`false` and `test`/`[ … ]` (string `=`/`!=`, numeric `-eq`/`-ne`/`-lt`/`-gt`, `-f`/`-d`/`-e`/`-z`/`-n` over the memfs); (6) **command lists** — `run_list` splits on `;`/`&&`/`||` and short-circuits on `$?` (bash skip-propagation), differentially tested; (7) **pipelines** — `run_pipeline` splits on `|` and stages each stage's stdout through a memfs temp the next reads as stdin (in-process, no fork yet, but real pipeline *semantics* — `cat f | grep p | wc`); per-stage redirects override the pipe, temps are unlinked at end. This is the **process-driven** feature in miniature; real concurrent `fork`/`exec` remains the Stage-1 epic; (8) **shell variables** — a flat name/value table: `NAME=VALUE` assignment, whole-token `$NAME`/`$?` expansion in any argument position (shell vars shadow the environment), and `export` promoting a var into the personality env via `setenv` (op 12); (9) **`sort`/`uniq`** — line-buffered sort (insertion, ≤64 lines) and adjacent-dup collapse, proving the classic `cat f | sort | uniq` three-stage pipe; (10) **globbing** — `*`/`?` tokens (`fnmatch_` + `glob_expand`) expand against the memfs into sorted `dir/name` matches feeding multi-file builtins (`echo`/`cat`/`rm`), literal on no match; (11) **`if/then/else/fi`** — single-line conditional (`run_if`/`run_top`) whose condition (any command, incl. a pipeline) picks the branch by `$?`, running multi-command bodies that themselves go back through `run_list`; (12) **`grep -v`/`-c`** — invert-match and count-only flags. **Stage 1 started** (`STAGE1.md`): the `posix_spawn`+`wait` core is proven differentially (`stage1_spawn_wait.rs`) — a parent seeds an `argv` token into a child's carve, `instantiate_module`s a separate host-verified `Module` (op 5), `join`s (op 1) for the child's exit status, and reads the child's output back through the §14 nested-carve superset; the carve is not zeroed so the seed is the delivery channel, confinement holds, interp==JIT. This rides existing substrate (no confinement-path change). **Slice 2 done** (`stage1_stdio_child.rs`): a same-module BusyBox-applet child inherits a granted `stdout` (`instantiate_named`, op 11) and echoes its parent-seeded `argv` to it — a real external `echo` (argv in, bytes out through inherited stdio, status back), interp==JIT. **Slice 3 done** (`stage1_applet_dispatch.rs`): a multi-applet binary (`true`→0, `false`→1, `echo`→writes+3) — spawning a chosen entry yields that applet's own `(stdout, status)`, the guarantee the shell's command dispatch rests on (look up → spawn entry → thread `$?`), interp==JIT; plus the **foreign-program variant** (`stage1_foreign_command.rs`): the general `exec` case where the command is a *separate* verified `Module` spawned via `instantiate_module` (op 5), its output forwarded by the parent-as-pager (no stdio-grant op for module children), interp==JIT. Next slices: a `PATH`→`Module` registry + `spawn` in `temen-posix` so the shell dispatches unknown commands to real children, then OS-thread children for concurrent pipelines. **Stage 1 exec now runs real compiled-C commands** (`STAGE1.md`): an unmodified `int main(int argc, char **argv)` compiled by chibicc (its `_start` parses the §3e args buffer) runs both top-level and — via the new **`Instantiator.instantiate_module_named` (op 13)** — as a **spawned child with inherited stdout** (a shell re-grants its `stdout` by name; the command's `write(1,…)` lands in the shell's sink), differential interp==JIT, carve masking untouched. Op 13 = the union of op 5 (run a foreign `Module`) + op 11 (re-grant caps by name) — capability "Power 1" (forwarding a held cap). **Endpoint gap (Power 2, deferred/S9):** shell-side *interception* of a command's output (`cmd > file`, `cmd | cmd2`) needs a guest-served `Endpoint` (a parent minting a cap backed by its own code), not forwarding — the `Endpoint` primitive (PROCESS.md §4) is unbuilt, so redirection/pipelines of external commands wait on it. BusyBox `ash` port itself: todo |
| S8 | Bash stage-0 port (interpreter-only; autoconf cross-config, `--noediting`) | fs ops | todo |
| S9 | ~~`Endpoint` v1~~ — **CLOSED 2026-07-23** (see §4 "S9 rescope after §3.6" + the answered block): serve/park/reply/cancel/FIFO/data-plane all landed via §3.6; self-mint **dropped by owner decision** (authority-mediated transfer only); the `serve`/`reply` split drop **confirmed**; the "Endpoint" name retired — the substrate keeps live offers + service points. Follow-on work moved to ISSUES.md I36 (native serve loops, promoted) and I37–I40 | §3.6, S1b | closed — superseded by §3.6 as-built + I36 |
| S10 | **Interposition gate** (stage 2.5): guest-implemented virtualizing-fs personality runs the BusyBox suite unmodified | S9 | todo |
| S11 | R8 closure (`call.dyn` durable coverage); `clone` of parked domains (full-copy) + fork endpoint with duplicated reply token | S9, durable | todo |
...
| S15 | **§7 late-binding completion — retire the fixed powerbox.** The fixed 8-slot `_start` / `__vm_cap(slot)` convention is an *implicit positional agreement* (a silent slot numbering duplicated across chibicc's `*_SLOT` defines, `include/temen.h`'s `VM_CAP_*`, and every runner's grant order — nothing checks it; a wrong grant order is a runtime CapFault at best). DESIGN.md §7 [SETTLED] already names the replacement: "late binding is the **general form** of the powerbox — a module declares its capability imports by name and the host resolves each to a registered implementation **+ handle**"; the module's **import section is the manifest** (discoverable, fail-closed, signature-checkable), and `self.*` reflection audits the held set at runtime. Stages: **(a) done** — `temen_ir::Resolved::CapBound` (name → `(type_id, op)` **+ granted handle**, patched into a `ConstI32` placeholder like `Resolved::Slot`; grant-before-resolve ordering) + the POSIX personality binds handle-free (`temen_posix::resolve_bound`; real-C differential `c_posix.rs` — plain `open(path, flags)` / `getenv(name)` / `malloc(n)` signatures, no slot anywhere); **(b) done** — a guest **definition** of `write`/`read`/`exit` now **shadows** chibicc's Stream/Exit builtin (gate the interception on `!is_definition` — a real function beats a compiler builtin; a bare `extern write` still gets the builtin, so fixed-powerbox programs are unchanged). So the POSIX libc shim uses the *standard* names — `write(fd, buf, n)` reaches the personality with `fd` preserved (the builtin dropped it), forwarding to a placeholder-handle `call.import` bound by (a). Chose the definition-shadow path over re-emitting the builtins as fd-preserving imports: contained to programs that link the personality libc, no default-runner/`c_frontend.rs` migration. Regression tests: `c_defined_write_shadows_the_builtin` / `c_undefined_write_still_hits_the_builtin` (`c_frontend.rs`); `c_write_then_exit_through_the_personality` (`c_posix.rs`). **(c)** `_start` takes no positional handles — stdout/stdin/exit/memory/… migrate to named imports, `__vm_cap`/`VM_CAP_*`/the reserved-region stash retire (`self.*` remains the reflective enumeration). A staged migration (two frontends + the escape-gated run path + dozens of positional test sites): **(c1) done** — runner-side `temen_run::powerbox_resolver` (name → `(type_id, op)` **+ granted handle**, `CapBound`; `Stream` disambiguated by op; runtime-minted `SharedRegion` handles decline, to compose with a plain-`Cap` resolver), proven against a hand-authored **paramless `_start`** reaching the real `Stream`+`Exit` caps by name on both backends (`powerbox_named.rs`); **(c2) done** — chibicc `emit_start` takes **no params**; its prologue resolves each powerbox cap **by name** (`self.resolve("stdout")`, …) into the reserved stash slot (so the builtins, which still *load* the stash, and `__vm_cap`, which reads it, are **unchanged** — the reserved region is now a private guest cache, not a guest/host slot-index contract). The chosen path is *runtime name resolution*, not load-time `CapBound`: a powerbox cap is used both as a builtin (would-be placeholder) **and** with an explicit handle (`vm_page_size(__vm_cap(3))`), and one name can't be `CapBound` at both sites — self.resolve sidesteps that. `_start` is tagged `export "_start" 0`; the runner's `is_named_powerbox_entry` recognizes it, grants + registers the fixed powerbox by name (F7) and runs func 0 with `&[]`. The **positional path is untouched** (temen-llvm, hand-written IR, `run.rs` still bind 3–8 `i32` params), so the flip is backward-compatible. Migrated the `powerbox()`/`run_c*` harnesses in `c_frontend.rs` (grant+register, run `&[]`) and `c_posix.rs` (personality-only: no powerbox grant; the by-name resolves stash `-errno`, never loaded). Verified: `c_frontend` 90 + `c_posix` 3 + the `temen-run` CLI on a real `.c`. **(c2-followup) done** — a per-program **used-caps scan** (`scan_prog_caps` walks the AST; bit per `VM_CAP_*` index, stdio counted only when unshadowed, `__vm_cap` ⇒ all): `_start` resolves *only* the caps the program reaches, so a **personality-only program resolves none** and depends on no powerbox grant. **(c3)+(c4) done — the by-name unification of the whole embedding API.** The insight: the wasm-style arbitrary-imports entry loads its handle from the *same stash slots* (`i32.load 0/4`) as the fixed powerbox, so **one** paramless `_start` resolving a *name list* serves both — powerbox cap names for the fixed set, the module's own import names for the arbitrary set. Landed: `temen_ir::build_powerbox_start` is now paramless + resolves a name list (`self.resolve`); `synth_powerbox_start` passes `POWERBOX_CAP_NAMES[..n]` (a new `temen-ir` const), `synth_powerbox_start_with_names` / `synth_powerbox_start_for_imports` cover the arbitrary case; `temen-capi` gains `temen_module_synth_powerbox_start_for_imports`; `Instance::grant_caps`'s `Some(binding)` path registers names + runs `&[]` for a paramless entry (positional still supported for a legacy entry). **temen-llvm's own `synth_start`/`synth_start_argv`** now emit the paramless by-name `_start` + `export "_start" 0` too (each name staged in its stash slot — the mapped low region, since temen-llvm's data-SP sits at the window's mapped edge). Verified: the whole in-workspace embedding suite (`powerbox_run`/`imports`/`reactor`/`instantiate`, `abi_tests`) **and** the full off-workspace `temen-llvm` translate suite (313 tests, clang-backed, incl. argv + fs/mmap demos). **c4:** no frontend or synth path emits a positional `_start` any more — the fixed positional powerbox is fully retired; the runner *keeps* positional entry support as a low-level primitive (hand-written IR kernels that take cap handles as args — a valid pattern, `run.rs`), which is *not* dead code, so nothing is removed there. **(d)** resolve-time signature validation (structural compare of the import's declared sig against the registered op's, §7's "type-safety without an IDL") — the last piece | — | **(a)+(b) done** (PR #316), **(c1)+(c2)+(c2-followup)+(c3)+(c4) done** (PR #327/#332); (d) landed as IMPORTS.md phase-1 manifest validation. **Superseded (IMPORTS.md phases 1–4):** executable manifest slots replaced the resolve-time machinery — the S15 (a)/(c1)/(c3) artifacts (`Resolved::CapBound`, `resolve_bound`, `powerbox_resolver`, the `synth_powerbox_start*` family, the stash bootstrap, the runner's positional-entry powerbox) are **deleted** in phase 4; the §2.5 grep-clean gate (`crates/temen/tests/imports_gate.rs`) keeps them gone |
...
| O14 | Attest's `freeze_authority` field requires freeze authority to be *explicit* — **representation landed** (#1440): `Binding::FreezeAuthority(FreezeScope)`, granted at a §14 nested spawn over the instantiator's range (containment-covering, so one grant serves every carve from it rather than a handle slot per spawn), durable across freeze/thaw, and askable via `Host::holds_freeze_authority`. `attest.freeze_exposed` is now **derived** — `durable && holds_authority` — rather than inferred from placement, with the same observable value for every domain that exists today. A **detached** child has no sub-range to be named by, so `FreezeScope::DetachedProgeny` is the all-or-nothing form (owner decision, 2026-09-17: A decides whether B may freeze *any* of its detached children; per-child granularity deferred). Crucially it is never self-minted — a nested parent already reads its child's carve, so self-granting there documents a fact, while a detached child's window is not readable by its parent, so a self-mint would dissolve the isolation the spawn just asked for; it comes from an embedder grant or an ancestor's re-grant. **The op-15 gate is lifted (#1361 step 4, 2026-09-23):** a durable parent holding `DetachedProgeny` spawns detached children of attested-freezable modules on every engine (the rule sits in the shared `Host::admit_detached_spawn`), and its freeze captures each as its own artifact inside the parent's (DURABILITY.md §4); the module-handle prerequisite was cleared first (#1361, 2026-09-18: an attested-freezable grant is durable, carried by its §4 content digest) | §6 | **conceptual half closed** (INVARIANTS #14 ruling 2026-09-08, #1289 R1: freeze authority is an explicit, attenuating capability, per grant not per placement; `attest` reports `freeze_exposed = durable`, the conservative truth for a nested child whose parent's aliasing implies the grant). **Remaining plumbing (in flight):** a durable **freeze/thaw** must capture + restore a child's `Attestation` (the thaw re-attach path currently defaults it) so a detached child's placement/exposure survives — the prerequisite for the op-15 refusal to become "refuse unless a freeze-authority holder is registered" (**#1412, 2026-09-14:** R1's slice-1 spawn lift is deferred until that plumbing and the #1361 capture land — the op-15 `!durable` gate is restored on all three engines meanwhile, since R1's compensating freeze refusal was a `ThreadFault` and existed only on the tree-walk path) — a `DURABILITY.md` follow-up |
...
learns. One deliberate amendment to settled text: `self.*` gains a read-only,
