...
| `serve(serve_end, buf)` — park until a call arrives | `svc.wait` (`cap.call CAP_SELF 10`): parks the domain on its bounded FIFO inbound queue; a caller's enqueue wakes it | built — serve is a *service point*, not a per-endpoint object: one queue per domain, offers multiplexed by `(export, op)` |
...
| client `cap.call` parks its fiber, indistinguishable from a host cap | caller-side parking (slice 3): enqueue + `Blocked::CapReply`, fiber-level since 5a; the slot route (`import.attach` + `call.import`) rides it too | built |
...
(`cap.call` is precisely the `temen-durable` transform's suspension unit). So:
