...
// same source has no `__vm_*` symbols, so guard TEMEN-specific code accordingly if it must also build
// natively.
#ifndef __TEMEN_H
#define __TEMEN_H
...
//
// Submit serialized Temen IR (the binary `temen-encode` format) built at runtime in this window;
