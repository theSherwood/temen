...
| — | default actions + `EINTR` | doorbell (§9 L1/L2) | host signal state | **partial** — **L2 async delivery to a running loop is DONE** (#796): with a handler + `sigaltstack`, the interp redirects a caught, unmasked signal into `void handler(int)` at a per-op safepoint and resumes. **Still parked:** default actions (terminate/stop), `EINTR` on blocked calls (L1), and `SA_RESTART`/block-during-handler |
