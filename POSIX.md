...
| 49 | `isatty(fd)` | `-> 1 \| 0` | proto-terminal | **done (#800)** — the `tc*` ops' proto-terminal test as C's boolean (stdio fds `1`, anything else `0`); bash's interactive-mode probe |
| 50 | `getppid()` | `-> pid` | host process table | **done (#800)** — the minting process's pid (fork twins and re-grant clones; `0` only for the root); bash's `$PPID` |
| 52 | `pipe_adopt(read_h, write_h, fds_ptr)` | `-> 0 \| -errno` | host fd table (bookkeeping only) | **done (#972 slice 1)** — records two guest-minted core pipe-end handles (`__vm_pipe`/`CAP_SELF_PIPE`) as `CorePipe` fds; `read`/`write`/`close` on them return a **handle tag** (`PX_TAG_BASE - h`, below every errno — a personality↔shim private convention the core never interprets) redirecting the shim to `__vm_read`/`__vm_write`/`__vm_close` — the core blocking/EINTR/true-EOF/`-EPIPE` path; the shim raises SIGPIPE on `-EPIPE` per disposition. `dup`/`dup2`/`F_DUPFD` share a per-process token; the **last** close tags the release (powerbox count decrement → EOF/EPIPE wakes); `fork` re-splits tokens per process (the twin's duplicated powerbox counts are its own). Off the park tier the mint declines (`-EINVAL`) and `pipe()` surfaces it probeably. **Exec carry: DONE (#972)** — the image-replace keeps a personality process's pipe ends (counts bumped before the old powerbox releases — no EOF/EPIPE dip) and fires the provider's exec-remap hook with the `(old, new)` handle pairs so the fd tokens re-point; gated on the provider declaring fd-preservation, so capability-path guests keep their explicit-carry drop-by-default contract. The pipeline crosses the exec boundary (`dup2` + `execve`'d command writing into the pipe, true EOF at its exit) |
| 56 | `tcgetwinsize(fd, ws_ptr)` | `-> 0 \| -errno` | the terminal (`{i32 row; i32 col}`) | **done (#797)** — the `TIOCGWINSZ` shape; updated by the embedder's `Posix::set_winsize`, which fires **SIGWINCH** at the foreground group; `-ENOTTY` off-terminal |
| 57 | `ttyname(fd, buf, buflen)` | `-> 0 \| -errno` | the terminal (fixed name) | **done (#797)** — `ttyname_r` shape: writes `/dev/tty\0` to `buf` when `fd` is the terminal (the `isatty` proto-terminal test), `-ENOTTY` off it, `-ERANGE` when `buf` is too small. Replaces bash's shim fake (a blanket `/dev/tty`, wrong for a non-terminal fd) |
| 55 | `tcsetattr(fd, attr_ptr)` | `-> 0 \| -errno` | the terminal | **done (#797)** — applies immediately (TCSANOW; `when` is a shim concern); the discipline honors `ISIG`/`ICANON`/`ECHO` + the cc's, everything else round-trips |
| 54 | `tcgetattr(fd, attr_ptr)` | `-> 0 \| -errno` | the terminal (32-byte `{lflag, cc[8], vmin, vtime}`) | **done (#797)** — the **controlling terminal** (opt-in `Posix::enable_terminal`; preloaded-stdin embedders unchanged): input is a held-writer core pipe the embedder feeds (`Posix::feed_terminal`) with the **line discipline at feed time** — raw deposits immediately, canonical buffers with `VERASE`/`VKILL` editing + echo and flushes completed lines, `VEOF` on an empty line is one-shot true EOF, `ISIG` chars fire the #798 group machinery at the fg pgid and never enter the stream. `read(0)` tag-redirects to the input pipe (the #972 park/EINTR path — a `^C` during a parked prompt read composes as `-EINTR`/`SA_RESTART`). Core delta, flagged: `grant_terminal_input` (held-writer grant variant) + the `set_pipe_wake` door — the doors/grant families, no new mechanism |
| 53 | `exec_resolve(path, len)` | `-> module \| -errno` | host PATH registry + memfs (`register_executable`) | **done (#801 slice A)** — execve-of-a-file: resolve a **filesystem path** to its registered executable's pre-granted `Module` handle (minting happened at registration — the exec_lookup discipline); a memfs file without the registration is `-EACCES`, absent is `-ENOENT` (the split `execvp`'s PATH walk keys on). **#763:** a memfs file holding a runnable module's encoding is a *built* program (PROCESS.md: `cc x.c && ./a.out`). `exec_resolve` has no handle to report for it (`-EACCES`; a C guest promotes it through its own `ModuleLoader.from_bytes`), while the personality's `execve` (op 60) stages its bytes and the engine promotes them through the process's `ModuleLoader`, which every fork and exec carries (`-EACCES` without one, `-ENOEXEC` for bytes that do not decode and verify). The guest libc's `execve` follows a hit with the core `CAP_SELF_EXEC` image-replace (`__vm_exec_module`, empty grants — v1 self-contained commands), argv/envp packed into the preserved args region; `stat` reports the exec bits (`0o755`) on registered paths. The exec now **carries the domain's exit hooks + signal doors** (core fix: an exec'd twin stays reapable and signalable — POSIX exec-keeps-the-pid). `__px_`-linked commands need the import-resolver carry (sketch on #801) |
| 51 | `fork()` | `-> pid \| 0 \| -errno` | core fork engine (caller-request door) | **done (#799)** — **return-twice fork through the personality**: no offer topology, no manager. Fires `ParkEvent::ForkSelf`; a parkable route runs the core's fork engine (private window copy incl. page protections, duplicated powerbox → this personality's fork factory registers the twin with pid + doors at birth) and re-admits parent=twin-pid / twin=`0`. Refused fork ⇒ `-EAGAIN` (retryable); door-less route/tier ⇒ `-ENOSYS` (fork unavailable — bounded error, no infinite retry). Twin TaskIds skip the reserved pid band (`0`/`1`) |
