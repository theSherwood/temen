//! **Core-wasm → Temen IR transpiler** (a frontend, not part of the escape-TCB — its output is
//! re-verified). Takes a wasm binary and lowers the subset of core wasm that overlaps our IR — the
//! numeric ops, locals, and structured control flow — into an [`temen_ir::Module`]. The point is
//! *apples-to-apples* benchmarking and a second, non-chibicc proof that the IR is a real target: take
//! any wasm, run it on Temen, compare to Wasmtime on the same bytes.
//!
...
//! 1:1 onto Temen's IR atomics (`tests/atomics.rs`), the **narrow** (8/16-bit) `*.atomic.*` forms
//! emulate via a 32-bit word-CAS loop (the i64 32-bit forms are word-sized natives), **shared** +
//! **imported** memory are accepted, and the **wasi-threads** ABI lowers to Temen's *native*
...
/// i32))`. The synthesized spawn shim adapts Temen's `(i64 sp, i64 arg)` thread-entry ABI to it.
...
/// past it traps under Temen's trap-confinement), so for unbounded memory this is a modest cap
...
fn val_type(w: W) -> Result<ValType, Error> {
    match w {
        ...
        // §RT reference types: both `funcref` and `externref` are an opaque **i32 index** in Temen — a
        ...
        // Temen only needs the bit representation, so both lower to `i32`.
...
pub fn transpile(wasm: &[u8]) -> Result<Transpiled, Error> {
    ...
    let mut debug_blobs: Vec<temen_ir::ProducerBlob> = Vec::new();
    let mut exports: Vec<(String, u32)> = Vec::new();
    let mut mem: Option<wasmparser::MemoryType> = None;
    let mut data: Vec<temen_ir::Data> = Vec::new();
    ...
    let mut manifest: Vec<temen_ir::Import> = Vec::new();
    ...
    for payload in Parser::new(0).parse_all(wasm) {
        match payload? {
            ...
            Payload::ImportSection(reader) => {
                for imp in reader.into_imports() {
                    ...
                    let type_idx = match imp.ty {
                        ...
                        // shared across the per-thread instances. Temen treats it exactly like a defined
                    ...
                    let t = manifest_types
                        ...
                        .unwrap_or_else(|| {
                            manifest_types.push(temen_ir::TypeEntry::Func(sig.clone()));
                            manifest_types.len() - 1
                        }) as u32;
                    manifest.push(temen_ir::Import {
                        name,
            ...
            Payload::MemorySection(reader) => {
                for mt in reader {
                    let mt = mt?;
                    // wasm `shared` memory (the threads proposal) is accepted: Temen's window is already
            ...
            Payload::CustomSection(reader) if reader.name().starts_with(".debug") => {
                ...
                debug_blobs.push(temen_ir::ProducerBlob {
    ...
    // an access past the window traps under Temen's trap-confinement (matching wasm's OOB trap).
    ...
    // functions, so its index is `bodies.len()`). The shim adapts Temen's thread-entry ABI to the
    ...
    // `(start $f)`: run the start function once before any export. Temen has no instantiation hook (a
    ...
    // Our window is a power-of-two byte range (Temen's reserved domain rounds up to it); size it to hold
    // the linear memory (its full growable span) **and** the size cell + globals + function-table
    // regions. Both wasm and Temen bounds-check-and-trap on an out-of-range access; the residual
...
fn build_debug_info(
    ...
    blobs: Vec<temen_ir::ProducerBlob>,
) -> Option<DebugInfo> {
...
fn ingest_variables(
    ...
    let mut type_ids: BTreeMap<u32, u32> = BTreeMap::new(); // DWARF type-DIE offset → temen TypeId
...
/// Map a DWARF `DW_AT_encoding` byte to the neutral [`temen_ir::Encoding`].
fn dwarf_encoding(e: u8) -> temen_ir::Encoding {
    match e {
        0x02 => temen_ir::Encoding::Bool,
        0x04 => temen_ir::Encoding::Float,
        0x07 | 0x08 => temen_ir::Encoding::Unsigned, // unsigned, unsigned_char
        _ => temen_ir::Encoding::Signed,             // signed, signed_char, address, …
    }
}
...
/// `wasi_thread_start` export (IR index `wts`), then returns 0. The data-SP `sp` is unused (temen-wasm
...
fn lower_func(
    ...
    for t in &local_types[params.len()..] {
        let v = match t {
            ...
            // WASM never declares a `ref` local (it's an temen-only GC reservation); treat as i64.
            ValType::Ref => lo.emit(Inst::ConstI64(0)),
            // `cap` is temen-only (§3.5); wasm never declares one. i32-width zero.
...
// dropped: Temen does not trap on misalignment for non-atomic accesses, and the hint left the wire.
...
fn spawn_op(lo: &mut Lower) -> Result<(), Error> {
    ...
    // The shim ignores its data-SP (temen-wasm keeps the C stack in linear memory via `__stack_pointer`),
...
//
// The wasm threads atomics map 1:1 onto Temen's IR atomic surface (same widths, same
// trap-on-misalignment, all seq-cst). Only the **full-width** (i32/i64) ops are lowered here; the
// **narrow** forms (`*.atomic.load8_u`/`rmw8`/`rmw16`/…) have no IR analogue (Temen atomics are
...
//
// Temen IR atomics are 32/64-bit only, but wasm has 8/16-bit (and i64's 32-bit) atomic
...
//
// Note: like Temen's §1a OOB-masking (confine, don't trap), a *misaligned* narrow atomic is not
...
fn lower_op(lo: &mut Lower, op: Operator, fn_results: &[ValType]) -> Result<(), Error> {
    ...
    match op {
        ...
        // ---- §RT reference instructions (both ref types are an i32 index in Temen) ----
        O::RefNull { .. } => {
        ...
        // and lowered to a real hardware barrier by the JIT (all Temen atomics are already seq-cst).
        ...
        // get a genuine fused FMA; the rest alias to the deterministic op Temen already has. The two
