...
//! any export. Temen has no instantiation hook (a run calls one entry over a fresh window), so the
...
fn start_with_bad_signature_rejected() {
    ...
    match temen_wasm::transpile(&wasm) {
        ...
        Err(temen_wasm::Error::Unsupported(_) | temen_wasm::Error::Parse(_)) => {}
