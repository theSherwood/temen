//! §12 wasm threads — **atomics** (slice 1): the full-width `*.atomic.*` ops transpile to Temen's IR
...
//!
//! Narrow atomics (`*.atomic.rmw8`/`load16_u`/…) have no direct IR form (Temen atomics are 32/64-bit
...
//
// Temen IR atomics are 32/64-bit only; the narrow forms emulate via a CAS loop on the containing
