...
//!
//! Scope and the Temen impedance mismatch (read before trusting the numbers): Temen's interpreter runs
...
//! categorized `trap-divergence` — Temen **masks** the access into its window rather than trapping
...
//!   `SPEC_DIR=/path/to/spec/test/core cargo test -p temen-wasm --test spec -- --nocapture`
...
struct Tally {
    ...
    trap_divergence: u32, // assert_trap that Temen masks instead of trapping
...
fn transpile_instance(wasm: &[u8]) -> Option<Instance> {
    let t = temen_wasm::transpile(wasm).ok()?;
    temen_verify::verify_module(&t.module).ok()?;
...
fn run_file(text: &str) -> Tally {
    ...
    for d in wast.directives {
        match d {
            ...
            // (temen-verify, not temen-wasm), cross-instance state, exhaustion, etc.
            _ => t.skipped += 1,
        }
    }
    t
...
fn assert_trap(cur: &Option<Instance>, exec: WastExecute, t: &mut Tally) {
    ...
    match invoke(inst, inv.name, &args) {
        ...
        Ok(Some(_)) => t.trap_divergence += 1, // Temen masked/computed instead of trapping
    }
}
