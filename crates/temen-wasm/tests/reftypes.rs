...
//! i32 index in Temen (funcref → §3c function index, externref → §7 capability handle), so this all
//! lowers to existing IR (loads/stores/const/select + the synthesized fill loop) — no new ops.
//!
//! Table mutation is observed **within a single invoke** (Temen persists the window across the calls in
...
fn eval(wat: &str, entry: &str) -> i32 {
    ...
    let interp = temen_interp::run(&t.module, idx, &[], &mut fuel).expect("interp run");
    let jit = match temen_jit::compile_and_run(&t.module, idx, &[]).expect("jit") {
