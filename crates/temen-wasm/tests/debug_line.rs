...
fn wasm_dwarf_line_maps_into_the_debug_info_waist() {
    let t = temen_wasm::transpile(DLINE).expect("transpile");
...
fn wasm_dwarf_info_extracts_source_variables() {
    ...
    let info =
        temen_wasm::dwarf_info::parse(sec(".debug_info"), sec(".debug_abbrev"), sec(".debug_str"))
    ...
    let temen_wasm::dwarf_info::DwarfType::Base {
        name,
        encoding,
        size,
...
fn wasm_dwarf_ingests_aggregate_pointer_and_array_types() {
    ...
    let t = temen_wasm::transpile(AGG).expect("transpile");
    temen_verify::verify_module(&t.module).expect("verify");
...
fn wasm_dwarf_ingests_module_scoped_globals() {
    ...
    let t = temen_wasm::transpile(GLOBALS).expect("transpile");
    temen_verify::verify_module(&t.module).expect("verify");
    let di = t.module.debug_info.as_ref().expect("debug info");
...
fn wasm_global_reads_its_value_at_runtime() {
    ...
    use temen_interp::{Inspector, IrPc, Stop, Value, VarValue};
...
fn wasm_dwarf_ingests_lexical_scopes_for_shadowed_vars() {
    ...
    let t = temen_wasm::transpile(SHADOW).expect("transpile");
    temen_verify::verify_module(&t.module).expect("verify");
    let di = t.module.debug_info.as_ref().expect("debug info");
