//! Differential tests for **function imports / the host ABI** (the wasm `call` → Temen `call.import`
...
fn run_import(
    ...
    let jv = match t.module.funcs[idx as usize].results[0] {
        temen_ir::ValType::I32 => jit[0] as u32 as i64 & 0xFFFF_FFFF,
        _ => jit[0],
    };
    let iv_cmp = match t.module.funcs[idx as usize].results[0] {
        temen_ir::ValType::I32 => iv as u32 as i64 & 0xFFFF_FFFF,
        _ => iv,
    };
...
/// `temen_interp::Host::mix` so the test can compute the oracle independently.
fn mix(arg: i64) -> i64 {
...
fn imports_declare_manifest_entries() {
    ...
    assert!(t
        ...
        .imports
        .iter()
        .all(|i| i.mode == temen_ir::ImportMode::Required));
...
/// linear memory). Temen treats it exactly like a defined memory: the window's linear region at offset
...
fn import_memory_is_supported() {
    ...
    assert_eq!(
        temen_interp::run(&t.module, idx, &[], &mut fuel).expect("run"),
        vec![Value::I32(42)],
