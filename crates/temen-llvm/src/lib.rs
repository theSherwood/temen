//! LLVM-bitcode → TEMEN-IR translator (the AOT LLVM on-ramp, D54). See `LLVM.md` for the
...
//! `temen-verify`, so a translation bug is a clean error, never an escape. Correctness here is
...
//!   edges) becomes Temen's block-local form (§3a). Liveness makes every value live across a block's
...
//!   chunked load/stores (widest-first 8/4/2/1, the `temen-wasm` plan); copies load-all-then-store-all
...
//!   `copysign`/`fmin`/`fmax` (and `…f` f32 variants) lowers to the matching Temen float op inline —
//!   unless the guest defines its own. (`round` and transcendentals have no Temen op → still a call.)
...
//!   helper** (`__temen_memset`/`__temen_memcpy`, a real counted byte loop — the first multi-block helper)
...
//!   global, and each conversion lowers to the synthesized `__temen_utoa` (int→ASCII) + width/zero-pad
...
//! - **Z — `llvm.bswap`.** Byte-reverse synthesized inline (no Temen op): each source byte `i` →
...
//!   oversized-constant) `llvm.memmove` calls the synthesized **`__temen_memmove`** — a counted byte
...
//! - **AB — transcendental libm, bundled as guest code.** Math beyond the Temen float ops (`sin`/
...
/// Shorthand for the fail-closed chokepoint (the `temen-wasm` `unsup(...)` analog).
...
pub struct TranslateOptions {
    ...
    /// [`DEFAULT_STACK_PAGE`]. Pass `65536` when the `.temen` targets a **wasm** host (64 KiB pages,
...
struct StubTable {
    ...
    extern_sigs: HashMap<String, temen_ir::FuncType>,
}

impl StubTable {
    fn new(base: u32, extern_sigs: HashMap<String, temen_ir::FuncType>) -> Self {
        Self {
            base,
            order: Vec::new(),
            idx: HashMap::new(),
...
/// The Temen signature of a **declared** external (`m.func_declarations`): the threaded data-SP (§3d)
...
/// The Temen signature of a trap stub for a direct call to an undefined external: the threaded data-SP
...
/// types. Matches exactly the `args = [callee_sp, fixed…]` the call site builds, so `temen-verify`'s
/// type-id check passes.
fn extern_stub_sig(c: &crate::ll::ast::Call, types: &Types) -> Result<temen_ir::FuncType, Error> {
    let Type::FuncType {
        result_type,
        param_types,
        ..
...
/// llvm-ir`; this only matters for a `.bc` input. An explicit `TEMEN_LLVM_DIS` wins (pinning/testing).
/// Probed once and cached.
fn best_llvm_dis() -> &'static str {
    use std::sync::OnceLock;
    static DIS: OnceLock<String> = OnceLock::new();
    DIS.get_or_init(|| {
        if let Ok(p) = std::env::var("TEMEN_LLVM_DIS") {
            return p;
        }
...
/// large-program bring-up, or `stack_page` for a wasm target (a `.temen` whose RO/writable isolation
...
fn translate_impl(
    ...
    let stack_page = opts.stack_page.max(scratch + temen_ir::POWERBOX_ARGS_END);
    if !stack_page.is_power_of_two() || stack_page < temen_ir::POWERBOX_ARGS_END {
        return Err(Error::Unsupported(format!(
            "stack_page {stack_page} must be a power of two >= {} (POWERBOX_ARGS_END)",
            temen_ir::POWERBOX_ARGS_END
        )));
    }
    ...
    // Direct `<temen.h>` Memory-capability builtins (`__vm_map`/`unmap`/`protect`/`page_size`): register
    ...
    // A direct `strlen` call routes to the same synthesized `__temen_strlen` byte loop that `printf %s`
    ...
    // `puts`/`fputs` of a **non-literal** string emit a runtime `__temen_strlen` for the write length.
    ...
    // `%f` formatting (`__temen_dtoa_fixed`) rides on `printf`: it writes via the same `write` import and
    ...
    // definition plus an alias to it (e.g. temen-ir's `VIntUnOp::index` and `VPMinMaxOp::index`, both
    ...
    // `memchr(s, c, n)` (string/buffer scans — e.g. Embench `slre`) → the synthesized `__temen_memchr`
    // byte loop. Like `memcmp`, a pure address helper (no powerbox/`has_main`).
    let need_memchr = calls_external(m, &defined_names, "memchr");
    // `__cxa_end_catch` destroys the caught exception object via `__temen_eh_destroy` (gated on EH being
    ...
    // `__temen_eh_unwind` backs every throw/rethrow/resume — present whenever EH is active.
    ...
    // `__temen_memcmp` (after the atomics — matches the `take` order). Self-contained: it touches only
    ...
    // `__temen_udivmod128` (after `__temen_eh_unwind` — matches the `take` order). Self-contained: it
    ...
    if let Some(cell) = &stubs {
        ...
        for (i, (name, sig)) in table.order.iter().enumerate() {
            ...
            // backtrace's innermost frame resolves through `temen_interp::func_name` to the extern's name
...
const DEFAULT_STACK_PAGE: u64 = temen_ir::POWERBOX_STACK_PAGE;
// The §3e powerbox args buffer (`temen_ir::POWERBOX_ARGS_BASE..POWERBOX_ARGS_END`) must sit *above*
...
const _: () = assert!(temen_ir::POWERBOX_ARGS_BASE >= FMT_BUF_END);
const _: () = assert!(temen_ir::POWERBOX_ARGS_END == DEFAULT_STACK_PAGE);
/// The data-stack reserve (bytes) above the entry SP before the guard region — a stack overflow
/// past this faults rather than escaping the window ([`temen_ir::POWERBOX_STACK_RESERVE`]).
const STACK_RESERVE: u64 = temen_ir::POWERBOX_STACK_RESERVE;
...
/// onto the Temen `(func, block, inst)` pc it lowered to. This is the **source-line half** of the
...
impl DebugAcc {
    ...
    /// Record that the Temen ops at `func`/`block`/`inst in [start, end)` came from `dl`'s source
...
fn build_ctype_data(
    ...
    *end = end.div_ceil(stack_page) * stack_page;
    ...
    fn emit_ctype(bytes: Vec<u8>, elem: u64, data: &mut Vec<temen_ir::Data>, end: &mut u64) -> u64 {
    ...
    let case_table = |to_lower: bool, data: &mut Vec<temen_ir::Data>, end: &mut u64| -> u64 {
...
/// Map an LLVM type to an Temen value type. Narrow integers collapse to `i32` (§3b: `i8`/`i16`
...
fn vec_lane_shape(ty: &Type) -> Option<temen_ir::VShape> {
    use temen_ir::VShape;
    let Type::VectorType {
        element_type,
        scalable: false,
...
fn vec128_shape(ty: &Type) -> Option<temen_ir::VShape> {
...
/// step 1): the lane [`temen_ir::VShape`] its lanes pack into, the number of full 16-byte `v128`
...
/// e.g. `{ i64, ptr }`), each mapped to its Temen value type — the components of an Temen **multi-result**
...
/// The Temen result list for an LLVM return type: `[]` for `void`, the flattened scalar fields for a
...
/// The `FloatTy` (`f32`/`f64`) of a float-typed Temen value.
...
///
/// The temen-ir [`FCmpOp`] primitives carry Rust's semantics: `Eq`/`Lt`/`Le`/`Gt`/`Ge` are *ordered*
...
/// The `IntTy` (`i32`/`i64`) a value of this Temen type is computed at.
...
/// instruction results, in scan order). The translation works in terms of these; Temen block-local
...
struct Scan {
    ...
    /// `ValueId` → its Temen type. A wide-vector value (tracked in `wide`) carries a `V128` placeholder
...
fn translate_func(
    ...
    // stripped build deliberately carries no debug info to keep `.temen` lean — `translate.rs`'s
...
/// and instruction results), recording each one's Temen type and defining block. Also validates that
...
fn signed_load_op(loaded: u32, to: u32) -> temen_ir::LoadOp {
    use temen_ir::LoadOp as L;
    match (loaded, to) {
        (8, 32) => L::I32_8S,
        (8, 64) => L::I64_8S,
        (16, 32) => L::I32_16S,
...
/// The Temen signature of an indirect call's callee — the function type plus the prepended data-SP
/// param (§3d), so the runtime type-id check matches the callee's IR signature (§3c).
fn indirect_sig(c: &crate::ll::ast::Call, types: &Types) -> Result<temen_ir::FuncType, Error> {
    match c.function_ty.as_ref() {
        Type::FuncType {
            result_type,
            param_types,
            // For a `(...)` callee, `param_types` are the *fixed* params; the variadics are marshaled
            // into caller scratch (not IR args), so the Temen signature is `(sp, fixed-params…)` — the
...
/// The `HostProc` interface id (`temen_interp::cap_id::HOST_PROC`) — the **embedder-registered** capability
/// (§7 "host-defined capabilities"), reached from C via `__vm_host_call`. Pinned here numerically
/// (temen-llvm produces `temen-ir` and does not depend on the interpreter crate); `temen-run`'s
/// `host_proc_type_id_matches` test locks the two together.
const HOST_PROC_TYPE_ID: u32 = 13;
/// The `SharedRegion` interface id (`temen_interp::cap_id::SHARED_REGION`) — the §13 window-aliasing
...
/// ([`temen_ir::POWERBOX_HEAP_BRK`]), so the layout above it is unchanged.
const HANDLE_REGION_END: u64 = temen_ir::POWERBOX_HEAP_BRK;
/// The guest heap's bump pointer and committed boundary (`i64` each) — the allocator's only state,
/// placed just above the 8-handle region ([`temen_ir::POWERBOX_HEAP_BRK`]/[`temen_ir::POWERBOX_HEAP_TOP`]).
...
const HEAP_TOP: u64 = HEAP_BRK + 8; // 40
                                    ...
                                    // (`temen_ir::POWERBOX_HEAP_TOP`) so the two can never silently diverge.
const _: () = assert!(HEAP_TOP == temen_ir::POWERBOX_HEAP_TOP);
...
/// integer conversion: `__temen_utoa` writes the digits backward from `FMT_BUF_END`, and width padding
...
/// Scratch for the `%f` helper (`__temen_dtoa_fixed`): the 128-bit working limbs, the extracted decimal
...
/// when the handler completes (the `__temen_eh_destroy` helper guards the null case). `__cxa_rethrow`
...
/// match `temen-run`'s `default_cap_resolver`: `write`/`read` are `Stream` (`(i64 buf, i64 len) ->
...
fn cap_spec(name: &str) -> Option<CapSpec> {
    ...
    Some(match name {
        ...
        // stderr handle). The temen `std` PAL routes `Stderr` here so `eprintln!` captures separately.
...
fn import_sig(import: &str) -> temen_ir::FuncType {
    ...
    match import {
        ...
        // `page_size()` (op 3) — the rest of the §3e/§4 Memory surface a guest reaches from `<temen.h>`.
...
fn collect_cap_imports(
    m: &LModule,
    defined: &HashMap<String, u32>,
) -> (Vec<temen_ir::Import>, HashMap<String, u32>) {
    let mut imports: Vec<temen_ir::Import> = Vec::new();
...
/// Scan for direct `<temen.h>` Memory-capability builtin calls (`__vm_map`/…), registering each one's
...
fn register_vm_memory_imports(
    m: &LModule,
    defined: &HashMap<String, u32>,
    imports: &mut Vec<temen_ir::Import>,
...
/// (`temen_ir::POWERBOX_ARGS_BASE`, seeded by the host) into a C `argv[]`. It reads `argc`, walks the
...
struct Helpers {
    ...
    strlen: Option<u32>,
    /// `__temen_strcmp(a:i64, b:i64) -> i32` — NUL-terminated lexicographic byte compare (unsigned-char
    /// difference at the first mismatch). Backs `strcmp` and, in the C locale, `strcoll`.
    strcmp: Option<u32>,
    /// `__temen_strncmp(a:i64, b:i64, n:i64) -> i32` — `strcmp` bounded to `n` bytes. Backs `strncmp`.
    strncmp: Option<u32>,
    /// `__temen_strchr(s:i64, c:i32) -> i64` — first `(unsigned char)c` in `s`, or NULL (`c==0` → the
    /// terminating NUL). Backs `strchr`.
    strchr: Option<u32>,
    /// `__temen_strcpy(dst:i64, src:i64) -> i64` — copy `src` (incl. NUL) to `dst`, return `dst`.
    strcpy: Option<u32>,
    /// `__temen_strspn(s:i64, set:i64) -> i64` — length of the initial run of `s` within `set`.
    strspn: Option<u32>,
    /// `__temen_strcspn(s:i64, set:i64) -> i64` — length of the initial run of `s` *outside* `set`.
    strcspn: Option<u32>,
    /// `__temen_strrchr(s:i64, c:i32) -> i64` — last `(unsigned char)c` in `s`, or NULL (`c==0` → the
    /// terminating NUL). Backs `strrchr`.
    strrchr: Option<u32>,
    /// `__temen_strpbrk(s:i64, set:i64) -> i64` — first byte of `s` that is in `set`, or NULL.
    strpbrk: Option<u32>,
    /// `__temen_ldexp(x:f64, n:i32) -> f64` — `x · 2^n` (the `scalbn` algorithm), bit-exact to libc.
    ldexp: Option<u32>,
    ...
    pow_stub: Option<u32>,
    /// `__temen_fmod(x:f64, y:f64) -> f64` — the IEEE floating remainder via musl's exact 64-bit
    ...
    fmod: Option<u32>,
    /// `__temen_frexp(x:f64, eptr:i64) -> f64` — split `x` into mantissa∈[0.5,1) and exponent; writes the
    ...
    memcmp: Option<u32>,
    /// `__temen_memchr(s:i64, c:i32, n:i64) -> i64` — first occurrence of `(unsigned char)c` in the first
    /// `n` bytes at `s`, or NULL. Backs `memchr` (string/buffer scans).
    memchr: Option<u32>,
    /// `__temen_eh_destroy(sp:i64, exn:i64, dtor:i64)` — run an exception object's destructor for
    ...
    /// `__temen_eh_unwind(base:i64)` — the throw/rethrow/resume tail: pop the innermost handler and
    ...
    /// `__temen_getenv(name:i64) -> i64` — scan the §3e env strings for `name=`, returning the value
    /// pointer (just past the `=`) or NULL. Reads the blob in the reserved low scratch directly.
    getenv: Option<u32>,
    /// `__temen_udivmod128(nlo, nhi, dlo, dhi) -> (qlo, qhi, rlo, rhi)` — unsigned 128÷128 quotient +
    ...
    dtoa_sci: Option<u32>,
    /// `__temen_dtoa_gen(bits, prec, width, flags, scratch) -> i64` — the `%g`/`%G` formatter.
    dtoa_gen: Option<u32>,
    /// `__temen_dtoa_fix_big(bits, prec, width, flags, scratch) -> i64` — the `%f`/`%F` formatter.
    dtoa_fix: Option<u32>,
    /// Narrow (i8/i16) atomic CAS-loop helpers: `__temen_atomic_rmw_narrow` and
    /// `__temen_atomic_cas_narrow`. Present only when the module uses a narrow atomic rmw/store/cmpxchg.
...
/// the on-ramp lowers them to the synthesized 128÷128 long-division helper, `__temen_udivmod128`.)
...
/// Synthesize `__temen_eh_unwind(base:i64)`: the throw/rethrow/resume tail. Pop the innermost active
...
/// Synthesize `__temen_memcpy(dst:i64, src:i64, len:i64)`: a counted byte loop
...
/// Synthesize `__temen_memcmp(a:i64, b:i64, len:i64) -> i32`: compare `len` bytes as **unsigned**.
...
/// Synthesize `__temen_memchr(s:i64, c:i32, n:i64) -> i64`: scan the first `n` bytes at `s` for the byte
...
/// Synthesize `__temen_utoa(value:i64, base:i64, bufend:i64) -> i64` — write the **unsigned** `value`
...
/// Synthesize `__temen_udivmod128(n_lo, n_hi, d_lo, d_hi) -> (q_lo, q_hi, r_lo, r_hi)` — **unsigned
...
fn synth_udivmod128() -> Func {
    ...
    let trap = {
        ...
        let one = b.k(1);
        let z = b.k(0);
        let t = b.bin(temen_ir::BinOp::DivU, one, z); // traps
...
/// Synthesize `__temen_strlen(p:i64) -> i64` — the NUL-terminated byte length, for `printf` `%s`. A
...
/// Synthesize `__temen_strcmp(a:i64, b:i64) -> i32` — the lexicographic NUL-terminated byte compare.
...
/// Synthesize `__temen_strncmp(a:i64, b:i64, n:i64) -> i32` — compare at most `n` bytes of the two
...
/// Synthesize `__temen_strchr(s:i64, c:i32) -> i64` — the first occurrence of `(unsigned char)c` in the
...
/// Synthesize `__temen_strcpy(dst:i64, src:i64) -> i64` — copy the NUL-terminated string `src` into
...
/// Synthesize `__temen_strspn(s:i64, set:i64) -> i64` — the length of the initial segment of `s`
...
/// Synthesize `__temen_strcspn(s:i64, set:i64) -> i64` — the length of the initial segment of `s`
/// containing **no** byte from the NUL-terminated `set` (the complement of [`synth_strcspn`]'s
/// sibling `__temen_strspn`: identical outer/inner scan, inverted continue condition).
...
/// Synthesize `__temen_strrchr(s:i64, c:i32) -> i64` — a pointer to the **last** occurrence of
...
/// Synthesize `__temen_strpbrk(s:i64, set:i64) -> i64` — a pointer to the first byte of `s` that is in
...
/// Synthesize `__temen_ldexp(x:f64, n:i32) -> f64` — `x * 2^n`, the musl `scalbn` algorithm: scale by
...
/// Synthesize `__temen_frexp(x:f64, eptr:i64) -> f64` — split `x = m·2^e` with `m ∈ [0.5, 1)`, writing
...
/// Synthesize `__temen_fmod(x:f64, y:f64) -> f64` — the IEEE floating remainder, a faithful translation
...
/// Synthesize `__temen_getenv(name:i64) -> i64` (the C `getenv`). Scans the §3e args blob
...
/// A tiny auto-numbering block builder for synthesizing CFGs by hand. Value indices in temen-ir are
...
/// `__temen_dtoa_fixed` leans on (consts, i64 arith/compare, width-typed loads/stores, select).
struct Bdr {
    insts: Vec<Inst>,
    n: u32,
}
...
/// `__temen_big_is_zero(a) -> i64` — 1 if every limb of the big integer at `a` is zero, else 0.
#[allow(dead_code)]
...
/// `__temen_big_cmp(a, b) -> i64` — `-1`/`0`/`1` for `a < b` / `a == b` / `a > b` (unsigned, high limb
/// first).
#[allow(dead_code)]
...
/// `__temen_big_sub(a, b)` — `a -= b` in place (assumes `a ≥ b`); borrow-propagating, low limb first.
#[allow(dead_code)]
...
/// `__temen_big_mul_small(a, c)` — `a *= c` in place (small `c`; result must fit `BIG_NLIMBS` limbs);
...
/// `__temen_big_shl_bits(a, n)` — `a <<= n` bits in place. Processes high limb first (so the in-place
...
/// `__temen_big_zero(a)` — set every limb of the big integer at `a` to 0.
#[allow(dead_code)]
...
/// `__temen_big_copy(dst, src)` — copy all `BIG_NLIMBS` limbs from `src` to `dst`.
#[allow(dead_code)]
...
/// `__temen_dtoa_digits(bits, nsig, scratch) -> E` — the exact Dragon4-style digit generator: produce
...
/// `__temen_big_shr1(a) -> i64` — shift the big integer at `a` right by 1 bit, returning the bit shifted
...
/// `__temen_big_divmod10(a) -> i64` — divide the big integer at `a` by 10 in place, returning the
...
/// `__temen_big_inc(a)` — add 1 to the big integer at `a` in place (carry-propagating).
#[allow(dead_code)]
...
/// `__temen_dtoa_sci(bits, prec, width, flags, scratch) -> i64` — format the double `bits` in `%e`
...
/// `__temen_dtoa_gen(bits, prec, width, flags, scratch) -> i64` — format the double `bits` in `%g`
...
/// `__temen_dtoa_fix_big(bits, prec, width, flags, scratch) -> i64` — fixed-notation `%f` via exact
...
/// `__temen_atomic_rmw_narrow(word_addr, shift, width_mask, value, opcode) -> i64` — emulate a narrow
...
/// `__temen_atomic_cas_narrow(word_addr, shift, width_mask, expected, replacement) -> i64` — emulate a
...
enum FmtSeg {
    ...
    /// `flags`. Fixed uses the synthesized 128-bit `__temen_dtoa_fixed`; scientific/general use the
    /// exact bignum `__temen_dtoa_*` engine.
    Float {
        width: u32,
        prec: u32,
        flags: FmtFlags,
...
fn parse_format(fmt: &[u8]) -> Result<Vec<FmtSeg>, Error> {
    ...
    while i < fmt.len() {
        ...
        segs.push(match conv {
            ...
            // Fixed-notation float (`%f`/`%F`). Exact decimal via the bignum `__temen_dtoa_fix_big`
            ...
            // Scientific notation (`%e`/`%E`). Exact decimal via the bignum `__temen_dtoa_sci` (Dragon4
            ...
            // the bignum `__temen_dtoa_gen`. `prec` is significant digits (C default 6; 0 ⇒ 1).
            b'g' | b'G' => {
                let p = prec.unwrap_or(6);
                if p > 510 {
                    return Err(bad("float precision > 510 (digit-buffer cap)"));
...
/// Lower a `<temen.h>` low-level builtin (`crates/.../include/temen.h`, the chibicc oracle in
/// `frontend/chibicc/codegen_ir.c`) to the matching Temen IR op or `Memory` capability call. Returns
...
fn lower_vm_builtin(
    ...
    use temen_ir::{AtomicRmwOp, StoreOp};
    ...
    match name {
        ...
        "__vm_fiber_resume" => {
            ...
            ctx.push_effect(Inst::Store {
                ...
                //     drops the continuation on a park (§3.6 slice 5a; temen PR #442).
...
fn lower_io_call(ctx: &mut BlockCtx, c: &crate::ll::ast::Call, name: &str) -> Result<bool, Error> {
    ...
    match name {
        ...
        "puts" | "fputs" => {
            ...
            // pointer needs a runtime `strlen` (`__temen_strlen`) — e.g. the Lua stdlib's `fputs` of a
        ...
        // conversion lowers to `__temen_utoa` + width/zero-padding (a buffer pre-fill) → `Stream.write`.
        ...
        // `strlen(s)`: the synthesized `__temen_strlen` NUL-scan loop (also used by `printf %s`).
        "strlen" => {
        ...
        // `strcmp(a, b)` / `strcoll(a, b)`: the synthesized `__temen_strcmp` byte compare. `strcoll` is
        ...
        // `strncmp(a, b, n)`: the synthesized `__temen_strncmp` bounded byte compare.
        "strncmp" => {
        ...
        // `strchr(s, c)`: the synthesized `__temen_strchr` byte scan → pointer or NULL.
        "strchr" => {
        ...
        // `strcpy(dst, src)`: the synthesized `__temen_strcpy` copy loop → `dst`.
        "strcpy" => {
        ...
        // `ldexp(x, n)` / `scalbn(x, n)`: the synthesized `__temen_ldexp` (`x · 2^n`, bit-exact to libc).
        "ldexp" | "scalbn" => {
        ...
        // `fmod(x, y)`: the synthesized `__temen_fmod` — IEEE remainder (bit-exact, no libm dependency).
        "fmod" => {
        ...
        // `frexp(x, eptr)`: the synthesized `__temen_frexp` — mantissa/exponent split, writes `*eptr`.
        "frexp" => {
        ...
        // `memchr(s, c, n)`: the synthesized `__temen_memchr` byte scan → pointer or NULL.
        "memchr" => {
        ...
        // `getenv(name)`: the synthesized `__temen_getenv` scans the §3e env strings for `name=`.
        "getenv" => {
...
/// runtime `0/1` "is negative" flag for a signed conversion (`None` for unsigned). `__temen_utoa`
...
/// Temen `VShift` shifts every lane by one scalar amount, so a splat maps directly. `None` for a
...
/// `llvm.fptoui.sat.…`, which Rust's `as` casts from float emit) to temen-IR's `FToISat` — exactly the
...
/// `is_*_poison` `i1` arg is ignored — Temen defines the zero case); `llvm.abs` → `select(x<0,-x,x)`.
...
fn lower_int_intrinsic(
    ...
    if let Some(shape) = vec128_shape(args[0].get_type(types).as_ref()) {
        // A vector funnel shift (`llvm.fshl`/`fshr`) in the **rotate idiom** (`a == b`). temen-ir's
        ...
        if matches!(base, "llvm.fshl" | "llvm.fshr") {
            ...
            // The **rotate idiom** (`a == b`): temen-ir's lane `Rotl`/`Rotr` masks the count mod width
            ...
            // what dragon4's u128 bignum (temen-leng's float formatter) emits — W5 §3e path (a).
        ...
        if base == "llvm.ctpop" {
            if shape == temen_ir::VShape::I8x16 {
                let a = ctx.operand(args[0])?;
        ...
        let op = match base {
            "llvm.smax" => temen_ir::VIntBinOp::MaxS,
            "llvm.smin" => temen_ir::VIntBinOp::MinS,
            "llvm.umax" => temen_ir::VIntBinOp::MaxU,
            "llvm.umin" => temen_ir::VIntBinOp::MinU,
    ...
    let idx = match base {
        ...
        // *full* width (`i32`/`i64`), where the temen-ir rotate's width equals the i32/i64 container; a
        ...
        // `llvm.bswap` — reverse the value's bytes inline (no Temen op): each source byte `i` is
        ...
        // `llvm.bitreverse` — reverse the value's bits inline (no Temen op), via the log-N swap network.
        "llvm.bitreverse" => {
            let bits = src_bits(args[0], types)?;
            let v = ctx.operand(args[0])?;
        ...
        "llvm.uadd.sat" | "llvm.usub.sat" | "llvm.sadd.sat" | "llvm.ssub.sat" => {
            ...
            // Rust's `dec2flt` (float parsing, reached by temen-leng) emits `usub.sat.i8`, W5 §3e.
            if bits == 8 || bits == 16 {
                let ty = IntTy::I32;
...
/// Lower a call to an external **libm math** function that has a direct Temen float op (`sqrt`,
...
/// have no Temen op, so they fall through to the call path (currently `Unsupported`).
fn lower_libm_call(
    ctx: &mut BlockCtx,
    c: &crate::ll::ast::Call,
    types: &Types,
...
/// `temen-wasm` lowers `memory.copy`/`fill`. Copies **load all chunks then store all** (overlap-safe,
...
fn lower_mem_intrinsic(ctx: &mut BlockCtx, c: &crate::ll::ast::Call) -> Result<bool, Error> {
    ...
    // byte-loop `__temen_memcpy` helper for oversized/variable lengths), which paid one confinement
...
fn lower_float_intrinsic(
    ...
    if let Some(shape) = args
        ...
        .filter(|s| s.is_float())
    {
        use temen_ir::{VFloatBinOp as VB, VFloatUnOp as VU};
    ...
    // temen-ir op — scalarize: explode the two `f32` lanes, apply the scalar float op per lane, repack
...
/// Temen §3d data-stack grows *up* (`sp` increases with depth). Returning raw `sp` would invert the check
...
/// a **trap** (the Temen abort, §3b/§5): the on-ramp drops the call and relies on the `unreachable` that
...
fn lower_eh_unwinder<A>(
    ...
    match name {
        "__cxa_throw" => {
            ...
            // object when the catching handler completes (`__temen_eh_destroy` guards the null case).
...
fn lower_eh_call(ctx: &mut BlockCtx, c: &crate::ll::ast::Call, name: &str) -> Result<bool, Error> {
    ...
    match name {
        ...
        "__cxa_end_catch" => {
            ...
            // type). The null guard + indirect call live in the `__temen_eh_destroy` helper because this
...
impl<'a> BlockCtx<'a> {
    ...
    fn eh_unwind(&mut self, base: u64) -> Result<(), Error> {
        // Hand off to `__temen_eh_unwind(base)`: it pops the innermost handler and long-jumps into its
    ...
    /// (Temen has no constant pool — constants are instructions, §3b).
...
fn translate_block(
    ...
    for (idx, instr) in bb.instrs.iter().enumerate() {
        ...
        // §6 source map: the Temen ops this LLVM instruction lowers to inherit its `!DILocation`.
...
/// `IntTy`; `i8`/`i16` are **narrow** — temen-ir has no sub-word atomic (it keeps only `i32`/`i64`),
...
/// Map an LLVM `atomicrmw` binop to the temen-ir [`AtomicRmwOp`], if temen-ir has it natively. `nand`,
...
// Narrow-atomic RMW op codes passed to the `__temen_atomic_rmw_narrow` helper.
...
/// LLVM `atomicrmw` binop → narrow-helper opcode (the subset temen-ir/the helper supports).
...
// aligned word (DESIGN §3b note 2): keep the IR i32/i64-only and splice the field in `__temen_atomic_*`
...
fn lower_i128(ctx: &mut BlockCtx, instr: &Instruction, types: &Types) -> Result<bool, Error> {
    ...
    match instr {
        ...
        // div/rem: the synthesized 128÷128 long-division helper (`__temen_udivmod128`) returns both
...
fn translate_inst(ctx: &mut BlockCtx, instr: &Instruction, types: &Types) -> Result<(), Error> {
    ...
    // `<N x i1>` boolean masks (vector `icmp`/`fcmp`/`select`/movemask) have no first-class temen-ir
    ...
    if let I::Call(c) = instr {
        ...
        // A call to an external libm-math function with a direct Temen op (`sqrt`/`floor`/…) lowers
        ...
        // A `<temen.h>` low-level builtin (`__vm_map`/`__vm_fiber_*`/`__vm_atomic_*`/`__vm_cap`/…) lowers
        // to the matching Temen IR op or `Memory` capability call. Gated on the name being external (a
    ...
    // side-table — temen-IR has no 128-bit type. This covers what `-O2` actually produces: clang
    ...
    let (dest, idx) = match instr {
        ...
        I::Load(l) => {
            ...
            if l.atomicity.is_some() {
                ...
                // Non-power-of-two integer load (e.g. an `i56` niche-discriminant field): temen-IR has no
        ...
        I::Trunc(x) => {
            // A lane-wise vector `trunc` scalarizes through the unified converter (temen-ir has no
...
fn lane_load_op(shape: temen_ir::VShape) -> temen_ir::LoadOp {
    use temen_ir::{LoadOp as L, VShape};
    match shape {
...
fn lane_store_op(shape: temen_ir::VShape) -> temen_ir::StoreOp {
    use temen_ir::{StoreOp as S, VShape};
    match shape {
...
fn tail_const_inst(shape: temen_ir::VShape, w: u64) -> Inst {
...
enum WIntChunk {
    Arith(temen_ir::VIntBinOp),
    Bit(temen_ir::VBitBinOp),
}
...
fn lower_wide(ctx: &mut BlockCtx, instr: &Instruction, types: &Types) -> Result<bool, Error> {
    use temen_ir::{VBitBinOp as Bit, VIntBinOp as VI};
    use Instruction as I;
    match instr {
        I::Store(st) => {
        ...
        I::ShuffleVector(sv) => {
            ...
            // repack into the result's representation. temen-ir's in-block `Inst::Shuffle` covers the
...
/// [`lower_wide`], before the scalar match). Each form re-verifies under `temen-verify`.
    ...
    if let Some(shape) = vec128_shape(a.get_type(types).as_ref()) {
        ...
        let inst = match op {
            BinOp::Add => Inst::VIntBin {
                shape,
                op: temen_ir::VIntBinOp::Add,
                a: av,
                b: bv,
            },
            BinOp::Sub => Inst::VIntBin {
                shape,
                op: temen_ir::VIntBinOp::Sub,
                a: av,
                b: bv,
            },
            BinOp::Mul => Inst::VIntBin {
                shape,
                op: temen_ir::VIntBinOp::Mul,
                a: av,
                b: bv,
            },
            BinOp::And => Inst::VBitBin {
                op: temen_ir::VBitBinOp::And,
                a: av,
                b: bv,
            },
            BinOp::Or => Inst::VBitBin {
                op: temen_ir::VBitBinOp::Or,
                a: av,
                b: bv,
            },
            BinOp::Xor => Inst::VBitBin {
                op: temen_ir::VBitBinOp::Xor,
                a: av,
                b: bv,
            },
            ...
            BinOp::Shl | BinOp::ShrU | BinOp::ShrS => {
                if let Some(amt) = const_splat_int(b) {
                    let vop = match op {
                        BinOp::Shl => temen_ir::VShiftOp::Shl,
                        BinOp::ShrU => temen_ir::VShiftOp::ShrU,
                        _ => temen_ir::VShiftOp::ShrS,
                    };
...
fn vfbin_op(op: FBinOp) -> Result<temen_ir::VFloatBinOp, Error> {
    use temen_ir::VFloatBinOp as V;
    Ok(match op {
        FBinOp::Add => V::Add,
        FBinOp::Sub => V::Sub,
        FBinOp::Mul => V::Mul,
...
/// (≤32 → `i32` container, 64 → `i64`) and non-integers. temen-IR has only `i32`/`i64` memory ops, so a
...
fn load_op(ty: &Type) -> Result<temen_ir::LoadOp, Error> {
    use temen_ir::LoadOp as L;
    match ty {
...
fn store_op(ty: &Type) -> Result<temen_ir::StoreOp, Error> {
    use temen_ir::StoreOp as S;
    match ty {
...
fn build_v128_from_lanes(ctx: &mut BlockCtx, shape: temen_ir::VShape, lanes: &[ValIdx]) -> ValIdx {
...
/// Per-lane (**non-uniform**) vector shift of one `v128`: temen-ir's `VShift` takes a single scalar count
/// for *every* lane, so a per-lane amount vector is scalarized — extract each lane of `data` and its own
/// count from `amt`, shift in the lane's integer type, repack. `op` is `Shl`/`ShrU`/`ShrS` (the Temen
...
fn v128_lane_shift(
    ...
    if !matches!(shape, temen_ir::VShape::I32x4 | temen_ir::VShape::I64x2) {
        return unsup(format!(
...
fn try_native_vwiden(
    ...
    let (lo, hi) = if signed {
        (temen_ir::VWidenOp::LowS, temen_ir::VWidenOp::HighS)
    } else {
        (temen_ir::VWidenOp::LowU, temen_ir::VWidenOp::HighU)
    };
...
/// the auto-vectorizer's widen/narrow. temen-ir has no vector-convert op, so we scalarize: explode the
...
/// verifies under `temen-verify` exactly as the hand-written scalar form would.
...
fn translate_indirectbr(
    ...
    let default_edge: temen_ir::Edge = (default_blk as u32, args_for[&default_blk].clone());
    let mut targets: Vec<temen_ir::Edge> = vec![default_edge.clone(); nblocks];
    for &d in &dests {
...
fn translate_switch(
    ...
    let mut targets: Vec<temen_ir::Edge> =
...
/// The temen-IR types of a block's parameters, in the fan-out order `branch_args` supplies them: the
...
/// the next case; the last falls through to the default. temen-IR has no first-class sparse jump, and
...
mod bigint_tests {
    ...
    //! of the bignum `%e`/`%g` formatter. Each runs the helper in isolation via `temen_interp::run_capture`
    ...
    fn run(func: Func, args: &[i64], mem: &[(usize, &[u32])]) -> (Vec<Value>, Vec<u8>) {
        ...
        let (res, win) = temen_interp::run_capture(&m, 0, &argv, &mut fuel, &init);
