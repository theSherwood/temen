...
//!
//! Run: `cargo test -p temen-llvm --test lua_futamura_deopt -- --nocapture`
...
fn idiv_zero_arm_deopts_while_the_fast_path_folds() {
    ...
    let r = specialize_with_config(&m, luav, &args, &cfg).expect(
        "IDIV loop specializes with the zero arm deopting (run with temen-peval/trace on a wall)",
    );
...
fn run_with_heap(r: &Module, e: u32, w: &[u8], x: u64, p: &[i64]) -> i64 {
    ...
    match temen_interp::run(&m, wr, &vs, &mut fuel)
        .expect("no trap")
        .as_slice()
    {
        [Value::I64(v)] => *v,
...
fn bc_with_heap(r: &Module, e: u32, w: &[u8], x: u64, p: &[i64]) -> i64 {
    ...
    let mut fuel = u64::MAX;
    let vs: Vec<Value> = p.iter().map(|&a| Value::I64(a)).collect();
    match temen_interp::bytecode::compile_and_run(&m, wr, &vs, &mut fuel)
        .expect("subset")
        .expect("no trap")
        .as_slice()
    {
...
fn jit_with_heap(r: &Module, e: u32, w: &[u8], x: u64, p: &[i64]) -> i64 {
    let (m, wr) = wrap_with_heap(r, e, w, x, p.len());
    match temen_jit::compile_and_run(&m, wr, p) {
