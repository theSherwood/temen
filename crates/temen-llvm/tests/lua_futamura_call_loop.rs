...
//! of unrolling. The enabling piece is [`temen_peval::PoscallModel::selective`]: on the modeled return,
...
//!
//! Run: `cargo test -p temen-llvm --test lua_futamura_call_loop -- --nocapture`
...
fn entry_regs(insp: &mut temen_interp::Inspector, luav: u32) -> (i64, u64, u64) {
    let pc = IrPc {
        module: 0,
        func: luav,
        block: 0,
...
fn a_call_bearing_loop_rolls() {
    ...
    let mut residual = specialize_with_config(&m, luav, &args, &cfg)
        .expect("a call-bearing loop rolls (run with temen-peval/trace to see any wall)");
    ...
    temen_verify::verify_module(&residual).expect("residual re-verifies");
