//! **Real Rust `std` as an temen guest** — the std-on-temen differential (LLVM.md §10; the analog of
//! `w5_rust_guest.rs`, one rung up: `no_std + alloc` → full `std`). A `std` binary is built for the
//! custom `x86_64-unknown-temen` target via `-Zbuild-std` (the `crates/temen-llvm/rust-temen/` lane),
...
//!
//! **Auto-skips** unless the full lane is present: a `nightly` toolchain with `rust-src`, and the temen
//! `std` overlay already applied to that toolchain (`rust-temen/apply-overlay.sh`). It therefore does
...
/// The lane is ready iff nightly + rust-src exist **and** the temen overlay is applied (the allocator
...
fn lane_ready() -> Option<PathBuf> {
    ...
    let applied = std_src.join("sys/alloc/temen.rs").is_file()
        && std::fs::read_to_string(std_src.join("sys/alloc/mod.rs"))
            .map(|s| s.contains("target_os = \"temen\""))
            .unwrap_or(false);
...
/// `x86_64-unknown-temen-threads.json`) and return the fat-LTO'd `.ll`. The work dir is keyed on the
...
fn build_std_bin_ll_target(name: &str, src: &str, target_file: &str) -> Option<PathBuf> {
    let stem = Path::new(target_file)
        .file_stem()
        .and_then(|s| s.to_str())?;
    let work = std::env::temp_dir().join(format!("temen_std_{name}_{stem}_{}", std::process::id()));
    ...
    let out_ll =
        std::env::temp_dir().join(format!("temen_std_{name}_{stem}_{}.ll", std::process::id()));
...
/// Override the budget with `TEMEN_STD_BUILD_TIMEOUT_SECS`.
fn wait_bounded(child: &mut std::process::Child, what: &str) -> Option<std::process::ExitStatus> {
    let budget = std::env::var("TEMEN_STD_BUILD_TIMEOUT_SECS")
        .ok()
...
fn temen_run_std_posix(
    ...
    let exit = match out.outcome {
        ...
        temen_run::Outcome::Returned(ref v) => match v.first() {
            ...
            Some(temen_interp::Value::I64(x)) => *x as u8,
            _ => 0,
        },
    };
...
fn temen_run_std_net(
    ...
    temen_verify::verify_module(&t.module).expect("the translated std binary verifies");
    let (cap, posix) = temen_run::posix::posix_cap(0, 0, Vec::new());
    let net = temen_run::posix::net_cap(&posix);
    seed(&posix);
    let out = temen_run::instantiate(t.module)
        .expect("instantiate")
        .run_with_caps(
...
fn temen_run_std(name: &str, src: &str) -> Option<(Vec<u8>, u8)> {
    let ll = build_std_bin_ll(name, src)?;
    let t =
        temen_llvm::translate_ll_path(&ll).expect("on-ramp translates the std binary's LLVM IR");
    temen_verify::verify_module(&t.module).expect("the translated std binary verifies");
    ...
    let run = temen_run::run_powerbox(&t.module, b"").expect("powerbox run");
...
/// Translate + verify + run `src` built for the **threaded** target (`x86_64-unknown-temen-threads`,
...
/// (`run_with_caps_parallel`, THREADS.md 4c) → `(stdout, exit code)`. Unlike `temen_run_std_threads`
...
fn temen_run_std_with_args(name: &str, src: &str, argv: &[&[u8]]) -> Option<(Vec<u8>, u8)> {
    let ll = build_std_bin_ll(name, src)?;
    let t =
        temen_llvm::translate_ll_path(&ll).expect("on-ramp translates the std binary's LLVM IR");
    temen_verify::verify_module(&t.module).expect("the translated std binary verifies");
...
fn native_oracle_args(name: &str, src: &str, extra_args: &[&str]) -> Option<(Vec<u8>, u8)> {
    ...
    let dir = std::env::temp_dir().join(format!("temen_std_native_{name}_{}", std::process::id()));
...
fn temen_run_std_streams(name: &str, src: &str) -> Option<(Vec<u8>, Vec<u8>, u8)> {
    let ll = build_std_bin_ll(name, src)?;
    let t =
        temen_llvm::translate_ll_path(&ll).expect("on-ramp translates the std binary's LLVM IR");
    temen_verify::verify_module(&t.module).expect("the translated std binary verifies");
...
fn native_oracle_streams(name: &str, src: &str) -> Option<(Vec<u8>, Vec<u8>, u8)> {
    let host_src = src.replace("#![feature(restricted_std)]\n", "");
    let dir = std::env::temp_dir().join(format!("temen_std_natst_{name}_{}", std::process::id()));
    std::fs::create_dir_all(&dir).ok()?;
    let rs = dir.join("main.rs");
    let bin = dir.join("oracle");
    std::fs::write(&rs, host_src).ok()?;
...
/// code** through `Termination` runs on temen byte-identical to the native-equivalent oracle. This is
/// pure compute (no I/O), so it passes even with only the `unsupported` PAL for stdio.
#[test]
fn std_bin_runs_on_temen_via_powerbox() {
    if lane_ready().is_none() {
        eprintln!(
            "note: skipping std_guest (need nightly + rust-src + the temen std overlay applied — \
             run crates/temen-llvm/rust-temen/apply-overlay.sh)"
        );
        return;
    }
    ...
    let Some((stdout, exit)) = temen_run_std("temen_std_compute", src) else {
    ...
    assert_eq!(
        exit, 140,
        "Σ i² for i<8 = 140; ran on temen to the computed exit code"
    );
}

/// S1c — the temen `std::sys::temen` PAL: real `println!` reaches the host through the powerbox `write`
...
fn std_stdout_and_exit_match_native() {
    if lane_ready().is_none() {
        eprintln!("note: skipping std_guest stdout (need the temen std overlay — see rust-temen/)");
        return;
    }

    let src = "#![feature(restricted_std)]\n\
         fn main() {\n\
         \x20   println!(\"hello from std on temen\");\n\
         \x20   let v: Vec<u32> = (1..=5).collect();\n\
         \x20   println!(\"sum(1..=5) = {}\", v.iter().sum::<u32>());\n\
         \x20   std::process::exit(7);\n\
         }\n";

    let Some((temen_stdout, temen_exit)) = temen_run_std("temen_std_hello", src) else {
        eprintln!("note: skipping std_guest stdout (build-std produced no .ll)");
        return;
    };
    let Some((native_stdout, native_exit)) = native_oracle("temen_std_hello", src) else {
        // No host rustc — still assert the temen side against the known-good bytes.
        assert_eq!(temen_stdout, b"hello from std on temen\nsum(1..=5) = 15\n");
        assert_eq!(temen_exit, 7);
        return;
    };
    assert_eq!(
        temen_stdout, native_stdout,
        "real std `println!` on temen matches native stdout byte-for-byte"
    );
    assert_eq!(
        temen_exit, native_exit,
        "std `process::exit` code matches native"
    );
}

/// S1e — distinct `stderr`: the temen PAL's `Stderr` routes to the powerbox stderr `Stream` (the
...
fn std_stderr_is_distinct_from_stdout() {
    if lane_ready().is_none() {
        eprintln!("note: skipping std_guest stderr (need the temen std overlay — see rust-temen/)");
        return;
    }
    ...
    let Some((temen_out, temen_err, _)) = temen_run_std_streams("temen_std_stderr", src) else {
        eprintln!("note: skipping std_guest stderr (build-std produced no .ll)");
        return;
    };
    // The temen side is fully determined: stdout has only the `println!` lines, stderr only `eprintln!`.
    assert_eq!(
        String::from_utf8_lossy(&temen_out),
        "out line 1\nout line 2\n"
    );
    assert_eq!(
        String::from_utf8_lossy(&temen_err),
        "err line 1\nerr line 2\n"
    );

    // Cross-check both streams against a real native run.
    if let Some((n_out, n_err, _)) = native_oracle_streams("temen_std_stderr", src) {
        assert_eq!(temen_out, n_out, "temen stdout matches native stdout");
        assert_eq!(temen_err, n_err, "temen stderr matches native stderr");
    }
}

/// S1d — `std::env::args`: the temen PAL's `init` captures the powerbox-threaded `argv`, and the temen
...
fn std_env_args_match_native() {
    if lane_ready().is_none() {
        eprintln!("note: skipping std_guest args (need the temen std overlay — see rust-temen/)");
        return;
    }
    ...
    let temen_argv: &[&[u8]] = &[b"prog", b"alpha", b"beta gamma"];

    let Some((temen_stdout, _)) = temen_run_std_with_args("temen_std_args", src, temen_argv) else {
        eprintln!("note: skipping std_guest args (build-std produced no .ll)");
        return;
    };
    // The temen side is fully determined; assert it directly (argv[0] = "prog").
    let expected = "argc = 3\nargv[0] = prog\nargv[1] = alpha\nargv[2] = beta gamma\n";
    assert_eq!(
        String::from_utf8_lossy(&temen_stdout),
        expected,
        "std::env::args on temen echoes the powerbox-threaded argv"
    );
    ...
    if let Some((native_stdout, _)) = native_oracle_args("temen_std_args", src, &extra) {
        let temen_tail: Vec<&str> = std::str::from_utf8(&temen_stdout)
            .unwrap()
            .lines()
        ...
        let native_tail: Vec<&str> = std::str::from_utf8(&native_stdout)
            ...
            .collect();
        assert_eq!(
            temen_tail, native_tail,
            "argv[1..] on temen matches native (argc and the passed args agree)"
        );
        assert!(
...
/// the temen PAL's `__vm_host_call` bridge to the granted `posix` personality (`OP_CLOCK`). Run with a
...
fn std_time_reads_the_posix_clock() {
    if lane_ready().is_none() {
        eprintln!("note: skipping std_guest time (need the temen std overlay — see rust-temen/)");
        return;
    }
...
/// S2 (env) — `std::env::{var_os, set_var, remove_var}` via the posix-cap path: the temen `env` module
...
fn std_env_var_os_round_trips() {
    if lane_ready().is_none() {
        eprintln!("note: skipping std_guest env (need the temen std overlay — see rust-temen/)");
        return;
    }
...
/// S2 (fs) — `std::fs` via the posix-cap path: the temen `fs` module reaches the personality's in-memory
...
fn std_fs_round_trips() {
    if lane_ready().is_none() {
        eprintln!("note: skipping std_guest fs (need the temen std overlay — see rust-temen/)");
        return;
    }
    ...
    let src = "#![feature(restricted_std)]\n\
         ...
         \x20   f.write_all(b\"hello temen fs\").expect(\"write\");\n\
         \x20   drop(f);\n\
...
fn std_process_round_trips() {
    if lane_ready().is_none() {
        eprintln!(
            "note: skipping std_guest process (need the temen std overlay — see rust-temen/)"
        );
        return;
    }
...
fn std_fs_dir_ops() {
    if lane_ready().is_none() {
        eprintln!("note: skipping std_guest fs-dir (need the temen std overlay — see rust-temen/)");
        return;
    }
...
/// the temen PAL's `sys/random` leaf, which supplies a per-guest-deterministic seed (no host RNG op), so
...
/// byte-identical to native even though temen's seed differs from native's real random seed — the
/// differential holds on *semantics*, which is what a fixed/deterministic seed buys (LLVM.md §10).
#[test]
fn std_hashmap_round_trips() {
    if lane_ready().is_none() {
        eprintln!(
            "note: skipping std_guest hashmap (need the temen std overlay — see rust-temen/)"
        );
        return;
    }
    ...
    let Some((stdout, _)) = temen_run_std("temen_std_hashmap", src) else {
    ...
    if let Some((native, _)) = native_oracle("temen_std_hashmap_oracle", src) {
        assert_eq!(
            stdout, native,
...
fn std_net_round_trips() {
    if lane_ready().is_none() {
        eprintln!("note: skipping std_guest net (need the temen std overlay — see rust-temen/)");
        return;
    }

    struct Canned;
    impl temen_posix::NetStream for Canned {
    ...
    struct Scripted;
    impl temen_posix::NetDelegate for Scripted {
        fn connect(
            &mut self,
            addr: &temen_posix::NetAddr,
        ) -> Result<Box<dyn temen_posix::NetStream>, i64> {
            assert_eq!(addr.port, 80);
            assert_eq!(&addr.addr[..4], &[93, 184, 216, 34]);
            Ok(Box::new(Canned))
        }
        fn resolve(&mut self, host: &str) -> Result<Vec<temen_posix::NetAddr>, i64> {
            assert_eq!(host, "example.com");
            let mut addr = [0u8; 16];
            addr[..4].copy_from_slice(&[93, 184, 216, 34]);
            Ok(vec![temen_posix::NetAddr {
                v6: false,
                port: 0,
                addr,
            }])
    ...
    let Some((stdout, _)) = temen_run_std_net("temen_std_net", src, |p| p.set_net(Scripted)) else {
...
// ---------------------------------------------------------------------------------------------
// #821 — the threaded target spec (`x86_64-unknown-temen-threads`, `singlethread=false`). These build
...
/// The pure-compute Σ i² program (the lean spec's `std_bin_runs_on_temen_via_powerbox`) built for the
...
#[test]
fn std_threads_spec_bin_runs_on_temen() {
    if lane_ready().is_none() {
        eprintln!(
            "note: skipping std_guest threads (need the temen std overlay — see rust-temen/)"
        );
        return;
    }
    ...
    let Some((stdout, exit)) = temen_run_std_threads("temen_stdt_compute", src) else {
...
fn std_threads_spec_futex_sync_round_trips() {
    ...
    let Some((_, exit)) = temen_run_std_threads("temen_stdt_sync", src) else {
        eprintln!("note: skipping std_guest threads (build-std produced no .ll)");
        return;
    };
    ...
    if let Some((_, oracle)) = native_oracle("temen_stdt_sync_oracle", src) {
...
fn std_threads_spec_thread_local_round_trips() {
    ...
    let Some((_, exit)) = temen_run_std_threads("temen_stdt_tls", src) else {
        eprintln!("note: skipping std_guest threads (build-std produced no .ll)");
        return;
    };
    // 100 + 23 = 123.
    ...
    if let Some((_, oracle)) = native_oracle("temen_stdt_tls_oracle", src) {
...
/// outer value) and its return value comes back through `join`. Exercises the whole `sys/thread/temen`
...
fn std_threads_spec_spawn_join_returns_value() {
    ...
    let Some((_, exit)) = temen_run_std_threads("temen_stdt_spawn", src) else {
        eprintln!("note: skipping std_guest threads (build-std produced no .ll)");
        return;
    };
    assert_eq!(exit, 42, "spawn(move || 30+12).join() == 42");
    if let Some((_, oracle)) = native_oracle("temen_stdt_spawn_oracle", src) {
...
fn std_threads_spec_shared_atomic_counter() {
    ...
    let Some((_, exit)) = temen_run_std_threads("temen_stdt_counter", src) else {
        eprintln!("note: skipping std_guest threads (build-std produced no .ll)");
        return;
    };
    assert_eq!(exit, 40, "4 threads × 10 fetch_add = 40");
    if let Some((_, oracle)) = native_oracle("temen_stdt_counter_oracle", src) {
        assert_eq!(
            exit, oracle,
...
fn std_threads_spec_yield_and_sleep() {
    ...
    let Some((_, exit)) = temen_run_std_threads("temen_stdt_yieldsleep", src) else {
        eprintln!("note: skipping std_guest threads (build-std produced no .ll)");
        return;
    };
    assert_eq!(
    ...
    if let Some((_, oracle)) = native_oracle("temen_stdt_yieldsleep_oracle", src) {
        assert_eq!(
            exit, oracle,
...
fn std_threads_spec_two_thread_memnet_echo() {
    ...
    let Some((_, exit)) = temen_run_std_threads_net("temen_stdt_netecho", src) else {
        eprintln!("note: skipping std_guest threads (build-std produced no .ll)");
        return;
    };
    assert_eq!(
    ...
    if let Some((_, oracle)) = native_oracle("temen_stdt_netecho_oracle", src) {
        assert_eq!(
            exit, oracle,
...
fn std_threads_spec_mpsc_channel() {
    ...
    let Some((_, exit)) = temen_run_std_threads("temen_stdt_mpsc", src) else {
        eprintln!("note: skipping std_guest threads (build-std produced no .ll)");
        return;
    };
    assert_eq!(
    ...
    if let Some((_, oracle)) = native_oracle("temen_stdt_mpsc_oracle", src) {
...
fn std_threads_parallel_smoke_atomic_counter() {
    ...
    let Some((_, exit)) = temen_run_std_threads_parallel("temen_stdt_counter_par", src) else {
        eprintln!("note: skipping std_guest threads (build-std produced no .ll)");
        return;
    };
    assert_eq!(
...
fn std_threads_parallel_smoke_spawn_join() {
    ...
    let Some((_, exit)) = temen_run_std_threads_parallel("temen_stdt_spawn_par", src) else {
        eprintln!("note: skipping std_guest threads (build-std produced no .ll)");
        return;
    };
    assert_eq!(
...
fn std_threads_parallel_smoke_mpsc_channel() {
    ...
    let Some((_, exit)) = temen_run_std_threads_parallel("temen_stdt_mpsc_par", src) else {
        eprintln!("note: skipping std_guest threads (build-std produced no .ll)");
        return;
    };
    assert_eq!(
