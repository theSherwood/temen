...
//! as a confined child by an Temen parent, runs (it uses `snprintf`, which forces a synthesized powerbox
...
fn child_entry_module_is_instantiable() {
    ...
    // An Temen parent that `instantiate_module`s the child (op 5) into a carve one child-window up, then
