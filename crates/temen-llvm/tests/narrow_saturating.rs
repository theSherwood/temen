//! Narrow-width saturating intrinsics (`llvm.{u,s}{add,sub}.sat.i{8,16}`), run on temen.
//!
//! Rust's `u8`/`i8`/`u16`/`i16` `saturating_add`/`saturating_sub` emit these — and so does `std`'s
//! float parser (`dec2flt`, `usub.sat.i8`), which is why translating the real `temen-leng` as an temen
//! guest hit them (W5 §3e). The on-ramp previously supported only i32/i64 saturating; this pins the
//! i8/i16 extension: a Rust guest doing all four ops at both narrow widths runs on temen and matches a
...
#[test]
fn narrow_saturating_ops_run_on_temen() {
    ...
    let t = temen_llvm::translate_ll_path(&ll).expect("translate narrow-saturating guest");
    ...
    for n in [
        ...
        let jit = match temen_jit::compile_and_run(&t.module, entry, &[sp, n]).expect("jit") {
            temen_jit::JitOutcome::Returned(v) => v[0],
            o => panic!("jit: {o:?}"),
        };
        assert_eq!(interp, jit, "§9 interp/JIT parity for n={n:#x}");
        assert_eq!(interp, oracle(n), "§18 temen == native for n={n:#x}");
    }
}
