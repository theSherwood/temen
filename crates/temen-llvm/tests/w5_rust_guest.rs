//! **W5 first light** (NIM.md §3e, Path L): a real Rust program runs **as a guest on temen**.
//!
//! W5 self-hosts the toolchain — the payoff is running the Rust `temen-leng` translator on temen. The
//! measured recommendation (§3e) is to reach first light through the **LLVM on-ramp** (already the
//! proven Rust→temen primitive: `rustbench`, the `peval` fixtures), before the wasm on-ramp. This is
//! that first light on **`temen-leng`-shaped work**: a `no_std + alloc` Rust program that *emits IR-ish
//! text and re-parses/folds it* — the exact surface the real translator leans on (`String`/`Vec`
//! building, byte scanning, integer format/parse) — compiled `rustc --emit=llvm-ir` → `temen-llvm`, and
//! run on temen. The Rust analog of chibicc's C self-host, one rung down.
//!
...
/// `alloc`-personality stub) + a `run(n) -> i64` doing the leng-shaped text work. `temen-llvm` prepends
...
/// temen-run guest, `temen-llvm` mistranslated real Rust.
fn oracle(n: i64) -> i64 {
    let k = (n & 63) + 1;
    let mut prog = String::new();
    let mut i = 0i64;
...
#[test]
fn rust_guest_runs_leng_shaped_work_on_temen() {
    ...
    let t =
        temen_llvm::translate_ll_path(&ll).expect("temen-llvm translates the Rust guest's LLVM IR");
    temen_verify::verify_module(&t.module).expect("the translated Rust guest verifies");
    let entry = t
        .exports
        .iter()
        .find(|(n, _)| n == "run")
    ...
    for n in [0i64, 1, 2, 7, 8, 63, 64, 255, 1000, -1] {
        ...
        let jit = match temen_jit::compile_and_run(&t.module, entry, &[sp, n]).expect("jit run") {
        ...
        assert_eq!(
            interp,
            oracle(n),
            "§18 temen-run == native oracle for n={n} (Rust guest ran correctly on temen)"
        );
    }
}
