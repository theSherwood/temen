...
//!
//! This test drives the lua_eval harness (script on stdin) on the `temen_interp::Inspector` debug
...
//!
//! Run: `cargo test -p temen-llvm --test lua_futamura_capture -- --nocapture`
...
fn capture(m: &Module, luav: u32) -> (i64, i64, u64, Vec<u8>) {
    let inst = temen_run::instantiate(m.clone()).expect("instantiate lua_eval");
...
fn chase(w: &[u8], ci: u64) -> Located {
    ...
    let sizek = rd_i32(
        w,
        proto + PROTO_SIZECODE as u64 - (PROTO_SIZECODE - PROTO_SIZEK) as u64,
    );
    ...
    assert_eq!(
        savedpc, code_addr,
        "savedpc != Proto.code — a struct offset is wrong"
    );
    assert!(
        (0..4096).contains(&sizecode),
        "implausible sizecode {sizecode} — offsets wrong"
    );
    assert!(
        (code_addr as usize) + 4 * sizecode as usize <= w.len(),
        "code past capture"
    );
...
fn chase_to_bytecode_and_validate() {
    ...
    for (i, &word) in loc.code.iter().enumerate() {
        ...
        assert!(
            op <= 82,
            "instruction {i} opcode {op} out of range — not real bytecode"
        );
    }
