...
//!
//! Run: `cargo test -p temen-llvm --test lua_futamura_gc -- --nocapture`
...
use temen_ir::{Block, Data, Func, Inst, LoadOp, Module, Terminator, ValType};
...
fn gc_cut_rolls_the_allocating_loop() {
    ...
    let r = specialize_with_config(&m, luav, &args, &cfg).expect(
        "allocating loop rolls with the alloc/GC cut (run with temen-peval/trace on a wall)",
    );
...
fn has_br_table_f(f: &temen_ir::Func) -> bool {
    f.blocks
        .iter()
