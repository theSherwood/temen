//! One-off: compare the Temen JIT running the SAME C through two frontends — chibicc (naive) vs
...
// into the loop; chibicc (no inlining) emits a real `call` per iteration, and temen-jit does no
...
fn chibicc_ir(name: &str, src: &str) -> (temen_ir::Module, u32, Vec<i64>) {
    ...
    let m = temen_text::parse_module(&ir).unwrap();
    let e = run_entry(&m);
    (m, e, vec![0]) // chibicc sp lead = 0
}

fn llvm_ir(name: &str, src: &str) -> (temen_ir::Module, u32, Vec<i64>) {
...
fn run_entry(m: &temen_ir::Module) -> u32 {
    m.funcs
        .iter()
        .position(|f| {
...
fn call(m: &temen_ir::Module, e: u32, lead: &[i64], n: i64) -> i64 {
...
fn ns(m: &temen_ir::Module, e: u32, lead: &[i64]) -> f64 {
...
fn chibicc_vs_llvm_jit() {
    println!("\nTemen JIT, same C, two frontends (ns/iter; ratio = chibicc / llvm):");
    println!(
        "{:<12} {:>10} {:>10} {:>8}",
