...
fn assemble(name: &str, ll: &str) -> Option<PathBuf> {
    ...
    let llp = dir.join(format!("temen_alias_{}_{}.ll", std::process::id(), name));
...
fn on_ramp_resolves_function_alias() {
    ...
    let t = temen_llvm::translate_ll_path(&bc).expect("translate bitcode with a function alias");
    let module = t.module;
    ...
    let full = vec![
        ...
        temen_interp::Value::I32(5),
        temen_interp::Value::I32(1),
    ];
    ...
    let jit = match temen_jit::compile_and_run(&module, entry, &slots).expect("jit run") {
