//! **General (non-rotate) vector funnel shift** (`llvm.fshl.v4i32` with distinct operands), run on temen.
//!
...
//! `llvm.fshl.v4i32`, which is what blocked a *float-capable* `temen-leng` from running on temen (W5 §3e
...
fn assemble(name: &str, ll: &str) -> PathBuf {
    ...
    let llp = dir.join(format!("temen_vfsh_{}_{}.ll", std::process::id(), name));
...
#[test]
fn general_vector_funnel_shift_runs_on_temen() {
    let bc = assemble("fshl", FSHL_LL);
    let t = temen_llvm::translate_ll_path(&bc).expect("translate general vector funnel shift");
    let module = t.module;
    ...
    for s in [0u32, 1, 7, 16, 31, 32, 63, 0xffff_ffff] {
        let full = vec![
            temen_interp::Value::I64(t.entry_sp as i64),
            temen_interp::Value::I32(s as i32),
        ];
        let mut fuel = 10_000_000u64;
        let interp = match temen_interp::run(&module, entry, &full, &mut fuel)
            .expect("interp run")
            .as_slice()
        {
        ...
        let jit =
            match temen_jit::compile_and_run(&module, entry, &[t.entry_sp as i64, s as i32 as i64])
                .expect("jit run")
            {
        ...
        assert_eq!(interp, oracle(s), "§18 temen == native for s={s:#x}");
    }
}
