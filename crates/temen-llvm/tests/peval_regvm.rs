...
//! `clang -O2 → LLVM → temen-llvm → temen-IR`, then partial-evaluated against a fixed bytecode program
...
//! bytecode engine, and the Cranelift JIT). The four-backend ROI number — including temen-wasm-jit on
//! Wasmtime — lives in `bench/src/bin/peval_regvm.rs`.
//!
//! Run: `cargo test -p temen-llvm --test peval_regvm -- --nocapture`  (temen-llvm is workspace-excluded).
...
/// Compile C → LLVM (`clang -O2`) → temen-IR. `None` if clang is unavailable (skip).
