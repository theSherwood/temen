...
//!   `temen_run::PowerboxProgram` compiles a powerbox module *once* and runs it against many inputs,
...
//!
//! Run: `cargo test -p temen-llvm --release --test lua_futamura_bench -- --ignored --nocapture`
...
fn lua_futamura_roi() {
    ...
    let run_base = |n: u64| {
        let s = loop_script(n);
        move || {
            temen_run::run_powerbox(mr, s.as_bytes())
                .expect("baseline run")
                .stdout
        }
    };
    ...
    let (tb, ob) = best_of(reps, || {
        temen_run::run_powerbox(&b.baseline, script.as_bytes())
            .expect("baseline")
            .stdout
    });
    let (tr, or_) = best_of(reps, || {
        temen_run::run_powerbox(&b.residual, script.as_bytes())
            .expect("residual")
            .stdout
    });
    assert_eq!(ob, format!("{}\n", 3 * n).into_bytes());
...
use temen_interp::{IrPc, Stop, StopReason, Value, WatchKind};
...
fn rolled_residual_vs_interpreter_per_iteration() {
    ...
    temen_verify::verify_module(&wm).expect("verifies");
...
// ---------------------------------------------------------------------------------------------
// The compiled-module cache: `temen_run::PowerboxProgram` compiles a powerbox module ONCE and runs it
...
fn powerbox_program_amortizes_compile() {
    ...
    let mut prog =
        temen_run::PowerboxProgram::compile(m.clone()).expect("compile powerbox program");
    for s in scripts {
        let want = temen_run::run_powerbox(&m, s.as_bytes())
            .expect("run_powerbox")
            .stdout;
    ...
    for _ in 0..reps {
        ...
        for _ in 0..n {
            black_box(
                temen_run::run_powerbox(&m, script)
                    .expect("run_powerbox")
                    .stdout,
            );
        }
    ...
    for _ in 0..reps {
        ...
        let t = Instant::now();
        let mut prog = temen_run::PowerboxProgram::compile(m.clone()).expect("compile");
        for _ in 0..n {
