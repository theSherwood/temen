...
//! in-sandbox, *specializes* a module with `temen-peval`, *encodes* the residual with `temen-encode`, and
...
//!
//! The host translates the guest to temen-IR, reads the on-ramp-assigned window `size_log2` (the
...
fn peval_guest_specializes_and_jits_in_sandbox() {
    ...
    let t = temen_llvm::translate_ll_path(&bc).expect("translate the peval-jit guest to temen-IR");
    assert!(
    ...
    let module = t.module;
    temen_verify::verify_module(&module).expect("verify the translated guest");
