...
//!
//! The JIT confinement lowering ([`temen_jit`]'s `mask_addr`) is the security hinge: every guest access
...
fn build() -> Option<temen_llvm::Translated> {
    ...
    let c = dir.join(format!("temen_confine_{uniq}.c"));
    let ll = dir.join(format!("temen_confine_{uniq}.ll"));
    ...
    let t = temen_llvm::translate_ll_path(&ll).expect("translate confinement probe");
    let _ = std::fs::remove_file(&c);
    let _ = std::fs::remove_file(&ll);
    Some(t)
}

fn export(t: &temen_llvm::Translated, name: &str) -> u32 {
    t.exports
        .iter()
        .find(|(n, _)| n == name)
...
fn in_bounds_load_matches_interpreter() {
    ...
    let interp = match temen_interp::run(&t.module, e, &[Value::I64(sp), Value::I64(3)], &mut fuel)
        .expect("interp run")
        .as_slice()
    {
        [Value::I64(x)] => *x,
    ...
    let jit = match temen_jit::compile_and_run(&t.module, e, &[sp, 3]).expect("jit run") {
...
fn out_of_bounds_load_traps_memory_fault() {
    ...
    match temen_jit::compile_and_run(&t.module, e, &[sp, 1 << 28]).expect("jit run") {
