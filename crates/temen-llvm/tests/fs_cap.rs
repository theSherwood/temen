...
//! unless the test injects one via [`temen_run::Instance::run_with_caps`] — the fixed powerbox is
//! untouched.
//!
//! Two interchangeable backends behind the same protocol (dependency injection at the capability
//! boundary):
//! - [`temen_run::fs::mem_fs`] — deterministic in-memory fs, on **all three engines**;
//! - [`temen_run::fs::host_fs`] — the **real** filesystem attenuated to a temp root: the test
//!   pre-seeds `seed.txt`, the guest verifies it and leaves `out.txt` behind, and the test asserts
//!   the bytes really landed on disk.

use temen_run::{fs, Backend, Limits, Outcome, RunConfig, Value};

fn instance() -> temen_run::Instance {
    let bc = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/fixtures/fs_probe.ll");
    let t = temen_llvm::translate_ll_path(bc).expect("translate fs probe");
...
fn fs_probe_host_fs_all_backends() {
    for backend in [Backend::TreeWalk, Backend::Bytecode, Backend::Jit] {
        let root =
            std::env::temp_dir().join(format!("temen-fs-probe-{}-{backend:?}", std::process::id()));
