...
fn check(name: &str, src: &str) {
    ...
    let c = dir.join(format!("temen_snp_{}_{}.c", std::process::id(), name));
    let bc = dir.join(format!("temen_snp_{}_{}.ll", std::process::id(), name));
    let exe = dir.join(format!("temen_snp_{}_{}", std::process::id(), name));
