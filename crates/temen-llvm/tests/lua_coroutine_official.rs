...
//! production engines do — see `temen_interp::MAX_CALL_DEPTH`. No translator or coroutine/debug change was
...
fn run(backend: Backend) -> temen_run::Run {
    ...
    let t = temen_llvm::translate_ll_path(bc).expect("translate Lua coroutine.lua bitcode");
