...
//! matches `x0 + 3*(c0+1)` byte-identically on all three backends (tree-walk / bytecode / temen-jit).
//!
...
//!
//! Run: `cargo test -p temen-llvm --test lua_futamura_rolled -- --nocapture`
//! (add `--features temen-peval/trace` to print the first specializer blocker while iterating).
...
fn dynamic_counter_rolls_the_real_luav_execute_loop() {
    ...
    let r = specialize_with_config(&m, luav, &args, &cfg)
        .expect("real luaV_execute rolls (run with temen-peval/trace to see any wall)");
    temen_verify::verify_module(&r).expect("residual re-verifies");
    ...
    assert_eq!(
        jit(&wm, we, &cap_args),
        150,
        "temen-jit residual @ captured inputs"
    );
