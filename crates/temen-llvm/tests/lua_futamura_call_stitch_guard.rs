...
//!
//! Run: `cargo test -p temen-llvm --test lua_futamura_call_stitch_guard -- --nocapture`

use temen_interp::{IrPc, Stop, StopReason, Value};
use temen_ir::{Block, CmpOp, Func, Inst, IntTy, LoadOp, Module, Terminator, ValType};
use temen_peval::{
...
fn guarded_stitch_takes_the_right_arm() {
    ...
    temen_verify::verify_module(&r1).expect("match variant verifies");
    let out1 = temen_run::run_powerbox(&r1, SCRIPT.as_bytes()).expect("match variant");
    println!(
    ...
    temen_verify::verify_module(&r2).expect("mismatch variant verifies");
    let out2 = temen_run::run_powerbox(&r2, SCRIPT.as_bytes()).expect("mismatch variant");
    println!(
