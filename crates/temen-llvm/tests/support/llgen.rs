...
//! bounds, no div, no poison flags), so it must **never trap**, and all three temen backends
...
/// seed with no bytes is reproducible. Mirrors `temen`'s `irgen::Gen`.
pub struct Gen {
    data: Vec<u8>,
    pos: usize,
    rng: u64,
...
pub fn check(ll: &str, oracle: i64) -> Result<Ran, String> {
    ...
    let t = match temen_llvm::translate_ll_str(ll) {
        Ok(t) => t,
        Err(_) => return Ok(ran),
    };
    ran.translated = true;
    temen_verify::verify_module(&t.module).map_err(|e| format!("verify failed: {e:?}"))?;
    let run = t
        .exports
        .iter()
        .find(|(n, _)| n == "run")
    ...
    match temen_interp::run(&t.module, run, &[Value::I64(sp)], &mut fuel) {
    ...
    match temen_jit::compile_and_run(&t.module, run, &[sp]) {
        Err(temen_jit::JitError::Unsupported(_)) => {}
        Ok(temen_jit::JitOutcome::Returned(v)) if v == vec![oracle] => ran.jit = true,
