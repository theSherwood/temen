...
//!
//! Run: `cargo test -p temen-llvm --test lua_futamura_specialize -- --nocapture`
//! (add `--features temen-peval/trace` to replay the breadcrumb that pinpointed the `SetJmp` site).
...
fn dispatch_folds_with_constant_bytecode() {
    ...
    // THE WALL (characterized 2026-08-05, with the `temen-peval/trace` breadcrumb). Both the baseline
