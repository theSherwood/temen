...
pub fn auto(script: &str) -> AutoResult {
    ...
    let baseline = temen_run::run_powerbox(&b.baseline, script.as_bytes()).expect("baseline run");
    let residual = temen_run::run_powerbox(&b.residual, script.as_bytes()).expect("residual run");
    AutoResult {
...
pub fn auto_build(script: &str) -> AutoBuild {
    ...
    for _ in 0..MAX_EVENTS {
        ...
        if pc.block == pblock as usize || pc.block == tblock as usize {
            ...
            let rd_val = |insp: &temen_interp::Inspector, idx: u32| match insp
