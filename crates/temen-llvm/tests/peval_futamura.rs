...
//! sandbox, builds a small accumulator-machine *interpreter* in temen-IR, **specializes it against a
...
fn peval_guest_specializes_interpreter_and_jits_in_sandbox() {
    ...
    let t =
        temen_llvm::translate_ll_path(&bc).expect("translate the peval-futamura guest to temen-IR");
    assert!(
