//! End-to-end W5 over the **LLVM frontend**: a real LLVM-bitcode → TEMEN-IR translation (with its §6
...
fn compile_o0g(name: &str, src: &str) -> Option<PathBuf> {
    ...
    let c = dir.join(format!("temen_llvm_dap_{}_{}.c", std::process::id(), name));
    let bc = dir.join(format!("temen_llvm_dap_{}_{}.ll", std::process::id(), name));
...
";
    ...
    let text = temen_text::print_module(&t.module);
    assert!(
        temen_text::parse_module(&text).is_ok(),
