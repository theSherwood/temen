...
//! compiled `clang -O2 → LLVM → temen-llvm → temen-IR`, then **partial-evaluated** by `temen-peval` against
...
//!
//! Run:   `cargo test -p temen-llvm --test lisp_demo -- --nocapture`            (correctness)
//! Bench: `cargo test -p temen-llvm --test lisp_demo -- --ignored --nocapture`  (size + speed)
//! (temen-llvm is workspace-excluded; run from `crates/temen-llvm`.)
...
use temen_ir::Module;
use temen_peval::{optimize_module, specialize, specialize_with_config, SpecArg, SpecConfig};
...
/// Compile C → LLVM bitcode (`clang -O2`) → temen-IR. Returns the module, the frontend's entry
/// data-stack pointer (`sp`), and the exported `name → func index` table. Mirrors `peval_demo.rs`.
fn compile_c_to_temen(name: &str, src: &str) -> (Module, i64, Vec<(String, u32)>) {
    ...
    let t = temen_llvm::translate_ll_path(&bc).expect("temen-llvm translate");
    let _ = std::fs::remove_file(&cf);
    let _ = std::fs::remove_file(&bc);
    (t.module, t.entry_sp as i64, t.exports)
}

/// The index of an exported function by name (here, `run`); temen-llvm prepends the `sp` data-stack
...
fn run_interp(m: &Module, entry: u32, args: &[i64]) -> i64 {
    let vals: Vec<_> = args.iter().map(|&a| temen_interp::Value::I64(a)).collect();
    let mut fuel = u64::MAX;
    match temen_interp::run(m, entry, &vals, &mut fuel) {
...
fn sizes(m: &Module) -> (usize, usize) {
    ...
    let bytes = temen_encode::encode_module(m).len();
    (blocks, bytes)
}
...
fn expr_specializes_and_matches_interpreter() {
    let (m, sp, exports) = compile_c_to_temen("expr", LISP_C);
    let entry = func_named(&exports, "run");
...
fn fib_specializes_and_matches_interpreter() {
    let (m, sp, exports) = compile_c_to_temen("fib", LISP_C);
    let entry = func_named(&exports, "run");
    verify_module(&m).expect("interpreter verifies");
...
fn lisp_futamura_roi() {
    let (m, sp, exports) = compile_c_to_temen("roi", LISP_C);
    let entry = func_named(&exports, "run");
