...
//! `clang -O2 → LLVM → temen-llvm → temen-IR`, then **partial-evaluated** by `temen-peval` against a fixed
...
//!
//! Run:        `cargo test -p temen-llvm --test peval_demo -- --nocapture`            (correctness)
//! Bench:      `cargo test -p temen-llvm --test peval_demo -- --ignored --nocapture`  (size + speed)
...
/// Compile C → LLVM bitcode (`clang -O2`) → temen-IR. Returns the module and the frontend's entry
/// data-stack pointer (`sp`). Mirrors `frontend_bench.rs`.
fn compile_c_to_temen(name: &str, src: &str) -> (Module, i64) {
...
/// The `run(sp, prog, input) -> i64` entry (temen-llvm prepends the `sp` data-stack pointer).
...
fn specialize_bf(m: &Module, entry: u32, sp: i64) -> Result<Module, temen_peval::SpecError> {
    specialize(
        m,
        entry,
        &[
...
fn bf_specializes_and_matches_interpreter() {
    let (m, sp) = compile_c_to_temen("correct", BF_C);
    let entry = run_entry(&m);
...
fn bf_futamura_roi() {
    let (m, sp) = compile_c_to_temen("roi", BF_C);
    let entry = run_entry(&m);
    ...
    let interp_interp = || {
        ...
        match temen_interp::run(
            &m,
            entry,
            &[
                temen_interp::Value::I64(sp),
                temen_interp::Value::I64(pa),
                temen_interp::Value::I64(n),
            ],
            &mut fuel,
        ) {
