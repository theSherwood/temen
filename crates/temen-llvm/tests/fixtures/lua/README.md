...
(`crates/temen-run/demos/libm/libm.c`, `crates/temen-run/demos/strtod/strtod.c`), running a float script
...
`RunConfig::stdin` and asserts the exact stdout on all three engines. The `.temen` for the page is built
...
## Lua warm-runtime-snapshot fixture (issue #805)

`lua_snapshot.ll` is the **two-phase warm-runtime-snapshot** twin of `lua_eval.ll` — same Lua core +
editor libs + guest layers, but built with **`lua_snapshot_harness.c`** instead of `lua_eval_harness.c`.
That harness splits the single-shot `main` into a program-independent `warmup` (create the `lua_State` +
open the editor libs into a static) and a per-Run `eval_run` (read the chunk from stdin, run it over the
warm state), so the browser warm session (`temen_warm_open`/`temen_warm_eval`, WASM_AOT.md) can snapshot the
window after `warmup` and restore-and-eval per Run — the Lua runtime init is paid once, not per Run. It
uses the **on-ramp heap** (`luaL_newstate`'s default `realloc`/`free` allocator → the translator's
`__temen_malloc`), not `lua_eval`'s 48 MiB static arena, so the snapshot image is the small live prefix
(~4 MiB, like QuickJS) and the declared window stays under the warm session's mapped window. Fresh-per-Run
isolation holds (each Run restores the same post-`warmup` image). The playground's warm Lua card and the
snapshot worker (issue #804) drive it; `browser/lua-warm-snapshot-test.mjs` asserts cold ≡ warm ≡ warm+JIT
byte-for-byte + isolation.

The shipped artifact is the committed **`browser/web/assets/lua_snapshot.temen`** (a generated binary,
like the vendored `doom1.wad`). Unlike `lua_eval.ll`, the ~76k-line linked `lua_snapshot.ll` is **not**
committed (no Rust test consumes it — only the browser `.temen` does), and `build-onramp-assets.mjs` does
**not** rebuild it — so it is regenerated by hand and the resulting `.temen` committed:

**Regenerating:** run the `lua_eval` recipe above with `lua_snapshot_harness.c` swapped in for
`lua_eval_harness.c` (produce `harness.ll` from it) to get `lua_snapshot.ll`, then translate and commit
the `.temen`:

```sh
temen-llvm-translate lua_snapshot.ll -o browser/web/assets/lua_snapshot.temen --host-page 65536 --null-guard
```
...
`temen_run::Instance::run_with_caps`. Two interchangeable backends live in `temen_run::fs`:
...
`temen_interp::MAX_CALL_DEPTH`. Verified regression-free (durable + interp + `jit_diff` suites green).
