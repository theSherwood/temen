 ...
 * (`luaL_newstate`'s default `realloc`/`free` allocator -> the translator's `__temen_malloc` bump heap),
 * so the snapshot image is the small live prefix `[0, brk)` (like QuickJS's ~4 MiB) and the declared
 * window stays under the warm session's mapped window (temen_warm_open requires declared < mapped). `free`
 ...
 * session (`temen_warm_open` snapshots after `warmup`; `temen_warm_eval` restores + runs `eval_run`).
 */
#include "lua.h"
#include "lauxlib.h"
#include "lualib.h"
