...
//!   2. specializes it with `temen-peval` (inlining the call, folding the constants into one function),
//!   3. serializes the residual with `temen-encode` (the binary module blob),
...
//! `peval_jit.rs` test compiles this to temen-IR and runs it under `run_powerbox` (which grants the
...
use temen_ir::{BinOp, Block, Func, Inst, IntTy, Memory, Module, Terminator, ValType};
...
pub extern "C" fn main(argc: i32, argv: *const *const u8) -> i32 {
    ...
    let residual = match temen_peval::specialize(&m, 0, &[temen_peval::SpecArg::Dynamic, temen_peval::SpecArg::Dynamic]) {
        Ok(r) => r,
        Err(_) => {
    ...
    let blob = temen_encode::encode_module(&residual);
    puts(b"emitted ");
