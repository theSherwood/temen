//! The in-sandbox `temen-peval` probe (DESIGN.md §20c). A `no_std`/`panic=abort` powerbox program
//! that builds a small module, calls `temen_peval::specialize`, and prints a summary of the residual
//! (`funcs`, total `blocks`, total `insts`) — one decimal per line. The `peval_in_sandbox.rs` test
//! compiles this to temen-IR and runs it, asserting the printed summary equals the **same** specialization
...
use temen_ir::{Block, Func, IntTy, BinOp, Inst, Module, Terminator, ValType};
...
pub extern "C" fn main() -> i32 {
    let m = build_module();
    match temen_peval::specialize(&m, 0, &[]) {
        Ok(res) => {
