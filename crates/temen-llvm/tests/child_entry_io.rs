...
fn child_entry_writes_through_a_regranted_stdout() {
    ...
    // (`name_off:u32=18432 | name_len:u32=6`). The parent lays these in its window (all above the #1094
    // NULL guard), then op-13-spawns the child re-granting the `stdout` handle (arg `v2`) under that name.
