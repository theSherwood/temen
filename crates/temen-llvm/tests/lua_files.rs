...
//! - [`temen_run::fs::mem_fs`] — deterministic in-memory fs, asserted on **all three engines**;
//! - [`temen_run::fs::host_fs`] — the real filesystem attenuated to a fresh temp root, proving the
...
fn instance() -> temen_run::Instance {
    ...
    let t = temen_llvm::translate_ll_path(bc).expect("translate Lua files.lua bitcode");
...
fn lua_files_host_fs() {
    let root = std::env::temp_dir().join(format!("temen-lua-files-{}", std::process::id()));
