...
//!
//! Run:   `cargo test -p temen-llvm --test peval_corpus -- --nocapture`            (correctness + size)
//! Bench: `cargo test -p temen-llvm --test peval_corpus -- --ignored --nocapture`  (full matrix)
...
/// Compile C → LLVM bitcode (`clang -O2`) → temen-IR. `extra` carries per-interpreter clang flags
/// (the tree-walker needs `-fno-optimize-sibling-calls` to keep its recursion as real calls).
fn compile_c_to_temen(name: &str, src: &str, extra: &[&str]) -> (Module, i64, Vec<(String, u32)>) {
...
/// Reference interpreter (temen-interp) — the honest, slow "interpreted interpreter" baseline.
...
fn sizes(m: &Module) -> Sizes {
    Sizes {
        ...
        insts: m
            ...
            .map(|b| b.insts.len())
            .sum(),
        bytes: temen_encode::encode_module(m).len(),
    }
}

/// Emit one machine-readable metric row when `TEMEN_BENCH_CSV` is set: `CSV,<bench>,<case>,<metric>,
/// <value>` on stdout (run with `--nocapture` and `grep '^CSV,'`). Off by default.
fn csv(case: &str, metric: &str, value: f64) {
    if std::env::var_os("TEMEN_BENCH_CSV").is_some() {
...
fn bf_case(
    ...
    let (m, sp, exports) = compile_c_to_temen(slug, &bf_c(prog), &[]);
...
fn corpus() -> Vec<Case> {
    ...
    {
        let (m, sp, exports) =
            compile_c_to_temen("lisp_expr", LISP_C, &["-fno-optimize-sibling-calls"]);
        let entry = run_entry(&m, &exports);
    ...
    {
        let (m, sp, exports) =
            compile_c_to_temen("lisp_fib", LISP_C, &["-fno-optimize-sibling-calls"]);
        let entry = run_entry(&m, &exports);
...
/// guard, like `temen-peval`'s `size_corpus`).
#[test]
