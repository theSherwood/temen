...
//! (native `clang -O2`, temen tree-walk / bytecode / JIT), to see how the stack holds up on realistic,
...
//!   (push/add/mul/sub/xor/dup): an interpreter running on the Temen.
//!
//! Run: cd crates/temen-llvm && cargo run --release --example end_to_end
...
fn main() {
    ...
    for &(name, large_fast, large_bc, large_tw, src) in PROGRAMS {
        ...
        let jit_chk = match temen_jit::compile_and_run(&t.module, e, &[sp, VERIFY_N]).unwrap() {
        ...
        let tw_ns = per_iter(large_tw, |n| {
            let mut fuel = u64::MAX;
            let r = temen_interp::run(&t.module, e, &[Value::I64(sp), Value::I64(n)], &mut fuel);
            black_box(&r);
        });
    ...
    if !jit_ratios.is_empty() {
        ...
        println!(
            "\ntemen-jit vs native: geomean {geo:.2}x over {} end-to-end programs (all engines bit-exact with native)",
            jit_ratios.len()
        );
    }
}
