...
//! `temen_run::cap_thunk` (§9 trampoline: marshalling + indirect dispatch) and through the §9/D45
...
fn time_engines(m: &Module, has_cap: bool) -> (f64, f64, f64, f64) {
    ...
    let tw = per_iter(|n| {
        ...
        let r = temen_interp::run_with_host(m, 0, &args_iv(clk, n), &mut fuel, &mut host);
        black_box(&r);
    });
    let bc = per_iter(|n| {
        let mut host = Host::new();
    ...
    let jit = per_iter(|n| {
        ...
        let r = temen_jit::compile_and_run_with_host(
            m,
            0,
            &args_i64(clk, n),
            temen_run::cap_thunk,
    ...
    let jit_fast = per_iter(|n| {
        ...
        let clk = host.grant_clock();
        let ctx = &mut host as *mut Host as *mut c_void;
        let r = temen_jit::compile_and_run_with_host_fast(
            m,
            0,
            &args_i64(clk, n),
            temen_run::cap_thunk,
