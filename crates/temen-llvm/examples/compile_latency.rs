...
//! throughput — it sizes `n` huge precisely so `temen_jit::compile_and_run`'s per-call recompile
...
//! LLVM frontend):
//!   * **translate latency** — one-time `temen_llvm::translate_ll_path` (LLVM-bitcode → Temen IR), shared
...
fn main() {
    ...
    let bc = std::env::temp_dir().join(format!("temen_cl_{}.ll", std::process::id()));
    ...
    for _ in 0..9 {
        let t = Instant::now();
        let tr = temen_llvm::translate_ll_path(&bc).expect("translate");
    ...
    println!(
        "module translate (LLVM bitcode → Temen IR): {:.1} µs (one-time, shared by all engines)\n",
        translate_ns / 1e3
    );
    println!(
        "{:<14} {:>11} {:>11} {:>11} | {:>9} {:>9} {:>9}",
    ...
    for &(disp, sym) in KERNELS {
        ...
        let (jit_cold, jit_iter) = fit(|n| {
            let r = temen_jit::compile_and_run(&t.module, e, &[sp, n as i64]).expect("jit");
            black_box(&r);
        });
