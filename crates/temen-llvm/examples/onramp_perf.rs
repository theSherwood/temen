...
//!
//! Run with:  `cd crates/temen-llvm && cargo run --release --example onramp_perf`
//!
...
//! because `temen_jit` only exposes compile-and-run together.
//!
...
//! `temen_jit` only exposes compile-and-run together — then divide by the iteration delta and take the
...
fn time_interp(m: &temen_ir::Module, entry_sp: u64, n: i32) -> (i32, std::time::Duration) {
    let args = [Value::I64(entry_sp as i64), Value::I32(n)];
    let mut fuel = u64::MAX;
    let t = Instant::now();
    let r = temen_interp::run_fast(m, 0, &args, &mut fuel).expect("interp run");
    let dt = t.elapsed();
    (
        match r[0] {
            Value::I32(x) => x,
...
fn time_jit(m: &temen_ir::Module, entry_sp: u64, n: i32) -> (i32, std::time::Duration) {
    let slots = [entry_sp as i64, n as i64];
    let t = Instant::now();
    let out = temen_jit::compile_and_run(m, 0, &slots).expect("jit run");
    let dt = t.elapsed();
    let v = match out {
