...
//! is `clang -O2`; the Temen rows compile the same wrapper with `clang -O2 -emit-llvm`, translate, and
...
//!
//! **Cross-engine (temen-jit vs V8 vs Wasmtime).** The headline `temen-jit` ratio is reported alongside two
...
//! (in-process Cranelift — the same backend temen-jit uses — via `bench/cross-engine/wasmtime-rs`'s
...
//!
//! **Why two widths.** temen-jit consumes the *host* `-O2` bitcode, where `long` is 64-bit (LP64). `wasm32`
//! makes `long` 32-bit (ILP32), which lets LLVM's wasm frontend auto-vectorize kernels temen-jit can't
...
struct Bench {
    ...
    /// kernels — a unity build, so the Temen side stays a single module (no llvm-link) and native/Temen
...
const BENCHES: &[Bench] = &[
    ...
    Bench {
        ...
        // mis-size that only barely fit the old native+temen timing.
        large: 400,
    },
    Bench {
        name: "qrduino",
...
fn main() {
    ...
    // same backend temen-jit uses). Both are optional: a missing tool just leaves that column blank.
    ...
    // net that caught the qrduino bug). Under strict mode the TEMEN-side build/translate/export skips are
    ...
    println!(
        ...
        "benchmark", "native(ns)", "temen-jit", "v8/w32", "v8/w64", "wt/w32", "wt/w64"
    );
    // Per-engine perf ratios vs native (geomean at the end). temen-jit always present; the V8/Wasmtime
    // columns appear per kernel whose wasm built and whose runner is available, at BOTH memory widths:
    // `w32` is wasm32 (`long` 32-bit, often auto-vectorized — not the program temen-jit runs) and `w64` is
    // wasm64/memory64 (`long` 64-bit, LP64 — the same widths as the host bitcode temen-jit consumes, so
    ...
    for bench in BENCHES {
        ...
        let common = |c: &mut Command| {
            ...
            // the macro — applied to *both* native and Temen builds so the differential stays honest.
            if name == "aha-mont64" {
        ...
        // Temen: compile the wrapper to bitcode (no main), translate.
        let bc = dir.join(format!("emb_{name}.ll"));
        let mut sc = Command::new("clang");
        common(&mut sc);
        // On-ramp SIMD target (D64): temen-jit consumes *host* bitcode, so unlike wasm (capped at the
        ...
        if !sc.status().map(|s| s.success()).unwrap_or(false) {
            println!("{name:<16} (skipped: temen bitcode compile failed)");
            if strict {
                failures.push(format!("{name}: temen bitcode compile failed"));
            }
            continue;
        }
        let t = match temen_llvm::translate_ll_path(&bc) {
            Ok(t) => t,
            Err(e) => {
        ...
        let jitv = match temen_jit::compile_and_run(&t.module, e, &[sp, VERIFY_N]).unwrap() {
            temen_jit::JitOutcome::Returned(v) => v[0],
            o => panic!("jit: {o:?}"),
        };
        // wasm builds (shared freestanding shim) for the V8 + Wasmtime rows, at BOTH memory widths.
        // Same kernel flags as the native/Temen builds via `common`; adds the shim + bulk-memory
        ...
        // temen-jit runs. wasm64 (memory64): `long` is 64-bit (LP64), exactly the widths of the host
        // bitcode temen-jit consumes — the honest cross-engine comparison.
