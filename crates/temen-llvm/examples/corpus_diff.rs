//! Backend code-quality differential: how far is temen-jit from native `clang -O2` on *realistic*
...
//! temen-jit via `clang -O2 -emit-llvm` → `temen_llvm::translate_ll_path` → `temen_jit::compile_and_run`,
//! timed in-process the same way.
//!
//! It reports the per-kernel ratio (temen-jit / native) plus the distribution — quantifying "how many
...
//!
//! Run: cd crates/temen-llvm && cargo run --release --example corpus_diff
...
// (so it dominates both native process spawn and temen-jit's per-call recompile jitter).
...
fn temenjit_ns(name: &str, src: &str, large: i64) -> Option<f64> {
    let dir = std::env::temp_dir();
    let kf = dir.join(format!("cd_{name}.c"));
    ...
    temen_jit::compile_and_run(&t.module, e, &[sp, SMALL]).ok()?;
    let run = |n: i64| {
        black_box(temen_jit::compile_and_run(&t.module, e, &[sp, n]).unwrap());
    };
    let m = |n: i64| {
        run(n);
...
fn main() {
    println!(
        "{:<16} {:>12} {:>12} {:>8}",
        "kernel", "native(ns)", "temen-jit(ns)", "ratio"
    );
    let mut ratios = Vec::new();
    for &(name, large, src) in KERNELS {
        match (native_ns(name, src, large), temenjit_ns(name, src, large)) {
            (Some(nat), Some(jit)) => {
                let r = jit / nat;
                ratios.push((name, r));
