//! Real `std::thread::spawn`/`join` for the threaded temen target (copied to `sys/thread/temen.rs`).
//!
//! Modeled on `sys/thread/hermit.rs` — the "single C-like entry taking a raw pointer, join by
//! handle" shape — over the temen §12 thread primitives the on-ramp lowers (`crates/temen-llvm`
//! `lower_vm_builtin`):
...
//!
//! Only compiled for `x86_64-unknown-temen-threads` (`target_env = "threads"`); the lean target keeps
...
unsafe extern "C" {
    ...
    // `yield_now`/`sleep` here; the same op backs `sys/sync/futex/temen.rs`.
