//! temen filesystem: `File`, `read_dir`, `metadata` and friends reach the POSIX personality through the
//! PAL `host` bridge (temen-posix `OP_OPEN`/`OP_READ`/`OP_WRITE`/`OP_CLOSE`/`OP_LSEEK`/`OP_UNLINK`/
...
// own copy of the values whose authority is the host table `temen_ir::errno` (negative, Linux values).
const ENOENT: i64 = -2;
const EEXIST: i64 = -17;
...
/// `ErrorKind` (the temen `io/error` leaf otherwise decodes everything to `Uncategorized`). `EEXIST`
...
/// A path as the `(ptr, len)` UTF-8 byte slice the host ops expect. temen has no OS-specific `OsStr`
...
impl Iterator for ReadDir {
    ...
    fn next(&mut self) -> Option<io::Result<DirEntry>> {
        ...
        loop {
            ...
            // SAFETY: temen has no OS-specific `OsStr` encoding; the name bytes are the path component.
