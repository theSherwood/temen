//! temen time: `Instant`/`SystemTime` read the host clock through the POSIX personality's `clock` op
//! (temen-posix `OP_CLOCK`) — monotonic (`clock_id 1`) for `Instant`, realtime (`clock_id 0`) for
