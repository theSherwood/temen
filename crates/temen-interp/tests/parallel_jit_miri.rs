...
//! small.
//!
//! The host JIT validator is stubbed inline (parse a fixed unit text) so this stays in the `temen-interp`
//! crate's Miri-buildable suite — `temen-run`'s real validator would drag in the Cranelift JIT Miri
//! can't compile.
//!
...
/// Stub blob validator (the role `temen-run`'s `jit_blob_validator` plays in the heavyweight suite):
