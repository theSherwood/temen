...
fn two_pollers_split_one_queue_and_their_counts_sum() {
    ...
    assert!(
        bytecode::compile_module(&m.funcs, &m.types, m.memory.and_then(|x| x.shadow)).is_none(),
...
fn a_timed_svc_wait_with_no_work_returns_zero() {
    ...
    host.set_self_module(&m);
    let mut fuel = u64::MAX;
    let rf = temen_interp::run_with_host_fast(&m, 0, &[], &mut fuel, &mut host).expect("fallback");
