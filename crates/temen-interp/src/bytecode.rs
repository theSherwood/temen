                ...
                Op::CapCall {
                    ...
                    if *type_id == temen_ir::CAP_IMPORT_TYPE_ID {
                        if let Some(b) = host.with(|p| p.import_binding(*op)) {
                            // §5/§14: an import bound to an **engine-serviced** `Instantiator` op
                            // (`join`, `instantiate_detached`) likewise — the Host never routes an
                            // Instantiator through the generic dispatch (it services these in the
                            // driver). Surface the same `Outcome` the static `call.cap` lowers to,
                            // on the **binding's** handle (the manifest model: the granted handle
                            // rides the binding, the call site's operand is vestigial — a linked
                            // object unit emits a placeholder there), so a C `extern` reaches the
                            // same arm as hand-written IR. The remaining Instantiator ops keep
                            // failing closed here (`CapFault`), as before.
                            if b.bound
                                && b.type_id == super::cap_id::INSTANTIATOR
                                && matches!(b.op, 1 | 15)
                            {
                                let ih = b.handle;
                                ...
                                return Ok(match b.op {
                                    1 => {
                                        let Some(&child) = argv.first() else {
                                            return Err(Trap::CapFault);
                                        };
                                        Outcome::ThreadJoin {
                                            handle: child as i32,
                                    ...
                                    _ => {
                                        ...
                                        Outcome::InstantiateDetached {
                                            ...
                                            dst,
                                            grants: (argv[3] != 0)
                                                .then(|| (argv[2] as u64, argv[3] as u64)),
                                            args: (argv.len() >= 9)
                                                .then(|| (argv[7] as u64, argv[8] as u64)),
                                            premap: (argv.len() >= 11)
                                                .then(|| (argv[9] as i32, argv[10] as u64)),
                                        }
                                    }
                                });
                            }
