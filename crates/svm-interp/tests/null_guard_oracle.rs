...
fn tree(m: &svm_ir::Module, func: u32, arg: i64) -> Result<i64, Trap> {
    let mut fuel = 1_000_000u64;
    svm_interp::run(m, func, &[Value::I64(arg)], &mut fuel).map(|v| match v.first() {
        Some(Value::I64(x)) => *x,
        other => panic!("result {other:?}"),
    })
}

fn byte(m: &svm_ir::Module, func: u32, arg: i64) -> Result<i64, Trap> {
    let mut fuel = 1_000_000u64;
...
fn page_ops_below_the_guard_are_refused() {
    let win = 1usize << 17;
    let unmap = |m: &svm_ir::Module, off: i64, len: i64| -> i64 {
