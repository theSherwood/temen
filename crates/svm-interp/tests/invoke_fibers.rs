...
//! svm-interp suite with no Cranelift dep.
...
"#
    ...
    svm_verify::verify_module(&m).expect("verify guest");
    m
}
...
fn fiber_hosting_invoke_agrees_across_engines() {
    ...
    let tw = svm_interp::run_with_host(&m, 0, &args, &mut fuel, &mut host)
