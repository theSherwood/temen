...
use svm_interp::{run_with_host, Host, OffloadOutcome, StreamRole, Value};
...
fn a_punted_host_call_parks_the_fiber_not_the_vcpu() {
    let m = svm_text::parse_module(PUNT_IN_FIBER).expect("parse");
    svm_verify::verify_module(&m).expect("verify");
    let mut host = Host::new();
...
fn two_fibers_on_one_vcpu_overlap_their_punts() {
    let m = svm_text::parse_module(TWO_FIBER_OVERLAP).expect("parse");
    svm_verify::verify_module(&m).expect("verify");
    let mut host = Host::new();
...
fn root_return_abandons_a_cap_parked_fiber() {
    let m = svm_text::parse_module(RESUME_ONCE).expect("parse");
    svm_verify::verify_module(&m).expect("verify");
    let mut host = Host::new();
    let h = host.grant_host_proc_offloadable(Box::new(|_op, args| {
        let a = *args.first().unwrap_or(&0);
...
fn ordered_delivery_holds_a_ready_later_completion_for_an_earlier_park() {
    let m = svm_text::parse_module(ORDERED_FIBER_DELIVERY).expect("parse");
    svm_verify::verify_module(&m).expect("verify");
