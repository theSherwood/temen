...
//! and the explorer do exactly that — pinned in `svm/tests/fiber_blocking_resume.rs`); here we
...
fn block_resume_idles_on_a_timed_wait_then_completes() {
    let m = svm_text::parse_module(BLOCK_ON_TIMED_WAIT).expect("parse");
...
fn block_resume_wakes_on_a_cross_vcpu_notify() {
    let m = svm_text::parse_module(BLOCK_ON_NOTIFY).expect("parse");
    svm_verify::verify_module(&m).expect("verify");
    let mut host = Host::new();
...
fn a_sibling_trap_frees_a_blocked_resumer() {
    let m = svm_text::parse_module(BLOCK_THEN_SIBLING_TRAPS).expect("parse");
    svm_verify::verify_module(&m).expect("verify");
    let mut host = Host::new();
    let mut fuel = 50_000_000u64;
    let init = vec![0u8; 1 << 16];
