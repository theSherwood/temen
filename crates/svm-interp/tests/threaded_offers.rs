...
use svm_interp::{run_with_host, Host, OfferPolicy, Value};

fn module(t: &str) -> svm_ir::Module {
    let m = svm_text::parse_module(t).expect("parse");
...
fn buried_cycle_consumer(x: i32, y: i32, tid: u32) -> svm_ir::Module {
    module(&format!(
        "func () -> (i64) {{\n\
         block 0 () {{\n\
           vhx = i32.const {x}\n\
