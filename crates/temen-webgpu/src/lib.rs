//! **A headless WebGPU compute capability for the Temen (LLVM.md §"GPU via a WebGPU capability").**
//!
...
//!
//! Kept in its own workspace-excluded crate (like `temen-llvm`) so the heavy `wgpu` dependency never
//! enters the default `cargo build`/`test`. Promoting the cap into `temen-run` behind a feature is a
