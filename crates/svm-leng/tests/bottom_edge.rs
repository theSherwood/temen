...
use svm_leng::{bottom_edge_index, bottom_edge_runtime};

/// The runtime with a window declared (its `mem.*`/load/store leaves need one to run standalone; under
/// a real link the program's larger window wins by max — `svm_ir::link`).
fn runtime() -> Module {
    let mut m = bottom_edge_runtime();
    m.memory = Some(Memory { size_log2: 16 });
    svm_verify::verify_module(&m).expect("bottom-edge runtime verifies");
    m
}
...
fn run_both(m: &Module, idx: u32, args: &[i64], seed: &[u8]) -> (Vec<i64>, Vec<u8>) {
    ...
    let (ir, iwin) = svm_interp::run_capture(m, idx, &ivals, &mut fuel, seed);
    let iwords: Vec<i64> = ir
        ...
        .collect();
    let (jout, jwin) = svm_jit::compile_and_run_capture(m, idx, args, seed).expect("jit");
...
fn imports_bind_to_the_runtime_via_link() {
    ...
    // SVM imports; linking against the runtime binds each import to a pure-IR function — the #761 point.
    const USER: &str = r#"
memory 16
...
"#;
    let user = svm_text::parse_module(USER).expect("user module parses");
    ...
    let linked = svm_ir::link(&units).expect("link user + bottom-edge runtime");
    svm_verify::verify_module(&linked).expect("linked module verifies (imports all bound)");
