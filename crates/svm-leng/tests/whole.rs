...
//! `main` and the exportc gvars. `link_whole_units` compiles each module in full to a `.svmo` object
...
(stmts
 ...
 (proc :ini.0. . . .
  (stmts .
   ...
   (asgn iniGuard.0. (true))
   (call ini.0.modb))))";
    let linked = svm_leng::link_whole_units(&[
        WholeModule {
            stem: "moda",
            src: mod_a,
        },
    ...
    let interp = svm_interp::run(&linked, 1, &ivals, &mut fuel).expect("interp ini");
    assert!(interp.is_empty(), "ini is void: {interp:?}");
    match svm_jit::compile_and_run(&linked, 1, &[]).expect("jit ini") {
        svm_jit::JitOutcome::Returned(v) => assert!(v.is_empty(), "ini is void: {v:?}"),
        o => panic!("jit ini: {o:?}"),
    }
}
...
/// as a whole `.svmo` — no hand-picked procs. Their cross-module edges resolve against each other
...
fn real_two_modules_link_in_full() {
    ...
    let interp = svm_interp::run(&linked, 2, &iargs, &mut fuel).expect("interp C main");
    ...
    let jit = match svm_jit::compile_and_run(&linked, 2, &[0, 0, 0]).expect("jit C main") {
        svm_jit::JitOutcome::Returned(v) => v,
