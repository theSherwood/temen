...
fn verify_whole(src: &str) -> svm_ir::Module {
    let text =
        svm_leng::translate_to_text(src).unwrap_or_else(|e| panic!("translate whole module: {e}"));
    let m =
        svm_text::parse_module(&text).unwrap_or_else(|e| panic!("parse emitted IR: {e:?}\n{text}"));
...
fn whole_addtwo_user_main_runs() {
    ...
    let interp = svm_interp::run(&m, 1, &[], &mut fuel).expect("interp run of user main");
    let jit = match svm_jit::compile_and_run(&m, 1, &[]).expect("jit compile") {
