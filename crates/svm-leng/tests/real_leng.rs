...
fn translates_real_hexer_proc_addtwo() {
    ...
    let module = svm_leng::translate_proc(REAL, "addTwo.0.")
    ...
    for (a, b) in [(20i64, 22i64), (-5, 8), (1_000_000, 337)] {
        ...
        let jit = match svm_jit::compile_and_run(&module, 0, &[a, b]).expect("jit compile") {
...
fn missing_proc_is_reported() {
    match svm_leng::translate_proc(REAL, "nope.0.") {
