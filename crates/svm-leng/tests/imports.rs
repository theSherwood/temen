...
//! a declared SVM `import` + `call.import`, with its signature fixed from the call site. The runtime
...
fn cross_module_call_becomes_import() {
    let text = svm_leng::translate_to_text(MOD).unwrap();
    ...
    let m = svm_leng::translate(MOD).unwrap_or_else(|e| panic!("translate: {e}"));
    // The frontend's job is a well-typed, verifier-accepted module; the embedder binds the import.
    svm_verify::verify_module(&m).expect("verify (imports are bound by the embedder)");
}
...
fn real_nimony_sumto_with_import() {
    ...
    let m = svm_leng::translate_proc(REAL, "sumto.0.")
        .unwrap_or_else(|e| panic!("translate real sumto: {e}"));
    svm_verify::verify_module(&m).expect("verify real sumto");
    // The module declares the `inc` import and frames the address-taken counter.
    let text = svm_leng::translate_proc_to_text(REAL, "sumto.0.").unwrap();
    assert!(
        text.contains("call.import"),
