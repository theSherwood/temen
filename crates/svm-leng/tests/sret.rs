...
/// seeded length).
fn run_void_capture(module: &svm_ir::Module, idx: u32, args: &[i64], win: usize) -> Vec<u8> {
    svm_verify::verify_module(module).unwrap_or_else(|e| panic!("verify: {e:?}"));
    let seed = vec![0u8; win];
    let ivals: Vec<Value> = args.iter().map(|&n| Value::I64(n)).collect();
    let mut fuel = u64::MAX;
    let (ir, imem) = svm_interp::run_capture(module, idx, &ivals, &mut fuel, &seed);
    ir.expect("interp run");
    let (jout, jmem) =
        svm_jit::compile_and_run_capture(module, idx, args, &seed).expect("jit compile");
    assert!(
        matches!(jout, svm_jit::JitOutcome::Returned(ref v) if v.is_empty()),
...
fn real_nimony_sret_direct() {
    ...
    let m = svm_leng::translate_proc(REAL, "mk.0.")
...
fn real_nimony_sret_caller() {
    ...
    let m = svm_leng::translate_procs(REAL, &["mk.0.", "mkSum.0."])
