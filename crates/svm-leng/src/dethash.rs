//! A **deterministic, seedless** hasher for svm-leng's maps.
//!
...
//! **untranslatable** when svm-leng is compiled to run *as a guest on svm* (W5, NIM.md §3e): the
//! on-ramp synthesizes the allocator but cannot supply `getrandom`. FNV-1a is seedless and needs no
//! entropy. svm-leng's emitted output is already iteration-order-independent — its tests pass under
