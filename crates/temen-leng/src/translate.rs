//! Leng-NIF → TEMEN-text translation (NIM.md Phase 2).
//!
//! Emits Temen text (chibicc's `codegen_ir.c` model), let `temen_text::parse_module` build the IR.
...
/// A cross-module callee lowered to an Temen `import` — its declared signature and slot. The runtime
...
/// Escape raw bytes as an temen-text string body: every byte as `\xHH` (round-trips through the
...
/// The temen-text prefix for a value type (`i32`/`i64`). This subset only produces integer types.
...
pub(crate) struct Translator {
    ...
    /// module stem and emits a `temen_ir::DataFuncref` the linker resolves to the merged funcidx. Empty
    ...
    globals_top: u64,
    /// **Link-unit mode.** When true, the module is emitted as a relocatable link unit (`.temeno`
    ...
    /// region. When false (the default, runnable `.temen` shape), globals sit at fixed absolute
...
impl Translator {
    ...
    /// points a host or `temen-run` binds to (the C `main`, `cmdCount`, `cmdLine`, `nimEnviron`), in
    ...
    /// C-ABI surface findable (`temen-run --link` can enter at `main`).
    pub fn exportc_exports(
        &self,
        root: &Node,
    ) -> Result<(Vec<temen_ir::Export>, Vec<temen_ir::DataExport>), LengError> {
    ...
    /// The Temen value type of a **param**: a funcref (`proctype`) param is an `i32` function index;
    ...
    /// Translate a `(stmts TopLevelConstruct*)` module to Temen text (all procs).
    pub fn module(&mut self, root: &Node) -> Result<String, LengError> {
        Ok(self.module_with_names(root)?.0)
    }

    /// Translate an **entire** module (every proc, in source order) to Temen text, also returning each
    ...
    fn propagate_frames(&mut self, nodes: &[&Node]) -> Result<(), LengError> {
        loop {
            ...
            for node in nodes {
                ...
                // `map_or(true, …)` not `is_none_or` — temen-leng must compile under the W5 guest
...
struct FuncGen<'a> {
    ...
    /// operand in this set lowers to the unsigned Temen op (`lt_u`/`le_u`) so a high-bit-set value
...
impl<'a> FuncGen<'a> {
    ...
    fn stmt(&mut self, s: &Node) -> Result<(), LengError> {
        ...
        match s.tag() {
            ...
            Some("keepovf") => {
                ...
                // Overflow checks are off (temen integer arithmetic wraps), so it's a plain store of
    ...
    fn expr(&mut self, e: &Node) -> Result<Val, LengError> {
        match e {
            ...
            Node::List(_) => match e.tag() {
                ...
                // The overflow flag (`(ovf)`) — always false: temen integer arithmetic wraps and we
    ...
    /// `(call Callee Expr*)` — a direct call to a module proc, or an Temen `import` for a cross-module
    ...
    fn call(&mut self, e: &Node, ret_hint: Option<ValType>) -> Result<Val, LengError> {
        ...
        // Cross-module callee (not defined in this module) → an Temen import.
    ...
    /// One call argument → its Temen value id. An **aggregate** argument is passed by address: an
    ...
    /// A float constant (`{:?}` round-trips through temen-text's `parse_float`). Gated on `float`: the
    /// `{f:?}` decimal formatter is `core::num::flt2dec` (dragon4, u128/vector), which `temen-llvm`
...
/// Temen value type of a Leng type: pointers (`ptr`/`aptr`) and named aggregates (held by address)
...
/// unsigned Temen ops (`lt_u`/`le_u`), not the signed ones — a `uint32` bitmap word with bit 31 set
