//! End-to-end tests for the `temen-leng` walking skeleton (NIM.md Phase 2): hand-written Leng-NIF
//! (faithful to `doc/leng-spec.md`) → TEMEN-IR → verify → run on **both** the interpreter and the
//! JIT, asserting the computed values agree with the expected result. The two-backend shape is the
//! §9 parity discipline the other frontends follow (`temen-wasm`'s `run_both`).

use temen_interp::Value;
use temen_leng::{translate, LengError};
...
fn run_i64(leng: &str, idx: usize, args: &[i64]) -> i64 {
    ...
    let interp = temen_interp::run(&module, idx as u32, &ivals, &mut fuel).expect("interp run");
    let interp_n = match interp.as_slice() {
        [Value::I64(n)] => *n,
        other => panic!("expected one i64 result, got {other:?}"),
    };

    let jit = match temen_jit::compile_and_run(&module, idx as u32, args).expect("jit compile") {
...
fn run_i64_i32arg(leng: &str, idx: usize, arg: i32) -> i64 {
    ...
    let mut fuel = u64::MAX;
    let interp =
        temen_interp::run(&module, idx as u32, &[Value::I32(arg)], &mut fuel).expect("interp");
    let interp_n = match interp.as_slice() {
        [Value::I64(n)] => *n,
        other => panic!("expected i64, got {other:?}"),
    };
    // JIT: an i32 param is passed in a raw i64 word (low 32 bits).
    let jit = match temen_jit::compile_and_run(&module, idx as u32, &[arg as i64]).expect("jit") {
        temen_jit::JitOutcome::Returned(v) => v,
...
fn overflow_keeping_arithmetic() {
    ...
    // With checks off, temen arithmetic wraps and `(ovf)` is always false, so it's a plain wrapping
