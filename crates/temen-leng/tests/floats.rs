...
fn run_f64(module: &temen_ir::Module, idx: u32, args: &[Value]) -> f64 {
    temen_verify::verify_module(module).unwrap_or_else(|e| panic!("verify: {e:?}"));
    let mut fuel = u64::MAX;
    let interp = temen_interp::run(module, idx, args, &mut fuel).expect("interp run");
    ...
    let jargs: Vec<i64> = args
        ...
        .collect();
    let jit = match temen_jit::compile_and_run(module, idx, &jargs).expect("jit compile") {
 ...
 (proc :trunc_neg.0 (params (param :x.0 . (f +64))) (i +64) .
  (stmts .
   (ret (conv (i +64) (neg (f +64) x.0))))))";
    ...
    let mut fuel = u64::MAX;
    let interp = temen_interp::run(&m, 0, &[Value::F64(3.7)], &mut fuel).unwrap();
    assert_eq!(
        interp.as_slice(),
        &[Value::I64(-3)],
        "int(-3.7) = -3 (trunc toward zero)"
    );
    let jit = match temen_jit::compile_and_run(&m, 0, &[3.7f64.to_bits() as i64]).unwrap() {
        temen_jit::JitOutcome::Returned(v) => v,
        other => panic!("jit: {other:?}"),
    };
...
(stmts
 (proc :fmax.0 (params (param :a.0 . (f +64)) (param :b.0 . (f +64))) (f +64) .
  (stmts .
   ...
   (ret r.0))))";
    let m = temen_leng::translate(leng).unwrap_or_else(|e| panic!("translate: {e}"));
    assert!(temen_leng::translate_to_text(leng)
        .unwrap()
        .contains("f64.lt"));
...
fn real_nimony_floats() {
    ...
    let m = temen_leng::translate_proc(REAL, "favg.0.")
