...
//! modules as `P.<stem>`. `temen_leng::link_units` translates each module's procs, exports them under
//! those global names, and resolves every unit's cross-module calls (named imports) against the
//! exports via `temen_ir::link` — one merged, import-free temen module. Both engines, §9 parity.
 ...
 (proc :mk.0. (params (param :v.0 . (i +64))) (i +64) .
  (stmts .
   ...
   (ret (add (i +64) (dot p.0 x.0 0) (dot p.0 y.0 0))))))";
    ...
    assert!(
        temen_leng::compile_object(&LengModule {
            stem: "modu",
            src: mod_u,
            names: &["mk.0."],
        })
...
fn manifest_link_retains_unresolved_syscall_leaves() {
    ...
    let m = temen_leng::link_whole_with_runtime_manifest(&[unit()], vec![])
    ...
    assert!(
        temen_leng::link_whole_units(&[unit()]).is_err(),
...
(stmts
 ...
 (proc :=destroy.2. (params (param :s.0 . (ptr string.0.))) . . (stmts . (ret .))))";
    ...
    let (_jout, jmem) = temen_jit::compile_and_run_capture(&linked, 0, &[sp, sret as i64], &seed)
        .expect("jit greet");
...
(stmts
 (proc :top.0. . (i +64) . (stmts . (ret (call missing.0.modx)))))";
    match temen_leng::link_units(&[LengModule {
        stem: "moda",
        src: mod_a,
        names: &["top.0."],
    }]) {
...
/// the linker only knows once it assigns func bases, so `temen-leng` emits a `data.funcref` relocation
