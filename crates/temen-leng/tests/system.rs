...
//! **translated from the actual nimony `system` module** — real compiled stdlib code running on Temen.
//!
...
(stmts
 (proc :clr.0. (params (param :p.0 . (i +64))) . .
  (stmts . (call =wasMoved.2.sysvq0asl p.0))))";
    ...
    let (_j, jmem) =
        temen_jit::compile_and_run_capture(&linked, 0, &[s as i64], &seed).expect("jit");
...
(stmts
 (proc :del.0. (params (param :p.0 . (i +64))) . .
  (stmts . (call =destroy.2.sysvq0asl p.0))))";
    ...
    let mut fuel = u64::MAX;
    let (ir, imem) =
        temen_interp::run_capture(&linked, 0, &[Value::I64(s as i64)], &mut fuel, &seed);
...
(stmts
 (proc :cp.0. (params (param :dest.0 . (i +64)) (param :src.0 . (i +64))) . .
  (stmts . (call =copy.2.sysvq0asl dest.0 src.0))))";
    ...
    ir.expect("interp cp");
    let (_j, jmem) =
        temen_jit::compile_and_run_capture(&linked, 0, &[d as i64, s as i64], &seed).expect("jit");
    let n = imem.len().min(jmem.len());
    assert_eq!(imem[..n], jmem[..n], "§9 interp/JIT window parity");
...
fn real_ref_type_dup_hook_increments_refcount() {
    ...
    let mut fuel = u64::MAX;
    let (ir, imem) = temen_interp::run_capture(&m, 2, &[Value::I64(r as i64)], &mut fuel, &seed);
    let ival = ir.expect("interp dupDriver");
    let (jout, jmem) = temen_jit::compile_and_run_capture(&m, 2, &[r as i64], &seed).expect("jit");
    let jval = match jout {
