...
fn run_void_capture(m: &temen_ir::Module, idx: u32, args: &[i64], win: usize) -> Vec<u8> {
    ...
    ir.expect("interp");
    let (_j, jmem) = temen_jit::compile_and_run_capture(m, idx, args, &seed).expect("jit");
 ...
 (proc :getVt.0 (params (param :d.0 . (ptr Derived.0.))) (i +64) .
  (stmts .
   (ret (dot (deref d.0) vt.00 2)))))";
    ...
    let mut fuel = u64::MAX;
    let (ir, _) = temen_interp::run_capture(&m, 0, &[Value::I64(obj as i64)], &mut fuel, &seed);
    assert_eq!(ir.expect("interp").as_slice(), &[Value::I64(0xABCD)]);
    let (jout, _) = temen_jit::compile_and_run_capture(&m, 0, &[obj as i64], &seed).expect("jit");
    match jout {
        temen_jit::JitOutcome::Returned(v) => assert_eq!(v, vec![0xABCD], "§9 parity"),
        o => panic!("jit: {o:?}"),
    }
}
...
fn real_nimony_inheritance() {
    ...
    temen_verify::verify_module(&m).unwrap();
    let mut fuel = u64::MAX;
    let (ir, _) = temen_interp::run_capture(&m, 0, &[Value::I64(base as i64)], &mut fuel, &seed);
    assert_eq!(
        ir.expect("interp").as_slice(),
        &[Value::I64(123)],
        "e.value"
    );
    let (jout, _) = temen_jit::compile_and_run_capture(&m, 0, &[base as i64], &seed).expect("jit");
    assert!(matches!(jout, temen_jit::JitOutcome::Returned(ref v) if v == &[123]));

    // mkDerived constructs an inherited object (vtable + base + derived fields) — translate + verify.
    let m2 = temen_leng::translate_proc(REAL, "mkDerived.0.")
        .unwrap_or_else(|e| panic!("translate real mkDerived: {e}"));
    temen_verify::verify_module(&m2).unwrap_or_else(|e| panic!("verify mkDerived: {e:?}"));
}
