//! `.temeno` object-dialect tests (NIM.md W2 — the narrow waist). `temen_leng::compile_object` emits a
//! nimony module as a **binary link object** (the counterpart of `temen-llvm-translate -o out.temeno`):
//! a relocatable unit with its procs exported in-band. These objects link through the *same* format
//! and `temen_ir::link` any other producer uses — so a nimony object composes with objects a different
...
use temen_encode::{decode_unit, encode_unit};
use temen_interp::Value;
use temen_ir::{Export, LinkUnit, Module};
use temen_leng::{LengModule, WholeModule};
...
fn nimony_object_round_trips_and_runs() {
    // Compile a real 2-module nimony program to `.temeno` objects, then link them *only* through the
    ...
    let a_obj = temen_leng::compile_object(&LengModule {
        stem: "modywjwgs",
        src: MODA,
        names: &["useit.0."],
    })
    .unwrap();
    let b_obj = temen_leng::compile_object(&LengModule {
        stem: "modwru7vt1",
        src: MODB,
        names: &["helper.0."],
    })
    ...
    let b_decoded = decode_unit(&b_obj).expect("decode modb.temeno");
    assert!(
        b_decoded
            .exports
            .iter()
    ...
    let linked = temen_ir::link(&[unit_of(decode_unit(&a_obj).unwrap()), unit_of(b_decoded)])
        .expect("link objects");
    temen_verify::verify_module(&linked).expect("verify");
    let mut fuel = u64::MAX;
    let r = temen_interp::run(&linked, 0, &[Value::I64(5)], &mut fuel).unwrap();
    assert_eq!(r.as_slice(), &[Value::I64(16)]);
}

/// A whole-module object exposes its **`exportc`** symbols under their C names — the conventional
/// entry points a host / `temen-run --link` binds to. Real `moda` (main module) marks its C `main`
...
fn whole_object_exposes_exportc_c_names() {
    const MODA: &str = include_str!("fixtures/real_moda.leng.nif");
    let obj = temen_leng::compile_whole_object(&WholeModule {
        stem: "modywjwgs",
        src: MODA,
    })
    .expect("compile whole moda object");
    let m = decode_unit(&obj).expect("decode moda.temeno");
...
#[test]
fn cross_producer_link_through_temeno() {
    // A nimony `.temeno` links against a **separately-produced, non-nimony** runtime `.temeno` through the
 ...
 (proc :dbl.0. (params (param :x.0 . (i +64))) (i +64) .
  (stmts .
   (ret (call ext.0.rt x.0)))))";
    let nim_obj = temen_leng::compile_object(&LengModule {
        stem: "seqmod",
        src: nim_src,
        names: &["dbl.0."],
    })
    ...
    let mut runtime = temen_text::parse_module(
        "func (i64) -> (i64) {\nblock 0 (v0: i64) {\n  v1 = i64.const 2\n  v2 = i64.mul v0 v1\n  return v2\n  }\n}",
    )
    .expect("runtime text");
    runtime.exports = nim
        ...
        .collect();
    let rt_obj = encode_unit(&runtime); // the runtime, as its own `.temeno`
    ...
    // dbl is func 0: (x) -> 2x.
    let mut fuel = u64::MAX;
    let ir = temen_interp::run(&linked, 0, &[Value::I64(21)], &mut fuel).unwrap();
    assert_eq!(ir.as_slice(), &[Value::I64(42)]);
    match temen_jit::compile_and_run(&linked, 0, &[21]).expect("jit") {
