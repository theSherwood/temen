...
//! for temen-text (the `[]` operator mangles to `\5B\5D…`), aggregate **args** to imports passed by
 ...
 (proc :sumSeq.0 (params (param :s.0 . Seq.0.)) (i +64) .
  (stmts .
   ...
   (ret acc.0))))";
    ...
    let mut fuel = u64::MAX;
    let (ir, _) =
        temen_interp::run_capture(&m, 0, &[Value::I64(seq_addr as i64)], &mut fuel, &seed);
    assert_eq!(ir.expect("interp").as_slice(), &[Value::I64(60)]);
    let (jout, _) =
        temen_jit::compile_and_run_capture(&m, 0, &[seq_addr as i64], &seed).expect("jit");
    match jout {
        temen_jit::JitOutcome::Returned(v) => assert_eq!(v, vec![60], "§9 parity"),
        o => panic!("jit: {o:?}"),
    }
}
...
fn break_outside_loop_is_fail_closed() {
    ...
    match temen_leng::translate(leng) {
        Err(temen_leng::LengError::Malformed(_)) | Err(temen_leng::LengError::Unsupported(_)) => {}
