...
//! for the semantics (name lookup, member/index/deref, scalar load). The caller (`temen-dap`)
//! supplies the resolver that reads the interpreter's window through the neutral debug info — so
//! this module has no dependency on `temen-ir`/`temen-interp` or any frontend.
