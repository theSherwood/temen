...
//! requests onto the `temen-interp` [`Inspector`] — which already does the real work (breakpoints,
...
impl Session {
    ...
    fn resolve_var(&self, frame_idx: usize, func: u32, name: &str) -> Option<i64> {
        ...
        let var = debug
            .vars
            .iter()
            .find(|v| (v.func == func || v.func == temen_ir::GLOBAL_SCOPE) && v.name == name)?;
...
impl DapServer {
    ...
    fn on_launch(&mut self, args: Option<&Json>) -> (bool, Json, Vec<Event>) {
        ...
        // Synthesize debug info for a program with no explicit `debug` section, so hand-written Temen
        // text is debuggable (breakpoints by line, SSA values by name). An explicit section wins.
        let Ok(module) = temen_text::parse_module_debug(&text) else {
            return (false, Json::Null, vec![]);
        };
    ...
    fn frame_locals(&mut self, vref: i64) -> Option<Vec<Json>> {
        ...
        let in_scope = |v: &VarInfo| {
            (v.func == frame.pc.func || v.func == temen_ir::GLOBAL_SCOPE)
    ...
    fn on_set_variable(&mut self, args: Option<&Json>) -> (bool, Json, Vec<Event>) {
        ...
        let Some((ty_name, type_id)) = session.debug.as_ref().and_then(|debug| {
            debug
                ...
                .iter()
                .filter(|v| {
                    (v.func == func || v.func == temen_ir::GLOBAL_SCOPE)
                        && scope_covers(v.scope, line)
                        && v.name == name
                })
                .max_by_key(|v| v.scope.map_or(0u32, |(s, _)| s))
    ...
    fn on_evaluate(&mut self, args: Option<&Json>) -> (bool, Json, Vec<Event>) {
        ...
        let bare = session.debug.as_ref().and_then(|d| {
            d.vars
                .iter()
                .find(|v| (v.func == func || v.func == temen_ir::GLOBAL_SCOPE) && v.name == expr)
                .map(|v| (d, v))
        });
