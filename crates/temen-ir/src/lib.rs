...
pub mod cap_id {
    ...
    /// shared object (`memfd`) **or a real host file** (`temen-run`'s `FileBacking`, minted by an
...
/// the servicers (temen-fs, temen-exec, temen-posix), the runtime (temen-interp), the embedder (temen-run),
/// and the JIT thunks (temen-jit). The convention is **negative**: this is what crosses the ABI, since
...
/// `temen-llvm/rust-temen/*-imp.rs` are a separate compilation universe (guest code cannot link this
...
pub enum Inst {
    ...
    /// **currently executing** this op. temen carries one i64 of per-vCPU state; it is read *at the
    ...
    /// transform (`temen-durable`) to address that context's *own* per-context shadow-SP word, so
    ...
    /// (IEEE-754 FMA). Float shapes (`f32x4`/`f64x2`); `a`/`b`/`c`/result are `v128`. Temen picks the
...
/// the optimizer (`temen-opt`) consults for legality (see `OPT.md`, "the equivalence contract"). The
...
impl Inst {
    ...
    ///
    /// This lives beside [`Inst::effects`] so temen-ir is the single source of truth for both "what
...
impl Terminator {
    ...
    /// [`Inst::for_each_operand_mut`], so temen-ir owns terminator operand traversal too (#913).
    pub fn for_each_operand_mut(&mut self, f: &mut impl FnMut(&mut ValIdx)) {
        match self {
            Terminator::Br { args, .. } => {
                for v in args.iter_mut() {
...
/// constant so they stay in differential lockstep, and the masking unit (`temen-mask`) never
...
/// for a powerbox program (`temen-llvm`'s `STACK_PAGE`). The args blob must fit in
...
/// a fresh host page **above** the read-only region on every host — the `_start` layout (`temen-llvm`'s
...
/// — the §3e manifest-module entry shape) so a frontend that emits TEMEN-IR directly (e.g. the
...
/// convention inside it (`posix.fs`, `app.log`; `temen.` reserved for platform interfaces), and
