...
//! `temen-opt` — the generic temen-IR AOT optimizer (see `OPT.md`, `DESIGN.md` §20a/§20c).
//!
//! A pure, closed-module `Module -> Module` transform: it rewrites verified IR into faster/smaller
//! verified IR, and its output is **re-verified** (`temen_verify::verify_module`) before it runs, so a
...
//! which the specializer in `temen-peval` (the first Futamura projection) reuses — `temen-peval` depends
...
//! `temen_verify::verify_module` before it runs, so a bug here is a clean verify error, never
...
//! `call` inlining, narrow renameable cells, value-stack renaming) live in `temen-peval`.
//!
//! **`no_std` + `alloc`.** This crate compiles `no_std` (gated on `not(test)`; its own test harness
//! gets `std`) so it can itself be translated to temen-IR through the Rust on-ramp and run *inside* temen
...
pub struct OptConfig {
    /// Sparse conditional constant propagation (`temen_opt::sccp`).
    pub sccp: bool,
    /// Constant reassociation of `(x OP c1) OP c2` chains (`temen_opt::reassociate`).
    pub reassociate: bool,
    /// Global value numbering / cross-block redundancy elimination (`temen_opt::gvn`).
    pub gvn: bool,
    /// Loop-invariant code motion (`temen_opt::licm`).
    pub licm: bool,
    ...
    /// Constant-funcref devirtualization (`temen_opt::interproc::devirtualize`) — module-level, runs
    /// before inlining (a devirtualized call becomes an inlining candidate).
    pub devirt: bool,
    /// Interprocedural constant propagation (`temen_opt::interproc::const_prop`) — module-level;
    /// substitutes a parameter that is the same constant at every direct call site into the callee.
    pub const_prop: bool,
    /// Budgeted direct-call inlining (`temen_opt::interproc::inline_calls`) — module-level, runs once
    /// before the per-function passes.
    pub inline: bool,
    /// Dead-function elimination (`temen_opt::interproc::dead_func_elim`) — module-level, runs once
...
// ----- scalar float constant folding (mirrors `temen-interp`'s scalar helpers exactly) -----
//
...
/// (the in-temen temen-IR build, which can't translate libm's inline-asm/i128) the op is never reached —
...
/// even. `abs`/`neg` are pure `core`. The in-temen build (no `libm-floats`) leaves these unfolded.
...
fn fun32(op: FUnOp, a: f32) -> f32 {
    match op {
        ...
        // feature; the in-temen build leaves them unfolded (the dispatch returns `None` first).
        FUnOp::Abs => fabs32(a),
        FUnOp::Neg => -a,
...
pub fn fold_fun(ty: FloatTy, op: FUnOp, a: Known) -> Option<Known> {
    // The in-temen build (no `libm-floats`) can't fold the libm ops — pass them through unfolded.
...
/// the `libm-floats` feature; the in-temen build leaves `fma` unfolded (sound — it runs at runtime).
#[cfg(feature = "libm-floats")]
...
pub fn fold_fma(_ty: FloatTy, _a: Known, _b: Known, _c: Known) -> Option<Known> {
    None // in-temen build: FMA needs libm (i128/inline-asm), so leave it unfolded
}
...
// ----- v128 (SIMD) constant folding (mirrors `temen-interp`'s `simd_*` lane helpers exactly) -----
//
...
pub fn fold_simd(inst: &Inst, get: impl Fn(ValIdx) -> Option<Known>) -> Option<Known> {
    ...
    Some(match *inst {
        ...
        // Exotic lane ops — each mirrors the matching `temen-interp` `simd_*` helper bit-for-bit (the
...
// ---- exotic lane-op helpers (ported from `temen-interp`'s `simd_*`, kept bit-identical) ----------
...
fn simd_vsat_bin(shape: VShape, op: VSatBinOp, a: [u8; 16], b: [u8; 16]) -> [u8; 16] {
    ...
    // no need for `i128` (which the temen-IR on-ramp can't translate; see DESIGN.md §20c).
...
// way); the in-temen build leaves SIMD FMA unfolded.
#[cfg(feature = "libm-floats")]
...
/// memory access, no side effect — [`temen_ir::Inst::effects`]) whose operation *and* operands exactly
...
/// temen_mask's **trap-confinement**: two admitted accesses off one base differ by exactly their offset
...
/// `mem.copy`/`fill`, a call (via [`temen_ir::Inst::effects`]) — clobbers the whole map. Recomputed
    ...
    for (i, inst) in b.insts.iter().enumerate() {
        match inst {
            ...
            Inst::Store {
                ...
                // disjoint byte range. Under trap-confinement (temen_mask), two admitted accesses off the
...
/// behavior.
///
/// This delegates to the single source of truth, [`temen_ir::Inst::effects`] (see `OPT.md` Phase 1a),
...
/// Remap **every value operand** of an instruction through `f`, in place — the renumbering
/// primitive the SSA/DCE/inlining passes rely on. Thin adapter over
/// [`temen_ir::Inst::for_each_operand_mut`] (which owns the exhaustive, wildcard-free operand
/// traversal beside `effects()`, #913). `FuncIdx` immediates (`RefFunc`/`ThreadSpawn::func`) are
/// *not* value operands and are left alone.
pub fn map_operands(inst: &mut Inst, f: &mut impl FnMut(ValIdx) -> ValIdx) {
    // Adapter over the single source of truth in temen-ir (#913): the exhaustive operand
    // traversal lives beside `Inst::effects`. Remapping is `for_each_operand_mut` with a
...
/// [`temen_ir::Terminator::for_each_operand_mut`], the sibling of `map_operands` (#913).
