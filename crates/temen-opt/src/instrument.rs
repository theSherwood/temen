...
//! the output (`temen_verify::verify_module`) before running it, so a bug here is a clean verify
...
mod tests {
    use super::*;
    use temen_ir::{Block, LoadOp, Memory, StoreOp, Terminator};
    ...
    fn inserts_one_capcall_per_memory_op_and_reverifies() {
        let m = sample();
        temen_verify::verify_module(&m).expect("sample verifies");
        ...
        let caps = out.funcs[0].blocks[0]
            ...
            .count();
        assert_eq!(caps, 3);
        temen_verify::verify_module(&out).expect("instrumented module re-verifies");
    ...
    fn instrumented_module_computes_the_same_result_and_reports_the_accesses() {
        ...
        temen_verify::verify_module(&out).expect("re-verify");

        // Reference run of the pristine module.
        let mut h0 = temen_interp::Host::new();
        let mut fuel0 = 1_000_000u64;
        let r0 = temen_interp::run_with_host(&m, 0, &[], &mut fuel0, &mut h0);
        ...
        let r = temen_interp::run_with_host(&out, 0, &[], &mut fuel, &mut h);
