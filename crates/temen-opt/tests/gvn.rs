...
use temen_ir::{BinOp, Block, Func, Inst, IntTy, LoadOp, Memory, Module, Terminator, ValType};
...
fn a_dispatch_loop_is_de_relooped() {
    ...
    // the dispatch (recovering the TEMEN-legal irreducible CFG) — but *only* if GVN hasn't first threaded
