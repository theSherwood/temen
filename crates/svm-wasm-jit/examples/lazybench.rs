...
//!   cargo run --release -p svm-wasm-jit --example lazybench -- <guest.svmb> <touched.json>

use std::time::Instant;
use svm_wasm_jit::{compile_module_reactor, compile_module_reactor_keep};
...
fn main() {
    ...
    let guest = args
        .next()
        .expect("usage: lazybench <guest.svmb> <touched.json>");
    let touched_path = args
        .next()
        .expect("usage: lazybench <guest.svmb> <touched.json>");
    let m = svm_encode::decode_module(&std::fs::read(&guest).unwrap()).expect("decode");
    let n = m.funcs.len();
