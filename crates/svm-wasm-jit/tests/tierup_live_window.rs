...
//! `SVM_BROWSER_TIERUP_FINDINGS.md` postmortem hit (emitted `f{func}` trapped `MemoryFault →
...
//! running over that **same live window**. That loop exists only in the browser (`svm_par_run` +
...
fn oracle(m: &svm_ir::Module, entry: u32, arg: i64) -> Outcome {
    let mut fuel = FUEL;
    match svm_interp::run(m, entry, &[Value::I64(arg)], &mut fuel) {
        Ok(v) => Outcome::Vals(v.iter().map(as_i64).collect()),
        Err(Trap::MemoryFault) => Outcome::Trap(TrapKind::MemoryFault),
        Err(Trap::OutOfFuel) => Outcome::Trap(TrapKind::OutOfFuel),
        Err(_) => Outcome::Trap(TrapKind::Other),
