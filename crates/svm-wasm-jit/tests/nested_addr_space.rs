...
//! cross-tier transport: [`svm_wasm_jit::outline_nested_cap_calls`] hoists each such `cap.call` into
//! an int-signature wrapper leaf and [`svm_wasm_jit::compile_module_nested`] routes it through
...
use svm_wasm_jit::{compile_module_nested, outline_nested_cap_calls};
use wasmi::{Caller, Engine, Linker, Memory, MemoryType, Module as WModule, Store, Val};

const WIN_BASE: i32 = 0x1_0000;
const ENV_PTR: i32 = 1024;
const ENV_SCRATCH_OFF: usize = 16; // svm_wasm_jit's cross-tier scratch offset in the env cell
...
fn oracle(m: &svm_ir::Module, with_inst: bool) -> i64 {
        ...
        .func_wrap(
            ...
            |mut caller: Caller<'_, HostState>,
             ...
             -> i32 {
                ...
                let r = match svm_interp::run(&m, entry as u32, &[Value::I64(0)], &mut fuel) {
                    Ok(v) => match v.first() {
                        Some(Value::I64(x)) => *x,
...
fn scratch_layout_assumption() {
    ...
    assert!(ENV_PTR as usize + svm_wasm_jit::ENV_CELL_BYTES < WIN_BASE as usize);
}
