...
//!
//! This crate is the single home for that `unsafe`, kept tiny and auditable (the way `temen-mem` is for
...
/// `fiber_rt` cfg that `temen-jit` derives for the same target set.
...
// Overflow protection is the always-on temen-jit software stack-check (there is no per-fiber guard page).
