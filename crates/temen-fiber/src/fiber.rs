...
struct Control {
    ...
    /// `temen-jit` arbiter enforces). This is the only cross-thread-visible field, so a violation of that
    ...
    /// [`Yielder::stack_low`] so a caller (the temen-jit software stack-overflow guard) can pass it as
...
impl Yielder {
    ...
    /// `temen-jit` `gc.roots` walker as the conservative low bound for scanning the **root
...
impl Fiber {
    ...
    pub fn resume(&mut self, val: u64) -> State {
        ...
        if self.control.running.swap(true, Ordering::Acquire) {
            eprintln!("temen-fiber: fiber resumed while already running (single-owner violation)");
            std::process::abort();
        }
