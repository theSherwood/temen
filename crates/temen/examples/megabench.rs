//! Cross-engine micro-benchmark for the Temen backends — **tree-walker**, **bytecode engine**, and
...
//! *example* (not the `temen-bench` binary) because the JIT lives in `temen-jit`, a dev-dependency.
//!
...
//!   cargo run --release --example megabench -p temen
...
fn main() {
    ...
    for (name, src, small, large) in kernels {
        ...
        println!("temen-tree-walk,{name},{tw:.4}");
        ...
        println!("temen-bytecode,{name},{bc:.4}");

        let jit = per_iter(small, large, |n| {
            let r = temen_jit::compile_and_run(&m, 0, &[n as i64]).expect("jit compiles + runs");
            std::hint::black_box(&r);
        });
        println!("temen-jit,{name},{jit:.4}");
    }
}
...
// The kernels mirror `src/bin/bench.rs` exactly (so the Temen numbers are comparable run-to-run), and
...
/// to vector adds, while a scalar backend (the Temen JIT's Cranelift, and the interpreters) stays scalar
