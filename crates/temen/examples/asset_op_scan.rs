//! One-shot §3d survey tool: decode each committed `.temen` asset and report which
...
fn main() {
    ...
    for dir in ["browser/web/assets", "browser/tests/fixtures"] {
        if let Ok(rd) = std::fs::read_dir(dir) {
            for e in rd.flatten() {
                let p = e.path();
                if p.extension().is_some_and(|x| x == "temen") {
                    paths.push(p);
                }
            }
        }
    ...
    for p in paths {
        ...
        let m = match temen_encode::decode_module(&bytes)
            .or_else(|_| temen_encode::decode_unit(&bytes))
        {
            Ok(m) => m,
            Err(e) => {
        ...
        for f in &m.funcs {
            for b in &f.blocks {
                for i in &b.insts {
                    if let temen_ir::Inst::CapCall { type_id, op, .. } = i {
                        if *type_id == 6 {
                            ops.insert(*op);
                        }
                        ifaces
