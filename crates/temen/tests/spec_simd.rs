...
//! (`temen_spec::simd`), plus the two memory-facing SIMD ops (`v128.load`/`store`) on
...
use temen_interp::{bytecode, run, run_capture_reserved, Host, Trap, Value};
use temen_ir::{Block, Func, Inst, Memory, Module, Terminator, VShape, ValType};
use temen_jit::{compile, JitOutcome, TrapKind};
use temen_spec::simd::{
    encode_probe_module, module_for_simd_batch, simd_rows, simd_vectors_for, SimdRow,
};
use temen_spec::{SpecVal, MEM_LOG2, MEM_SIZE};
...
fn check_batch(row: &SimdRow, batch: &[Vec<SpecVal>]) {
    ...
    // JIT bail (temen-jit's `ensure_supported`: no legalizable Cranelift lowering on
...
fn spec_v128_load_store_boundary_lattice() {
    ...
    for (addr, offset) in v128_mem_lattice() {
        ...
        temen::verify::verify_module(&m).unwrap();
...
fn spec_simd_encoding_conformance() {
    use temen::encode::{decode_module, encode_module};
    use temen_spec::Enc;
