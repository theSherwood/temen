...
//!
//! Run: cargo test -p temen --release --test bytecode_spike -- --nocapture --ignored

use std::hint::black_box;
use std::time::Instant;

use temen_interp::Value;
use temen_ir::{BinOp, CmpOp, Func, Inst, IntTy, Module, Terminator};
...
fn interp(m: &Module, n: i64) -> i64 {
    ...
    let v = temen_interp::run(m, 0, &[Value::I64(n)], &mut fuel).expect("interp");
    match v[0] {
        Value::I64(x) => x,
        o => panic!("{o:?}"),
    }
...
fn bytecode_spike_alu() {
    let m = temen::text::parse_module(ALU).expect("parse");
...
fn bytecode_spike_call() {
    let m = temen::text::parse_module(CALL).expect("parse");
    temen::verify::verify_module(&m).expect("verify");
    let arities: Vec<usize> = m.funcs.iter().map(|f| f.results.len()).collect();
