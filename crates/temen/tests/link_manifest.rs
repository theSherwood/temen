//! **Separate-artifact linking with a retained import manifest** (`temen_ir::link_with_manifest` +
//! `temen_ir::synth_manifest_start`) — the direct-IR frontend on-ramp (first consumer: jacl). A
...
use temen_ir::{link, link_with_manifest, synth_manifest_start, LinkError, LinkUnit};
use temen_run::{
...
fn runtime_with_capability_imports_links_into_a_powerbox() {
    ...
    temen_verify::verify_module(&module).expect("linked powerbox module verifies");
...
fn retained_slots_reindex_into_the_merged_manifest() {
    ...
    let main = linked.resolve_export("main").expect("main exported");
    let module = synth_manifest_start(linked, main, false).expect("synthesize _start");
    temen_verify::verify_module(&module).expect("verifies — slot indices reindexed consistently");
...
/// `seed_heap` seeds the guest heap words ([`temen_ir::POWERBOX_HEAP_BRK`]/`_TOP`) to the window's
