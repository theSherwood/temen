...
fn run(src: &str) -> Vec<Value> {
    ...
    let jit =
        match compile_and_run_with_host(&m, 0, &jslots, temen_run::cap_thunk, host_ptr(&mut hj))
            .expect("jit compile")
        {
...
fn get_out_of_range_traps() {
    ...
    let jslots: Vec<i64> = grant3(&mut hj).iter().map(as_i64).collect();
    let outcome =
        compile_and_run_with_host(&m, 0, &jslots, temen_run::cap_thunk, host_ptr(&mut hj))
            .expect("jit compile");
    assert!(
...
/// The reflection intrinsics survive the binary form (`temen-encode`) — opcodes 0x7A/0x7B/0x7E
...
fn discover_then_use_a_granted_capability() {
    ...
    let mut hj = Host::new();
    let jslots: Vec<i64> = grant3(&mut hj).iter().map(as_i64).collect();
    let jit = compile_and_run_with_host(&m, 0, &jslots, temen_run::cap_thunk, host_ptr(&mut hj))
        .expect("jit compile");
    assert!(
