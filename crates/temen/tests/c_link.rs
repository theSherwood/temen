...
//! unit on its own with `chibicc --emit-object`, then `temen_ir::link` the units into one program.
//!
...
//!     Temen signature (leading data-SP, then the C params), so the linker's direct call type-checks.
//!
...
/// wire** (`.temeno`, the v9 object dialect): encode with `encode_unit`, re-decode with
...
fn unit_via_temeno(m: temen_ir::Module, tag: &str) -> LinkUnit {
    let bytes = temen_encode::encode_unit(&m);
    let re =
        temen_encode::decode_unit(&bytes).unwrap_or_else(|e| panic!("decode {tag} object: {e:?}"));
...
fn selfhost_dir() -> PathBuf {
    repo_root().join("crates/temen-run/demos/chibicc_selfhost")
}
...
fn emit_object_real(cfile: &Path, include_dirs: &[PathBuf], tag: &str) -> LinkUnit {
    ...
    let m = temen_text::parse_module(&ir).unwrap_or_else(|e| panic!("parse {tag}: {e:?}"));
    unit_via_temeno(m, tag)
}
...
///
/// **Linux-only** (like `temen-llvm` and the self-host build scripts): `chibicc.h` pulls system headers
...
/// Compile the cc1 **entry** TU — `crates/temen-run/demos/chibicc_selfhost/cc1_main.c`, which replaces
...
fn object_unit_emit_libc() -> LinkUnit {
    ...
    let dirs = [
        ...
        selfhost_dir(),
        repo_root().join("crates/temen-run/demos/postgres"),
    ];
...
fn object_unit(tag: &str, src: &str) -> LinkUnit {
    ...
    static N: AtomicUsize = AtomicUsize::new(0);
    let id = N.fetch_add(1, Ordering::Relaxed);
    let base = std::env::temp_dir().join(format!("temen_clink_{tag}_{}_{id}", std::process::id()));
    ...
    let ir = std::fs::read_to_string(&irfile).unwrap();
    let m =
        temen_text::parse_module(&ir).unwrap_or_else(|e| panic!("parse {tag} unit: {e:?}\n{ir}"));
...
fn run_i32(m: &temen_ir::Module, idx: u32, sp: i64, args: &[i32]) -> i32 {
    ...
    let jit = match temen_jit::compile_and_run(m, idx, &jargs).expect("jit compile") {
        temen_jit::JitOutcome::Returned(v) => v[0] as i32,
...
fn links_two_separately_compiled_c_units() {
    ...
    temen_verify::verify_module(&linked).expect("verify the linked program");
...
fn unresolved_cross_unit_call_fails_closed() {
    ...
    assert_eq!(err, temen_ir::LinkError::Unresolved("nowhere".into()));
}
...
/// functions `load` from the window, so `temen_interp::run` materializes the data segments and the SP
...
fn data_sp(linked: &temen_ir::Module) -> i64 {
    let data_top = linked
        .data
        .iter()
...
fn cross_tu_data_symbol() {
    ...
    temen_verify::verify_module(&linked).expect("verify linked data program");
...
fn cross_tu_data_ptr_sym() {
    ...
    assert!(
        holder.module.data_ptrs.iter().any(|dp| matches!(
            &dp.target,
            temen_ir::DataPtrTarget::Sym { name, .. } if name == "target"
        )),
...
fn links_and_runs_under_powerbox() {
    ...
    temen_verify::verify_module(&linked).expect("verify linked powerbox program");
    ...
    let sp = temen_ir::powerbox_entry_sp(&linked);
    let data_top = linked
        .data
        .iter()
        .map(|d| d.offset + d.bytes.len() as u64)
...
fn links_and_runs_under_powerbox_with_argv() {
    ...
    let linked =
        temen_ir::link_with_manifest(&[main_unit, provider]).expect("link argv entry + provider");
    temen_verify::verify_module(&linked).expect("verify");
    assert!(
...
fn links_and_runs_emit_libc_under_powerbox() {
    ...
    let demo = object_unit_file("crates/temen/tests/fixtures/emit_libc/demo1.c");
    ...
    let linked = temen_ir::link_with_manifest(&[demo, libc]).expect("link demo + emit-object libc");
    assert!(
        linked.imports.iter().any(|i| i.name == "write"),
        "write retained as a host-bound manifest import: {:?}",
        linked.imports.iter().map(|i| &i.name).collect::<Vec<_>>()
    ...
    temen_verify::verify_module(&linked).expect("verify linked emit-object libc program");
...
/// [`object_unit_real`]).
/// A stand-in **owner** for the shared bump-allocator state (`__temen_brk`/`__temen_committed`/
/// `__temen_page`/`__temen_grow_lock`). The emit-object cc1 TUs now *import* these four globals from the
/// libc (`stdlib.h`'s `__TEMEN_LIBC_EXTERN` sharing — one bump pointer across all units, so the per-TU
...
fn alloc_state_stub() -> LinkUnit {
    object_unit(
        "alloc_state",
        "long __temen_brk = 268435456L;\n\
         long __temen_committed = 268435456L;\n\
         long __temen_page = 0;\n\
         int __temen_grow_lock = 0;\n",
    )
}
...
fn real_chibicc_type_tu_emits_and_links() {
    ...
    let linked =
        temen_ir::link_with_manifest(&[type_tu, alloc_state_stub()]).expect("link real type.c");
    assert!(
        linked.data_ptrs.is_empty(),
        "data.ptr relocations resolved and cleared at link"
    );
    temen_verify::verify_module(&linked).expect("verify real type.c");
}
...
fn links_multiple_real_chibicc_tus() {
    ...
    let linked = temen_ir::link_with_manifest(&units).expect("link four real cc1 TUs");
    temen_verify::verify_module(&linked).expect("verify the linked cc1 subset");
...
fn all_cc1_tus_compile_link_and_verify_under_emit_object() {
    ...
    //   * the shared bump-allocator state (`__temen_brk`/…) — each TU's `#include <stdlib.h>` allocator
    //     now imports one shared bump pointer (see `stdlib.h`'s `__TEMEN_LIBC_EXTERN`) instead of minting
    ...
    let linked = temen_ir::link_with_manifest(&units).expect("link all cc1 TUs");
    temen_verify::verify_module(&linked).expect("verify the whole linked cc1");
    ...
    // Its *state* (`__temen_brk`/…) is shared cross-TU (one bump pointer, `__TEMEN_LIBC_EXTERN`), stubbed
...
///
/// The guard this pins: **no temen-llvm intrinsic survives.** The on-ramp's `os_shim.c`/`printf_shim.c`
...
fn emit_object_libc_core_compiles_and_is_intrinsic_free() {
    ...
    // The load-bearing guard: not one on-ramp temen-llvm intrinsic is referenced. If `os_shim.c`/
...
fn emit_object_libc_printf_runs_byte_exact_under_powerbox() {
    use temen_run::{instantiate_with_imports, Outcome, RunConfig, Value};

    // Entry unit first so `demo2`'s `_start` is merged function 0; the libc has no `main`.
    let demo = object_unit_file("crates/temen/tests/fixtures/emit_libc/demo2.c");
    let libc = object_unit_emit_libc();

    let linked =
        temen_ir::link_with_manifest(&[demo, libc]).expect("link demo2 + emit-object libc");
...
fn emit_object_libc_float_runs_byte_exact_under_powerbox() {
    ...
    let demo = object_unit_file("crates/temen/tests/fixtures/emit_libc/demo_float.c");
    let libc = object_unit_emit_libc();

    let linked =
        temen_ir::link_with_manifest(&[demo, libc]).expect("link demo_float + emit-object libc");
    temen_verify::verify_module(&linked).expect("verify linked emit-object float program");
...
#[cfg(target_os = "linux")]
fn link_whole_cc1() -> temen_ir::Module {
    let mut units = vec![object_unit_cc1_entry()];
    for tu in [
        "tokenize.c",
        "preprocess.c",
    ...
    let linked = temen_ir::link_with_manifest(&units).expect("link whole cc1 + emit-object libc");
    temen_verify::verify_module(&linked).expect("verify the whole linked cc1 + libc");
    linked
}
...
/// substrate (guest libc + Temen engine vs system libc + native CPU). Built with `-mlong-double-64` (F3:
...
///     (`crates/temen-run/src/fs.rs`). `files`/`dirs` seed the memfs — a `#include` header lives here.
#[cfg(target_os = "linux")]
fn cc1_imports(files: Vec<(String, Vec<u8>)>, dirs: Vec<String>) -> temen_run::Imports {
...
#[cfg(target_os = "linux")]
fn cc1_compile(
    linked: &temen_ir::Module,
    args: &[&[u8]],
    stdin: &[u8],
    files: Vec<(String, Vec<u8>)>,
    dirs: Vec<String>,
) -> Vec<u8> {
    use temen_run::{instantiate_with_imports, Backend, RunConfig};
    let cfg = RunConfig {
    ...
    assert!(
        ...
        "the compiler emitted Temen IR (interp stderr: {})",
    ...
    temen_text::parse_module(&ir)
        .unwrap_or_else(|e| panic!("emitted IR should parse: {e:?}\n{ir}"));
    interp.stdout
}

/// **Benchmark: chibicc self-compile on native vs Temen bytecode vs Temen JIT.** The tree-walk interpreter
/// is the correctness oracle, not a consumer engine, so it's excluded here — this measures the two
/// engines a consumer actually runs. For each TU it times: native `chibicc_ref`; the Temen **bytecode**
/// engine (instantiate = IR→bytecode compile; run = execute); and the Temen **JIT** (instantiate =
...
/// is visible apart from the per-invocation run. Opt-in (`TEMEN_SELFHOST_BENCH=1`); not a CI gate.
#[cfg(target_os = "linux")]
#[test]
#[ignore = "benchmark: chibicc self-compile native vs bytecode vs jit; opt-in via TEMEN_SELFHOST_BENCH=1"]
fn bench_self_compile_native_vs_bytecode_vs_jit() {
    use std::time::Instant;
    use temen_run::{instantiate_with_imports, Backend, RunConfig};
    if std::env::var_os("TEMEN_SELFHOST_BENCH").is_none() {
        eprintln!("skipping self-compile benchmark; set TEMEN_SELFHOST_BENCH=1 to run");
        return;
    }
    let linked = link_whole_cc1();
    ...
    // a genuine constant. Measure it in isolation (pure `temen_jit::compile`, no execution) so the table
    ...
    for _ in 0..3 {
        let t = Instant::now();
        let _ = temen_jit::compile(&linked, 0).expect("compile whole cc1");
    ...
    println!(
        "\n=== chibicc self-compile: native vs Temen bytecode vs Temen JIT (best-of-3, ms) ==="
    );
    println!(
    ...
    println!(
        "\nlinked cc1: {nfuncs} funcs, {ninsns} IR insns.  \
         pure temen_jit::compile (Cranelift, opt_level=speed, no execution, best-of-3): {jit_compile_ms} ms [{profile} build].\n\
...
/// variables are the substrate (guest libc + Temen vs system libc + native CPU).
...
#[cfg(target_os = "linux")]
const SELFHOST_PRELUDE: &str = "crates/temen-run/demos/chibicc_selfhost/selfhost_prelude.h";
...
#[cfg(target_os = "linux")]
fn guest_emit_object(linked: &temen_ir::Module, tu: &str) -> Vec<u8> {
...
#[cfg(target_os = "linux")]
fn assert_guest_tu_matches_native(linked: &temen_ir::Module, tu: &str) {
    let guest = guest_emit_object(linked, tu);
    // Native reads the real header tree (absolute `-I`); header paths never reach the IR, so the only
    // variables are the substrate (guest libc + Temen vs system libc + native CPU). `-Ifrontend/chibicc`
...
#[cfg(target_os = "linux")]
fn guest_cc1_unit(linked: &temen_ir::Module, tu: &str, tag: &str) -> LinkUnit {
    let ir = guest_emit_object(linked, tu);
    let m = temen_text::parse_module(&String::from_utf8(ir).expect("guest IR is UTF-8"))
...
/// differ **only** by substrate (system libc + native CPU vs guest libc + Temen). This is what makes the
...
fn native_cc1_unit(tu: &str, tag: &str) -> LinkUnit {
    ...
    let m = temen_text::parse_module(&String::from_utf8(ir).expect("native IR is UTF-8"))
...
/// check: the TEMEN-executed compiler is faithful to native not just on crafted programs but on the
...
/// per-PR CI budget — so this is **opt-in**: it no-ops fast unless `TEMEN_SELFHOST_GIANTS=1` is set
...
#[cfg(target_os = "linux")]
#[test]
#[ignore = "very heavy (~1h): giant chibicc TUs on two engines; opt-in via TEMEN_SELFHOST_GIANTS=1"]
fn whole_cc1_compiles_giant_tus_matching_native() {
    if std::env::var_os("TEMEN_SELFHOST_GIANTS").is_none() {
        eprintln!("skipping giant-TU self-compile; set TEMEN_SELFHOST_GIANTS=1 to run (~1h)");
        return;
    }
    let linked = link_whole_cc1();
    for tu in [
...
/// (guest-compiles all nine, incl. the giants); opt-in via `TEMEN_SELFHOST_GIANTS=1`, on the nightly lane.
#[cfg(target_os = "linux")]
#[test]
#[ignore = "very heavy (~10 min): relink the whole cc1 from the guest's own objects; opt-in via TEMEN_SELFHOST_GIANTS=1"]
fn whole_cc1_relinks_from_guest_objects_equals_native() {
    if std::env::var_os("TEMEN_SELFHOST_GIANTS").is_none() {
        eprintln!("skipping guest-object relink; set TEMEN_SELFHOST_GIANTS=1 to run (~10 min)");
        return;
    }
    let entry = "crates/temen-run/demos/chibicc_selfhost/cc1_main.c";
    let upstream = [
        "tokenize.c",
        "preprocess.c",
        "parse.c",
    ...
    let chibicc1 =
        temen_ir::link_with_manifest(&native_units).expect("link native chibicc1 (relative paths)");
    ...
    let chibicc2 = temen_ir::link_with_manifest(&guest_units)
        .expect("link chibicc2 from the guest's own objects");
    temen_verify::verify_module(&chibicc2).expect("verify chibicc2");

    assert_eq!(
        temen_text::print_module(&chibicc2),
        temen_text::print_module(&chibicc1),
...
/// through `_start` under the powerbox and **compiles C to Temen IR inside the sandbox** — the self-host
...
/// byte-identical guest-vs-native, catching any construct where the TEMEN-executed compiler would diverge.
///
...
/// `cargo test -p temen --test c_link -- --ignored self_compiles`.
#[cfg(target_os = "linux")]
#[test]
...
fn whole_cc1_self_compiles_a_program_matching_native_on_interp_and_jit() {
    ...
    let incdir = std::env::temp_dir().join(format!("temen_cc1_inc_{}", std::process::id()));
