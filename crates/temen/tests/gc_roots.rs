...
fn gc_roots_is_unsupported_on_the_jit_without_fibers() {
    use temen_jit::{compile_and_run, JitError};
    let src = "memory 16\n\
        func () -> (i64) {\n\
        block 0 () {\n\
        \x20 v1 = i64.const 0\n\
...
/// (`fiber_rt::temen_gc_roots_flush`). The Tail ABI preserves the callee-saved registers, so under
...
fn build_multi_fiber_module(roots: &[i64], lo: i64, hi: i64) -> temen_ir::Module {
...
fn run_multi_fiber_words(m: &temen_ir::Module) -> [Vec<i64>; 2] {
    ...
    let (outcome, jmem) =
        temen_jit::compile_and_run_capture(m, 0, &[], &init_mem).expect("jit compile/run");
    assert!(
...
fn gc_roots_rejects_fold_down_mask_in_verifier() {
    ...
    assert!(
        matches!(
            verify_module(&m),
            Err(temen_verify::VerifyError::GcRootsMaskUnsafe { .. })
        ),
