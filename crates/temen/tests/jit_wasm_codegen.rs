...
/// A [`temen_interp::JitWasmEmitter`]: emit the wasm that runs a validated closed unit's entry as
/// `f0(win, env, args…)`, or `None` if it is outside the emitter subset (then `invoke` falls back to
/// the interpreter). A bare `fn`, so the `temen-wasm-jit` dependency stays out of the core.
fn emit_unit(blob: &[u8]) -> Option<Vec<u8>> {
    let m = temen_encode::decode_module(blob).ok()?;
    match temen_wasm_jit::compile_jit(&m, temen_wasm_jit::Shape::Batch { entry: 0 }, false) {
...
fn oracle_unit(unit: &Module, argv: &[i64]) -> Result<Vec<i64>, Trap> {
    ...
    temen_interp::run(unit, 0, &args, &mut fuel).map(|vals| {
        vals.iter()
            .map(|v| match v {
...
/// mirroring the browser's `temen_par_powerbox_jit_runtime` + `par_resolve_unit_rt`.
