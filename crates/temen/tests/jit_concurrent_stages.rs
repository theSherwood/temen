...
/// window offset 65536 (#1094: above the NULL guard AND aligned to the largest real granule — the
/// Windows 64 KiB allocation granularity `MapViewOfFile3` requires for the placement address; an
/// unaligned 16384 EINVALs there and the ring never aliases, hanging the pipeline), and move 1..=4
/// through the one-slot ring (flag at region byte 0, datum at 8).
