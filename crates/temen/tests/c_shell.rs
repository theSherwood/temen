...
//! end on `temen-posix` (the libc-as-host-caps personality), and it's the scaffold BusyBox `ash` slots
...
//! (`temen_ir::Resolved::Cap`, link-time symbol resolution); the guest discovers the granted handles
...
fn c_to_ir_with(src: &str, extra: &[&str]) -> String {
    ...
    let base = std::env::temp_dir().join(format!("temen_cshell_{}_{id}", std::process::id()));
...
fn c_to_ir_child_with(src: &str, extra: &[&str]) -> String {
    ...
    static N: AtomicUsize = AtomicUsize::new(0);
    let id = N.fetch_add(1, Ordering::Relaxed);
    let base = std::env::temp_dir().join(format!("temen_cshcmd_{}_{id}", std::process::id()));
    let cfile = base.with_extension("c");
    let irfile = base.with_extension("temen");
    std::fs::write(&cfile, src).unwrap();
...
const SHIM: &str = include_str!("../../temen-run/demos/shell/shim.c");
...
const RING: &str = include_str!("../../temen-run/demos/shell/ring.c");
...
const SHELL_MAIN: &str = include_str!("../../temen-run/demos/shell/shell_main.c");
...
fn run_shell_ex(
    ...
    let setup = |host: &mut Host| -> (temen_posix::Posix, i32, i32) {
    ...
    let m = temen_ir::resolve_imports_with(&raw, link_shim)
...
fn shell_bytecode_stdout(
    ...
    let cmd_handles: Vec<(&str, i32, u8)> = cmd_mods
        ...
        .collect();
    let (_px_h, posix) = temen_posix::grant(
        &mut host,
        (win - (64 << 10)) as u64,
        win as u64,
        stdin.to_vec(),
...
const STAGE_RUNNER_MAIN: &str = include_str!("../../temen-run/demos/shell/stage_runner_main.c");
...
const UPPER_MAIN: &str = include_str!("../../temen-run/demos/shell/upper_main.c");
...
/// uses), verifies, and encodes it to `browser/tests/fixtures/shell.temen` — the exact module bytes the
...
fn gen_browser_shell_fixture() {
    ...
    let out = dir.join("shell.temen");
    std::fs::write(&out, &bytes).expect("write shell.temen");
    ...
    std::fs::write(&rout, &rbytes).expect("write stage_runner.temen");
    ...
    for (name, source) in [("primes", PRIMES_MAIN), ("upper", UPPER_MAIN)] {
        ...
        let cout = dir.join(format!("{name}.temen"));
        std::fs::write(&cout, &cbytes).unwrap_or_else(|e| panic!("write {name}.temen: {e:?}"));
