...
fn c_to_ir(src: &str) -> String {
    ...
    let base = std::env::temp_dir().join(format!("temen_argvmain_{}_{id}", std::process::id()));
...
fn run(backend: Backend, args: &[&[u8]]) -> (i64, Vec<u8>) {
    let ir = c_to_ir(PROG);
    let module = temen_text::parse_module(&ir).expect("parse chibicc IR");
    let inst = temen_run::instantiate(module).expect("instantiate (resolve + verify)");
    let cfg = RunConfig {
