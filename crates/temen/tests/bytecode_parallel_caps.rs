...
use std::sync::Arc;
use temen_interp::{bytecode, Host, Region, StreamRole, Value};
...
fn run_cooperative() -> (Result<Vec<Value>, temen_interp::Trap>, Vec<u8>) {
...
fn run_parallel() -> (Result<Vec<Value>, temen_interp::Trap>, Vec<u8>) {
    let m = parse_module(CAPS).unwrap();
    let mut host = Host::new();
    let h = host.grant_stream(StreamRole::Out);
