...
fn host_with_unit(guest: &temen_ir::Module) -> (Host, i32, i32) {
...
fn compile_unit(host: &mut Host, jit: i32, src: &str) -> i32 {
    ...
    host.jit_compile(jit, &temen_encode::encode_module(&m))
        .expect("no trap")
        .expect("compile ok")
        .handle
}
...
fn invoke_setup(guest: &temen_ir::Module) -> (Host, i32, i32, i32) {
    let mut host = Host::new();
