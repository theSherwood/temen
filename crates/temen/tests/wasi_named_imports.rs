//! The WASI preview1 named-import path on **real WASI bytes** (§7) — the coverage that used to live in
//! the `temen-wasi` crate, folded here when that demo crate was retired (#919, audit §8).
//!
//! A deliberately tiny WASI preview1 shim (`fd_write` to stdout/stderr, `proc_exit`) as an embedder
//! host capability: temen-wasm transpiles a hand-written WASI module (each WASI import becomes one §7
//! `"<module>.<name>"` manifest import), [`resolve`] maps each name to a `(HOST_PROC, op)` over one
//! granted handle, and [`handler`] implements the ops over the guest window. WASI *semantics* live
//! here — outside both temen-wasm and the interp TCB — exactly the boundary DESIGN.md §7 draws: the
//! binding mechanism is in scope; WASI-the-standard is a host-layer shim a guest reaches only through
//! a granted, masked, type-checked handle. The tests drive the whole path on **both** backends.
#![forbid(unsafe_code)]

use std::sync::{Arc, Mutex};

use temen_interp::{cap_id, run_with_host, BoundImport, GuestMem, Host, HostProc, Trap};
...
/// temen-wasm declares them, `"<module>.<name>"`) to the [`cap_id::HOST_PROC`] capability + op. Unknown
...
/// (IMPORTS.md phase 3): each import name resolves through [`resolve`] to `(HOST_PROC, op)` over the
...
fn bind(m: &temen_ir::Module, host: &mut Host) -> Option<WasiOut> {
...
/// Build the WASI [`HostProc`] handler over `out`. `fd_write` captures into `out`; `proc_exit`
/// terminates the domain with the given code (a non-error [`Trap::Exit`]).
fn handler(out: WasiOut) -> HostProc {
    Box::new(
        move |op, args, mem, _minter: Option<&mut dyn temen_interp::RegionMinter>| match op {
...
/// `wasm32-wasi` (minimal, hand-written so the test needs no wasi toolchain). temen-wasm transpiles
/// it (the WASI import → a `CallImport`), [`resolve`] binds the name to the WASI `HostProc`
...
fn wasi_hello_world() {
    ...
    let t = temen_wasm::transpile(&wasm).expect("transpile WASI module");
    ...
    temen_verify::verify_module(&t.module).expect("verify manifest module");
    ...
    // The HostProc capability dispatches through the same `cap_dispatch_slots` the thunk calls, so
    // the demo runs on the production backend unchanged.
    let mut hj = Host::new();
    let jout = bind(&t.module, &mut hj).expect("bind WASI imports");
    let outcome = temen_jit::compile_and_run_with_host(
        &t.module,
        entry,
        &[],
        temen_run::cap_thunk,
    ...
    .expect("jit compile");
    assert!(
        matches!(outcome, temen_jit::JitOutcome::Returned(_)),
