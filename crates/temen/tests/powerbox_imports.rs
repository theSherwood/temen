//! **Phase 2 — name-based dynamic import binding.** A hand-written IR module (no C, no `temen-llvm`)
...
//! matches each `call.sym "<name>"` to a [`temen_run::HostCap`] and binds slot `i` ↔ import `i` at
...
fn arbitrary_named_host_capabilities_bind_and_run() {
    let module = temen_text::parse_module(SRC).expect("frontend IR parses");
...
/// encoding `temen-wasm` emits for `(import "kv" "set" …)` — and wasm semantics require both fields to
...
fn module_bindings_share_one_instance() {
    for backend in [Backend::TreeWalk, Backend::Bytecode, Backend::Jit] {
        let module = temen_text::parse_module(MODULE_SRC).expect("parse");
...
fn resolve_capability_by_name_at_runtime() {
    for backend in [Backend::TreeWalk, Backend::Bytecode, Backend::Jit] {
        let module = temen_text::parse_module(RESOLVE_SRC).expect("parse");
        let imports = Imports::new().provide("write", HostCap::stdout());
        let instance = instantiate_with_imports(module, imports).expect("instantiate");
        let run = instance
            .run(backend, &RunConfig::default())
...
fn resolve_unknown_name_is_fail_closed() {
    let module = temen_text::parse_module(RESOLVE_BOGUS_SRC).expect("parse");
...
fn canonical_powerbox_names_resolve_to_working_handles() {
    let module = temen_text::parse_module(CANON_SRC).expect("parse");
...
fn label_a_handle_to_its_registered_name() {
    for backend in [Backend::TreeWalk, Backend::Bytecode, Backend::Jit] {
        let module = temen_text::parse_module(LABEL_SRC).expect("parse");
        let imports = Imports::new().provide("write", HostCap::stdout());
        let instance = instantiate_with_imports(module, imports).expect("instantiate");
        let run = instance
            .run(backend, &RunConfig::default())
...
fn label_too_small_buffer_returns_full_length() {
    let module = temen_text::parse_module(LABEL_SMALL_SRC).expect("parse");
    let imports = Imports::new().provide("write", HostCap::stdout());
    let instance = instantiate_with_imports(module, imports).expect("instantiate");
    let run = instance.call("_start", &[]).expect("run");
    assert_eq!(
