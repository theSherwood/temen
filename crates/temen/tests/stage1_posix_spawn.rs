...
use core::ffi::c_void;
use temen_interp::{run_capture_reserved_with_host, Host, StreamRole, Trap};
use temen_ir::Resolved;
...
fn c_to_ir(src: &str, child_entry: bool) -> String {
    ...
    let base = std::env::temp_dir().join(format!("temen_pxspawn_{}_{id}", std::process::id()));
...
/// The shell, running on the `temen-posix` personality. It reads its own `argv` via the personality
