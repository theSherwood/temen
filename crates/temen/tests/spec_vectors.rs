...
//! (`temen-spec`), run its boundary-value input vectors on **all three backends** — the
...
use temen_jit::{compile, CompiledModule, JitOutcome, TrapKind};
use temen_spec::{all_rows, module_for, vectors_for, OpRow, Shape, SpecTrap, SpecVal};
...
fn to_value(v: SpecVal) -> Value {
    match v {
        ...
        SpecVal::V128(_) => unreachable!("no scalar row takes a v128"),
    }
}
...
fn slot_matches(expected: SpecVal, slot: i64) -> bool {
    match expected {
        ...
        SpecVal::V128(_) => unreachable!("no scalar row yields a v128"),
    }
}
...
fn check_vector(row: &OpRow, m: &temen_ir::Module, cm: &mut CompiledModule, vector: &[SpecVal]) {
...
fn spec_vectors_match_all_backends() {
        ...
        match row.shape {
            ...
            Shape::Immediate => {
                ...
                for vector in vectors_for(&row) {
                    ...
                    temen::verify::verify_module(&m).unwrap_or_else(|e| {
    ...
    // (all unary/binary rows take their full cross product — see temen-spec's
