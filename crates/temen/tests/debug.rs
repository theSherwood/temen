...
"#;
    ...
    assert_eq!(di.vars[5].func, temen_ir::GLOBAL_SCOPE);
    assert_eq!(di.vars[5].loc, temen_ir::VarLoc::Fixed { addr: 64 });
    assert_eq!(di.vars[5].type_id, Some(0));

    // The runtime-base window var (winvia) parsed its base loclist + offset.
    let temen_ir::VarLoc::WindowVia { base, off } = &di.vars[4].loc else {
    ...
    let temen_ir::VarLoc::SsaList(locs) = &di.vars[3].loc else {
    ...
    let temen_ir::TypeDef::Aggregate { fields, size, .. } = &di.types[3] else {
