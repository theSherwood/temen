...
fn child_ir(src: &str) -> String {
    ...
    let base = std::env::temp_dir().join(format!("temen_exec_{}_{id}", std::process::id()));
...
fn run_interp(cmd: &temen_ir::Module, argv: &[&[u8]]) -> Result<Vec<Value>, Trap> {
...
fn run_jit(cmd: &temen_ir::Module, argv: &[&[u8]]) -> JitOutcome {
    let parent = parse_module(&parent_src(argv)).expect("parse parent");
    verify_module(&parent).expect("verify parent");
    let mut host = Host::new();
    let ih = host.grant_instantiator(0, WIN as u64);
