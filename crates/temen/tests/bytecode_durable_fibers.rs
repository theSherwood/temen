//! Multi-fiber durability, commit 1 (INTERP_PERF.md; DURABILITY.md §12.8): the bytecode mirror of
//! `durable_fibers.rs`. A **durable** run keeps the active shadow-SP word pointing at the *running*
//! context's per-fiber shadow region, swapping it on every fiber switch (D-fiber-cont **option A** —
//! the swap lives in the engine's `cont.*` execution, where the resume chain is known). This is the
//! invariant the freeze path rests on: a poll that fires while a fiber runs must spill into that
//! fiber's own region, never a sibling's.
//!
//! Until the freeze driver lands (commit 2), the swap is observed directly: a host-fn capability
//! reads the active shadow-SP each time it is called, and we drive a root that probes, runs two
//! fibers (each probes, then **suspends** so both stay concurrently live in their own slots), and
//! probes again — proving each context sees a distinct region base and control is restored to the
...
#[test]
fn bytecode_durable_fiber_switch_routes_shadow_sp_per_context() {
    ...
    let (res, _snap) = bytecode::compile_and_run_capture_reserved_with_host(
        &m,
        0,
        &[Value::I32(hf)],
        &mut fuel,
    ...
    .expect("bytecode engine must drive a NORMAL multi-fiber durable module (commit 1)");
...
fn bytecode_non_durable_fiber_run_leaves_the_reserve_untouched() {
    ...
    .expect("bytecode engine drives a non-durable multi-fiber module");
    assert_eq!(
        res.expect("run ok"),
        vec![Value::I64(42)],
        "fiber returns 42"
