...
//! validated cross-platform by `jit_fuzz`/`escape_oracle` + the `temen-jit` PAL conformance test.
#![cfg(unix)]
...
use temen_jit::{compile_and_run_with_host, JitOutcome, TrapKind};
use temen_run::cap_thunk; // the shared JIT-CapThunk → reference-Host bridge (§9)
...
/// the slot bindings at instantiation ([`bind_imports`], mirroring `temen_run`'s `grant_caps`).
fn parse_module(ir: &str) -> Result<temen_ir::Module, temen_text::ParseError> {
    parse_module_raw(ir)
}
...
fn c_to_ir(src: &str) -> String {
    ...
    let base = std::env::temp_dir().join(format!("temen_cfe_{}_{id}", std::process::id()));
...
fn struct_operand_arithmetic_is_a_clean_error_not_a_crash() {
    ...
    for src in [
        ...
        let id = N.fetch_add(1, Ordering::Relaxed);
        let base = std::env::temp_dir().join(format!("temen_cfe_neg_{}_{id}", std::process::id()));
        let cfile = base.with_extension("c");
        std::fs::write(&cfile, src).unwrap();
        let status = Command::new(chibicc())
            .args([
...
///
/// Driven through the public, frontend-independent embedding API (F1): [`temen_run::instantiate`]
/// resolves + verifies (the resolve is a no-op here — [`parse_module`] already lowered the §7
/// imports), and [`temen_run::Instance::run_diff`] runs `_start` on the tree-walker *and* the JIT under
...
fn run_c_full(src: &str) -> CRun {
    ...
    let inst = temen_run::instantiate(m)
    ...
    let outcome = match run.outcome {
        temen_run::Outcome::Returned(v) => Outcome::Returned(v),
        temen_run::Outcome::Exited(c) => Outcome::Exited(c),
    };
    CRun {
        outcome,
        stdout: run.stdout,
...
fn native_run(src: &str) -> (u8, Vec<u8>) {
    ...
    static N: AtomicUsize = AtomicUsize::new(0);
    let id = N.fetch_add(1, Ordering::Relaxed);
    let base = std::env::temp_dir().join(format!("temen_gcc_{}_{id}", std::process::id()));
    let cfile = base.with_extension("c");
...
fn c_guest_mn_scheduler_demo() {
    let src = include_str!("../../temen-run/demos/mn_sched/mn_sched.c");
    let run = run_c_full(src);
    assert_eq!(
        run.stdout, b"1024\n",
...
/// that JITs itself — it emits serialized Temen IR at runtime (the binary `temen-encode` format,
...
#[cfg(all(unix, target_arch = "x86_64"))]
fn c_guest_jit_demo() {
    let src = include_str!("../../temen-run/demos/jit/jit_demo.c");
    let run = run_c_full(src);
...
fn c_guest_jit_link_demo() {
    let src = include_str!("../../temen-run/demos/jit/jit_link.c");
    let run = run_c_full(src);
    let out = String::from_utf8_lossy(&run.stdout);
    assert!(
...
fn c_guest_jit_dlopen_demo() {
    let src = include_str!("../../temen-run/demos/jit/jit_dlopen.c");
    let run = run_c_full(src);
    let out = String::from_utf8_lossy(&run.stdout);
    assert!(
...
fn c_guest_jit_hotreload_demo() {
    let src = include_str!("../../temen-run/demos/jit/jit_hotreload.c");
    let run = run_c_full(src);
    let out = String::from_utf8_lossy(&run.stdout);
    assert!(
...
fn c_guest_jit_threads_demo() {
    let src = include_str!("../../temen-run/demos/jit/jit_threads.c");
    let run = run_c_full(src);
    assert_eq!(
        run.stdout, b"0\n",
...
/// `temen_run::JitSession`: re-enter the C `_start` (func 0) once per prompt over a persistent window,
...
fn run_jit_repl_session(
    ...
    let mut session = temen_run::JitSession::new(
        &m,
        0,
...
fn c_guest_jit_repl_compacts() {
    let src = include_str!("../../temen-run/demos/jit/jit_repl.c");
    let n = 30;
...
fn c_guest_work_stealing_demo() {
    let src = include_str!("../../temen-run/demos/work_stealing/work_stealing.c");
    let run = run_c_full(src);
    assert_eq!(
        run.stdout, b"256\n",
...
/// native stack migrates (on the JIT, a real cross-thread `temen-fiber` switch claimed through the
...
fn c_guest_steal_fibers_demo() {
    let src = include_str!("../../temen-run/demos/steal_fibers/steal_fibers.c");
    let run = run_c_full(src);
    assert_eq!(
        run.stdout, b"256\n121920\n",
...
fn c_guest_thread_safe_malloc() {
    let src = include_str!("../../temen-run/demos/malloc_threads/malloc_threads.c");
    let run = run_c_full(src);
    assert_eq!(
        run.stdout, b"0\n",
