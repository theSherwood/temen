...
//! Slice 1c-5a). The random module generator never emits `call.cap`, so — per the TDD plan for the
...
//! does not *support* a synchronous `call.cap` (returns `None` → would fall back), the test fails.
...
fn check_cap(src: &str, extra_args: &[Value]) {
    ...
    let bc = bytecode::compile_and_run_with_host(&m, 0, &args_bc, &mut f_bc, &mut h_bc)
        .expect("bytecode engine must support a synchronous call.cap module (Slice 1c-5a)");

    assert!(
        !matches!(tw, Err(Trap::OutOfFuel)) && !matches!(bc, Err(Trap::OutOfFuel)),
        "unexpected OutOfFuel\n tw={tw:?}\n bc={bc:?}\n{src}"
    );
    assert_eq!(tw, bc, "call.cap: tree-walker != bytecode\n{src}");
}
...
block 0 (v0: i32, v1: i64, v2: i64) {
  v3 = call.cap 13 0 (i64, i64) -> (i64) v0 (v1, v2)
  return v3
  }
}
"#;
...
block 0 (v0: i32, v1: i64) {
  v2 = call.cap 13 5 (i64) -> (i64) v0 (v1)
  return v2
  }
}
"#;
...
func (i32, i64, i64) -> (i64) {
block 0 (v0: i32, v1: i64, v2: i64) {
  v3 = call.cap 13 0 (i64) -> (i64) v0 (v1)
  v4 = call.cap 13 0 (i64, i64) -> (i64) v0 (v3, v2)
  v5 = i64.add v3 v4
  return v5
  }
}
...
/// A call.cap inside a loop, accumulating — exercises the seam across back-edges and with the
...
block 2 (v9: i32, v10: i64, v11: i64, v12: i64) {
  v13 = call.cap 13 0 (i64) -> (i64) v9 (v11)
  v14 = i64.add v12 v13
  v15 = i64.const 1
  v16 = i64.add v11 v15
  br 1(v9, v10, v16, v14)
...
// ---- §7 reflection (self.count / self.get) --------------------------------------------
...
fn check_self(src: &str, args: &[Value]) {
    ...
    let bc = bytecode::compile_and_run_with_host(&m, 0, args, &mut f_bc, &mut h_bc)
        .expect("bytecode engine must support self.* (Slice 1c-5a)");

    assert_eq!(tw, bc, "self.*: tree-walker != bytecode\n{src}");
}
...
/// `self.get(i)` returns `(handle, type_id)`; sum them so the result depends on both.
