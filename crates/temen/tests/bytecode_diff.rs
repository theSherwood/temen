...
//!
//! The bytecode engine (`temen_interp::bytecode`) must be a *bug-for-bug* match of the reference
...
//!      bytecode engine supports, assert it agrees with `temen_interp::run`.
...
use temen_interp::{bytecode, run, run_fast, run_traced, Host, Trap, Value};
...
fn check(m: &temen_ir::Module, args: &[Value], seed: u64) -> bool {
    if m.funcs.is_empty() {
        return false;
    }
...
fn interp_call(m: &temen_ir::Module, n: i64) -> i64 {
...
fn bc_call(m: &temen_ir::Module, n: i64) -> i64 {
    let mut fuel = u64::MAX;
