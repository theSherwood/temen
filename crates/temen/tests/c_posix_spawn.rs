...
//! embedder wires the personality's spawn delegate ([`temen_posix::Posix::set_spawn`]) to **instantiate and
...
fn c_to_ir(src: &str) -> String {
    ...
    let base = std::env::temp_dir().join(format!("temen_cpxspawn_{}_{id}", std::process::id()));
...
/// ([`temen_posix::resolve`] → `(HOST_PROC, op)`), phase-4 no-rewrite. Same helper as `c_posix.rs`.
...
fn up_delegate(
    ...
    move |name: &str, _argv: &[String], stdin: &[u8]| {
        ...
        let mut ch = Host::new();
        let (px, cposix) = temen_posix::grant(&mut ch, win / 2, win, stdin.to_vec());
...
fn run_shell(
    ...
    let (px, posix) = temen_posix::grant(&mut host, win / 2, win, stdin.to_vec());
