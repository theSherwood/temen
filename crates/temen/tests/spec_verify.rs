...
//! §3b/§3c validity rules — `temen-verify` (production, the TCB contract) and
//! `temen_spec::verify` (the reference, written from the prose) — must agree:
//!
...
//! `temen-verify` now needs the same bug, independently written, in the reference.

#[path = "support/irgen.rs"]
mod irgen;

use irgen::Gen;
use temen::verify::{verify_module, VerifyError};
use temen_ir::*;
use temen_spec::{all_rows, mem_rows, module_for, module_for_mem, vectors_for, Shape};
...
fn reject(m: &Module, what: &str, pat: impl Fn(&VerifyError) -> bool) {
    ...
    assert!(
        temen_spec::verify::verify(m).is_err(),
...
fn directed_rule_rejects() {
    ...
    m.add_func_import("out", stream_sig.clone(), temen_ir::ImportMode::Rebindable);
    accept(&m, "attach to a rebindable import");
    let mut m = module(vec![attach_fn.clone()]);
    m.add_func_import("out", stream_sig, temen_ir::ImportMode::Required);
    ...
    let offer_module = || {
        let mut m = module(vec![func(vec![], vec![], vec![], T::Return(vec![]))]);
        m.types = vec![
            temen_ir::TypeEntry::Func(FuncType {
                params: vec![],
                results: vec![],
            }),
    ...
    let offer = |interface: u32, ops: Vec<u32>| temen_ir::ImplExport {
        name: "logger".into(),
        interface,
        ops,
        threaded: false,
    ...
    m.types[0] = temen_ir::TypeEntry::Func(FuncType {
        params: vec![],
        results: vec![V::I64],
    });
    m.impl_exports.push(offer(1, vec![0]));
    ...
    let mut m = offer_module();
    m.types[1] = temen_ir::TypeEntry::Interface(vec![
        temen_ir::IfaceOp {
            name: "a".into(),
            ty: 0,
        },
...
fn verifier_agreement_on_generated_modules() {
    for seed in 0..300u64 {
        ...
        for kind in 0..6u64 {
            ...
            let refv = temen_spec::verify::verify(&mm).is_ok();
            assert_eq!(
                prod,
                refv,
                "verifier disagreement (seed {seed}, mutation {kind}): production={prod}, \
                 reference={refv}\n{}",
                temen::text::print_module(&mm)
            );
        }
    }
}
...
fn spec_agreement_on_v7_grouped_surface() {
    ...
    let base = |op: u32| {
        ...
        m.types = vec![
            temen_ir::TypeEntry::Func(i64_sig.clone()),
            temen_ir::TypeEntry::Interface(vec![temen_ir::IfaceOp {
                name: "dbl".into(),
                ty: 0,
            }]),
        ];
        m.imports = vec![temen_ir::Import {
            name: "env.svc".into(),
            shape: temen_ir::ImportShape::Interface(1),
    ...
    let mut m = base(0);
    m.imports[0].shape = temen_ir::ImportShape::Interface(0);
