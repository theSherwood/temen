...
//! `crates/temen/tests/spec_fuzz_smoke.rs`, exactly as `irgen.rs` is shared by `diff`/`jit_fuzz`:
//!
...
//!   mutation, and assert `temen-verify` and the reference verifier (`temen_spec::verify`) agree
...
use temen_jit::{compile, JitOutcome, TrapKind};
use temen_spec::{all_rows, module_for, OpRow, Shape, SpecTrap, SpecVal};
...
fn read_val(r: &mut Reader, t: temen_ir::ValType) -> SpecVal {
    use temen_ir::ValType as V;
    match t {
...
fn check_one(
    row: &OpRow,
    m: &temen_ir::Module,
    cm: &mut temen_jit::CompiledModule,
    vector: &[SpecVal],
) {
...
// ---------------------------------------------------------------------------------------------
// Driver B — verifier accept/reject agreement between temen-verify and the reference verifier.
// ---------------------------------------------------------------------------------------------

use irgen::Gen;
use temen_ir::{Inst, Module, Terminator, ValType};
