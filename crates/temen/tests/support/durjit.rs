...
//!      `temen-snapshot` §12 codec to a **byte-identical artifact** (header digest + handle
...
//!
//! Shared by the stable `cargo test` driver (`crates/temen/tests/durable_jit.rs`) and the
//! libFuzzer target (`fuzz/fuzz_targets/durable_jit.rs`), mirroring `irgen`/`durgen`.

#![allow(dead_code)] // each includer uses a subset

#[path = "../../../temen-durable/tests/support/durgen.rs"]
pub mod durgen;
...
pub fn fuzz_recycle_fiber_one_xbackend(g: &mut Gen) {
    ...
    temen_verify::verify_module(&inst).expect("instrumented recycling IR must verify");
