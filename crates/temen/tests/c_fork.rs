...
fn c_to_ir(src: &str) -> String {
    ...
    let base = std::env::temp_dir().join(format!("temen_cfork_{}_{id}", std::process::id()));
...
fn a_shell_redirects_a_command_output_to_a_file() {
    ...
    let (factory, memfs) = temen_run::fs::mem_fs_shared_factory(vec![], vec![]);
...
fn a_shell_appends_a_command_output_to_a_file() {
    ...
    let (factory, memfs) = temen_run::fs::mem_fs_shared_factory(
...
fn a_shell_redirects_a_file_into_a_command_stdin() {
    ...
    let (factory, _memfs) = temen_run::fs::mem_fs_shared_factory(
...
fn opshift_fork(base: temen_interp::HostProcFork) -> temen_interp::HostProcFork {
    Arc::new(move |pid| {
        let forked = base(pid);
        let mut inner = forked.handler;
        temen_interp::ForkedProc {
...
/// [`temen_posix::Posix::kill_pid`]`(1000, SIGINT)` — the parent's personality pid (the first
