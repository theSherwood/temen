...
//! over that map (Stage 2) — round-tripped here through the existing `temen_wasm::dwarf_line` reader,
...
fn jit_emits_debug_line_that_round_trips_through_the_reader() {
    ...
    let prog = temen_wasm::dwarf_line::parse(&bytes).expect("the emitted line program parses");
...
fn jit_emits_debug_info_subprograms_that_round_trip() {
    ...
    let parsed = temen_wasm::dwarf_info::parse(&info, &abbrev, &[]).expect("the emitted CU parses");
...
fn jit_wraps_dwarf_in_an_elf_whose_sections_round_trip() {
    ...
    let line = temen_jit::gdb::elf_section(&elf, ".debug_line").expect(".debug_line in the ELF");
    assert_eq!(
        line,
        cm.debug_line_section(),
        "the embedded .debug_line is the section verbatim"
    );
    let prog = temen_wasm::dwarf_line::parse(line).expect("embedded line program parses");
    ...
    let info = temen_jit::gdb::elf_section(&elf, ".debug_info").expect(".debug_info in the ELF");
    let abbrev =
        temen_jit::gdb::elf_section(&elf, ".debug_abbrev").expect(".debug_abbrev in the ELF");
    let parsed = temen_wasm::dwarf_info::parse(info, abbrev, &[]).expect("embedded CU parses");
    assert_eq!(
        parsed.subs.len(),
        1,
...
fn jit_registers_and_unregisters_with_the_gdb_jit_interface() {
    ...
    // state directly.
    let cm = compile(COMPUTE_DBG);
    let (_, before) = temen_jit::gdb::descriptor_state();

    {
        let _reg = cm.register_with_gdb().expect("a -g module registers");
        let (action, during) = temen_jit::gdb::descriptor_state();
        ...
        let (_, two) = temen_jit::gdb::descriptor_state();
    ...
    let (action, after) = temen_jit::gdb::descriptor_state();
...
fn jit_emits_type_dies_that_round_trip() {
    ...
    use temen_wasm::dwarf_info::DwarfType;
...
fn jit_emits_variable_loclists_matching_the_value_locations() {
    ...
    let (info, abbrev) = cm.debug_info_sections();
    let parsed = temen_wasm::dwarf_info::parse(&info, &abbrev, &[]).expect("the CU parses");
    assert_eq!(
        parsed.subs.len(),
        1,
