...
use temen_ir::{Block, Func, Inst, IntTy, Module, Terminator, ValType};
...
fn generated_fiber_programs_never_panic_and_are_deterministic() {
    ...
    for _ in 0..2_000 {
        ...
        for fi in 0..m.funcs.len() as u32 {
            let args = [temen_interp::Value::I64(4096), temen_interp::Value::I64(1)];
            // Fuel is metered at IR safepoints (function entries, taken back-edges, and — since the
            // fiber-safepoint fix — `cont.resume`), not per op. With resume metered, fuel again bounds
            // fiber-recursion, so this corpus runs in ~5 s debug at the original 8_000 budget.
...
/// on *both* backends and must agree — hardening the `temen-fiber` native stack-switch the JIT lowers
...
fn generated_fiber_programs_agree_on_interp_and_jit() {
    ...
    for _ in 0..iters {
        ...
        let (jit, _) = cm.run(&[4096, 1], None, None).expect("jit fiber run");
...
/// real `temen-fiber` stack switch **cross-thread** through the shared-table claim. The interpreter
    ...
    for _ in 0..iters {
        ...
        match compile_and_run(&m, 0, &[]).expect("jit compile/run") {
            ...
            other => {
                ...
                #[cfg(windows)]
                if matches!(other, JitOutcome::Trapped(temen_jit::TrapKind::FiberFault)) {
                    alloc_skips += 1;
                    continue;
                }
