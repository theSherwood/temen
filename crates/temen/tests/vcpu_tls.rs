...
//!
//! One i64 of temen per-vCPU state, read/written **at the execution point**, seeded at vCPU creation to
//! a dense id (root = 0, children sequential). In temen's M:N model a fiber migrates between vCPUs
...
fn interp_i64s(m: &temen_ir::Module) -> Vec<i64> {
...
mod threaded {
    ...
    fn jit_i64s(m: &temen_ir::Module) -> Vec<i64> {
        match compile_and_run(m, 0, &[]).expect("jit compile/run") {
    ...
    /// seed temen assigned) into `slot[idx]`; the root reads the three slots back. Each vCPU's seed must
