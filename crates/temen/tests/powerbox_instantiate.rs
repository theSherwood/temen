//! The **frontend-independent powerbox** path: a hand-written IR module (no C, no `temen-llvm`) with
//! a paramless `_start` (func 0) and a `write` manifest import, instantiated and run through
//! [`temen_run::instantiate`] / [`temen_run::Instance::call`]. This is the `run_c_full` experience —
...
//!
//! Gated `#![cfg(unix)]` like the other JIT differential suites (`temen-jit`'s window/guard page is
//! unix-only).
#![cfg(unix)]

use temen_run::{instantiate, Outcome, Value};
...
/// binds the `write` slot at instantiation — exactly what a non-C frontend that targets TEMEN-IR +
...
fn handwritten_ir_runs_through_the_powerbox_wrapper() {
    ...
    let module = temen_text::parse_module(HELLO).expect("frontend IR parses");
...
/// [`temen_run::Instance::call`] on a non-`_start` export: it resolves the name to its funcidx and runs
