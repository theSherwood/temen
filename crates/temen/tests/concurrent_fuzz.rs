...
//!
//! - **Deterministic sweep:** [`temen_interp::run_scheduled`] across many scheduler seeds — systematic,
//!   reproducible coverage. A failure is replayable from `(program_seed, scheduler_seed)`.
//! - **Real-executor stress:** [`temen_interp::run`] on the M:N pool — OS scheduling supplies extra
//!   interleaving variety and the run is ThreadSanitizer-clean.

use temen_interp::{run, run_scheduled, Trap, Value};
