...
fn resolve_imports_calls_are_linker_only() {
    ...
    let callers: &[&str] = &[
        "crates/temen-ir/src/lib.rs",    // the pass itself + its linker tests
        "crates/temen-run/src/lib.rs",   // the guest-JIT `compile_linked` symbol-table path
        "browser/src/lib.rs",            // browser_jit_validator — the same `compile_linked` path
        "crates/temen-text/src/lib.rs",  // parse → link integration test
        "crates/temen/tests/dynlink.rs", // linker tests (Func/Slot)
        "crates/temen/tests/dynlink_runtime.rs",
        "crates/temen/tests/dynlink_resolve.rs",
        "crates/temen/tests/dynlink_repl.rs",
        "crates/temen/tests/c_shell.rs", // link a compiled C object's symbols (link-time, no handles)
        "crates/temen/tests/c_shell_exec.rs",
        "crates/temen/tests/stage1_posix_spawn.rs",
    ];
    let internals: &[(&str, &[&str])] = &[
        ("patch_placeholder", &["crates/temen-ir/src/lib.rs"]),
        (
            "SlotHandleNotConst",
            &[
                "crates/temen-ir/src/lib.rs",
                "crates/temen/tests/dynlink.rs",
            ],
        ),
    ];
    let violations = scan(|rel, text, out| {
