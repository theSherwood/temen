...
//! `temen-interp/tests/concurrent_stages.rs` pins on the tree-walk oracle — byte-identical module,
//! same 410 result.
//!
...
/// sums them (park while empty) → 10. Byte-identical to `temen-interp/tests/concurrent_stages.rs`.
...
fn grants(m: &Arc<temen_ir::Module>) -> (Host, i32, i32) {
...
fn bytecode_drives_two_concurrent_stages_through_a_shared_region_ring() {
    let m = Arc::new(temen_text::parse_module(PIPELINE).expect("parse"));
