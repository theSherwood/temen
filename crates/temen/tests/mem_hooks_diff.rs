...
//! `temen_opt::instrument::instrument_mem_hooks`, HOOKS.md).
//!
...
use temen_run::{instantiate, Backend, MemEvent, MemHookFn, RunConfig};
...
fn hooked_run(src: &str, backend: Backend) -> (Vec<MemEvent>, Result<temen_run::Run, String>) {
...
fn multi_vcpu_guest_is_observed_without_crashing() {
    for backend in [Backend::TreeWalk, Backend::Bytecode] {
        ...
        assert_eq!(
            run.outcome,
            temen_run::Outcome::Returned(vec![temen_run::Value::I64(43981)]),
