...
fn run_f64(m: &Module, x: f64) -> f64 {
    let mut fuel = 10_000_000u64;
    match temen_interp::run(m, 0, &[Value::F64(x)], &mut fuel) {
...
fn specializes_float_interpreter() {
    ...
    for x in [0.0f64, 0.5, 1.5, 2.5, 3.0, -4.0] {
        ...
        let jit = match temen_jit::compile_and_run(&residual, 0, &[x.to_bits() as i64]) {
            Ok(temen_jit::JitOutcome::Returned(v)) => f64::from_bits(v[0] as u64),
...
fn inlines_direct_call_leaf_helper() {
    ...
    for x in [0i64, 1, 2, -3, 7, 1000, i64::MIN] {
        ...
        let jit = match temen_jit::compile_and_run(&residual, 0, &[x]) {
...
fn outline(m: &Module, func: u32, args: &[SpecArg]) -> Result<Module, temen_peval::SpecError> {
...
fn selective(m: &Module, func: u32, args: &[SpecArg]) -> Result<Module, temen_peval::SpecError> {
    specialize_with_config(
        m,
        func,
        args,
