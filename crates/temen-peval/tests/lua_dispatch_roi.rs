...
//! substrate (see `temen-llvm/tests/lua_futamura_specialize.rs`). But the win a Futamura projection is
...
//!
//! Run: `cargo test -p temen-peval --test lua_dispatch_roi -- --nocapture`

use temen_interp::Value;
use temen_ir::{Data, Inst, LoadOp, Module, Terminator};
...
/// `temen-llvm/tests/lua_futamura_specialize.rs`; opcodes verified there).
...
fn run_fuel(m: &Module, x0: i64) -> (Result<Vec<Value>, temen_interp::Trap>, u64) {
    let start = 50_000_000u64;
    let mut fuel = start;
    let r = temen_interp::run(m, 0, &[Value::I64(x0)], &mut fuel);
    (r, start - fuel)
}
