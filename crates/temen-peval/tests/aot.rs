//! Ahead-of-time pipeline proof. `temen-peval` is a pure host-side `Module -> Module` transform, so
...
use temen_ir::{
    BinOp, Block, Data, Func, Inst, IntTy, LoadOp, Memory, Module, Terminator, ValType,
};
use temen_jit::JitOutcome;
use temen_peval::{specialize, SpecArg};
