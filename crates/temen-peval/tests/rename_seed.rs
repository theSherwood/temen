...
fn count<F: Fn(&temen_ir::Inst) -> bool>(m: &Module, pred: F) -> usize {
    m.funcs
        .iter()
        .flat_map(|f| &f.blocks)
        .flat_map(|b| &b.insts)
...
fn n_loads(m: &Module) -> usize {
    count(m, |i| matches!(i, temen_ir::Inst::Load { .. }))
}
fn n_stores(m: &Module) -> usize {
    count(m, |i| matches!(i, temen_ir::Inst::Store { .. }))
}
...
";
    ...
    for x in [1i64, -5, 0x0BAD_F00D, i64::MIN] {
        ...
        let (r_ref, w_ref) = temen_interp::run_capture(&m, 0, &[Value::I64(x)], &mut f1, &init);
        let mut f2 = 1_000_000u64;
        let (r_res, w_res) =
            temen_interp::run_capture(&residual, 0, &[Value::I64(x)], &mut f2, &init);
    ...
    let (_, w_scratch) = temen_interp::run_capture(&scratch, 0, &[Value::I64(42)], &mut f, &init);
    assert_eq!(
...
fn two_disjoint_seeded_regions_fold_and_write_back() {
    // Disjoint regions A@16512 and B@16640 (as a lua_State / stack / CallInfo would be), both seeded and
...
";
    ...
    m.data.push(Data {
        offset: 16512, // above the #1094 NULL guard
        readonly: false,
        bytes: a0.to_le_bytes().to_vec(),
    });
    m.data.push(Data {
        offset: 16640, // above the #1094 NULL guard
        readonly: false,
    ...
    let cfg = SpecConfig {
        rename: Some((16512, 16520)),
        rename_extra: vec![(16640, 16648)],
        rename_is_private: true,
        rename_seed_from_image: true,
        const_regions: vec![(16512, 16520), (16640, 16648)],
        ..SpecConfig::default()
    };
    let residual = specialize_with_config(&m, 0, &[SpecArg::Dynamic], &cfg).expect("specializes");
    verify_module(&residual).expect("residual verifies");
