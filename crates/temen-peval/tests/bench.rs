...
//!
//! Run with:  cargo test -p temen-peval --test bench -- --ignored --nocapture
...
/// encoded `.temen` byte length — the three things specialization is meant to shrink (or, for a
...
/// Emit one machine-readable metric row when `TEMEN_BENCH_CSV` is set in the environment, as
...
fn interp_run(m: &Module, input: i64) -> i64 {
    ...
    match temen_interp::run(m, 0, &[Value::I64(input)], &mut fuel) {
...
fn jit_run(m: &Module, input: i64) -> i64 {
    match temen_jit::compile_and_run(m, 0, &[input]) {
...
// the table with:  cargo test -p temen-peval --test bench size_corpus -- --nocapture
// ===========================================================================================
...
//
//   cargo test -p temen-peval --test bench outline_rename -- --nocapture
// ===========================================================================================
...
//
//   cargo test -p temen-peval --test bench fuzz -- --nocapture
// ===========================================================================================
...
fn interp_try(m: &Module, input: i64, fuel: u64) -> Option<i64> {
    let mut f = fuel;
    match temen_interp::run(m, 0, &[Value::I64(input)], &mut f) {
        Ok(v) => match v.as_slice() {
            [Value::I64(x)] => Some(*x),
            _ => None,
        },
...
fn fuzz_shape(
    ...
    specialize: impl Fn(&Module) -> Result<Module, temen_peval::SpecError>,
) -> Tally {
    ...
    for _ in 0..n {
        ...
        let residual = match specialize(&interp) {
            Ok(r) => r,
            Err(temen_peval::SpecError::Budget) => {
                t.budget += 1;
                t.ex_budget.get_or_insert_with(|| prog.clone());
                continue;
            }
            Err(temen_peval::SpecError::Unsupported) => {
                t.unsupported += 1;
    ...
    for &seed in seeds {
        ...
        for _ in 0..n {
            ...
            match (np, pv) {
                ...
                (Err(temen_peval::SpecError::Unsupported), Ok(rp)) => {
                    np_unsup += 1;
                    rescues += 1;
                    icheck(&rp, "pv");
                    if first_ok.is_some() {
...
/// it to map the bail surface: `cargo test -p temen-peval --test bench fuzz_specialization_thorough --
/// --ignored --nocapture`.
#[test]
...
//
//   cargo test -p temen-peval --test bench gain_spectrum -- --ignored --nocapture
// ===========================================================================================
