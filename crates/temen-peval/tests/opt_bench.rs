...
//! temen-opt pass buys us, by *leave-one-out*: run the full pipeline, then the full pipeline with one
...
//!
//!   cargo test -p temen-peval --test opt_bench -- --nocapture              # size table (+ guards)
//!   cargo test -p temen-peval --test opt_bench -- --ignored --nocapture    # + run-time table
//!
//! Set `TEMEN_BENCH_CSV=1` to also emit `CSV,<bench>,<case>,<metric>,<value>` rows for the report script.
...
fn interp_run(m: &Module, args: &[Value]) -> Result<Vec<Value>, temen_interp::Trap> {
    let mut fuel = 50_000_000u64;
    temen_interp::run(m, 0, args, &mut fuel)
}
...
fn opt_runtime_ablation() {
    ...
    println!(
        "\nTwo backends, because they answer different questions. The **JIT** runs its own optimizer\n\
         over the IR, so temen-opt's scalar passes are largely redundant there (native run time barely\n\
         ...
         temen-opt adds on the interp path (and inside temen, DESIGN.md §20c)."
    );
