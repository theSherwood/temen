...
//! `temen-peval` — the partial-evaluation / Futamura on-ramp (see `DESIGN.md` §20c).
//!
...
//!
//! The **generic IR→IR optimizer** it builds on now lives in the [`temen_opt`] crate: the specializer
//! reuses that crate's constant-fold machinery ([`temen_opt::Known`] + the `fold_*` helpers) and its
//! CFG cleanup ([`optimize_module`], re-exported here so existing consumers keep their `temen_peval::`
...
//! escape-TCB: its output is re-verified with `temen_verify::verify_module` before it runs, so a bug
...
//! (DESIGN.md §20c). The `libm-floats` feature is forwarded to [`temen_opt`] (the in-temen build drops it,
...
pub use specialize::{
    ...
    PretailcallModel, SpecArg, SpecConfig, SpecError, TailSite,
};

// The generic optimizer + remap helpers moved to `temen-opt` (see `OPT.md` Phase 0). Re-exported so
// existing consumers (`temen-run`, the `temen-llvm` demos, this crate's own tests) keep working through
// the `temen_peval::` path.
pub use temen_opt::{
