...
pub struct SpecConfig {
    ...
    /// expressed generically over `(func, block)` so it fits any interpreter ported to temen, not just
    ...
    /// `String`-cloning the manifest out of the on-ramped `temen-peval` guest build).
...
pub fn specialize_with_config(
    ...
    // resolve driftsort's `sqrt_approx`, and temen-peval is itself on-ramped in the guest peval tests).
    ...
    if config.carry_whole_module {
        ...
        // source `imports` manifest (temen-peval itself leaves it empty — cloning the `String`-named
...
impl Spec<'_> {
    ...
    fn build_block(&mut self, task: Task) -> Result<temen_ir::Block, SpecError> {
