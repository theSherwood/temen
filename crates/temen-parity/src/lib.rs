...
//!
//! * [`parity`] — an **exhaustive** classifier over [`temen_ir::Inst`]/[`temen_ir::Terminator`]. No
//!   wildcard arm, exactly like [`temen_ir::Inst::effects`]: a new op fails to compile until it is
...
//!   built from `temen-ir`'s own sub-op vocabulary so the row set tracks the IR.
//!
...
//! same data by the `temen-parity-gen` bin and kept fresh by a golden test.

use temen_ir::*;

/// The four execution strategies (DESIGN.md §3), in matrix-column order. The names match the
/// crate/short-id vocabulary the owner uses (`temen-tree-walk`, `temen-bytecode`, `temen-jit`,
/// `temen-wasm-jit`).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum Backend {
    /// `temen-interp::run` — walks the SSA IR directly. The **oracle**; runs every op.
    TreeWalk,
    /// `temen-interp::bytecode` — operand-resolved threaded bytecode. Held bit-exact; runs every op.
    Bytecode,
    /// `temen-jit` — the Cranelift native speed path. Fail-closed: folds its non-subset to the oracle.
    Cranelift,
    /// `temen-wasm-jit` — emits wasm for a host engine's optimizing tiers. A **leaf accelerator**
...
impl Backend {
    ...
    pub fn short(self) -> &'static str {
        match self {
            Backend::TreeWalk => "temen-tree-walk",
            Backend::Bytecode => "temen-bytecode",
            Backend::Cranelift => "temen-jit",
            Backend::WasmJit => "temen-wasm-jit",
        }
    }
}
...
const FORK_GAP: &str =
    "native on tree-walk + bytecode; Cranelift still folds (serve loop in temen-run, native-frame twin) — the next slice (FORK.md §9.1)";
...
fn parity_capcall(type_id: u32, op: u32) -> [Cell; 4] {
    ...
    match (type_id, op) {
        ...
        // Cranelift (its serve loop is in temen-run, its twin continuation is a native frame with no
...
///
/// Ground truth for the two JIT columns: the Cranelift support gate (`temen_jit`'s `ensure_supported`)
/// and the wasm-JIT subset (`temen_wasm_jit`'s `block_value_types` typing pass). The conformance test
