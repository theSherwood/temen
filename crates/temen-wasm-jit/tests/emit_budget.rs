...
fn over_budget_whole_program_declines_before_emitting() {
    ...
    let err = temen_wasm_jit::compile_module_reactor(&m, 0, false)
...
fn over_budget_unit_emit_declines() {
    let m = wide_module(20, 48_000);
    let err = temen_wasm_jit::compile_module_b2(&m, false, 10)
...
fn reactor_budget_boundary() {
    ...
    let err =
        temen_wasm_jit::compile_module_reactor_budgeted(&m, 0, false, usize::MAX, 20_000, None)
...
fn over_budget_tierup_degrades_to_fit() {
    ...
    let (wasm, emitted) = temen_wasm_jit::compile_module_tierup_b2_budgeted(&m, false, 10, 30_000)
    ...
    let (_, emitted) = temen_wasm_jit::compile_module_tierup_b2_budgeted(&m, false, 10, usize::MAX)
        .expect("emits");
    assert!(emitted.iter().all(|&e| e), "everything emits unbudgeted");
}
