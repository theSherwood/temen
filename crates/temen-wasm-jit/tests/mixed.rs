...
use temen_wasm_jit::{compile_jit, DriveMode, Shape, ENV_CELL_BYTES};
...
fn oracle(m: &temen_ir::Module, arg: i64) -> i64 {
    ...
    match temen_interp::run(m, 0, &[Value::I64(arg)], &mut fuel) {
...
fn mixed(m: &temen_ir::Module, arg: i64) -> Result<i64, ()> {
            ...
            move |mut caller: Caller<'_, i32>,
                ...
                let args: Vec<Value> = {
                    ...
                    callee
                        ...
                        .enumerate()
                        .map(|(i, t)| match t {
                            temen_ir::ValType::I32 => Value::I32(slot(data, i) as i32),
...
fn oracle_traps(m: &temen_ir::Module, arg: i64) -> bool {
    let mut fuel = u64::MAX;
    temen_interp::run(m, 0, &[Value::I64(arg)], &mut fuel).is_err()
}
