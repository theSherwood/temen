...
/// What one engine produced: values, or a trap kind (the Temen taxonomy).
...
/// unreachable) map from `wasmi::core::TrapCode`; the TEMEN-specific ones (guard fault, fuel)
...
fn oracle(m: &temen_ir::Module, args: &[Value], fuel: u64) -> Outcome {
    let mut fuel = fuel;
    ...
    match temen_interp::run(m, 0, args, &mut fuel) {
...
fn wasm_run(m: &temen_ir::Module, wasm: &[u8], args: &[Value], fuel: u64) -> Outcome {
...
func (i64, i64) -> (i64) {
block 0 (vn: i64, vacc: i64) {
  vz = i64.const 0
  vcmp = i64.eq vn vz
  br_if vcmp 1(vacc) 2(vn, vacc)
}
block 1 (vr: i64) {
  return vr
}
block 2 (vn2: i64, vacc2: i64) {
...
fn bulk_span_masking_sweep() {
    let fill = temen_text::parse_module(MASK_FILL).unwrap();
    temen_verify::verify_module(&fill).unwrap();
    let fill_wasm = compile_module(&fill).unwrap();
    let copy = temen_text::parse_module(MASK_COPY).unwrap();
    temen_verify::verify_module(&copy).unwrap();
...
block 0 (v0: i64) {
  ...
  v2 = i64.const 100
  v3 = i64.const 200
  br_table v1 [1(v2), 2(v3), 1(v3)] 3(v0, v0)
}
block 1 (v4: i64) {
  v5 = i64.const 1
  v6 = i64.add v4 v5
...
fn fuel_parity_exact() {
    for (name, src, arg) in [("alu", ALU, 40i64), ("call", CALL, 25i64)] {
        ...
        let mut fuel = FUEL;
        assert!(
            temen_interp::run(&m, 0, &args, &mut fuel).is_ok(),
...
// (`temen_ir::bounds::in_window` over the tracked upper bound), the emitter drops the bounds-trap
...
fn elision_actually_removes_bytes() {
    let bounded = temen_text::parse_module(ELIDES_BOUNDED).unwrap();
    temen_verify::verify_module(&bounded).unwrap();
    let unbounded = temen_text::parse_module(NO_ELIDE_UNBOUNDED).unwrap();
    temen_verify::verify_module(&unbounded).unwrap();
...
fn elision_boundary_sweep() {
    let mix = temen_text::parse_module(ELIDE_MIX).unwrap();
    temen_verify::verify_module(&mix).unwrap();
    let mix_wasm = compile_module(&mix).unwrap();
    let edge = temen_text::parse_module(NOELIDE_WIDTH8_EDGE).unwrap();
    temen_verify::verify_module(&edge).unwrap();
