//! **`call.import` outlining** ([`temen_wasm_jit::outline_cap_calls`], IMPORTS.md phase 3) — the same
...
//! module's [`temen_interp::BoundImport`] bindings — no import-resolution rewrite anywhere, so the
...
use temen_interp::{bytecode, BoundImport, Host, Value};
