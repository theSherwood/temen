...
//! Temen function — the SQLite VDBE dispatcher in the shipped cards, whose bulk-memory body rejoined
...
fn module_with_padded_callee(n: usize) -> temen_ir::Module {
...
fn oversized_function_stays_interpreter_resident_and_the_module_still_emits() {
    ...
    let (wasm, emitted) = temen_wasm_jit::compile_module_reactor(&big, 0, false).expect(
    ...
    let (_, emitted) = temen_wasm_jit::compile_module_reactor(&small, 0, false).expect("emits");
    assert!(
        emitted[0] && emitted[1],
