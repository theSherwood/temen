//! **Temen IR → WebAssembly emitter** — slice 1 of the browser wasm-JIT tier (`BROWSER.md`
//! § "wasm-JIT tier — design & implementation plan").
//!
//! Compiles a verified [`temen_ir::Module`]'s functions into one WebAssembly module (binary bytes,
...
//! like `temen-jit`**: anything outside the supported subset returns [`Error::Unsupported`] and the
//! caller keeps the function on the bytecode-interpreter tier.
//!
//! ## Emitted shape
//!
//! One exported wasm function `f{i}` per Temen function `i`. Each takes two prepended environment
//! params ahead of the Temen signature:
//!
...
//! `epoch_cell` (temen-run), never this counter.
//!
...
fn fbin_opcode(ty: temen_ir::FloatTy, op: temen_ir::FBinOp) -> u8 {
    use temen_ir::FBinOp::*;
    ...
    match ty {
        temen_ir::FloatTy::F32 => 0x92 + idx,
        temen_ir::FloatTy::F64 => 0xa0 + idx,
    }
}

fn fun_opcode(ty: temen_ir::FloatTy, op: temen_ir::FUnOp) -> u8 {
    use temen_ir::FUnOp::*;
    ...
    match ty {
        temen_ir::FloatTy::F32 => f32op,
        temen_ir::FloatTy::F64 => f64op,
    }
}

fn fcmp_opcode(ty: temen_ir::FloatTy, op: temen_ir::FCmpOp) -> u8 {
    use temen_ir::FCmpOp::*;
...
fn ftoisat_subop(op: temen_ir::FToI) -> u8 {
    ...
    let base = match ity {
        ...
        temen_ir::FloatTy::F32 => 0,
        temen_ir::FloatTy::F64 => 2,
    };
    base + if signed { 0 } else { 1 }
}

/// `i32/i64.trunc_f32/f64_{s,u}` — the trapping float→int opcodes (NaN / out-of-range trap).
fn ftoitrap_opcode(op: temen_ir::FToI) -> u8 {
    let (fty, ity, signed) = op.parts();
    match (ity, fty, signed) {
        (IntTy::I32, temen_ir::FloatTy::F32, true) => 0xa8,
        (IntTy::I32, temen_ir::FloatTy::F32, false) => 0xa9,
        (IntTy::I32, temen_ir::FloatTy::F64, true) => 0xaa,
        (IntTy::I32, temen_ir::FloatTy::F64, false) => 0xab,
        (IntTy::I64, temen_ir::FloatTy::F32, true) => 0xae,
        (IntTy::I64, temen_ir::FloatTy::F32, false) => 0xaf,
        (IntTy::I64, temen_ir::FloatTy::F64, true) => 0xb0,
        (IntTy::I64, temen_ir::FloatTy::F64, false) => 0xb1,
    }
}

/// `f32/f64.convert_i32/i64_{s,u}` — int→float.
fn itof_opcode(op: temen_ir::IToF) -> u8 {
    let (ity, fty, signed) = op.parts();
    match (fty, ity, signed) {
        (temen_ir::FloatTy::F32, IntTy::I32, true) => 0xb2,
        (temen_ir::FloatTy::F32, IntTy::I32, false) => 0xb3,
        (temen_ir::FloatTy::F32, IntTy::I64, true) => 0xb4,
        (temen_ir::FloatTy::F32, IntTy::I64, false) => 0xb5,
        (temen_ir::FloatTy::F64, IntTy::I32, true) => 0xb7,
        (temen_ir::FloatTy::F64, IntTy::I32, false) => 0xb8,
        (temen_ir::FloatTy::F64, IntTy::I64, true) => 0xb9,
        (temen_ir::FloatTy::F64, IntTy::I64, false) => 0xba,
    }
}

/// `demote`/`promote`/`reinterpret` cast opcode.
fn cast_opcode(op: temen_ir::CastOp) -> u8 {
    use temen_ir::CastOp::*;
    match op {
        Demote => 0xb6,
        Promote => 0xbb,
...
fn vbitbin_sub(op: temen_ir::VBitBinOp) -> u32 {
    use temen_ir::VBitBinOp::*;
    match op {
        And => 78,
        Or => 80,
        Xor => 81,
...
fn vconvert_sub(op: temen_ir::VCvtOp) -> u32 {
    use temen_ir::VCvtOp::*;
    match op {
...
/// Public for the tier-up opens (the browser's `temen_coop_open`) to key their paged flip on actual
        ...
        !matches!(
            ...
            !matches!(
                ...
                Inst::Load { .. }
                        ...
                        // so it is excluded here explicitly rather than by accident of the
                        // call-free rule.
                        | Inst::GcRoots { .. }
            )
        })
...
fn compile_module_tierup_inner(
    ...
    // spike did — `jacl_impl/docs/TEMEN_BROWSER_TIERUP_FINDINGS.md`, where an emitted leaf over a
...
/// Assemble the wasm module: emit the functions listed in `emitted` (Temen indices, in the order they
...
fn emit_module(
    ...
    // * **Local (default, `reserved_table_log2 = None`).** Slot `s` = Temen function `s`, power-of-two
    ...
    // trace — e.g. a `temen_warm_jit` decline (issue #865) — then names the culprit function instead of a
    ...
    // names already ship in every `.temen` via the semantic exports table — no `-g` flag, no format change,
...
/// Emit a **cross-tier indirect trampoline** body for Temen function `fi` (its `Func` is `f`): a wasm
...
fn emit_trampoline(f: &Func, fi: u32) -> Result<Vec<u8>, Error> {
    ...
    for (i, p) in f.params.iter().enumerate() {
        ...
        code.push(OP_LOCAL_GET);
        uleb(&mut code, (2 + i) as u64); // the i-th Temen param local
...
fn emit_edge(
    ...
    // target block index is `<= from_block`, the oracle's exact definition (temen-interp `drive`).
...
fn emit_block_body(
    ...
    let mut next_val = b.params.len(); // where the next instruction's results land
                                       ...
                                       // address value's bound via the shared `temen_ir::bounds` proof to decide whether the runtime
    ...
    let get = |code: &mut Vec<u8>, cx: &FnCtx, v: temen_ir::ValIdx| {
        code.push(OP_LOCAL_GET);
            ...
            Inst::Call { func, args } => {
                ...
                match wasm_of[*func as usize] {
                    ...
                    None => {
                        ...
                        // env.call_interp(func_temen_idx, args_ptr = env + ENV_SCRATCH_OFF).
                        code.push(OP_I32_CONST);
                        sleb32(code, *func as i32);
                        code.push(OP_LOCAL_GET);
                        uleb(code, 1); // env
