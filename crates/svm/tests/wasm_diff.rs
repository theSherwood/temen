...
//! emit it with `svm_wasm_jit::compile_module`, run its entry under `wasmi`, and assert it agrees
...
//! hand-written kernels (`svm-wasm-jit/tests/differential.rs`). Stable-toolchain (deterministic
...
fn generator_reaches_the_wasm_memory_oracle() {
    ...
    for seed in 0..iters {
        ...
        if m.memory.is_some()
            ...
            && int_entry
            && ro_free
            && svm_wasm_jit::compile_module(&m).is_ok()
        {
            oracle += 1;
        }
    }
