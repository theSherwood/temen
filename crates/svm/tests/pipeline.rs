...
use svm_interp::{bytecode, run, run_with_host, run_with_host_fast, Host, StreamRole, Trap, Value};
use svm_ir::{BinOp, Inst, IntTy, DEFAULT_RESERVED_LOG2};
...
// storage: a slot's generation advances only at (re)grant, so every generation `1..=current`
// was once live — a dead-but-issued generation IS the tombstone. Differential across all three
// backends; the JIT case additionally exercises the D45 `Clock.now` fast-cap path.
// ---------------------------------------------------------------------------
...
fn rev_module(src: &str) -> svm_ir::Module {
    let m = parse_module(src).expect("parse");
    verify_module(&m).expect("verify");
    m
}
