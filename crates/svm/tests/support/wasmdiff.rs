...
//! `emit_confine`/`emit_span_check` (`svm-wasm-jit`) is the tree's *third* confinement lowering, and
//! until now it had only hand-written kernels (`svm-wasm-jit/tests/differential.rs`) — no generative
...
//! `svm-wasm-jit/tests/differential.rs` + `entry_data.rs`): a generated verifier-valid module is
//! emitted with [`svm_wasm_jit::compile_module`] and run under `wasmi` against the tree-walk oracle
...
/// layout as `svm-wasm-jit/tests/differential.rs`.
...
pub fn run_differential_wasm(m: &Module, args: &[Value]) {
    ...
    let wasm = match svm_wasm_jit::compile_module(m) {
        Ok(w) => w,
        Err(_) => return,
    };
