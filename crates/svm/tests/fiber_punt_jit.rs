...
//! `svm-interp/tests/fiber_punt_diff.rs` (no fixture crate — the `host_park.rs` precedent).
//!
//! Stack switching exists on x86-64 unix, aarch64 unix, and x86-64 Windows today
//! (`svm_fiber::supported()`); elsewhere the JIT bails `Unsupported`, so gated like
//! `jit_fibers.rs`.
#![cfg(any(
...
fn pin_three(m: &svm_ir::Module, mk_host: &dyn Fn() -> (Host, i32), want: i64, label: &str) {
    // Tree-walk oracle.
    ...
    let jo = svm_jit::compile_and_run_with_host(
        m,
        0,
        &[hjh as i64],
        svm_run::cap_thunk,
