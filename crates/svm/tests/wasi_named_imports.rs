...
//! the `svm-wasi` crate, folded here when that demo crate was retired (#919, audit §8).
//!
//! A deliberately tiny WASI preview1 shim (`fd_write` to stdout/stderr, `proc_exit`) as an embedder
//! host capability: svm-wasm transpiles a hand-written WASI module (each WASI import becomes one §7
...
//! here — outside both svm-wasm and the interp TCB — exactly the boundary DESIGN.md §7 draws: the
...
use svm_interp::{cap_id, run_with_host, BoundImport, GuestMem, Host, HostProc, Trap};
...
/// svm-wasm declares them, `"<module>.<name>"`) to the [`cap_id::HOST_PROC`] capability + op. Unknown
...
fn bind(m: &svm_ir::Module, host: &mut Host) -> Option<WasiOut> {
