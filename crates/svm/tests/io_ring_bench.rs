...
use svm_interp::{
    bytecode, run_with_host, Host, Region, Trap, Value, OFFLOAD_POOL_THREADS,
};
...
fn run_ring_once(
    ...
    assert_eq!(got, checksum(n), "ring lane checksum");
    dt
}

/// Run the seq lane once on `host`, returning elapsed.
fn run_seq_once(m: &svm_ir::Module, blocking: i32, host: &mut Host, n: u64) -> Duration {
    let mut fuel = 500_000_000u64;
    let t = Instant::now();
    ...
    assert_eq!(got, checksum(n), "seq lane checksum");
    dt
}
...
fn ring_vs_seq_vs_threaded() {
    ...
    // ---- regime A: pure ABI overhead (block_for = 0) — the batching question -----------------
    {
        ...
        // warm
        for _ in 0..20 {
            run_ring_once(&ring_m, ring_h, rb_h, &mut hr, n);
            run_seq_once(&seq_m, sb_h, &mut hs, n);
        }
        let mut t_ring = Duration::ZERO;
        let mut t_seq = Duration::ZERO;
        for _ in 0..reps {
            t_ring += run_ring_once(&ring_m, ring_h, rb_h, &mut hr, n);
            t_seq += run_seq_once(&seq_m, sb_h, &mut hs, n);
        }
