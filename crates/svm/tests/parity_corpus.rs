...
//!
//! This test makes the [`svm_parity`] manifest the enforced oracle over that hand-maintained set, so
...
//! `svm-parity/tests/conformance.rs`, so the chain is closed: corpus ⊆ manifest-supported ⊆
//! backend-supported.
...
use svm_parity::{catalog, supports, supports_term, Backend, Focus};
