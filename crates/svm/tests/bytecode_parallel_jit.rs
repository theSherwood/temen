...
// The browser's cross-Worker runtime JIT (`svm_par_child` now shares the `ParJitCfg` host) therefore
...
fn run_parallel_runtime_compile() -> Result<Vec<Value>, svm_interp::Trap> {
    let unit = {
        let m = parse_module(SERVICE).expect("parse service");
        verify_module(&m).expect("verify service");
        svm_encode::encode_module(&m)
