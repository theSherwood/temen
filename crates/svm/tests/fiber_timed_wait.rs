...
//!
//! The contract these kernels pin (extending `svm-interp/tests/fiber_parks.rs`, whose
...
/// The notify wake on the fast backends — `fiber_parks.rs`'s futex kernel through the svm-run
...
/// svm-run path: the cell (`mem[0]`) already differs when the fiber waits, so the
...
fn f_the_explorer_finishes_the_poll_loop_on_its_logical_clock() {
    ...
    for seed in 0..20 {
        assert_eq!(
            svm_interp::run_scheduled(&m, 0, &[], 50_000_000, seed),
            Ok(vec![svm_interp::Value::I64(2_042)]),
            "explorer seed {seed}"
        );
    }
}
