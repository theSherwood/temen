...
//!
//! The tier-up arm of the same contract is `svm-wasm-jit/tests/tierup_grow_window.rs`; this test
...
fn emit_unit(unit: &svm_ir::Module) -> Vec<u8> {
    ...
    let a = svm_wasm_jit::compile_jit(&e, svm_wasm_jit::Shape::Batch { entry: 0 }, false)
        .expect("unit emits");
...
fn run_unit_over_live_window(
    ...
    if let Some(mapped) = sync {
        // The #717 host-sync contract (worker.js: `unit.mapped.value = ex.svm_par_ev_b(v)`).
        instance
    ...
    let out = match f.call(&mut store, &params, &mut results) {
        ...
        Err(_) => Err(match *store.data() {
            svm_wasm_jit::TRAP_OUT_OF_FUEL => Trap::OutOfFuel,
            _ => Trap::MemoryFault,
        }),
    };
    // SAFETY: see fn doc.
