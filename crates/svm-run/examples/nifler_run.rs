//! Run `nifler`-the-guest (`nifler.svmb`, a real nimony phase on-ramped to SVM) over an input Nim
//! file, and print the `.nif` it emits — the guest half of the nifler-on-SVM differential (NIM.md
...
//!
//! ```text
//! cargo run -q --release -p svm-run --example nifler_run -- <nifler.svmb> <input.nim> <out.nif>
//! ```
//!
//! The memfs maps an absolute guest path to its cap-relative key (fs.rs `norm`), so `/x.nim` resolves
//! against the seed key `x.nim`. `SVM_NIFLER_BACKEND` selects the engine (treewalk default).
...
fn main() {
    ...
    let svmb = a
        .next()
        .expect("usage: nifler_run <nifler.svmb> <input.nim> <out.nif>");
    ...
    let (factory, handle) = svm_run::fs::mem_fs_shared_factory(vec![(in_key.clone(), src)], vec![]);
    ...
    let backend = match std::env::var("SVM_NIFLER_BACKEND").as_deref() {
        Ok("bytecode") => Backend::Bytecode,
        Ok("jit") => Backend::Jit,
        Ok("treewalk") | Err(_) => Backend::TreeWalk,
        Ok(other) => panic!("SVM_NIFLER_BACKEND: unknown engine {other:?}"),
    };
