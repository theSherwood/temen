//! Run a nimony compiler phase (`nifler`/`hexer`/`nimony`/…) on the SVM over an in-window `fs` memfs,
//! and dump every file the phase *produced* to an output dir — the generic guest half of the
//! nimony-on-SVM differentials (NIM.md §3c/§3e, "compile Nim in the browser"). The phase's whole-
//! program `.svmb` runs `main(argc, argv)` with the given argv; the memfs is seeded from a fixture
...
//!
//! ```text
//! cargo run -q --release -p svm-run --example nimphase_run -- \
//!     <phase.svmb> <fixture-dir> <out-dir> -- <argv0> <argv1> ...
//! ```
//!
//! `SVM_PHASE_BACKEND` selects the engine (treewalk default · bytecode · jit). The guest's cwd is `/`,
...
fn main() {
    ...
    let svmb = a
        .next()
        .expect("usage: nimphase_run <phase.svmb> <fixture-dir> <out-dir> -- <argv...>");
    ...
    let (factory, handle) = svm_run::fs::mem_fs_shared_factory(seed, vec![]);
    ...
    let backend = match std::env::var("SVM_PHASE_BACKEND").as_deref() {
        Ok("bytecode") => Backend::Bytecode,
        Ok("jit") => Backend::Jit,
        Ok("treewalk") | Err(_) => Backend::TreeWalk,
        Ok(other) => panic!("SVM_PHASE_BACKEND: unknown engine {other:?}"),
    };
