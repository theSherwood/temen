...
//! `main` on the tree-walker (the oracle engine, CLAUDE.md), and forwards the guest's stdout/stderr
//! + exit code. The driving script (`run_selfhost_diff.sh`) compares this stdout byte-for-byte
//! against the native reference built from the *same* frontend sources.
//!
...
fn main() {
    ...
    let svmb = a.next().expect("usage: chibicc_run <svmb> <input.c> <argv-path> [include-dir]");
    ...
    let cfg = RunConfig {
        limits: Limits { fuel: None, deadline: None, max_fibers: 0, max_vcpus: 0 },
        stdin: vec![],
    ...
    let run = inst
        .run_with_caps(Backend::TreeWalk, &cfg, &[("fs", fs::mem_fs_seeded(files, dirs))])
