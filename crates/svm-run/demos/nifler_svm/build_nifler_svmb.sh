#!/usr/bin/env bash
# nifler-on-SVM (NIM.md §3c/§3e, "nimony in the browser" slice 1) — compile the FIRST real nimony
# phase, `nifler` (Nim source → NIF), all the way to a runnable SVM module and prove it parses a Nim
...
#   whole-program bitcode --(svm-llvm-translate --stub-externs)--> SVM --(prep_svmb: verify)--> .svmb
...
#
# The bottom edge (nifler_shim.c) reuses the shims that already run Postgres on SVM (../postgres/
# os_shim.c + stdio_shim.c) over the `fs` cap; see that file. The ~17 MB `.svmb` is a build artifact,
# NOT committed (too big for an asset lane) — this script IS the gate, run when the toolchain is present.
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(cd "$HERE/../../../.." && pwd)"
CACHE="${SVM_NIFLER_CACHE:-/tmp/svm_nifler_build}"
...
"$CLANG" -O2 -fno-vectorize -fno-slp-vectorize -DSVM_GUEST -emit-llvm -c -I"$NIMLIB" "$HERE/nifler_shim.c" -o "$CACHE/nifler_shim.bc"
...
echo "=== [4/5] translate (--stub-externs) + prep_svmb (verify) ==="
...
"$TR" "$CACHE/nifler.linked.bc" -o "$CACHE/nifler_raw.svmb" --binary --host-page 65536 --stub-externs
cargo run -q --release -p svm-run --example prep_svmb -- "$CACHE/nifler_raw.svmb" "$CACHE/nifler.svmb" | grep -E "funcs|wrote"

echo "=== [5/5] run nifler-on-SVM over each input, diff vs native (treewalk, bytecode, jit) ==="
fail=0
for src in "$HERE"/inputs/*.nim; do
  ...
  for eng in treewalk bytecode jit; do
    set +e
    SVM_NIFLER_BACKEND="$eng" cargo run -q --release -p svm-run --example nifler_run -- \
      "$CACHE/nifler.svmb" "$src" "/out.nif" > "$CACHE/svm_${name}_$eng.nif" 2>"$CACHE/svm_${name}_$eng.err"; rc=$?
    set -e
    if [ "$rc" = 0 ] && diff -q "$CACHE/nat/out.nif" "$CACHE/svm_${name}_$eng.nif" >/dev/null; then
      echo "  $name [$eng]: OK (byte-identical, $(stat -c%s "$CACHE/svm_${name}_$eng.nif") bytes)"
    else
      echo "  $name [$eng]: MISMATCH (exit=$rc)"; head -c 200 "$CACHE/svm_${name}_$eng.err" | sed 's/^/    /'; fail=1
    fi
  done
done
