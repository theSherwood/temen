#!/usr/bin/env bash
# Self-host differential (SELFHOST_C.md §7 step 5): for each test .c, assert the SVM IR emitted by
# chibicc-the-GUEST (chibicc.svmb, run on the SVM tree-walker via the chibicc_run example) is
# byte-identical to the NATIVE reference (chibicc_ref, built from the *same* cc1 TUs + cc1_main.c).
# Same frontend code both sides → the only variables are the substrate (guest libc + SVM interpreter
# vs system libc + native CPU). A mismatch is a guest-libc or on-ramp bug.
#
# Prereqs: build_chibicc_svmb.sh has produced $CACHE/chibicc.svmb; clang ≥18 on PATH.
...
SVMB="$CACHE/chibicc.svmb"
[ -f "$SVMB" ] || { echo "missing $SVMB — run build_chibicc_svmb.sh first"; exit 1; }
...
cargo build -q -p svm-run --example chibicc_run
