#!/usr/bin/env bash
# stage_bin.sh — compile the #801 coreutils (demos/posix_utils, the chibicc world) to `.svm` text
...
ROOT="$HERE/../../../.."
OUT="${1:-/tmp/svm_bash_cache/bin}"
mkdir -p "$OUT"
make -s -C "$ROOT/frontend/chibicc"
CC="$ROOT/frontend/chibicc/chibicc"
UT="$HERE/../posix_utils"
PL="$HERE/../posix_libc"
for t in true false echo cat seq head wc sort uniq ls pwd grep; do
  tu="$OUT/_$t.c"
  if [ "$t" = grep ]; then
  ...
  "$CC" -cc1 --emit-ir --child-entry -cc1-input "$tu" -cc1-output "$OUT/$t.svm" "$tu"
done
echo "staged: $OUT"
