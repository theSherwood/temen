...
| `exec.c` | `execve`/`execv`/`execvp` — resolve via op 53, pack argv/envp into the powerbox args region, image-replace via the core `CAP_SELF_EXEC` self-op; `execvp` walks `PATH` (EACCES remembered, ENOENT continues); returns only on failure (`-ENOENT`/`-EACCES`/`-E2BIG`/`-EINVAL`) | exec_resolve/getenv ops (53/11) + `__vm_exec_module` |
