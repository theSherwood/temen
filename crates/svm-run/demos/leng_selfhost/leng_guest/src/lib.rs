//! The `svm-leng.svmb` guest (W5 capstone, NIM.md §3e): the real `svm-leng` translator, wrapped as a
//! **powerbox program**. It reads a Leng-NIF module from stdin, translates it with
//! `svm_leng::translate_to_text`, and writes the emitted SVM text to stdout — the leng analog of
//! `chibicc.svmb` (which reads C and emits SVM IR). Built once through the LLVM on-ramp into the
//! committed `svm-leng.svmb`; a translation bug is a clean error (svm-verify re-checks it), never an
//! escape (DESIGN.md §2a).
//!
...
/// construct), `0` = the SVM text was emitted to stdout.
#[no_mangle]
pub extern "C" fn main() -> i32 {
    ...
    match svm_leng::translate_to_text(src) {
        Ok(text) => {
