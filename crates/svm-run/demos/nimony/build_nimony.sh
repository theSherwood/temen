...
# genuine Leng->C output to SVM, running it there against nimony's native run. This is the
...
REPO="$(cd "$HERE/../../../.." && pwd)"
CACHE="${SVM_NIMONY_CACHE:-/tmp/svm_nimony_demo}"
...
echo "  undefined: $(tr '\n' ' ' < "$CACHE/undef.txt")"
ONRAMP='^(write|read|exit|_exit|__vm_[a-z_0-9]+|__svm_[a-z_0-9]+)$'
if UNEXPECTED=$(grep -Ev "$ONRAMP" "$CACHE/undef.txt"); then
  echo "  UNEXPECTED undefined (shim gap): $(echo $UNEXPECTED)"; exit 1
fi

echo "=== [5/6] translate (no --stub-externs) + prep_svmb gate ==="
...
"$TR" "$CACHE/nimony.linked.bc" -o "$CACHE/nimony_raw.svmb" --binary --host-page 65536
cargo run -q --release -p svm-run --example prep_svmb -- "$CACHE/nimony_raw.svmb" "$CACHE/nimony.svmb" | grep funcs

echo "=== [6/6] run genuine nimony output on SVM (treewalk, bytecode, jit) and diff vs native ==="
fail=0
for eng in treewalk bytecode jit; do
  set +e
  SVM_PROBE_BACKEND="$eng" cargo run -q --release -p svm-run --example nimony_probe -- \
    "$CACHE/nimony.svmb" > "$CACHE/svm_$eng.out" 2>"$CACHE/svm_$eng.err"; SVM_EXIT=$?
  set -e
  if diff -q "$CACHE/native.out" "$CACHE/svm_$eng.out" >/dev/null && [ "$SVM_EXIT" = "$NATIVE_EXIT" ]; then
    echo "  $eng: OK (stdout byte-identical to native nimony, exit=$SVM_EXIT)"
  else
...
[ "$fail" = 0 ] && echo "ALL ENGINES MATCH NATIVE NIMONY (genuine Leng->C output runs on SVM)" || { echo "FAILED"; exit 1; }
