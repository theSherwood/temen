#!/usr/bin/env bash
# The nimony FRONT-END on SVM (NIM.md §3c/§3e, "compile Nim in the browser" slice 2b): prove the real
# `nimsem` (sema) phase runs on the SVM as a multi-binary DRIVER — it spawns the real `nifler` (parse)
...
# (nifler_svm/nifler_shim.c `system()`) to the `exec` capability; `nifler.svmb` runs as an isolated
...
#          llvm-18, cargo. Fail-soft SKIP if absent (NIM.md §2). The .svmb are build artifacts.
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(cd "$HERE/../../../.." && pwd)"
CACHE="${SVM_FRONTEND_CACHE:-/tmp/svm_frontend_build}"
...
NM=llvm-nm; TR="$REPO/crates/svm-llvm/target/release/svm-llvm-translate"
...
echo "=== [1/4] build nifler.svmb + nimsem.svmb (-d:skipPostSemValidator) ==="
build_svmb "$REPO/nimony/src/nifler/nifler.nim" "$CACHE/nifler.svmb"
build_svmb "$REPO/nimony/src/nimony/nimsem.nim" "$CACHE/nimsem.svmb" -d:skipPostSemValidator
echo "  nifler.svmb $(stat -c%s "$CACHE/nifler.svmb") B; nimsem.svmb $(stat -c%s "$CACHE/nimsem.svmb") B"
...
echo "=== [4/4] nimsem-on-SVM drives nifler via exec; diff (path-normalized) vs native ==="
rm -f /tmp/svm_sys.s.nif
cargo run -q --release -p svm-run --example nim_frontend_driver -- \
  "$CACHE/nimsem.svmb" "$CACHE/nifler.svmb" "$BIN/../lib" "$W/nimcache/$sys.p.nif" "$sys"
...
if diff -q <(norm "$CACHE/native.s.nif") <(norm /tmp/svm_sys.s.nif) >/dev/null; then
  echo "ALL MATCH NATIVE — nimsem-on-SVM sema is byte-identical to native (modulo embedded paths); the real nimony FRONT-END runs on the SVM"
else
  echo "FAILED: residual semantic differences after path normalization:"; diff <(norm "$CACHE/native.s.nif") <(norm /tmp/svm_sys.s.nif) | head -20; exit 1
fi
