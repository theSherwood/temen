# hexer-on-SVM — nimony's middle-end lowering Nim to Leng on the sandbox
...
lowering — to **Leng** (`.x.nif`), the IR that `svm-leng` (already in the browser) turns into SVM-IR.
This demo compiles the **real** `hexer` (~18k loc of Nim, from the vendored `nimony` submodule) to a
runnable SVM module and proves it lowers a real semchecked module **byte-for-byte identically to
...
1. **Bottom edge — one new symbol.** hexer reads NIF files through `std/memfiles` (`nifreader.nim`'s
   ...
   were added to the shared shim (`../nifler_svm/nifler_shim.c`) as a malloc + read-the-region over
...
2. **Multi-file fixture.** hexer's input is a *semchecked* `.s.nif`, so the build first runs the real
   ...
   the `system` module's) to both native hexer and hexer-on-SVM and diffs **every file each
...
NIMONY_BIN=<repo>/.nimtool/nimony/bin/nimony  HEXER_BIN=<repo>/.nimtool/nimony/bin/hexer \
  bash build_hexer_svmb.sh
```

Fail-soft **SKIP** without the toolchain (nimony bootstrap needs Nim 2.3.x devel — **not** in the
per-PR CI, NIM.md §2). The ~3 MB `.svmb` is a build artifact, not committed; this script is the gate.

## Status

✅ **hexer-on-SVM done.** Lowers `inputs/*.nim`'s semchecked NIF to Leng on the SVM, every produced
...
it compiles to C, on-ramps clean (zero shim delta), and translates + verifies to an SVM module, but
...
(as `mmap`/`readlink` were) — it's a genuine svm-llvm on-ramp codegen gap reached before any I/O,
i.e. a lowering issue to pin in the translator, not the bottom edge. Then the W4 driver chains the
real phases (slice 3) and feeds the Leng into `svm-leng.svmb` → the browser card (slice 4).
