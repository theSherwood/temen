//! **The committed `nifler.svmb.gz` playground asset, run in-sandbox** (NIM.md §3c/§3e — the "compile
//! Nim in the browser" front-end card, slice 4). `nifler` — the first real nimony compiler phase (Nim
//! source → parsed NIF) — is a Nim program on-ramped to a verified SVM module through the LLVM/C
...
//! `crates/svm-run/demos/nifler_svm/build_nifler_svmb.sh` (needs the nimony toolchain).
//!
...
//! caused the drift — regenerate the asset + fixtures with `build_nifler_svmb.sh` and commit them (see
//! `crates/svm-run/demos/nifler_svm/README.md`).
...
const ASSET_GZ: &[u8] = include_bytes!("../../../browser/web/assets/nifler.svmb.gz");

/// The Nim inputs the asset is exercised over, paired with their committed native-`nifler` `.p.nif`.
const CORPUS: &[(&str, &str)] = &[
    (
        include_str!("../demos/nifler_svm/inputs/basic.nim"),
        include_str!("../demos/nifler_svm/expected/basic.p.nif"),
    ),
    (
        include_str!("../demos/nifler_svm/inputs/control.nim"),
        include_str!("../demos/nifler_svm/expected/control.p.nif"),
    ),
];

/// Inflate the committed gzip asset to the raw `.svmb` bytes via `gzip -dc` (Linux-only, like the
...
/// main thread drains stdout — the inflated `.svmb` (~17.7 MB) overflows the OS pipe buffer, so a
...
fn committed_asset_decodes_and_verifies() {
    let Some(svmb) = inflate_asset() else {
        eprintln!("SKIP: gzip unavailable to inflate nifler.svmb.gz");
        return;
    };
    ...
    let module = svm_encode::decode_module(&svmb).expect("decode nifler.svmb");
    svm_verify::verify_module(&module).expect("verify nifler.svmb (the trusted floor)");
...
fn asset_parses_nim_byte_identical_to_native_nifler() {
    ...
    let module = svm_encode::decode_module(&svmb).expect("decode nifler.svmb");
    svm_verify::verify_module(&module).expect("verify nifler.svmb");
    ...
    for (src, expected) in CORPUS {
        ...
        let (factory, handle) = svm_run::fs::mem_fs_shared_factory(
        ...
        let run = svm_run::instantiate(module.clone())
            .expect("instantiate nifler.svmb")
            .run_with_caps(Backend::TreeWalk, &cfg, &[("fs", fs_cap)])
            .expect("run nifler.svmb");
        assert!(
        ...
        assert_eq!(
            emitted,
            expected.as_bytes(),
            "nifler-on-SVM must parse byte-identically to native nifler (the committed fixture)"
        );
    }
}
