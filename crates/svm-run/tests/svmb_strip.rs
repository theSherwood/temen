//! Tests for [`svm_run::demote_exports`] — the `svmb-strip` packaging step (#907): the
...
//! ships), and only what [`Module::resolve_export`](svm_ir::Module::resolve_export) can reach
//! changes. `_start` is always kept — it is the powerbox-entry *marker*
//! ([`svm_run::is_named_powerbox_entry`]), semantics, not observability.

use svm_ir::{FuncName, Module};
use svm_run::{demote_exports, is_named_powerbox_entry, run_powerbox};
...
fn demoted_name_survives_encode_roundtrip() {
    ...
    let rt = svm_encode::decode_module(&svm_encode::encode_module(&m)).expect("roundtrip");
...
fn keep_list_and_source_name_precedence() {
    ...
    let mut m = two_func_module();
    m.debug_info = Some(svm_ir::DebugInfo {
