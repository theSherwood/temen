...
//! `crates/temen/tests/spec_*.rs` are generated.
//!
//! **Independence rule (SPEC.md):** the `eval` closures here are written from the
//! `DESIGN.md` §3b prose, never lifted from `temen-interp` — this crate is a *second,
...
pub struct OpRow {
    /// Text mnemonic, matching `temen-text` exactly (e.g. `"i32.div_s"`).
    pub id: String,
    ...
    pub class: Class,
    pub shape: Shape,
    /// The op's wire encoding, restated independently of `temen-encode` (suite 3).
    pub encoding: Enc,
...
//
// A second, independent statement of `temen-encode`'s `mod op` opcode map, written as
...
//
// Trap-confinement semantics per DESIGN.md §4 / the `temen-mask`
