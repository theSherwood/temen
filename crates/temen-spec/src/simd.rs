...
//! `v128` value op, written from the `temen-ir` op documentation (wasm-parity lane
//! rules), never from a backend.
//!
//! Shape notes that drive the harness (`crates/temen/tests/spec_simd.rs`):
    ...
    rows.push(SimdRow {
        ...
        eval: Box::new(|x| {
            ...
            let out: Vec<u64> = (0..8)
                .map(|i| {
                    let p = (xa[2 * i] * xb[2 * i]).wrapping_add(xa[2 * i + 1] * xb[2 * i + 1]);
                    wrap_lane(VShape::I16x8, p) as u64 // wraps at i16 (temen-ir doc)
                })
                .collect();
