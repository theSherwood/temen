...
to the guard-paged backend with `temen-fiber/guard-page-stacks`. See `STACK_GUARD_FLIP.md` for the
...
2. **[done — this increment]** Functional escape-grade check: `emit_stack_check` now does the
   ...
   `temen_set_current_fiber`) and the root stays `limit = 0` (inert). Test `tests/stack_check.rs`: an
