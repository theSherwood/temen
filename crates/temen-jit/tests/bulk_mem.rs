...
//!
//! The three engines — tree-walker (`temen_interp::run`), bytecode interp
//! (`temen_interp::bytecode::compile_and_run`), and the JIT (`temen_jit::compile_and_run`, which lowers
...
fn run_all(src: &str) -> Result<i64, ()> {
    ...
    let tw = temen_interp::run(&m, 0, &[], &mut fuel);
    let mut fuel = u64::MAX;
    ...
    let norm = |r: Result<i64, temen_interp::Trap>| r.map_err(|_| ());
