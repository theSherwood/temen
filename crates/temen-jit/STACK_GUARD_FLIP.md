...
promote `temen-fiber/arena-stacks` + `temen-jit/stack-check` from off-by-default prototypes to the default
...
```yaml
      - run: cargo test -p temen-jit --features stack-check --test stack_guard_fuzz
```
...
```yaml
      ...
      - run: cargo test -p temen-jit --features stack-check,guard-page-stacks --test stack_guard_fuzz
```

The arena backend stays covered by `cargo test -p temen-jit --features stack-check` (same
...
guard-page line on `main`; (2) flip `temen-fiber/arena-stacks` + `temen-jit/stack-check` on by default,
