...
/// Generation bits a fiber **guest handle** carries (recycling step 1): the handle is laid out
/// `[tag:8][generation:32][slot:24]` (DESIGN.md §3c "Uniform pointer tagging"). A slot fits in the low
/// 24 bits (`MAX_FIBERS = 1<<24`); the **top byte is reserved for a guest pointer-tag**, so an `i64`
/// handle conveys the low **32** bits of the slot's full word generation. [`Ownership::claim_gen`]
/// validates a handle against `gen_of(word) & FIBER_HANDLE_GEN_MASK`, so the cross-backend handle
/// namespace (shared with `temen_interp`, whose `FIBER_GEN_SHIFT` is likewise 24) matches. 32 bits puts
/// wraparound at ~4.3 billion recycles of one slot — beyond any real churn, so the ABA guard never
/// silently re-matches in practice. (Index widened 16→24 once the arena removed the
/// `vm.max_map_count` VMA wall; the generation field narrowed 48→40→32, the last step to free the top
/// byte for tagging across all pointer kinds — INVARIANTS.md §11.)
pub(crate) const FIBER_HANDLE_GEN_MASK: u64 = (1 << 32) - 1;
...
// `RUSTFLAGS="--cfg loom" cargo test -p temen-jit --lib fiber_registry::loom`.
#[cfg(all(test, loom))]
mod loom_tests {
    use super::*;
