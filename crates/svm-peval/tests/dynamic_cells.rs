...
//! memory: `savedpc@0` (the renamed pc), `reg0@8`, `out@16`, and the loop bound `N@24`. Seeding the
//! whole `[0,32)` region folds `savedpc` (so the dispatch collapses) — but marking `N@24` a
...
//!
//! Run: `cargo test -p svm-peval --test dynamic_cells -- --nocapture`

use svm_interp::{Trap, Value};
use svm_ir::{Module, Terminator};
use svm_peval::{specialize_with_config, SpecConfig};
...
fn run_oracle(m: &Module, n: i64) -> Result<Vec<Value>, Trap> {
    ...
    svm_interp::run(m, 1, &[Value::I64(n)], &mut fuel)
}
...
fn a_dynamic_cell_rolls_a_captured_loop() {
    ...
    // Point the specializer straight at `interp` (func 0), no wrapper. Seed the whole [0,32) VM-state
    // region from a zero overlay (savedpc=0, reg0=0, out=0) so `savedpc` folds and the dispatch
    // collapses — but mark `N@24` a runtime-input cell so it enters as a dynamic residual parameter.
    let cfg = SpecConfig {
        rename: Some((0, 32)),
    ...
    for n in [0i64, 1, 2, 4, 5, 6, 20, 100, 1000, 100_000] {
        ...
        assert_eq!(jit(&r, 0, &[n]), want, "svm-jit residual @ {n}");
    }
}
