...
//!
//! Run: `cargo test -p svm-peval --test cut_read_state -- --nocapture`
...
fn run_oracle(m: &Module, n: i64) -> Result<Vec<Value>, Trap> {
    ...
    svm_interp::run(m, 2, &[Value::I64(n)], &mut fuel)
}
...
fn n_calls(f: &svm_ir::Func) -> usize {
    f.blocks
        .iter()
        .flat_map(|b| &b.insts)
        .filter(|i| matches!(i, svm_ir::Inst::Call { .. }))
        .count()
}
...
fn read_state_cut_keeps_the_dispatch_folded_and_rolls() {
    ...
    for n in [0i64, 1, 2, 5, 6, 20, 100, 1000] {
        ...
        assert_eq!(jit(&r, 0, &[n]), want, "svm-jit @ {n}");
    }
}
