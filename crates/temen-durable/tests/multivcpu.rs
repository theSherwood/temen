...
//! (`temen-durable` is unchanged; `thread.spawn` is not a checkpoint, so the reload lives in the runtime).
//!
...
// runs before each level's `call.cap` unwind point, so every level spawns its child *before* it
