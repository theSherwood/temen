...
fn fiber_module_is_inert_under_instrumentation_in_normal() {
    ...
    temen_verify::verify_module(&inst).expect("instrumented fiber'd IR verifies");
...
/// [`temen_interp::FrozenFiber`] residue). Thawing re-seeds the fiber, re-enters the root under
...
/// byte-level snapshot Section-2 codec is the follow-up `temen-snapshot` slice.)
#[test]
