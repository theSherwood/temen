...
//! a frame whose top would cross `ShadowArena::LEGACY.end` into the guest's region. Reaching a real
...
use temen_durable::{
    init_durable_window, transform_module, write_state, ShadowArena, STATE_UNWINDING,
};
...
fn freeze_with_sp(inst: &Module, sp: u64) -> Result<Vec<Value>, Trap> {
    ...
    win[ShadowArena::LEGACY.region_base(0) as usize
...
fn shadow_overflow_traps_instead_of_corrupting() {
    ...
    // `ShadowArena::LEGACY.end` into guest memory → the check traps instead.
    assert!(
        freeze_with_sp(&inst, ShadowArena::LEGACY.end - 8).is_err(),
        "a push past the reserve traps, never writes guest memory"
    );

    // From the base, the same freeze fits and returns its placeholder.
    assert!(
        freeze_with_sp(&inst, ShadowArena::LEGACY.region_base(0)).is_ok(),
