...
//!
//! Run: cargo run --release --example durable_overhead -p temen-durable
...
fn main() {
    ...
    for &(name, src) in KERNELS {
        let orig = parse(src);
        temen_verify::verify_module(&orig).expect("verify original");
        let dur = transform_module(&orig).expect("transform");
        temen_verify::verify_module(&dur).expect("verify durable");
