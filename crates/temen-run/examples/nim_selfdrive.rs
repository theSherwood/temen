//! **Compile Nim on the Temen with NO native `nimony` bootstrap** (NIM.md §3c/§3e; #958 — the in-browser
...
//! dependency graph by parsing each module with `nifler.temen` and reading its `.p.deps.nif`, then runs
//! `nimsem.temen` (dependency-ordered) and `hexer.temen` over the closure and links + runs the result —
//! every phase a sandboxed Temen guest, nothing native in the loop but the guest `.temen` build.
//!
//! ```text
//! cargo run -q --release -p temen-run --example nim_selfdrive -- \
//!     <nifler.temen> <nimsem.temen> <hexer.temen> <libdir> <proj-dir> <main.nim> <export> <expected> [arg...]
//! ```
//!
...
fn main() {
    ...
    // ---- phase 1: parse + crawl the import closure with nifler.temen (plays nifmake) -----------------
    ...
    while let Some((file, role)) = work.pop() {
        ...
        eprintln!(
            "phase 1  nifler.temen: {} → stem {stem} ({} import(s))",
            file,
            dep_stems.len()
        );
        mods.insert(
    ...
    // ---- phase 2: nimsem.temen (sema), dependency-ordered, driving nifler.temen via exec -------------
    for stem in &order {
        let m = &mods[stem];
        ...
        eprintln!("phase 2  nimsem.temen: {stem} → {} B .s.nif", s.len());
    }

    // ---- phase 3: hexer.temen (lower) — main gets the app-entry glue --------------------------------
    ...
    let run_hexer = |stem: &str| -> Vec<u8> {
        ...
        eprintln!("phase 3  hexer.temen: {stem} → {} B Leng .x.nif", x.len());
        x
    };
    ...
    if io_mode {
        ...
        println!(
            "✅ COMPILED NIM ON Temen, NO NATIVE BOOTSTRAP (I/O): {} modules → stdout = {got:?}",
            order.len()
        );
        return;
    }
    ...
    println!(
        "✅ COMPILED NIM ON Temen, NO NATIVE BOOTSTRAP: {} modules → {export}({call_args:?}) = {iword} \
         (interp == JIT == {expected})",
        order.len()
    );
}
