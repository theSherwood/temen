...
//! `.temen` by `demos/nimony/build_probe.sh`) on the Temen and forward its stdout + exit code, so the
...
//!
//! ```text
//! TEMEN_PROBE_BACKEND={treewalk|bytecode|jit} \
//!   cargo run -q --release -p temen-run --example nimony_probe -- <probe.temen>
//! ```
...
fn main() {
    let temen = std::env::args()
        .nth(1)
        .expect("usage: nimony_probe <probe.temen>");

    let backend = match std::env::var("TEMEN_PROBE_BACKEND").as_deref() {
        Ok("bytecode") => Backend::Bytecode,
        Ok("jit") => Backend::Jit,
        Ok("treewalk") | Err(_) => Backend::TreeWalk,
        Ok(other) => panic!("TEMEN_PROBE_BACKEND: unknown engine {other:?}"),
    };
