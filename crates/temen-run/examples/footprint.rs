...
//! benchmarks miss, from the perspective of the **actual runtime**: this binary links only the Temen
//! runtime crates (`temen-ir`/`encode`/`interp`/`jit`) — **no LLVM anywhere**. The LLVM frontend is an
//! *AOT* tool (`temen-llvm-translate`) that produces Temen IR (`.temt`/`.temen`); that IR is what travels
...
//!
//! For a module file (`.temen` binary or `.temt` text) it reports:
//!   * **IR** bytes (`temen_encode::encode_module`) + instruction count — the shippable program;
...
/// Load an Temen IR module from a `.temen` (binary) or `.temt` (text) file — pure runtime, no libLLVM.
fn load(path: &str) -> Module {
    let bytes = std::fs::read(path).unwrap_or_else(|e| panic!("read {path}: {e}"));
    if path.ends_with(".temen") {
        temen_encode::decode_module(&bytes).expect("decode .temen")
    } else {
        temen_text::parse_module(&String::from_utf8(bytes).expect("utf8 .temt"))
            .expect("parse .temt")
    }
}

/// Compile the whole module on the JIT (every function is lowered regardless of entry) and hold it.
fn jit_compile(m: &Module) -> (temen_jit::CompiledModule, usize) {
    let cm = temen_jit::CompiledModule::compile(
        m,
        0,
        temen_jit::INERT_CAP_THUNK,
...
fn main() {
    ...
    let path = args.get(1).map(String::as_str).unwrap_or_else(|| {
        eprintln!(
            "usage: footprint <module.temen|module.temt>\n  (produce the input AOT with \
             temen-llvm-translate; this probe links no libLLVM — see the file header)"
        );
        std::process::exit(2);
    });
    ...
    let m = load(path);
    let ir_bytes = temen_encode::encode_module(&m).len();
    ...
    println!("module: {file} — {n_funcs} functions, {ir_insts} IR instructions (Temen IR off disk; no libLLVM)\n");
    println!(
        "{:<22} {:>12} {:>16}",
    ...
    println!(
        "\n(This binary links no libLLVM — the LLVM frontend is AOT-only (temen-llvm-translate → Temen IR);\n \
