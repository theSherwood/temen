...
//!
//!   cargo run --release -p temen-run --example build_nim_stdlib_image -- <nimony-lib-dir> <out.img>
//!
//! Ship it gzipped alongside the phase `.temen.gz` (see `browser/web/assets/`); the card fetches +
//! inflates it and hands the bytes to `temen_compile_nim_fs`.
...
fn main() {
    ...
    let img = temen_run::fs::encode_image(&files, &[]);
    ...
    let _ = temen_run::fs::decode_image(&img).expect("image round-trips");
}
