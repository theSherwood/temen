//! **The full nimony compiler, running on the Temen: Nim source → a module that RUNS** (NIM.md §3c/§3e,
...
//! as **real Temen guests** over a shared in-window `fs`, producing the program's + `system` module's
//! Leng (`.x.nif`); `temen_leng::link_whole_with_runtime` then links that to one verified, import-free
...
//!     <nifler.temen> <nimsem.temen> <hexer.temen> <libdir> <nimcache-dir> <main-stem> \
//!     <export> <expected> [arg...]
//! ```
//!
...
//! ordered. It re-runs **sema and lowering on the Temen** for every module (the parse is checked too —
//! nifler.temen re-parses the main and its bytes must match), then links and runs in one of two modes:
//!
...
//!   (`temen_leng::link_nim_powerbox`), run `_start` under the powerbox (host grants stdout), and assert
//!   the captured **stdout** (`\n`/`\t` decoded) — a Nim program compiled by the Temen printing for real.
...
/// the in-set edges. nimony's plan is already a DAG; this pins a valid processing order for the Temen
...
fn main() {
    ...
    let nifler_p = next("nifler.temen");
    let nimsem_p = next("nimsem.temen");
    let hexer_p = next("hexer.temen");
    ...
    let flat: Vec<(String, Vec<u8>)> = files
        ...
        .collect();
    files.extend(flat);
    // Seed the parsed nif of every module the plan mentions (bootstrapped by native nifler; nifler.temen
    // re-parses the main below to prove the parse runs on the Temen too).
    for step in &steps {
    ...
    // nifler.temen registered under the names nimsem uses for argv[0] (deps.nim shells out to `nifler`).
    ...
    // ---- phase 1: nifler.temen re-parses prog.nim on the Temen; its bytes must match the bootstrap ----
    {
        let run = instantiate(temen_encode::decode_module(&nifler).expect("decode nifler"))
            .expect("instantiate nifler")
            .run_with_caps(
                Backend::TreeWalk,
                &cfg(
        ...
        assert_eq!(
            reparsed, prog_pnif,
            "nifler.temen re-parse must match the bootstrap .p.nif"
        );
        eprintln!(
            "phase 1  nifler.temen: parsed prog.nim → {} B .p.nif (== bootstrap)",
            reparsed.len()
        );
    }

    // ---- phase 2: nimsem.temen (sema), driving nifler.temen via exec — each module in dependency order.
    ...
    for &i in &order {
        ...
        let run = instantiate(temen_encode::decode_module(&nimsem).expect("decode nimsem"))
    ...
    // ---- phase 3: hexer.temen (lower) — each semchecked module → Leng `.x.nif` ----------------------
    ...
    let run_hexer = |stem: &str| -> Vec<u8> {
        ...
        let run = instantiate(temen_encode::decode_module(&hexer).expect("decode hexer"))
        ...
        eprintln!(
            "phase 3  hexer.temen: {stem} → {} B Leng .x.nif",
            xnif.len()
        );
        xnif
    };
    ...
    // ---- phase 4 (host link + run): link the TEMEN-produced Leng, execute --------------------------
    ...
    if io_mode {
        ...
        assert_eq!(
            got.as_ref(),
            want,
            "Temen stdout must match the expected output (got {:?})",
            got
        );
        println!(
            "✅ FULL CHAIN ON Temen (I/O): nifler → nimsem → hexer (all sandboxed) → nim→powerbox link → \
             ran under the powerbox → stdout = {:?}. A Nim program compiled by the Temen prints for real.",
            got
        );
        return;
    }
    ...
    let (jout, _) = temen_jit::compile_and_run_capture(&m, idx, &call_args, &seed).expect("jit");
    ...
    assert_eq!(
        iword, expected,
        "{export}({call_args:?}) — Temen run must equal the expected result"
    );

    println!(
        "✅ FULL CHAIN ON Temen: nifler → nimsem(→nifler via exec) → hexer, all sandboxed guests → \
         linked → {export}({call_args:?}) = {iword} (interp == JIT == {expected}). The nimony \
         compiler runs on the Temen, and its output runs too."
    );
}
