...
//! (`temen-llvm` guests, which reach the host through `__vm_cap_resolve` + `__vm_host_call`) use the
//! `temen-posix` process/fd/signal ABI (POSIX.md), the same surface the chibicc/name-binding world already
//! links against.
//!
//! `temen-posix` normally grants itself on a `Host` by name binding ([`temen_posix::grant`], the chibicc
...
//! factory ([`temen_posix::cap`]) in a [`HostCap`] at [`temen_interp::cap_id::HOST_PROC`], so an embedder can
...
fn posix_cap_inner(
    ...
    let (posix, make) = temen_posix::cap(heap_base, heap_end, stdin);
    ...
    // #802 slice 4 — the grant installs everything `temen_posix::grant` installs on the name-binding
...
/// embedder's [`temen_posix::NetDelegate`] ([`Posix::set_net`]) or fails closed.
pub fn net_cap(posix: &Posix) -> HostCap {
    HostCap::host_proc(0, temen_posix::net_cap_factory(posix))
}
