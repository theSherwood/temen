//! `temen-strip` — the `.temen` packaging step (#907): demote non-entry-point exports to diagnostic
...
//!
//! ```text
//! temen-strip <in.temen> -o <out.temen> [--keep name,name,...] [--strip-names]
//! ```
//!
...
//! guaranteed inert. Only sound per-artifact for a known-single-module `.temen` — a parent guest can
...
fn try_main() -> Result<(), String> {
    ...
    if args.is_empty() || args.iter().any(|a| a == "-h" || a == "--help") {
        eprintln!(
            "usage: temen-strip <in.temen> -o <out.temen> [--keep name,name,...] [--strip-names]\n\
    ...
    let mut m =
        temen_encode::decode_module(&bytes).map_err(|e| format!("decode `{input}`: {e:?}"))?;

    let before = m.exports.len();
    let keep_refs: Vec<&str> = keep.iter().map(String::as_str).collect();
    let demoted = temen_run::demote_exports(&mut m, &keep_refs);
    if strip_names {
    ...
    let out_bytes = temen_encode::encode_module(&m);
    std::fs::write(&out, &out_bytes).map_err(|e| format!("write `{out}`: {e}"))?;
    eprintln!(
        "temen-strip: exports {before} -> {} ({demoted} demoted{}), {} -> {} bytes",
        m.exports.len(),
        if strip_names {
