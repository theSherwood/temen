...
//! compiled through the chibicc frontend, located via `$TEMEN_CHIBICC` or the in-repo build). The module is verified,
...
fn try_main() -> Result<(), String> {
    ...
    // `--link` (D-LINK): statically link several units (`.temt`/`.temeno`/`.temen`) into one module.
    let mut link = false;
    // `--assemble`: text unit → binary object (`.temeno`), the tiny assembler for frontends that
    ...
    if assemble {
        ...
        let [unit] = link_files.as_slice() else {
            return Err("--assemble takes exactly one text unit (.temt)".into());
        };
        is_text_ext(
            Path::new(unit)
                .extension()
        ...
        let out = out_path.ok_or("--assemble needs -o OUT.temeno")?;
        let text = fs::read_to_string(unit).map_err(|e| format!("read `{unit}`: {e}"))?;
        let m = temen_text::parse_module(&text).map_err(|e| format!("parse text IR: {e:?}"))?;
        fs::write(&out, temen_encode::encode_unit(&m))
            .map_err(|e| format!("write `{out}`: {e}"))?;
        return Ok(());
    }
    if link {
        if link_files.is_empty() {
            return Err("--link needs at least one unit (.temt/.temeno/.temen)".into());
        }
    ...
    if !module.imports.is_empty() && !temen_run::is_named_powerbox_entry(&module) {
        return Err(
    ...
    if is_named_powerbox_entry(&module) {
        // §5 kill-path: `TEMEN_DEADLINE_MS` (CLI policy) bounds a possibly-runaway guest so it is
        // detect-and-killed after the deadline instead of hanging the process; unset ⇒ unbounded.
        let deadline = std::env::var("TEMEN_DEADLINE_MS")
            .ok()
...
fn run_specialize(
    ...
    if let Some(path) = out_path {
        let bytes = temen_encode::encode_module(&residual);
        fs::write(&path, &bytes).map_err(|e| format!("write {path}: {e}"))?;
        eprintln!(
            "temen-run: wrote specialized residual to {path} ({} bytes, {} block(s))",
            bytes.len(),
            residual.funcs[0].blocks.len()
        );
        Ok(())
    } else if emit_text {
        print!("{}", temen_text::print_module(&residual));
        Ok(())
    } else {
...
fn print_usage() {
    eprintln!(
        "usage: temen-run <file.temt|.temen|.c> [--stdin FILE] [-- <guest args>]\n\
         ...
         \n  Units are text IR (.temt; .temt deprecated) or binary objects (`.temeno`, the v9\n\
         ...
         \n  (--emit-text).\n\
         \n  temen-run --assemble <unit.temt> -o OUT.temeno\n\
         ...
         \n  temen-run <file> --specialize [--func N] [--arg BIND]... [--const-region lo:hi]...\n\
         \n                 [--rename lo:hi] [--rename-private] [--outline] [--selective]\n\
         \n                 [--no-optimize]\n\
         \n                 [-o OUT.temen | --emit-text | --run-args v,v,...]\n\
         ...
         \n  -o OUT.temen  write the (re-verified) residual as a binary artifact; else --emit-text\n\
...
/// The `--link` path (D-LINK): load each unit (`.temt` text, `.temeno` binary object, or an
/// already-resolved `.temen`), pair its first-class export tables into a [`temen_ir::LinkUnit`]
...
/// here), and write it (`-o out.temen|.temt`) or print it (`--emit-text`).
fn run_link(files: &[String], out_path: Option<String>, emit_text: bool) -> Result<(), String> {
    ...
    let linked = temen_ir::link(&units).map_err(|e| format!("link failed: {e:?}"))?;
    ...
    match out_path {
        Some(p) => {
            if p.ends_with(".temt") || p.ends_with(".ir") || p.ends_with(".txt") {
                ...
                fs::write(&p, temen_encode::encode_module(&linked))
                    .map_err(|e| format!("write `{p}`: {e}"))?;
            }
        ...
        None if emit_text => {
            print!("{}", temen_text::print_module(&linked));
            Ok(())
        }
        None => Err("--link needs -o OUT.temen (or --emit-text)".into()),
    }
}
...
/// Load a **link unit** by extension: `.temt` text (which may carry the link forms; `.temt`
/// deprecated), or `.temeno`/`.temen`/`.bin` binary via `decode_unit` (the object dialect; an
...
fn load_unit(path: &Path) -> Result<Module, String> {
    ...
    if is_text_ext(ext, path) {
        ...
        return temen_text::parse_module(&text).map_err(|e| format!("parse text IR: {e:?}"));
    }
    match ext {
        "temeno" | "temen" | "bin" => {
            let bytes = fs::read(path).map_err(|e| format!("read {}: {e}", path.display()))?;
            temen_encode::decode_unit(&bytes).map_err(|e| format!("decode unit: {e:?}"))
        }
        other => Err(format!(
            "unknown unit extension `.{other}` — expected .temt, .temeno, or .temen"
        )),
    }
}

/// Load a module by file extension: `.temt` text IR (`.temt` deprecated), `.temen`/`.bin` binary,
...
fn load_module(path: &Path) -> Result<Module, String> {
    ...
    match ext {
        "temeno" => Err("`.temeno` is an object (a pre-link unit) — link it first: \
             temen-run --link <units...> -o out.temen"
            .into()),
        "temen" | "bin" => {
            let bytes = fs::read(path).map_err(|e| format!("read {}: {e}", path.display()))?;
            temen_encode::decode_module(&bytes).map_err(|e| match e {
                temen_encode::DecodeError::ObjectInput => "this file is an object (a pre-link \
                     unit) — link it first: temen-run --link <units...> -o out.temen"
                    .to_string(),
        ...
        "c" => {
            ...
            temen_text::parse_module(&ir).map_err(|e| format!("parse frontend IR: {e:?}"))
        }
        other => Err(format!(
            "unknown extension `.{other}` — expected .temt, .temen, or .c"
        )),
    }
}
...
/// `out.temt`. `$TEMEN_CHIBICC`/`.c` compilation is a trusted-operator path (the binary it runs is
...
fn fresh_temp_dir() -> Result<PathBuf, String> {
    ...
    let dir = env::temp_dir().join(format!("temen_run_{rnd:016x}"));
