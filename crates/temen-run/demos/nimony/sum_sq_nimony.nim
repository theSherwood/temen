...
# whose genuine Leng->C output (via `nimony c`) is on-ramped to Temen and runs there. This is the
...
#
# Deterministic output (sum_sq=385 / count=10) so the Temen run diffs against nimony's native run.
