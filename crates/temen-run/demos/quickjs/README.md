...
Bellard's **QuickJS** (2024-01-13, MIT) driven through the LLVM→TEMEN-IR on-ramp:
...
`cargo run --example try_translate` (in `crates/temen-llvm`) walks the fail-closed
...
the scalar path and fail-closed. Fixed in `temen-llvm` (`agg_fields`); test
...
*downward* native stack, but the Temen data-stack grows *up*, so `frameaddress(0)`
...
translates to `qjs_repl.temen` at `--host-page 65536` (~4.3 MB); `web/play.js`
...
decimal, but the Temen float ops are round-to-nearest only (no rounding-mode op).
...
# native oracle
QJS=/tmp/temen_quickjs_cache/quickjs-2024-01-13
...
cd ../../../temen-llvm && cargo run --example try_translate -- \
