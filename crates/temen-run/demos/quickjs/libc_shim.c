 ...
 * The Temen float ops are round-to-nearest-even only, with no rounding-mode primitive. QuickJS's
 * shortest-round-trip Number->string (`js_ecvt1`) toggles FE_DOWNWARD/FE_UPWARD to find the
 * shortest decimal; that directed-rounding path is a KNOWN FOLLOW-UP (needs an Temen rounding-mode
