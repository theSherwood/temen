...
frame, which drains the `keyboard` capability into the Controller device, fires the Screen vector,
and presents the composed frame through `display`.
...
- **`varvara.c`** — the devices a framebuffer-and-keyboard host can serve: **System** (palette,
  ...
  **Datetime** (a deterministic virtual clock — the on-ramp grants no wall clock, and determinism is
  what the differential wants). Mouse, Audio and File are absent: the playground has no such
  capabilities yet; their ports are inert bytes.
- **`main.c`** — the reactor entry (`#include`s the two above as one translation unit). Maps JS
  ...
  Space/Enter/Backspace/Tab/Esc as the key byte). A halted ROM (System/state) exits the guest, which
  ends the reactor loop.
...
- **`demo.tal`** — the demo ROM: a 256×192 striped screen, a title set in a 1bpp font, a swarm of
  2bpp sprites bouncing on the foreground layer, a player steered by the arrow keys; any letter
  cycles the palette, Space resets the swarm.
...
To run your own ROM in the playground, replace `uxn_demo.rom` (the guest opens whatever the page serves
as `boot.rom`); to write one, assemble it with `uxnasm` from this directory.
