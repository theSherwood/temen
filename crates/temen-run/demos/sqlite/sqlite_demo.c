 ...
 * vendored; see `demo_sqlite_vs_native` in temen-llvm's translate.rs) into a self-contained guest
 ...
 * `datetime('now')` agree between the native and Temen runs. `:memory:` + `SQLITE_TEMP_STORE=3`
...
/* xRandomness: a fixed-seed SplitMix64 — identical bytes native vs Temen, so random() agrees. */
