...
Two whole-program demos driving the **unmodified SQLite amalgamation** through the LLVM→TEMEN-IR
...
(`fetch_sqlite_amalgamation` in `crates/temen-llvm/tests/translate.rs`, currently 3.50.2) and skip
...
embedder-granted `fs` capability (`temen-run`'s `mem_fs`/`host_fs` — see `crates/temen-run/src/fs.rs`
...
- `-DTEMEN_GUEST` → `SQLITE_OS_OTHER=1` + the capability VFS (translated, run in the sandbox);
...
their own SQL client-side; the `.temen` is built by `browser/build-onramp-assets.mjs` at
...
clang -O2 -emit-llvm -c -fno-vectorize -fno-slp-vectorize [-DTEMEN_GUEST] \
