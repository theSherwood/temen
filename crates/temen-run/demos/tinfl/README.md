...
writing the result to stdout. `temen-run`'s output must match a native `cc` build byte-for-byte.

```sh
cargo run -p temen-run -- crates/temen-run/demos/tinfl/tinfl_demo.c
```
