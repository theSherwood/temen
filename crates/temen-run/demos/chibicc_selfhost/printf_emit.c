 ...
 * *only* on-ramp dependency is the float path, which delegates to the temen-llvm intrinsics
 ...
 * generator (a faithful C port of temen-llvm's `synth_dtoa_*` IR reference, `crates/temen-llvm/src/lib.rs`)
...
static int temen_fmt_sign(char *o, double *x, int flags) {
  int n = 0;
...
static int temen_fmt_special(char *o, double x, int up, int had_sign) {
  if (x != x) {
...
static int temen_fmt_exp(char *o, int E, int up) {
  int n = 0;
  o[n++] = up ? 'E' : 'e';
  o[n++] = E < 0 ? '-' : '+';
  int ae = E < 0 ? -E : E;
...
int __vm_fmt_sci(char *o, double x, int prec, int width, int flags) {
  ...
  int n = temen_fmt_sign(o, &x, flags);
  int sp = temen_fmt_special(o, x, (flags >> 3) & 1, n);
  if (sp >= 0) return sp;
  int up = (flags >> 3) & 1;
  ...
  if (x == 0.0) {
    ...
    return n + temen_fmt_exp(o + n, 0, up);
  }
  ...
  return n + temen_fmt_exp(o + n, E, up);
}
