...
# no fetch: per-TU bitcode → llvm-link → temen-llvm-translate → prep_temen (decode/verify/
...
# The self-host guest libc (SELFHOST_C.md §5 B / Appendix B): the aggregating -DTEMEN_GUEST driver
...
echo "=== [1b/4] guest libc (chibicc_libc.c, -DTEMEN_GUEST) ==="
"$CLANG" -O2 -emit-llvm -c -DTEMEN_GUEST -mlong-double-64 \
  ...
  -I"$SRC" -I"$REPO/crates/temen-run/demos/postgres" \
...
# chibicc-internal. Allowlist = the recognizer set (temen-llvm/src/lib.rs): the __vm_* primitives,
...
ONRAMP='^(__vm_[a-z_0-9]+|write|read|exit|_exit|_Exit|malloc|calloc|realloc|free|memcpy|memmove|memset|memcmp|bcmp|printf|puts|putchar|putc|fputc|fwrite|fputs|__temen_[a-z_0-9]+)$'
...
if [ ! -x "$TR" ]; then
  ...
  cargo build --release --bin temen-llvm-translate \
    --manifest-path "$REPO/crates/temen-llvm/Cargo.toml"
fi
...
cargo run --release -p temen-run --example prep_temen -- \
  "$CACHE/chibicc_raw.temen" "$CACHE/chibicc.temen"
echo "OK: $CACHE/chibicc.temen"
