 ...
 * `build_chibicc_temen.sh` emit-object build.
 */
#ifndef TEMEN_SELFHOST_PRELUDE_H
#define TEMEN_SELFHOST_PRELUDE_H
 ...
 * would hand out overlapping addresses and corrupt each other at run time. Defining `__TEMEN_LIBC_EXTERN`
 * (this prelude is force-included in every emit-object TU) makes that state a single **shared** symbol:
 * each TU sees an `extern`, and the libc unit (`emit_libc.c`, which sets `__TEMEN_LIBC_OWNER`) holds the
 * one definition — resolved cross-TU by `temen_ir::link` exactly like chibicc's shared `ty_int`. */
#define __TEMEN_LIBC_EXTERN 1
...
#endif /* TEMEN_SELFHOST_PRELUDE_H */
