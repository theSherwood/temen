...
`.o` so the rule prints), rewrite it to `-emit-llvm -o X.bc`, and run it. See README.md.
    ...
    TEMEN_PG_SRC=/path/to/postgresql-XX.Y python3 emit_bc.py

Writes bc_manifest.txt next to the source tree's parent (TEMEN_PG_CACHE)."""
import os, subprocess, shlex, concurrent.futures

SRC = os.environ.get("TEMEN_PG_SRC")
CACHE = os.environ.get("TEMEN_PG_CACHE", "/tmp/temen_pg_cache")
assert SRC and os.path.isdir(SRC), "set TEMEN_PG_SRC to the extracted postgres tree"
