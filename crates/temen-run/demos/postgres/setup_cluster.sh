...
# `initdb`/`postgres` that `build_bitcode.sh` produced (`make install` into `$TEMEN_PG_CACHE/inst`),
...
# guest's first boot — the BOOTSPEED lever). The result is `$TEMEN_PG_DATA`, which `build_image` turns
# into the shippable `pgdata.img`.
#
#   needs: a native `initdb`/`postgres` installed at $INST (build_bitcode.sh + `make install`)
#   env:   TEMEN_PG_CACHE (default /tmp/temen_pg_cache), TEMEN_PG_DATA (default $TEMEN_PG_CACHE/pgdata)
...
INST="$CACHE/inst"
DATA="${TEMEN_PG_DATA:-$CACHE/pgdata}"
...
# resolves. It is never executed (the Temen module is), so a bare shell stub suffices.
