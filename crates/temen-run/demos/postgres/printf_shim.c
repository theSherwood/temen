 ...
 * (`write(1/2,…)` → `__temen_stream_write`), `fprintf(file, …)` → the fs cap (`write(fd≥3,…)`). A single
 ...
 *
 * `#include`d into a driver under `-DTEMEN_GUEST`, after `os_shim.c` + `stdio_shim.c` (it `fwrite`s).
 * Adapted from `crates/temen-llvm/tests/fixtures/lua/lua_fmt_snprintf.c` (the Lua `string.format`
