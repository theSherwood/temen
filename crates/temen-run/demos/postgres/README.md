...
Artifacts land in `$TEMEN_PG_CACHE` (default `/tmp/temen_pg_cache`):
...
| 11a | **`fs` cap: the metadata + directory surface** | — | **DONE (slice BZ):** the runtime's first blocker — the `fs` capability could open/read/write/seek *files* but had no way to **walk a tree** (no `stat`/`mkdir`/`rmdir`/`opendir`/`readdir`), which a natively-`initdb`'d cluster needs pervasively. Added ops 14–19 (`temen-run/src/fs.rs`): `stat` fills a fixed 72-byte little-endian `StatBuf` (the `S_IF*` type bits + size + mtime + ino/dev) with **lstat** semantics (a symlink can't be used to probe a type outside the granted root); `mkdir`/`rmdir`; `opendir` snapshots a directory's entries and `readdir` yields them sorted, one per call. Both backends at protocol parity — `mem_fs` models directories over its flat name table (a path is a dir if `mkdir`'d or a strict prefix of a file), `host_fs` uses the real tree — so a differential runs identically on either. Tests `os_metadata_ops_parity_mem_vs_host` + `readdir_is_sorted_and_bounded` |
...
| 11m | **boot: drive the named boot to real logging** | — | **DONE (slice CL):** with every trap self-naming (#11k), a tight run→name→fix loop cleared the run of gaps between `find_my_exec` and Postgres's first log output. One on-ramp fix: **`__sigsetjmp`** (the symbol the `sigsetjmp` macro expands to) joins the setjmp recognizer (test `sigsetjmp_recognized`). Guest shims (each named by the diagnostic): **`realpath`** (canonicalize→absolute over the fs cap), a mutable **environment** (`setenv`/`getenv`/…), **`getpwuid`**/`getpwnam`, deterministic **`random`**/`srandom`, the **event-loop backend** (`event_shim.c`: `signalfd`/`epoll_*`/`eventfd` → fake fds + no-op registration; the single-process `WAIT_USE_EPOLL` latch is never blocked), real **`memcpy`/`memmove`/`memset`** (`mem_shim.c`, so an *address-taken* one resolves to a real function not a funcref stub), **`pow`** (then a `math_shim.c` shim exact for `pow(10,n)`; superseded by bundled openlibm in CO), and **`strerror`** + GNU **`strerror_r`** (its own `_GNU_SOURCE`-isolated TU `strerror_shim.c`). Composes with #11l's `strtod`/`scanf`. The boot now runs real startup through GUC init + config-file processing, emitting timestamped `LOG:`/`FATAL:` lines — no trap, a clean `Exited(1)` |
...
| 11o | **★★★ `SELECT 1+1` → `2`: the capstone** | — | **DONE (slice CN):** the checkpoint `fn0` trap was an **address-taken `memcmp`** — dynahash's `HASH_BLOBS` table stores `hashp->match = memcmp` and calls it *through the pointer*, but `memcmp` was only synthesized for direct calls, never defined, so its funcref fell to a trap stub (CL had defined `memcpy`/`memmove`/`memset` for this but missed the comparators). `mem_shim.c` now also defines `memcmp`/`strlen`/`strcmp`/`strncmp`; differential `demo_pg_funcptr_vs_native`. With the fix `postgres --single` completes recovery + the checkpoint, reaches the `backend>` prompt, **executes `SELECT 1+1;` and prints `2` (int4)**, then shuts down cleanly (`Exited(0)`). PostgreSQL 17.5 runs a real query on the Temen bytecode engine under the fs + powerbox caps |
| 11p | **★★ real-table round-trip: `CREATE TABLE`/`INSERT`/`SELECT * FROM t`** | — | **DONE (slice CO):** DDL/DML worked immediately (heap read/write/`UPDATE`/`DELETE`/`WHERE` ride the WAL-recovery machinery), but `ORDER BY` **trapped** in the planner: `cost_tuplesort`'s `N·LOG2(N)` cost model calls `log`, which was an **undefined external → trap stub**. In fact the *whole* transcendental surface was undefined — `postgres_libm.bc` was byte-identical to `postgres.linked.bc`; slice BQ built the bundled-openlibm mechanism but never wired it into the boot link. **Fix:** `link_shims.sh` now `llvm-link`s openlibm's double set (the `OPENLIBM_SRCS` 28-file set) into `postgres_shimmed.bc`, so `log`/`exp`/`pow`/… are defined guest code (retiring the hand-written `math_shim.c` `pow` for openlibm's bit-exact `e_pow`). Round-trip runs clean (`Exited(0)`): `SELECT *`, `count`/`sum`/`avg`, **`ORDER BY … DESC`**, `UPDATE`, `DELETE`, `GROUP BY … ORDER BY` all correct |
...
`cap.call`) — a **clean `temen-verify` pass** (BY fixed the one remaining `TypeMismatch`). What's left is
...
first printed output, and the varargs **`scanf` input engine** + a correctly-rounded **`strtod`** (gap
#11l, slice CJ: `scanf_shim.c` + `../strtod/strtod.c`) parse config/version values. The boot then trapped
**silently** in early init, so slice CK (gap #11k) makes stub-traps **self-naming** — the on-ramp records
each stub's missing-extern name in the §6 function-name table, so the trap-time backtrace's innermost frame
resolves (via `temen_interp::func_name`) to the extern, the stub body staying a pure `Unreachable`. That
legible loop drove slice CL (gap #11m): the on-ramp **`__sigsetjmp`** fix plus guest shims for `realpath`, a
mutable **environment**, `getpwuid`, deterministic **`random`**, the `signalfd`/`epoll` **event-loop
...
len 4) with a clean `Exited(0)` — the full lifecycle. Slice CO then widened it to a **real-table
round-trip** — `CREATE TABLE` / `INSERT` / `SELECT * FROM t`, plus `ORDER BY` / `GROUP BY` / aggregates /
`UPDATE` / `DELETE`, all correct — by **llvm-linking the bundled guest libm (openlibm)** into the module
(the sort-cost model's `LOG2` needs `log`, an undefined transcendental until then; the capstone's constant
`SELECT` never reached one). *Boot speed* — a cold 15k-function boot reaches the prompt in ~74 s on the
bytecode engine — is now the gating concern for a browser demo, pointing at snapshot/restore of the
post-boot state. See `LLVM.md` slices BM–CO, and "Booting" below.

## Booting

`build_bitcode.sh` produces `postgres_libm.bc` (the whole-program module). `link_shims.sh` `llvm-link`s
the guest shims **and the bundled guest libm (openlibm)** into it → `postgres_shimmed.bc`: `pg_shims.c`
(mem/os/libc/locale/time/proc/event/ipc/stdio/printf + `../strtod/strtod.c` + `scanf_shim.c`),
`strerror_shim.c` compiled separately with `-D_GNU_SOURCE`, and openlibm's double set (the 28-file
`OPENLIBM_SRCS` — the 18 transcendentals `log`/`exp`/`pow`/`sin`/… the Temen has no op for; `sqrt`/`fabs`/
`ceil` still lower to on-ramp float ops). A driver `translate_bc_path_with_options(…, stub_unresolved_externs:
...
As of slice CO the module runs real backend init, opens the catalog, performs **WAL crash recovery**
(`redo starts…`/`redo done`/`checkpoint starting`), completes the end-of-recovery checkpoint, reaches the
`backend>` prompt, and runs a **real-table round-trip** end-to-end (`Exited(0)`): `CREATE TABLE t(x int, s
text)`, `INSERT`s, then `SELECT *`, `count`/`sum`/`avg`, **`ORDER BY … DESC`**, `UPDATE`, `DELETE`, and
`GROUP BY … ORDER BY` all return the correct rows. (Slice CO's fix: `ORDER BY` cost estimation calls `log`
via `cost_tuplesort`'s `LOG2` model — an undefined transcendental until openlibm was llvm-linked into the
module; the capstone's `SELECT 1+1` never reached one.) Cutting the ~100 s cold-boot time (snapshot/restore
of the post-recovery state) is the remaining work for a browser demo.
