 ...
 * Phase B and LMDB use (`crates/temen-run/src/fs.rs`), now with the metadata + directory surface
 ...
 *
 * Built by being `#include`d into a driver under `-DTEMEN_GUEST` (like SQLite's cap VFS) — a single
...
/* The `fs` op protocol — crates/temen-run/src/fs.rs. */
enum {
