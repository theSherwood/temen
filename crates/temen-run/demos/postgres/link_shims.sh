...
clang -O2 -emit-llvm -c -DTEMEN_GUEST -fno-vectorize -fno-slp-vectorize \
...
clang -O2 -emit-llvm -c -DTEMEN_GUEST -D_GNU_SOURCE -fno-vectorize -fno-slp-vectorize -I"$HERE" \
  "$HERE/strerror_shim.c" -o "$CACHE/strerror_shim.bc"

# --- Bundled guest libm (slice BQ). The Temen has no transcendental op, so `log`/`exp`/`pow`/`sin`/… stay
...
OL_VER="${TEMEN_OPENLIBM_VER:-0.8.5}"
OL_CACHE="${TEMEN_OPENLIBM_CACHE:-/tmp/temen_openlibm_cache}"
...
OL_DIR=""
for cand in "${TEMEN_OPENLIBM_DIR:-}" /tmp/openlibm "$OL_CACHE/openlibm-$OL_VER"; do
...
if [ -z "$OL_DIR" ]; then
  ...
  [ -f "$OL_DIR/src/e_log.c" ] \
    || { echo "OPENLIBM FETCH FAILED (set TEMEN_OPENLIBM_DIR to a local tree)"; exit 21; }
fi
