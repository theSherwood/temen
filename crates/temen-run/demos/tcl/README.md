...
The reference **Tcl 8.6.14** interpreter (Tcl/BSD license) driven through the LLVM→TEMEN-IR
...
- `tcl_shim.c` — the Tcl-specific OS/libc waist the on-ramp neither synthesizes, resolves to
  an temen-posix capability, nor covers via a reused shim: the `clock` time surface (deterministic
...
| **posix caps** — `open`/`read`/`write`/`close`/`lseek`/`stat`/`opendir`/`readdir`/`getcwd`/`chdir`/`getenv`/`setenv`/`unlink`/`exit` | 18 | **temen-posix** ops 0–20, resolved at load (POSIX.md) |
...
are trap-stubbed via `TEMEN_STUB_EXTERNS` — never called on the eval path.
...
`build-onramp-assets.mjs` builds `tcl_repl.temen` (`--stub-externs`, 64 KiB pages) and `web/play.js`
...
**Why the VFS, not temen-posix.** The VFS callbacks fill Tcl's *own* `Tcl_StatBuf` in C, so there is
**no libc `struct stat` ABI gap** (temen-posix's `stat` writes a minimal `{mode,size}`, which Tcl's
glibc-layout `struct stat` would misread); it needs **no filesystem capability** (the scripts live in
the guest binary); and it runs identically native, on temen, and in the browser. This is how embedded
Tcl applications ship their library. `build_bitcode.sh` links both variants (`tcl_linked.ll` +
`tcl_init_linked.ll`); the playground card uses the full-init `tcl_init.temen`.
...
**DONE.** `_start` is **wasm-JIT-emittable**: the full-init module (`tcl_init.temen`) runs on the
emitted-wasm tier in real V8, **byte-identical to the interpreter** — `browser-jit-module-test.mjs`
now drives Tcl (alongside hello_c/Lua/SQLite/QuickJS) through both `temen_run_onramp` and `runJitModule`
...
./build_bitcode.sh                       # → /tmp/temen_tcl_cache/tcl_linked.ll

# native oracle by hand (the minimal-embedding REPL against real libtcl):
cd /tmp/temen_tcl_cache/tcl8.6.14
...
cd ../../crates/temen-llvm && cargo build --release --example try_translate
./target/release/examples/try_translate /tmp/temen_tcl_cache/tcl_linked.ll
```
