#!/usr/bin/env bash
# temen-leng-the-guest bitcode pipeline (NIM.md §3e / W5 capstone) — the chibicc/Postgres asset-lane
# pattern for the Rust `temen-leng` translator: build-std → llvm-link → temen-llvm-translate → prep_temen
...
CACHE="${TEMEN_LENG_CACHE:-/tmp/temen_leng_cache}"
...
# chibicc.temen); no --stub-externs, so translation proves every call resolves. The guarded powerbox
# layout (#964/#1094) is unconditional now — the powerbox low scratch shifts one 16 KiB guard up so a
# host seeds [0, guard) unmapped and a NULL deref in the compiler traps instead of reading zeros.
# --null-guard below is a redundant no-op (the marker export is retired), kept for compatibility.
...
echo "[4/4] prep_temen (decode/verify/bytecode-compile) ..."
cargo run --release -p temen-run --example prep_temen -- "$CACHE/leng_raw.temen" "$CACHE/temen-leng.temen"

echo "done: $CACHE/temen-leng.temen"
