...
# and execve/execv/execvp are the #801 staged-exec (slice 4) — linked as ordinary guest code
# (their `__px_*` externs bridge in bash_shim.c; `__vm_exec_module` is a core builtin).
for f in fnmatch regex exec; do
...
if [ -x "$TR" ]; then
  # TEMEN_STUB_EXTERNS=1: trap-stub the OS surface the guest never reaches. The surface bash DOES
  # reach binds to the temen-posix ops by bare libc name (Lane C) — slice 3's run wires the rest.
  ...
  TEMEN_STUB_EXTERNS=1 "$TR" "$LINKED" 2>&1 | grep -v '\[stub\]' | head -5 || true
else
