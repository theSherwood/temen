 ...
 * to an temen-posix capability, nor covers via a reused shim (#802 slices 2-3). Ordinary guest C; a
 ...
 *   0. the temen-posix personality — the OS surface (open/read/write/stat/fork/signals/…): the
 *      embedder grants the personality under the name "posix" (`temen_run::posix::posix_cap`); this
 ...
 *      (`(ptr, len)` strings, compact LE structs — layouts from `crates/temen-posix/src/lib.rs`);
 ...
 *
 * Anything not here and not reached by the current slice rides `TEMEN_STUB_EXTERNS` trap stubs —
...
/* --- band 0: the temen-posix personality --------------------------------------------------------
 ...
 * and marshaling contracts track `crates/temen-posix/src/lib.rs` (the vtable is the ABI). */
...
static struct shim_tm shim_tm_;
/* Real UTC calendar math + a bounded `strftime` format engine (ported from the postgres
 * `time_shim.c`, gap #11e — glibc C-locale exact). The sandbox is single-TZ UTC, so
 * `localtime` == `gmtime`. Before this, `localtime` returned a fixed epoch-0 struct and
 * `strftime` ignored the format entirely (always "1970-01-01"), so bash's `printf '%(fmt)T'`
 * was broken. Civil date from a day count is Howard Hinnant's algorithm. */
static struct shim_tm *shim_gmtime_r(const long *tp, struct shim_tm *out) {
  long t = *tp;
  ...
  int yday = (int)d - 1;
  for (int i = 0; i < out->tm_mon; i = i + 1) yday += mdays[i] + (i == 1 ? leap : 0);
  out->tm_yday = yday;
  out->tm_gmtoff = 0;
  out->tm_zone = "UTC";
  return out;
}
struct shim_tm *gmtime(const long *t) { return shim_gmtime_r(t, &shim_tm_); }
struct shim_tm *gmtime_r(const long *t, struct shim_tm *out) { return shim_gmtime_r(t, out); }
struct shim_tm *localtime(const long *t) { return shim_gmtime_r(t, &shim_tm_); }
struct shim_tm *localtime_r(const long *t, struct shim_tm *out) { return shim_gmtime_r(t, out); }
...
static unsigned long shim_tput(char *b, unsigned long pos, unsigned long cap, const char *s) {
  while (*s) {
    if (pos + 1 < cap) b[pos] = *s;
    pos++;
    s++;
  }
  return pos;
}
static unsigned long shim_tnum(char *b, unsigned long pos, unsigned long cap, long v, int width,
                               char pad) {
  char tmp[24];
  int n = 0, neg = v < 0;
  ...
  while (n > 0) {
    char c = tmp[--n];
    if (pos + 1 < cap) b[pos] = c;
    pos++;
  }
  return pos;
}
unsigned long strftime(char *s, unsigned long max, const char *fmt, const void *tmv) {
  const struct shim_tm *tm = (const struct shim_tm *)tmv;
  unsigned long pos = 0;
  for (const char *p = fmt; *p; p++) {
    ...
    p++;
    switch (*p) {
      case 'Y': pos = shim_tnum(s, pos, max, tm->tm_year + 1900, 4, '0'); break;
      case 'y': pos = shim_tnum(s, pos, max, (tm->tm_year + 1900) % 100, 2, '0'); break;
      case 'C': pos = shim_tnum(s, pos, max, (tm->tm_year + 1900) / 100, 2, '0'); break;
      case 'm': pos = shim_tnum(s, pos, max, tm->tm_mon + 1, 2, '0'); break;
      case 'd': pos = shim_tnum(s, pos, max, tm->tm_mday, 2, '0'); break;
      case 'e': pos = shim_tnum(s, pos, max, tm->tm_mday, 2, ' '); break;
      case 'H': pos = shim_tnum(s, pos, max, tm->tm_hour, 2, '0'); break;
      case 'I': {
        int h = tm->tm_hour % 12;
        if (h == 0) h = 12;
        pos = shim_tnum(s, pos, max, h, 2, '0');
        break;
      }
      case 'M': pos = shim_tnum(s, pos, max, tm->tm_min, 2, '0'); break;
      case 'S': pos = shim_tnum(s, pos, max, tm->tm_sec, 2, '0'); break;
      case 'j': pos = shim_tnum(s, pos, max, tm->tm_yday + 1, 3, '0'); break;
      case 'w': pos = shim_tnum(s, pos, max, tm->tm_wday, 0, '0'); break;
      case 'u': pos = shim_tnum(s, pos, max, tm->tm_wday == 0 ? 7 : tm->tm_wday, 0, '0'); break;
      case 'p': pos = shim_tput(s, pos, max, tm->tm_hour < 12 ? "AM" : "PM"); break;
      case 'a': pos = shim_tput(s, pos, max, shim_wday_ab[tm->tm_wday & 7]); break;
      case 'A': pos = shim_tput(s, pos, max, shim_wday_full[tm->tm_wday & 7]); break;
      case 'b':
      case 'h': pos = shim_tput(s, pos, max, shim_mon_ab[tm->tm_mon % 12]); break;
      case 'B': pos = shim_tput(s, pos, max, shim_mon_full[tm->tm_mon % 12]); break;
      case 'n': pos = shim_tput(s, pos, max, "\n"); break;
      case 't': pos = shim_tput(s, pos, max, "\t"); break;
      case '%': pos = shim_tput(s, pos, max, "%"); break;
