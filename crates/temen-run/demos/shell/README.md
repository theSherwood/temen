# The `temen-posix` shell — canonical C source

The Stage-0/Stage-1 shell (STAGE1.md): a real command interpreter compiled by the
in-tree **chibicc** onto the `temen-posix` personality (libc by name). These `.c`
files are the single source of truth, `include_str!`d by the differential test
`crates/temen/tests/c_shell.rs` and (as of the playground-shell epic) compiled into
the `shell.temen` asset the browser playground runs.
