#!/bin/sh
# Build the Doom guest for the Temen sandbox: compile the fetched doomgeneric sources + this platform
# layer + the libc shim, link to one module, and translate through the LLVM on-ramp to a `.temen`.
...
# (`cargo build --release --bin temen-llvm-translate` in crates/temen-llvm), and the sources fetched
...
LUA="$REPO/crates/temen-llvm/tests/fixtures/lua"           # reused fs + printf guest shims
...
clang $FL -I"$SRC" "$HERE/doomgeneric_temen.c" -o "$OUT/_platform.bc"
...
"$TR" "$OUT/doom.bc" -o "$OUT/doom.temen" --host-page 65536 --null-guard
echo "built $OUT/doom.temen ($(wc -c < "$OUT/doom.temen") bytes)"
