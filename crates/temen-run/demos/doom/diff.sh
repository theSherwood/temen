...
# artifacts the `temen-run` test `doom_diff` compares:
#   - the guest  $OUT/bcdiff/doom_diff.temen   (doom_diff.c through the on-ramp)
...
SRC="$CACHE/dg"
LUA="$REPO/crates/temen-llvm/tests/fixtures/lua"
...
"$TR" "$G/doom_diff.linked.bc" -o "$G/doom_diff.temen" --host-page 65536 --null-guard
echo "guest: $G/doom_diff.temen ($(wc -c < "$G/doom_diff.temen") bytes)"
...
echo "now run:  cargo test -p temen-run --test doom_diff -- --ignored --nocapture"
