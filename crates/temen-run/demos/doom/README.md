...
LLVM→TEMEN-IR on-ramp, in the browser playground, driven by the reactor run model. This directory holds
...
network. Only the *engine* module (`doom.temen`, built from doomgeneric's source by `fetch.sh` +
...
- **`doom1.wad`** — the vendored shareware IWAD (above); the browser opens the reactor over it via the
  `fs` capability, and the `temen-run` diff/reactor tests read it (default `/tmp/doomgeneric_cache`,
  overridable via `DOOM_WAD` — point it at this file to run them without a fetch).
- **`doomgeneric_temen.c`** — the platform layer (real, compiles against doomgeneric's headers). `DG_*`
...
- **`doom_diff.c`** — the headless **frame-hash differential** platform: `DG_DrawFrame` prints an FNV
  ...
  renders byte-identically to native. Driven by the `temen-run` test `doom_diff` (over an in-memory WAD).
- **`fetch.sh` / `build.sh` / `diff.sh`** — fetch-and-cache the sources (not vendored — id Software's
  ...
  differential (guest `.temen` + native oracle). CI-style fetch uses the GitHub archive with a
  `raw.githubusercontent.com` per-file fallback.

The reused Lua guest shims (`crates/temen-llvm/tests/fixtures/lua/`) do the heavy lifting:
...
1. **Compiles clean.** All 79 Doom TUs build to LLVM-18 bitcode with stock `clang -O2 -emit-llvm`
   (79/79, zero errors; the X11 platform is replaced by `doomgeneric_temen.c`).
2. **Translates whole-program — no IR gaps.** `llvm-link` → one ~900 KB module → `temen-llvm-translate`
   produces a **797 KB `doom.temen`** with `main`/`tick` exported. There are **no unsupported IR
...
4. **Renders — byte-exact vs native `cc`.** `sh diff.sh` builds the guest `doom_diff.temen` + a native
   oracle and the `temen-run` test `doom_diff` runs the guest over an in-memory WAD, asserting its
...
- **Playground wiring** (slice 4): the reactor `.temen` (`build.sh`) + the shareware WAD as browser
