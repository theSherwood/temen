 ...
 * `crates/temen-run/src/fs.rs`) — so the whole memory-mapped data plane flows through explicitly
 ...
 *   - guest (`-DTEMEN_GUEST`): the mmap/pthread/file shims below over the Fs capability;
...
#ifdef TEMEN_GUEST
extern void temen_fs_crash_arm(long n);
#else
static void temen_fs_crash_arm(long n) { (void)n; }
#endif
...
static int run_crash(long n) {
  ...
  if (rc) return rc;
  REOPEN(env);
  temen_fs_crash_arm(n);            /* arm the simulated power loss for txn2's commit */
