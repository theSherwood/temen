...
see `crates/temen-run/src/fs.rs`): a guest shim bridges LMDB's `mmap`/`msync`/`pread`/`open`/… to
...
test fetches and caches it from the upstream GitHub mirror (`fetch_lmdb` in temen-llvm's
...
- **`lmdb_shim.c`** — guest-only (`-DTEMEN_GUEST`): the Fs-capability bridge for the file + mmap
...
# guest bitcode (the test does this automatically): compile each -DTEMEN_GUEST, llvm-link, translate
```
