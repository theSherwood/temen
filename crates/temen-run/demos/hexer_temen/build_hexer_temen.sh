#!/usr/bin/env bash
# hexer-on-Temen (NIM.md §3c/§3e, "nimony in the browser" slice 2) — compile nimony's MIDDLE-END phase,
# `hexer` (semchecked NIF → Leng, ~18k loc), to a runnable Temen module and prove it lowers a real
# semchecked module to Leng **byte-for-byte identically to native hexer**, across all three engines.
#
# Same C on-ramp as nifler (slice 1), same shared bottom-edge shim (../nifler_temen/nifler_shim.c —
...
"$CLANG" -O2 -fno-vectorize -fno-slp-vectorize -DTEMEN_GUEST -emit-llvm -c -I"$NIMLIB" "$SHIM" -o "$CACHE/shim.bc"
"$LINK" "$CACHE"/c/*.c.bc "$CACHE/shim.bc" -o "$CACHE/hexer.linked.bc"

echo "=== [3/6] translate (--stub-externs) + prep_temen (verify) ==="
...
fail=0
for src in "$HERE"/../nifler_temen/inputs/*.nim; do
  name="$(basename "$src")"
  ...
  echo "=== [6/6] $name: hexer-on-Temen + diff every produced file (treewalk, bytecode, jit) ==="
  for eng in treewalk bytecode jit; do
    out="$CACHE/temen_${name}_$eng"; rm -rf "$out"; mkdir -p "$out"
    set +e
    TEMEN_PHASE_BACKEND="$eng" cargo run -q --release -p temen-run --example nimphase_run -- \
      "$CACHE/hexer.temen" "$fixture" "$out" -- hexer c "$main.s.nif" >/dev/null 2>"$out/err.txt"; rc=$?
    set -e
    ok=1; [ "$rc" = 0 ] || ok=0
