...
//
//   cargo run -p temen-run -- crates/temen-run/demos/jit/jit_link.c
...
// --- the byte emitter: serialized Temen IR (binary `temen-encode` v2) ---------------------------
// One-byte opcodes + LEB128, mirroring `crates/temen-encode`. Value operands are block-local indices
