...
// it and emits serialized Temen IR — the binary `temen-encode` format, built byte-by-byte in guest
...
static long emit_unit(Ins *prog, char *buf, int abi_sp) {
  ...
  uleb(buf, ni);
  // The straight-line body. Opcodes mirror temen-encode: 0x11 = i64.const (+ sleb immediate);
