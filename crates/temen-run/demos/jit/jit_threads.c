...
// sibling: `NWORKERS` pthreads each build serialized Temen IR for a *distinct* unit at runtime, submit
...
// The binary layout mirrors `crates/temen-encode` (LEB128 + one-byte opcodes), exactly as
...
// genuinely distinct unit. Opcodes mirror temen-encode: 0x11 = i64.const (+ sleb), 0x40 = i64.add,
...
static long emit_unit(char *buf, long k, long w) {
  ...
  eb(&e, (13) & 0xff); eb(&e, (13) >> 8); // version (u16)
  ...
  eb(&e, 1);
  eb(&e, 17);
  eb(&e, 0); // no shadow arena (v11: the durable arena is module-declared; this unit spawns nothing)
  eb(&e, 0); // no data segments
  eb(&e, 0); // no data-image funcref slots (v13 section)
