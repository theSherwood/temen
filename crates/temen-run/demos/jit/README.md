...
2. **JITed** — at runtime, the guest walks the same bytecode and *emits serialized Temen IR*
   (the binary `temen-encode` format, built byte-by-byte in its own window), submits the blob
...
```sh
cargo run -p temen-run -- crates/temen-run/demos/jit/jit_demo.c
```
...
```sh
cargo run -p temen-run -- crates/temen-run/demos/jit/jit_threads.c
```
...
The product-path smoke test is `run.rs::demo_jit_threads_runs` (through the `temen-run` binary, which
...
standalone `cargo run` program: it is driven by the embedder's `temen_run::JitSession`, which re-enters
...
```sh
cargo run -p temen-run -- crates/temen-run/demos/jit/jit_repl.c
# prompt 1: +14 -> acc=14
```
