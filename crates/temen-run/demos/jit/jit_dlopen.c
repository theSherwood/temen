...
// a growing symbol table where definitions persist and compose by name — `dlopen` for Temen, in-sandbox.
//
...
//
//   cargo run -p temen-run -- crates/temen-run/demos/jit/jit_dlopen.c
...
// --- the byte emitter: serialized Temen IR (binary `temen-encode` v2), as in jit_link.c -----------
static int n_out;
...
// Magic + version + memory(16) + 0 data segments + 0 funcref slots (common to every unit).
