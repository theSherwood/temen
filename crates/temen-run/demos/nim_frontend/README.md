# nimony front-end on Temen — nimsem drives nifler via `exec`
...
to `nifler`. This demo runs `nimsem` as an Temen guest and routes that shell-out to the Temen **`exec`
capability**, so the real `nifler.temen` runs as an isolated child domain — the W4 multi-binary driver
...
- **nimsem's shell-out → `exec`.** nimsem builds `nifler --portablePaths --deps parse <src>
  ...
  (`../nifler_temen/nifler_shim.c` `system()`) parses that command into argv and calls the `exec`
...
`system` module — `nimsem-on-Temen` returns 0 and emits the `.s.nif`.
...
- `fixtures/sysvq0asl.x.nif.gz` — hexer's expected Leng output (step 10).
...
discrepancy and no Temen/on-ramp gap. Remaining polish: exact path-matching for raw byte-identity, and
