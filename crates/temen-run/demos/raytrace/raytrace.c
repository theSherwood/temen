 ...
 * rim falloff, rendered to a character ramp and written row-by-row. `sqrt`/`floor` lower to Temen
 ...
 * IEEE / unfused on both sides). temen-run's output must match a native `cc` build byte-for-byte. */
#include <stddef.h>
