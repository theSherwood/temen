...
//! no frontend) exercised through `run_powerbox`/`run_kernel`, the shipped demo, and the `temen-run`
//! binary itself driving a `.temt` file with real stdout + exit code.
...
///   1. `TEMEN_DEADLINE_MS` arms the §5 detect-and-kill, so a *guest-side* wedge — a spinning **or**
...
fn run_demo_failfast(rel: &str) -> std::process::Output {
    ...
    match rx.recv_timeout(HARD_TIMEOUT) {
        Ok(status) => {
            let status = status.expect("wait temen-run");
...
fn runs_shipped_demo() {
    let src = std::fs::read_to_string(demo("hello.temt")).expect("read hello.temt");
    let m = load(&src);
    let run = run_powerbox(&m, b"").expect("run");
...
/// Drive the actual `temen-run` binary on the demo `.temt`: it must print the greeting to stdout
/// and exit 0 — the "a program runs in the sandbox from the command line" milestone.
#[test]
fn cli_runs_temen_file() {
...
/// Compile a demo `.c` with native `cc`, run both it and `temen-run`, and assert identical
...
fn assert_demo_matches_cc(name: &str) {
    ...
    if !temen.status.success() {
        ...
        panic!("temen-run on {name} failed: {err}");
    }
    assert_eq!(
        String::from_utf8_lossy(&temen.stdout),
        String::from_utf8_lossy(&native.stdout),
        "{name}: temen-run vs native cc stdout differ"
    );
}
...
fn demo_clay_layout_runs() {
    ...
    if !out.status.success() {
        ...
        panic!("temen-run on clay_demo.c failed: {err}");
    }
    assert_eq!(
...
fn cli_compiles_and_runs_c() {
    ...
    if !out.status.success() {
        ...
        panic!("temen-run on hello.c failed: {err}");
    }
    assert_eq!(out.stdout, b"hello, sandbox!\n");
}
...
/// `temen_capture_explicit_trap`, threaded the trapping frame pointer via `get_frame_pointer`) runs on
...
fn powerbox_region_minting_round_trips() {
    let m = load(
        "memory 17\n\
         ...
         \x20 v7 = cap.call 4 0 (i64, i64, i64, i32) -> (i64) v3(vg, v5, v4, v6)\n\
         \x20 v8 = cap.call 4 0 (i64, i64, i64, i32) -> (i64) v3(vg2, v5, v4, v6)\n\
...
/// `temen-run` binary: 4 worker threads each cooperatively scheduling 8 fibers over the VM's
...
fn demo_mn_scheduler_runs() {
    ...
    assert!(out.status.success(), "temen-run on mn_sched failed: {err}");
...
/// end to end through the `temen-run` binary — must print the interleaving-invariant total `256`. The
...
fn demo_work_stealing_runs() {
    ...
    assert!(
        out.status.success(),
        "temen-run on work_stealing failed: {err}"
    );
...
/// (`demos/steal_fibers`, D57 complete), end to end through the `temen-run` binary: suspended
...
fn demo_steal_fibers_runs() {
    ...
    assert!(
        out.status.success(),
        "temen-run on steal_fibers failed: {err}"
    );
    assert_eq!(
        out.stdout, b"256\n121920\n",
...
/// through the `temen-run` binary: 4 worker threads each Cranelift-compile a distinct unit
...
fn demo_jit_threads_runs() {
    ...
    assert!(
        out.status.success(),
        "temen-run on jit_threads failed: {err}"
    );
    assert_eq!(
        out.stdout, b"0\n",
...
fn unknown_named_import_fails_closed() {
    ...
    let err = match temen_run::instantiate(m) {
