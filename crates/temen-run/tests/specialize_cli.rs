...
use temen_run::{run_kernel, specialize_module, SpecArg, SpecializeOpts, Value};
...
fn cli_specialize_runs_and_emits_artifact() {
    ...
    let src = dir.join(format!("peval_cli_{pid}.temen"));
    std::fs::write(&src, temen_encode::encode_module(&g())).unwrap();
    ...
    // to 0 -> 10).
    let art = dir.join(format!("peval_cli_{pid}_out.temen"));
