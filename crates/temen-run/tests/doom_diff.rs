...
//!
//! `#[ignore]`d — it needs build artifacts produced by `crates/temen-run/demos/doom/{fetch,build,diff}.sh`
//! (the guest `.temen`, the native oracle's frame list, and the shareware WAD). Paths are overridable
...
fn doom_frame_hashes_match_native() {
    let temen = std::fs::read(env_or(
        "DOOM_TEMEN",
        "/tmp/doomgeneric_cache/bcdiff/doom_diff.temen",
    ))
    .expect("guest doom_diff.temen — run demos/doom/diff.sh first");
    ...
    let m = temen_encode::decode_module(&temen).unwrap();
    assert!(
    ...
    // its `(type_id, op)` + granted handle — the same binding `temen_run::instantiate` performs.
