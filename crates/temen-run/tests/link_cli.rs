//! The `--link` CLI path (D-LINK, v9 object dialect): binary `.temeno` units carrying data
//! exports, `data.ptr` relocations, and the link-form data-address instructions link into one
//! runnable `.temen` — the end-to-end for the wire dialect that `temen-encode` v9 added.
...
fn write_unit(dir: &std::path::Path, name: &str, m: &Module) -> PathBuf {
    let p = dir.join(name);
    std::fs::write(&p, temen_encode::encode_unit(m)).unwrap();
    p
}

#[test]
fn link_temeno_units_into_runnable_temen() {
    let bin = env!("CARGO_BIN_EXE_temen-run");
    let dir = std::env::temp_dir().join(format!("temen_link_cli_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let a = write_unit(&dir, "a.temeno", &unit_a());
    let b = write_unit(&dir, "b.temeno", &unit_b());
    let out = dir.join("out.temen");
    ...
    assert!(
        ...
        "direct .temeno run should point at --link"
    );
    ...
    let linked = temen_encode::decode_module(&bytes).expect("linked output is runnable dialect");
...
fn assemble_text_unit_to_temeno() {
    let bin = env!("CARGO_BIN_EXE_temen-run");
    let dir = std::env::temp_dir().join(format!("temen_asm_cli_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let unit = unit_b();
    let text_path = dir.join("b.temt");
    std::fs::write(&text_path, temen_text::print_module(&unit)).unwrap();
    let obj_path = dir.join("b.temeno");
