...
fn mode_and_attach_round_trip() {
    ...
    assert_eq!(m.imports[0].mode, temen_ir::ImportMode::Rebindable);
    assert_eq!(m.imports[1].mode, temen_ir::ImportMode::Required);
  ...
  return\n\
    ...
    assert!(
        matches!(
            ...
            Err(temen_verify::VerifyError::AttachNotRebindable { .. })
        ),
...
block 0 () {\n\
  ...
  return\n\
    ...
    assert!(
        matches!(
            temen_verify::verify_module(&m),
            Err(temen_verify::VerifyError::UnresolvedImport { .. })
        ),
...
block 0 (v0: i32) {\n\
  ...
  return v2\n\
    ...
    .expect("parse");
    assert!(
        temen_verify::manifest_complete(&self_query),
