//! The svm futex primitive (copied to `sys/sync/futex/svm.rs` by `apply-overlay.sh`).
//!
//! A near-mechanical port of std's `sys/sync/futex/wasm.rs`: the same "an atomic word is a futex"
//! contract, but the wait/notify reach the svm §12 futex through the on-ramp intrinsics
//! `__vm_wait32` / `__vm_notify` (svm-llvm `lower_vm_builtin`) instead of the wasm
//! `memory.atomic.wait32` / `memory.atomic.notify` opcodes. The IR ops these lower to
//! (`MemoryWait` / `MemoryNotify`, svm-ir §12) share wasm's semantics exactly:
...
//!
//! Only compiled for the threaded target (`x86_64-unknown-svm-threads`, `target_env = "threads"`);
//! the lean single-threaded `x86_64-unknown-svm` target keeps std's `no_threads` sync and never
...
pub fn futex_wake_all(futex: &Atomic<u32>) {
    // `count = -1` reinterpreted as `u32::MAX` = "wake all" (svm-ir `MemoryNotify` caps at the real
