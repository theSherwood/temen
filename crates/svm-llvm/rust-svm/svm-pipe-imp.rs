//! svm anonymous pipes: the `sys::pipe::Pipe` backing `std::process`'s child stdio. A pipe is a pair of
//! fds over one in-personality byte FIFO (svm-posix `OP_PIPE`), read/written through the same
