//! svm environment variables: `getenv`/`setenv`/`unsetenv`/`vars` reach the POSIX personality through
//! the PAL `host` bridge (svm-posix `OP_GETENV_R`/`OP_SETENV`/`OP_UNSETENV`/`OP_ENVIRON`). The
...
pub fn getenv(name: &OsStr) -> Option<OsString> {
    ...
    // SAFETY: svm has no OS-specific OsStr encoding; the bytes are the value verbatim.
    ...
    if host::have_posix() {
        ...
        loop {
            ...
            if let Some(eq) = buf.iter().position(|&b| b == b'=') {
                // SAFETY: svm has no OS-specific OsStr encoding.
