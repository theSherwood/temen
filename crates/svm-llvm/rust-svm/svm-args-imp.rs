//! svm command-line arguments: captured from the powerbox-threaded `argv` at runtime init (the svm
...
/// One-time global initialization, called from the svm PAL's `init` with the values the powerbox
...
pub fn args() -> Args {
    ...
    for i in 0..argc {
        ...
        // SAFETY: the bytes came from a C string; svm has no OS-specific OsStr encoding.
