...
| `std-overlay.patch` | `cfg_select!` arm additions routing `target_os="svm"` to the right leaf-module impls: the minimal (no-OS, single-thread) ones for `sys/{alloc,thread_local,random,io/error}` (as `vexos`/`zkvm` do), the svm `stdio`/`pal`/`args`/`time`/`env`/`fs` modules, and the powerbox `exit` in `sys/exit.rs`. 46 added lines across 10 files. |
...
| `svm-pal.rs` | The svm PAL proper (copied to `sys/pal/svm.rs`). Mirrors the `unsupported` PAL, but its `init` captures the powerbox-threaded `argv` (so `std::env::args` works), and it hosts the `host` bridge — `__vm_cap_resolve("posix")` + per-op `__vm_host_call` wrappers — that the richer surface (`time`/`env`/`fs`) reaches the host through. |
...
- **Working today:** stdout (`println!`), `stderr` (`eprintln!`), `process::exit`,
  ...
  **`std::fs`** (`File` open/read/write/seek, `metadata`/`read_dir`/`remove_file`/
  `exists`, via a granted posix cap), heap/`Vec`, collections, `fmt`, iterators.
  (`std::env::var`/`vars` — the `str`-Debug paths — light up with the on-ramp
  entry-block slot-numbering fix, #755.)
- **The two paths:** the powerbox stream/exit handles carry stdio/exit/args (no
  ...
  via the PAL `host` bridge's `__vm_host_call`) carries `time`/`env`/`fs` — this is
  where the richer, many-op surface scales without growing the powerbox.
