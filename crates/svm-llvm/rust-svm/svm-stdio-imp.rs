//! svm stdio: the three standard streams reach the host through the powerbox's `write`/`read`
//! bindings (the svm-llvm on-ramp's "Lane C" — `write`/`read`/`exit` bind to the powerbox
...
//! in-band (negative), matching svm-posix's value-not-trap ABI.
use crate::io;
