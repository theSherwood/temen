...
//! dispatch-free loop (the `svm-peval` dispatch-ROI measurement on main shows that shape).
//!
...
//! feature's soundness itself is differential-tested in `svm-peval/tests/rename_seed.rs`.
//!
//! Run: `cargo test -p svm-llvm --test lua_futamura_vmstate -- --nocapture`
//! (`--features svm-peval/trace` prints the first blocker if a further wall remains).
