...
//! svm-llvm → svm-IR`): specialized against a constant bytecode program, decode + dispatch fold and
...
//! against genuinely ported C, not synthetic svm-IR (the register VM is the Lua/QuickJS execution
...
//!
//! Run: `cargo test -p svm-llvm --test peval_regvm_runtime -- --nocapture`
...
/// Compile a C source `src` → LLVM (`clang -O2`) → svm-IR. `None` if clang is unavailable (skip).
...
fn block_calling(f: &svm_ir::Func, target: u32) -> u32 {
    f.blocks
        .iter()
        .position(|b| {
            b.insts
