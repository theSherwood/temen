...
entry:\n\
  ret i64 add (i64 ptrtoint (ptr @g to i64), i64 7)\n\
    ...
    let t = svm_llvm::translate_ll_str(ir).expect("translate constexpr-add IR");
    svm_verify::verify_module(&t.module).expect("verify");
    let base = t
        .data_symbols
        .iter()
    ...
    let got = match svm_interp::run(&t.module, f, &[Value::I64(t.entry_sp as i64)], &mut fuel)
        .expect("interp")[0]
    {
        Value::I64(v) => v,
        other => panic!("unexpected {other:?}"),
...
fn demo_tcl_init_stdin() {
    ...
    let init_linked = std::env::temp_dir().join("svm_tcl_cache/tcl_init_linked.ll");
    ...
    let t =
        svm_llvm::translate_ll_path_with_options(&init_linked, opts).expect("translate tcl_init");
    let module = t.module;
    svm_verify::verify_module(&module).expect("verify tcl_init module");
    let run = svm_run::run_powerbox(&module, stdin).expect("powerbox run tcl_init");
    assert_eq!(
        run.stdout,
        native.stdout,
