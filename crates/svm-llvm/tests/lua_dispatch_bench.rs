...
//!
//! Run: `cargo test -p svm-llvm --test lua_dispatch_bench -- --nocapture`

use svm_interp::{IrPc, Stop, StopReason, Value};
use svm_ir::{Data, Inst, Module, Terminator};
...
fn run(m: &Module) -> Result<Vec<Value>, svm_interp::Trap> {
    let mut fuel = u64::MAX;
    svm_interp::run(m, 0, &[], &mut fuel)
}
