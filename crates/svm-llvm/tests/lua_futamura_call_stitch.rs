...
//!
//! Run: `cargo test -p svm-llvm --test lua_futamura_call_stitch -- --nocapture`
...
fn two_residuals_stitched_at_the_call_boundary() {
    ...
    svm_verify::verify_module(&r).expect("stitched module verifies");
    let base = svm_run::run_powerbox(&m, SCRIPT.as_bytes()).expect("baseline");
    let emb = svm_run::run_powerbox(&r, SCRIPT.as_bytes()).expect("stitched");
    println!(
        "baseline={:?} stitched={:?}",
