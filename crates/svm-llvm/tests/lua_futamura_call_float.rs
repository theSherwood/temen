//! **A float-returning Lua callee** — the selective return tag generalized beyond integers.
//!
//! ```lua
//! local function half(x) return x / 2 end
//! print(half(5) + 0.25)                    -- 2.5 + 0.25 = 2.75
//! ```
//!
//! Lua's `/` always produces a float, so `half`'s return is float-tagged and the caller's `OP_ADDK`
//! takes the float fast path. The [`svm_peval::PoscallModel::selective`] entry pins the result slot's
//! tag to the **discovered** runtime tag (read from the result cell at the `print` call's precall
//! breakpoint — self-discovering, not hardcoded), while the value reloads dynamic; the float
//! arithmetic then folds its tag checks and computes on the dynamic value at runtime. Everything else
//! is the `lua_futamura_call_arith` configuration. The embedded residual prints byte-identically to
//! the interpreter.
//!
//! Run: `cargo test -p svm-llvm --test lua_futamura_call_float -- --nocapture`
...
const SCRIPT: &str = "local function half(x) return x / 2 end\nprint(half(5) + 0.25)\n";
...
fn lm() -> Module {
    ...
    svm_llvm::translate_ll_path(&p).expect("x").module
}
...
fn entry(insp: &mut svm_interp::Inspector, luav: u32) -> (i64, u64, u64) {
    let pc = IrPc {
        module: 0,
        func: luav,
        block: 0,
...
fn float_returning_callee() {
    ...
    let inst = svm_run::instantiate(m.clone()).unwrap();
    ...
    // At print's precall the callee has returned; discover the result's runtime tag from its slot.
    let w2 = insp.read_window(0, CAP).unwrap();
    let ret_tag = w2[(ra_add + 8) as usize];
    println!("discovered return tag: {ret_tag:#x} (float expected)");
    assert_ne!(ret_tag, 0x03, "x / 2 must not be integer-tagged");
    insp.clear_breakpoint(pbp);
    let inst2 = svm_run::instantiate(m.clone()).unwrap();
    ...
    svm_verify::verify_module(&r).expect("verifies");
    let base = svm_run::run_powerbox(&m, SCRIPT.as_bytes()).unwrap();
    let emb = svm_run::run_powerbox(&r, SCRIPT.as_bytes()).unwrap();
    println!(
        "base={:?} emb={:?}",
        String::from_utf8_lossy(&base.stdout),
        String::from_utf8_lossy(&emb.stdout)
    );
    assert_eq!(base.stdout, b"2.75\n");
