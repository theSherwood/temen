...
//!
//! Run: `cargo test -p svm-llvm --test lua_futamura_call_stitch_chain -- --nocapture`
...
fn three_residuals_chain() {
    ...
    svm_verify::verify_module(&r).expect("chained module verifies");
    let base = svm_run::run_powerbox(&m, SCRIPT.as_bytes()).expect("baseline");
    let emb = svm_run::run_powerbox(&r, SCRIPT.as_bytes()).expect("chained");
    println!(
        "baseline={:?} chained={:?}",
