...
//! move phase orchestration *into* the sandbox: a Rust-on-SVM guest that spawns each nimony phase child
...
//! can issue the spawn. The C precedent is `svm/tests/c_shell_exec.rs`; this is its Rust counterpart via
...
use svm_interp::{run_capture_reserved_with_host, ForkedProc, Host, HostProc, Value};
...
fn parse_child() -> svm_ir::Module {
    let m = svm_text::parse_module(CHILD).expect("parse child");
    svm_verify::verify_module(&m).expect("verify child");
    m
}

/// The production [`GrantChildHooks`] table (svm-run's child build/bind/release/mint/thunk/serve) as the
/// granted-spawn suites install it on the JIT — the same table `svm/tests/c_shell_exec.rs` uses.
...
fn granted_host(child: &svm_ir::Module, win: u64) -> (Host, Arc<Mutex<i64>>) {
...
fn rust_guest_spawns_child_via_op13() {
    ...
    let t = svm_llvm::translate_ll_path(&ll).expect("svm-llvm translates the Rust guest");
    svm_verify::verify_module(&t.module).expect("the translated guest verifies");
    let entry = t
        .exports
        .iter()
        .find(|(n, _)| n == "run")
