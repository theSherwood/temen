...
//!
//! `savedpc_resume.rs` (svm-peval) showed mid-loop resume on a VM authored in svm-IR, where the
...
//! [`svm_llvm::Translated::data_symbols`] now exposes every global's name → window address; this test
//! uses it to rename a real C interpreter's `savedpc` and drive a mid-loop deopt/resume end to end.
//!
//! The VM (`clang -O2 → LLVM → svm-llvm → svm-IR`): a per-opcode `pc` lives in `savedpc`; the loop
...
//! genuine mid-loop resume — now on ported C, not synthetic svm-IR.
//!
//! Run: `cargo test -p svm-llvm --test peval_savedpc_ported -- --nocapture`
...
fn compile(name: &str, src: &str) -> Option<svm_llvm::Translated> {
...
fn fn_idx(t: &svm_llvm::Translated, name: &str) -> u32 {
    t.exports
        .iter()
        .find(|(n, _)| n == name)
...
fn data_addr(t: &svm_llvm::Translated, name: &str) -> (u64, u64) {
    let s = t
        .data_symbols
        .iter()
        .find(|s| s.name == name)
