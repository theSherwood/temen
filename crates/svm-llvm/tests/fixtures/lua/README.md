...
warm state), so the browser warm session (`svm_warm_open`/`svm_warm_eval`, WASM_AOT.md) can snapshot the
...
`__svm_malloc`), not `lua_eval`'s 48 MiB static arena, so the snapshot image is the small live prefix
...
The shipped artifact is the committed **`browser/web/assets/lua_snapshot.svmb`** (a generated binary,
like the vendored `doom1.wad`). Unlike `lua_eval.ll`, the ~76k-line linked `lua_snapshot.ll` is **not**
committed (no Rust test consumes it — only the browser `.svmb` does), and `build-onramp-assets.mjs` does
**not** rebuild it — so it is regenerated by hand and the resulting `.svmb` committed:
