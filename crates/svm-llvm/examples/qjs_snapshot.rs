...
//!
//! Uses the two-phase driver `crates/svm-run/demos/quickjs/qjs_snapshot.c`:
...
//! in svm-run), (2) seed the guest heap bump-pointer words (`POWERBOX_HEAP_BRK`/`_TOP`) to the window's
...
//!   `qjs_snapshot.c` instead of `qjs_repl.c` and translate to `/tmp/qjs_snapshot.svmb`.
//!
//! Usage: `cargo run --release --example qjs_snapshot -- /tmp/qjs_snapshot.svmb`

use std::alloc::{alloc_zeroed, dealloc, Layout};
use std::sync::Arc;
use std::time::Instant;

use svm_interp::bytecode::SharedProgram;
use svm_interp::{cap_id, BoundImport, Host, Region, StreamRole, Trap, Value};
use svm_ir::{Module, POWERBOX_HEAP_BRK, POWERBOX_HEAP_TOP};

const FUEL: u64 = 100_000_000_000;
/// The `Jit` install-table log2 (`svm-run`'s `CLI_JIT_TABLE_LOG2`); inert here (QuickJS never JITs).
...
/// same setup `svm-run`'s instantiation does for an on-ramp guest (`grant_powerbox_prefix` + the
...
fn powerbox_host(module: &Module, win: u64, stdin: &[u8]) -> Host {
    ...
    // match svm-run's; QuickJS only reaches write/read/exit + malloc's vm_map, but grant the full set.
...
fn main() {
    let path = std::env::args().nth(1).expect("usage: qjs_snapshot <svmb>");
    let bytes = std::fs::read(&path).expect("read svmb");
    let mut m = svm_encode::decode_module(&bytes).expect("decode module");
