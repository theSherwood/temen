...
//! `(func, block)` where the from-entry walk walls, build `svm-peval` with `--features trace`.
//!
...
fn main() {
    ...
    let m = svm_llvm::translate_ll_path(&p).expect("translate").module;
    let jci = m
        .exports
        .iter()
    ...
    match specialize_with_config(&m, jci, &args, &cfg) {
        ...
        Err(e) => {
            ...
            println!(
                "  → the wall is the C prologue's cold decision tree (build svm-peval with --features\n     \
...
fn decode_dispatch(m: &svm_ir::Module, jci: u32) {
