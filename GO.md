# GO.md — running Go on Temen: feasibility & work breakdown

Status: **scoping doc, pre-implementation**, written 2026-08-04. Answers "how much work
would it be to get Go running on Temen?" against what the tree provides today: migratable
...
on-ramps (`temen-llvm`, `temen-wasm`). It leans on the frontend trust model (`DESIGN.md`
...
- **The official `gc` toolchain ported natively is the wrong target.** Three structural
  mismatches, not effort gaps: (1) Go grows goroutine stacks by *copying*, but the Temen
...
- **Two recommended routes, cheap-first, both bounded:**
  - **Route A — stock Go via `GOOS=wasip1 GOARCH=wasm` → `temen-wasm`.** The `gc`
    ...
    module. temen-wasm is feature-complete for typical toolchain output; the gap is
  ...
  - **Route B — TinyGo → LLVM IR → `temen-llvm`, runtime retargeted to Temen
    primitives.** TinyGo is the Go-semantics compiler whose runtime already matches
    Temen's contract: **conservative, non-moving mark-sweep GC** and **fixed-size
    ...
    already proven in C). This is the route that uses what Temen uniquely provides.
- **Known temen-side gaps Route B hits (all small, listed in §4):** `fence` lowering in
  temen-llvm (parses, `Unsupported`), the LLVM TLS gap (avoidable in a retargeted
...
- **Route D — multi-threaded self-hosting Go (§6, added 2026-08-04):** a direct
  **Go-SSA → temen-ir backend** (a pure-Go compiler; TinyGo's LLVM dependency is
  ...
  3–5× the temen-leng effort** (backend ≈ months of slices, the go/types+go/ssa
  stdlib tail is the long pole), with the gc-via-wasm lane (§6.5) closing a
  toolchain-on-temen loop early for near-zero compiler work.

## 1. What Go needs, and what Temen answers

| Go runtime need | Temen answer | Status |
|---|---|---|
...
| `defer`/`panic`/`recover` | Frontend lowering (TinyGo lowers these itself); `SetJmp`/`LongJmp` core ops and SJLJ EH exist in temen-llvm if needed | Built |
...
(entry `br_table` re-entry dispatch — temen-wasm handles `br_table` natively). Nothing
about wasm-GC (temen-wasm's permanent non-goal) is involved.
...
### Route B — TinyGo retargeted to Temen primitives (the route that uses the machine)
...
LLVM, with a runtime *already shaped like Temen's contract*:

- **GC:** conservative, non-moving mark-sweep — literally the `GC.md` §1 model. Its
  stack scan is the piece the guest cannot do on Temen (SSA spills live on the
  unaddressable control stack) — replaced by one `__vm_gc_roots` call, which temen-llvm
...
temen-llvm is the mature on-ramp (QuickJS/SQLite/Lua/Postgres-scale; no libLLVM link;
...
- **B1 — single-vCPU goroutines + GC (order 4–8 weeks).** A TinyGo custom target
  (`temen.json` + `GOOS`-ish build tags) whose runtime package binds: task switch →
...
- **B2 — parallel M:N (order +4–6 weeks).** N vCPUs via `thread.spawn`, per-worker
  ...
  §2.1 `quiesce` barrier. `GOMAXPROCS` = vCPU count, passed in (temen exposes no count
...
`read()`) is exactly the contract a stackful switch needs. temen-llvm recognizes
...
check — compile via `clang -O2` → temen-llvm and match the chibicc-frontend build
...
A new `GOARCH`/`GOOS` backend in the `gc` compiler emitting Temen IR, plus a runtime
...
roots forbid relocation anyway. So goroutines on Temen are **fixed-stack**, sized at
...
1. **Pick sizes honestly.** Data stack: guest-chosen per fiber (16 KiB default is the
   ...
   `temen-jit/src/fiber_rt.rs`) — fine for thousands of goroutines, hostile to hundreds
   of thousands (§4.2 proposes the knob).
2. **Deep recursion is the app's problem.** Same posture as every Temen guest: the
   guard page catches it; a runtime that wants more spawns bigger-stack fibers.

## 4. temen-side gaps this surfaces (each small, each its own slice)

1. **`fence` in temen-llvm.** Parses but `Unsupported`; the IR has thread fences and
...
2. **Per-`cont.new` control-stack size.** Make the fiber control-stack size a
   ...
   `temen-fiber`/`fiber_rt`; needed for B2 at scale, not for B1.)*
...
- **Small core** (Invariant 1): proposed temen changes are two translator/parameter
...
## 6. Route D — multi-threaded self-hosting: a direct Go→temen-ir backend

Added 2026-08-04, answering the follow-up: *what would it take to run Go
multi-threaded and self-host it — a TinyGo-style backend that targets temen-ir, with
the compiler itself compiled to temen-ir?* "Self-hosting" here follows SELFHOST_C.md
...
TinyGo as-is cannot self-host on Temen: it links **libLLVM** (a multi-million-line
...
(`temen-llvm` §8 Q1b dropped the link in favor of an in-house `.ll` reader).
...
emits **temen-ir directly**, plus the Route B runtime as ordinary Go+intrinsics
code. Call it `temengo`. Whether it lives as a TinyGo fork with the LLVM emission
...
go/ssa is typed SSA with φ-nodes → block params, temen-ir takes irreducible CFGs,
multi-value returns (Go's bread and butter), and true tail calls natively. This is
the temen-llvm situation — "we already have SSA" — not the temen-wasm reconstruction
one. Like every frontend, `temengo` sits outside the escape-TCB (§2a): the verifier
...
|---|---|---|
| Scalars, structs, arrays, pointers | Window offsets + masking, §3d SysV-pinned layout | The chibicc/temen-leng model, proven twice |
...
| Goroutine-local (`g`) | Data-stack-base derivation + `vcpu.tls` for per-P state (§4.3) | No new temen feature |

The backend proper is bounded and slice-able exactly like `temen-leng` was
...
- **The compiler itself** parallelizes the way gc does (concurrent per-function
  compilation over a work queue) — on Temen that is goroutines over vCPUs, i.e. the
...
subprocesses) as its "biggest unknown" for self-hosting. `temengo` sidesteps this by
...
`.temeno` narrow waist and `temen_ir::link` already exist for exactly this). File I/O
bottoms out on the POSIX personality + `temen-fs` memfs; no `exec`, no process tree.
...
1. **Stage 0:** host Go toolchain builds `temengo` native. This is the build/dev
   oracle binary, exactly like chibicc's native form (SELFHOST_C.md §2) — it
   never ships, it gates.
2. **Stage 1:** native `temengo` compiles `temengo` + runtime + its stdlib closure →
   `temengo.temen`. Runs as a guest on interp + Cranelift JIT.
3. **Stage 2:** `temengo.temen` (on Temen, multi-vCPU) compiles the same sources.
...
`temengo.temen` could be built by **stock gc → `GOOS=wasip1` wasm → temen-wasm**
...
### 6.5 The near-free lane: the gc toolchain *on* Temen before any backend exists

Worth naming because it closes a toolchain-on-temen loop for ~zero compiler work,
as soon as Route A lands: the gc compiler (`cmd/compile`, `cmd/link`) is pure Go,
so it builds as a `wasip1` binary and runs on Temen through temen-wasm + the widened
WASI shim + memfs. Output targets `wasip1` too → feed the result back through
temen-wasm. That is *Go compiling Go on Temen, running the result on Temen* — no new
...
1. **Backend skeleton** — go/ssa → temen-text for scalars/control-flow/calls,
   fail-closed elsewhere; differential from the first commit. *(The temen-leng
   walking-skeleton move; that took days of slices.)*
2. **Core semantics** — slices/strings/maps/interfaces/closures/defer/panic,
   runtime in guest Go. *(The bulk of backend work; temen-leng's W1 analog, which
...
largest language project in the tree so far, sized roughly **3–5× temen-leng**
...
| Program/link shape | W2 linker landed in days, but **W4** (nifler→nimony→hexer→lengc subprocess pipeline) is NIM.md's "biggest unknown", still open | one binary, in-process, whole-program emit over the existing `.temeno`/`temen_ir::link` waist | **Go is easier** — W4 has no analog |
| Fixpoint strength | not a single-language fixpoint — temen-leng is Rust; NIM.md W5 *plans* to bootstrap it via rustc→wasm→temen-wasm (chosen over temen-llvm because full-**std** Rust needs the TLS/std bottom edge temen-llvm lacks, while rustc's wasip1 target solved all that upstream — the Rust lane on temen-llvm is deliberately `no_std`+`alloc`) | chibicc-grade stage1==stage2 in one language, parallel and deterministic | stronger result, dearer floor |
...
over (chibicc, temen-wasm/temen-llvm, temen-leng), and the self-host convention is
...
- **x/tools/go/ssa version coupling.** The backend couples to go/ssa's API — a
  vendored pin, refreshed deliberately (the LLVM-18→21 lesson from temen-llvm
...
- wasm-GC. Irrelevant here (Go doesn't emit it) and a standing temen non-goal.
