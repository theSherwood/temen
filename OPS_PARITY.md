...
| `gc.roots` | ✅ | ✅ | 🔶 | ⛔ | Full on x86-64-unix (fiber_rt); Declines to the interp elsewhere; leaf accelerator: folds to the bytecode interp underneath (DESIGN §3). wasm: **module-granular** — a module that can reach it emits nothing (#1546), since an emitted frame's roots are invisible to the scan |
